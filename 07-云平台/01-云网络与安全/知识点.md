...
先别动手。这一章后半全是你会动手的：**怎么划 VPC、怎么写安全组、怎么验 LB 探测源、NAT 端口满和黑洞分别怎么止血。** 前面的概念和原理，是为了让你动手时不选错层。
...
- [6. 常用命令](#6-常用命令)
- [7. 常用操作](#7-常用操作)
  - [7.1 暴露游戏端口](#71-暴露游戏端口)
  - [7.2 专线与 VPN](#72-专线与-vpn)
  - [7.3 DDoS 与 WAF](#73-ddos-与-waf游戏端口不走-waf)
- [8. 生产排障](#8-生产排障)
- [9. 必会 · 常考](#9-必会--常考)
- [合上书](#合上书问自己)
...
交换机 / 子网绑 AZ，创建后难改。ACK Terway / EKS AWS VPC CNI **按 Pod 占 ENI / 辅助 IP**，不是「一台 ECS 一个 IP」。

活动要加节点，节点池从 20 台扩到 64 台，每台大约 30 个 Pod。64 × 30 = 1920 个地址，按峰值 × 1.5 预留大约 2880。节点池子网如果是 `/24`，一共 254 个可用，远远不够。
...
AZ 是独立的供电、网络、冷却。单 AZ 挂了，这个 AZ 里的 ECS、单 AZ RDS、单 AZ NAT 一起没。多 AZ 抗的是 **机房级** 故障，不是地域级（地震 / 海缆走多地域 + DNS）。
...
正确表述：区服计算面单 AZ 部署，故障域写进预案——RTO 按「切到热备 AZ 或重开进程」计，不以「玩家无感」承诺。大厅、登录、匹配、HTTP API 可以多 AZ。别把「我们全多 AZ」说成默认最优。
...
一次支付回调随机超时：活动中充值偶尔失败，VPC 内互访正常，本机 `curl` 打支付网关偶发 `connect timeout`，换一个目标就好。系统里发生了什么：所有逻辑服打 **同一个支付 IP:443**。一个 NAT IP 大约 6 万可用源端口，五元组 `(srcIP, srcPort, dstIP, dstPort, proto)` 冲突时，SNAT 分不出新端口。

你眼睛看到的：

- VPC 内互访正常；出公网偶发超时
- 阿里云 NAT：端口水位、并发连接贴线
- AWS：`ErrorPortAllocation` 往上跳，`BytesOutToDestination` 不一定先爆
...
用一次上线把差别走一遍。战斗接入挂到 CLB / NLB，ECS 上 `curl 127.0.0.1:port` 200，从堡垒 ssh 再 curl 也通，办公网抽检也通。LB 控制台后端全 Unhealthy，玩家进不去。

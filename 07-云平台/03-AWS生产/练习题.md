...
**一句话**：游戏长连接用 NLB（L4），ALB 是 HTTP；WAF 挂不到 NLB TCP。
...
**一句话**：Spot 约 2 分钟中断，只给可中断任务；战斗服和 T3 / T4g 一律不用。
...
**一句话**：IRSA 把权限绑到具体 SA，Trust 必须限制 sub；EKS 托管控制面，节点组 / CNI 仍是你的。
...
**一句话**：IAM 求值：显式 Deny > SCP > Allow，跨账号要两边允许；模拟器不是真相。
...
**一句话**：EKS 拉 ECR 没 Endpoint 会穿 NAT，扩 NAT 是治标，要配 VPC Endpoint。
...
**一句话**：S3 生命周期按前缀，current 不过期；403 常是桶策略 / KMS / 公共访问块。
...
**一句话**：CloudWatch 管云资源指标，Prometheus 管业务 CCU；高基数自定义指标会打穿账单。
...
**一句话**：NLB ActiveFlow 涨但 Healthy 为 0 是 LB 黑洞，先查健康检查端口和 preserve client IP 的 SG。
...
**一句话**：中国区 AWS 与 Global 账号隔离，AK 不通，不能当阿里云用。
...
**一句话**：出海开服前夜：Endpoint、NLB 探测源、IRSA、心跳 < idle、On-Demand 战斗节点。

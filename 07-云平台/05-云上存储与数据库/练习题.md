...
**一句话**：热更走对象存储 + CDN，云盘绑实例无边缘；MySQL 数据目录禁 s3fs / NAS。
...
**一句话**：RDS 多 AZ 是 HA，只读是扩展读；写路径禁只读，切主后要重连连接池。
...
**一句话**：RPO 以校验成功的 restore 为准，只读副本和未验快照不算备份。
...
**一句话**：云 Redis 超时先看连接 / 带宽 / QPS 硬顶，不当唯一账本，禁公网。
...
**一句话**：max_connections 打满先杀泄漏再升规格，参数组改 innodb 是生产变更。
...
**一句话**：Redis 禁公网；RDS 切主后连接池还打旧连接，要滚动网关重连。
...
**一句话**：RDS / Redis / OSS 各有生产红线：多 AZ、无公网、私有桶 + Block Public Access。
...
**一句话**：NAS 不当库和战斗状态；慢先排除云盘 / RDS / Redis 规格上限再谈 SQL。

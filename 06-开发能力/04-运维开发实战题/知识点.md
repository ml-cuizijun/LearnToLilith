...
- 配置正确、第 50 台磁盘满 → 修第 50 台重入
- 变更单要求一致 → 对 49 台跑回滚 job（scp bak + nginx -t + reload）
- 战斗服不要和网关同一批并行 reload
...
开服 POST 非幂等——429 盲重试可能双写。拒绝时查是否已创建或用幂等键。
...
P0/P3 分 webhook；电话升级只绑 P0。

**Prom 发布门禁（口述要点）**：PromQL 带 **canary label**，对比发布前 5min baseline；`requests.get(..., timeout=5)`；没数据或 Prom 挂了 → 生产默认 **不放行**（`exit 1`）；超阈值调回滚 webhook。

**CI 扫描门禁（口述要点）**：`trivy --exit-code 1 --severity CRITICAL`；非 0 不 push 不可变 tag；`:latest` 禁当生产；豁免参数默认关、打日志。

**对账释放（口述要点）**：只读 AK + paginator + 限流；输出 orphan/diff；`protected=true` 标签跳过；**列出 ≠ 删除**，有 diff 就 `exit 1` 等人审。

...
每节 **一句话** + 图。讲清「为什么这样写」，不是语言 trivia。
...
死锁 vs 泄漏：都能 pprof；泄漏是**数量级**问题。修：ctx 传递、有界 queue、限制 worker；**重启能止血不能当根因**，活动后必须抓 pprof 再发版。
...
| 类型 | 语义 | 运维误用 |
|------|------|----------|
| Counter | 只增（登录次数） | 当成当前 CCU → 活动结束图还在涨 |
| Gauge | 可上可下（CCU、队列） | scrape 失败留脏值 → 大盘以为还在线 |
| Histogram | 延迟桶 | label 带 `user_id` → 基数爆炸 |

Histogram 桶按 SLA：游戏 API 常用 **5–50–100–200–500ms**，别用默认秒级桶。label：`server_id`/`region` 可以（封闭）；`user_id` 不行。已经炸了：**改代码丢 label + Prometheus drop 先止血**，不要靠加机器硬扛。
...
游戏 Agent 要在 SIGTERM 里**刷完指标再走**（K8s 先 SIGTERM 再 SIGKILL，默认宽限 **30s**）。下游不认 ctx：至少本进程能停；选 HTTP 客户端必须能 `NewRequestWithContext`。
...
**读法**：pprof 里线程多 + cgroup `throttled_usec` 涨 → 对齐 GOMAXPROCS 与 limit（Go 1.19+ 多数自动，老集群要查）。RSS 涨**不要一上来只看 cpu.max**，先分 heap vs goroutine。
...
| 3 | 后台 ticker 写 Gauge | 大厅一抖，scrape 全超时，大盘空白 |
| 4 | label 是否封闭 | Prometheus OOM |
| 5 | 出站 HTTP/Redis timeout | exporter 自己先卡死 |
| 6 | `Notify` → `Shutdown` | 探针失败，SIGKILL |

读代码先搜 `MustRegister` / `promauto` 和 `label` 字符串——两步就能审高基数。Operator 一句话：**Watch CR → Reconcile → 写集群 → 失败 requeue；幂等是灵魂**。有状态游戏服自动扩要非常谨慎——Reconcile 加副本 ≠ 玩家能进。
...
**典型行为**：SIGTERM 后不再接新连接，in-flight scrape 在宽限内完成；超时 K8s 发 SIGKILL。

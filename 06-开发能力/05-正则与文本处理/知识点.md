...
```bash
grep 'a+b' file        # BRE：字面量 a+b
grep -E 'a+b' file     # ERE：一个或多个 a 后跟 b
grep -P '(?<=\")GET' file   # PCRE 后顾（GNU grep）
grep -F 'error+' file  # 固定字符串，不走正则
```

PCRE 进阶（Python）：`(?P<name>...)` 命名组；`(?:...)` 非捕获；`(?=)` `(?!)` 前瞻；`re.I/M/S`。

RE2 vs PCRE 面试口径：PCRE 功能全但可能回溯爆炸；RE2 牺牲部分特性换可预测性能。日志清洗在 Python 里写死字符类比「加超时」靠谱。
...
回溯在干什么：引擎为让整段正则成功，把已吞字符一点点吐回再试。Python 标准 `re` 不好按行超时——控制输入长度 + 字符类 + 先 grep 过滤坏行。
...
|--|-----|-----|
| 擅长 | 替换、删、插 | 分列、聚合、计算 |
| 典型 | 改 `max_connections` | Top IP、QPS、状态码分布 |

combined 时间列 `$4` 形如 `[28/Aug/2026:10:00:00`：

```
  [28/Aug/2026:10:00:00
   12345678901234
               ^^ hour → substr($4, 14, 2)
```

每分钟 QPS：`substr($4, 14, 5)` → `10:00`。跨时区 / 跨天：key 用完整日期+小时，不要只取 2 位小时。

RT（响应时间）若不在 `$NF`：先 `awk '{print $(NF-1), $NF}' | head` 确认，再 `sort -rn`。
...
| 场景 | jq | awk 硬拆 |
|------|-----|---------|
| kubectl Pod 状态 | `select(.status.phase != "Running")` | 字段错位 |
| JSON 行日志 | `select(.status >= 500) \| .client_ip` | 引号内空格炸列 |
| 嵌套数组 | `.items[].spec.containers[].image` | 几乎不可维护 |

```bash
# JSON 行 access — 5xx IP
jq -r 'select(.status >= 500) | .client_ip' access.jsonl | sort | uniq -c | sort -rn | head

# 错：awk 当空格分列
awk '{print $3}' access.jsonl   # 无意义
```

YAML 同理：grep `^image:` 漏多行写法 → `yq` 或 `yaml.safe_load`。
...
|--------|------|------|
| < 100MB | awk 直接计数 | — |
| 100MB–10GB | `grep '" 5[0-9][0-9]' \| awk ...` | 全文件进 Python list |
| 压缩日志 | `zgrep` / `zcat \| grep` | 先解压再 grep |
| > 10GB 常查 | 进 ES / Loki | 跳板机反复 sort |
| 纯过滤 | grep+awk（C 实现） | 复杂嵌套正则全量跑 |

百万行 Python：`re.compile` + `for line in f` + `finditer`。结构化输出 / 接 API 用 Python；纯 Top N 往往 Shell 更快。
...
```text
2026-08-28 10:00:01 NullPointerException at LoginHandler.java:142
2026-08-28 10:00:05 connection refused: redis:6379
```

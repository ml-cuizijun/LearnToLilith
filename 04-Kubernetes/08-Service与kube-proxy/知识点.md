...
**四种 Service 类型 + Ingress 边界：ClusterIP 对内，NodePort/LB 对外，Headless 给 STS。**
...
**Endpoints 空就不转发——名单由 EndpointSlice 控制器按 selector + Ready 填。**
...
**iptables 链表 O(n)；IPVS 哈希 O(1)；包都走内核不走 proxy 进程。**
...
**摘流靠 readiness + preStop，Service 对象自己不会等容器感觉好了。**

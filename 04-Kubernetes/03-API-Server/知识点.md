...
1. **先看码**：401 / 403 / 429 还是超时 / readyz 失败——各走不同格子。
...
**Mutating 先改对象，Validating 再拒；Webhook Fail + 服务挂 = 写路径焊死。**
...
**Watch Cache 在 API 内存挡大部分读，写仍必须过 etcd。**
...
**429 是 APF 队列满，找肇事客户端而不是加 inflight。**
...
**exec/logs 过 API 再升级到 kubelet :10250，不是直连 Pod IP。**

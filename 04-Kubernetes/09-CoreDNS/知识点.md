...
**本章不讲：** 公网 DNS / CDN → [02-10 DNS与CDN](../../02-基础底座/10-DNS与CDN/)；VIP 怎么转到 Pod → [08 Service](../08-Service与kube-proxy/)；Pod 之间三层 → [10 CNI](../10-CNI与网络策略/)；上 NP 后 DNS 挂的策略写法 → 10，本章只要求知道要放行 53；Cilium 替 kube-proxy 后的 DNS → [23](../23-eBPF与Cilium/)。

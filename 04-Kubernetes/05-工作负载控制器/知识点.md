...
**四种控制器的差别是期望怎么定义：可互换、固定身份、每节点、跑完就停。**
...
**Deployment 滚动是新建 RS 步进，等 Ready 不是等 Running。**
...
**StatefulSet 保证名/序/盘/DNS，但不解决内存里的对局。**
...
**PDB 只拦自愿中断，不拦宕机也不拦 HPA 改 replicas。**
...
**DaemonSet 每节点一个；Job 跑完就停，Completed 要 TTL。**
...
**1.28+ 原生 sidecar 靠 initContainers + restartPolicy Always 保证时序。**

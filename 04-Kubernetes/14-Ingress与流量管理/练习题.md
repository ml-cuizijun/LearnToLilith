...
**一句话**：Ingress 是七层规则对象，Controller 才是执行者；没有 Controller，etcd 里有规则也没有流量。
...
**一句话**：Ingress 局限在 HTTP 和厂商注解；Gateway API 用 GatewayClass/Gateway/Route 分层，新集群优先。
...
**一句话**：HTTP 商城走七层 Ingress/Gateway；战斗长 TCP 走 L4 NLB，七层 reload 会断流。
...
**一句话**：502 是后端立刻失败先看 Endpoints；503 无可用后端；504 是等后端超时。
...
**一句话**：金丝雀用 HTTPRoute weight 准；副本比例不准，5% 已坏就归零权重。
...
**一句话**：TLS 常在 Controller 终止；通配符 *.example.com 不匹配裸域 example.com。
...
**一句话**：无 ADDRESS 查 Controller/class/LB；多 Controller 用 ingressClassName 认领。
...
**一句话**：限流防打爆、熔断防级联、降级保核心；支付不能降级，POST 不能乱重试。

限流防打爆；熔断防级联；降级保核心路径；重试只对幂等自愈抖动；隔离让故障不扩散。五个互补。

支付 **不能** 降级，必须强一致。排行榜、好友推荐可以返回空 / 缓存 / 跳过。熔断 Open 时不要重试。重试限制次数 + 退避。不用 Mesh 也能做：入口限流 + SDK + 业务开关 + K8s 资源隔离。网格是增强。Cilium / eBPF 网格见 23。Istio sidecar 成本见 [10-04](../../10-SRE实践/04-大规模可靠性架构/)。
...
**一句话**：X-Forwarded-For 只信最外层 hop；配错会把所有请求看成 Controller IP。
...
**一句话**：413 调 body 上限；金丝雀 5xx 立即归零；流量镜像不能代替金丝雀，写路径会双写。

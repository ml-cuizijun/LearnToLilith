# 配置与密钥

```mermaid
flowchart LR
    a[1 概念] --> b[2 架构]
    b --> c[3 原理]
    c --> d[4 安装部署]
    d --> e[5 配置]
    e --> f[6 命令与操作]
    f --> g[7 排障]
    g --> h[8 常考]
```

值班把数据库密码放进 Secret，以为「K8s 加密了」。你 `kubectl get secret -o yaml` 再 `base64 -d` 就是明文。etcd 那章已经说了：Secret 进 etcd **默认是明文 JSON**，静止加密是 API Server 的 `EncryptionConfiguration`。群里另一次是改了 ConfigMap，支付开关没变——YAML 是 envFrom。

先别动手。这一章后半全是你会动手的：**怎么开静止加密、怎么轮换密钥、env 改了怎么滚动、CreateContainerConfigError 先看引用。** 前面的注入方式和 base64，是为了让你动手时不把 Secret 当保险箱、不把热更新幻想成改完立刻变。

**本章主线（配置就按这三步）：**

1. **注入**：env 必须滚动；volume 文件会变但进程要 reload；subPath 常不热更。
2. **Secret**：base64 不是加密；静止加密在 API Server EncryptionConfiguration。
3. **轮换**：新 Secret → 改引用 → 滚动 → 删旧；应急 edit 当天补 Git。

**读完你能：** 白板上画出 CM/Secret → etcd → 三种注入；能讲 EncryptionConfiguration 链；轮换走新对象；大表不进 etcd。

## 本章目录

- [1. 基本概念](#1-基本概念)
- [2. 架构图](#2-架构图)
- [3. 原理](#3-原理)
  - [3.1 三种注入](#31-三种注入热更新各不相同)
  - [3.2 Secret 不是保险箱](#32-secret-不是保险箱)
  - [3.3 EncryptionConfiguration](#33-encryptionconfiguration静止加密)
  - [3.4 游戏热更](#34-游戏热更-cm-只适合当指针)
- [4. 安装部署](#4-安装部署)
- [5. 核心配置](#5-核心配置)
- [6. 命令与操作](#6-命令与操作)
- [7. 生产排障](#7-生产排障)
- [8. 必会 · 常考](#8-必会--常考)
- [合上书](#合上书问自己)

**本章不讲：** etcd 磁盘、quorum、直连禁令 → [02 etcd](../02-etcd/)（02 只点到「静止加密在 12」）；RBAC 少给 get secret、PSS 读文件权限 → [13](../13-RBAC与安全/)；镜像仓库 401、containerd → [16](../16-节点运行时与升级/)；Helm values、checksum 模板 → [21](../21-Helm与Kustomize/)；GitOps 同步把热修盖回去 → [22](../22-GitOps-ArgoCD与Tekton/)；热更 / 灰度发布单 → [15](../15-灰度与回滚/)、08-03。Vault / KMS **产品怎么搭**不在本库单开。

---

## 1. 基本概念

**值班里什么时候碰到**：改了 ConfigMap 业务行为没变、Pod 起不来报 CreateContainerConfigError、有人把密码 base64 当加密、合规问 etcd 里 Secret 是不是明文、活动热更表太大 apply 失败。

ConfigMap 放非敏感，Secret 放敏感——**两者都进 etcd**。Secret 默认只是 **base64，不是加密**。`kubectl get secret -o yaml` 再 `base64 -d` 就是密码。etcd 未做 Encryption at Rest 时，能碰控制面数据目录 ≈ 能读所有 Secret。

单对象 **1MiB**（etcd `--max-request-bytes` 常见上限，对齐 02）。大表、技能配置不进 CM，放对象存储，CM 只留地址或版本指针。跨 NS **不能引用**，要复制或用外部同步（ExternalSecret 一类）。

| 在 CM / Secret 里 | 不在这里 |
|-------------------|----------|
| 非敏感配置 / 敏感值的对象，落 etcd | Vault 集群怎么搭 |
| 三种注入：env、volume、imagePullSecrets | Helm 模板、GitOps 流水线（21、22） |
| API Server 写 etcd 前的 EncryptionConfiguration | etcd 自己的 TLS（02） |

三个角色不要混：

| 角色 | 干什么 |
|------|--------|
| **ConfigMap / Secret** | 对象；Secret 类型 Opaque / tls / dockerconfigjson |
| **kubelet** | 把 volume 同步成文件；env 只在创建时写入 |
| **API Server EncryptionConfiguration** | 写 etcd 前加密、读时解密；**etcd 里不再是明文 JSON** |

`stringData` 提交后 API 存 `data`（base64）。Git 不要提交 stringData 明文「图省事」。base64 不是加密，进了 Git 就等于进了密码本。

> **记住**：CM/Secret 都进 etcd；Secret = base64。静止加密是 API Server 的 EncryptionConfiguration，不是 etcd 自己的开关。

---

## 2. 架构图

先把整张图钉住。后面安装、命令、排障都对着这张图说话。

```mermaid
%%{init: {"flowchart": {"padding": 18, "nodeSpacing": 30, "rankSpacing": 50}, "theme": "base"}}%%
flowchart TB
    subgraph src ["① 值从哪来"]
        git[Git · 只放引用]
        kms[KMS / Vault / 云密钥]
        git -.-> kms
    end

    subgraph api ["② API Server"]
        obj[ConfigMap / Secret]
        enc["EncryptionConfiguration<br/>写前加密 / 读时解密"]
        obj --> enc
    end

    subgraph etcd ["③ etcd · 02"]
        kv["/registry/secrets/...<br/>默认明文 JSON；开了加密才是密文"]
        enc --> kv
    end

    subgraph inj ["④ 进进程"]
        env["env / envFrom · 创建时写入"]
        vol["volume / projected · kubelet 同步文件"]
        pull[imagePullSecrets · 只给 kubelet 拉镜像]
    end

    obj --> env
    obj --> vol
    obj --> pull
    kms --> obj

    classDef a fill:#334155,stroke:#0F172A,stroke-width:2px,color:#fff
    classDef b fill:#2563EB,stroke:#1E3A8A,stroke-width:2px,color:#fff
    classDef c fill:#7C3AED,stroke:#4C1D95,stroke-width:2px,color:#fff
    classDef d fill:#0891B2,stroke:#155E75,stroke-width:2px,color:#fff
    class git,kms a
    class obj,enc b
    class kv c
    class env,vol,pull d

    style src fill:#F1F5F9,stroke:#334155,stroke-width:3px,color:#0F172A
    style api fill:#DBEAFE,stroke:#1D4ED8,stroke-width:4px,color:#1E3A8A
    style etcd fill:#EDE9FE,stroke:#6D28D9,stroke-width:4px,color:#4C1D95
    style inj fill:#CFFAFE,stroke:#0E7490,stroke-width:3px,color:#155E75
```

**读图三句话：**

1. **从「改了不生效」进图**：先分叉注入方式——env 必须滚动；volume 等 sync + 进程 reload；subPath 当不热更。
2. **从「以为加密了」进图**：base64 在 API 层就 decode 了；etcd 明文要靠 EncryptionConfiguration，不是 etcd TLS。
3. **从 CreateContainerConfigError 进图**：对象不存在 / key 错 / 错 NS，业务容器还没起，不要先看应用日志。

图上三条线：

1. **etcd TLS ≠ 静止加密。** 02 的 peer/client 证书管的是谁能连 2379；EncryptionConfiguration 管的是盘上 / 快照里 Secret 是不是明文。
2. **能 `get secret` 就能看明文**（对 API 来说解密后给你）。RBAC 少给 get / list secrets。
3. **进进程的方式决定改了热不热。** env 必须滚动；volume 文件会变，进程要 reload。

生产不要幻想「改完立刻所有进程变了」。kubelet 同步周期常见秒到分钟级（`configMapAndSecretChangeDetectionStrategy`，常见 Cache + TTL），刚 apply 立刻 exec cat 可能还是旧文件，等一轮再判断「热更新坏了」。immutable 对象 kubelet 不再 watch 原地变更，正好配合「只发新对象」。

---

## 3. 原理

原理只保留动手和面试会用到的：三种注入、为什么 Secret 不是保险箱、EncryptionConfiguration 怎么链、大表为什么不进 CM。

**本节 4 块：** 3.1 注入 · 3.2 保险箱 · 3.3 静止加密 · 3.4 游戏热更

### 3.1 三种注入：热更新各不相同

**一句话：** env 创建时写入不能热更；volume 文件会变但进程要 reload。

**打个比方**：env 像出生证上的名字——生出来就定了，改户口得重生（重建 Pod）；volume 像房间里的公告板——kubelet 会换纸，但员工（进程）得自己抬头看。

**现场走一遍**（改了支付开关 ConfigMap，业务行为没变）：

1. 确认注入方式：

```bash
kubectl get deploy payment -n game -o yaml | grep -A20 'envFrom\|volumeMounts'
```

2. 屏幕出现 envFrom：

```yaml
envFrom:
- configMapRef:
    name: payment-config
```

3. **说明什么**：这是 env 型注入，apply 新 CM 后 **进程环境仍是旧的**，必须滚动。

4. 验证 CM 已是新的、容器 env 仍是旧的：

```bash
kubectl get cm payment-config -n game -o yaml | grep PAYMENT_ENABLED
kubectl exec deploy/payment -n game -- env | grep PAYMENT_ENABLED
```

5. 屏幕对比：

```text
# CM 里
PAYMENT_ENABLED: "true"

# 容器 env 里（旧 Pod）
PAYMENT_ENABLED=false
```

6. 滚动：

```bash
kubectl rollout restart deploy/payment -n game
kubectl rollout status deploy/payment -n game
kubectl exec deploy/payment -n game -- env | grep PAYMENT_ENABLED
# 现在应该是 true
```

7. 如果是 **volume 型**，apply 后等 sync（常见 10～60 秒），再 cat：

```bash
kubectl exec deploy/payment -n game -- cat /etc/config/payment.yaml
# 文件可能已是新的，但进程仍用旧配置——要 reload 或同样滚动
```

**输出解读**：

| 方式 | 热更新 | 用途 |
|------|--------|------|
| env / envFrom | **否**，创建时写入进程环境 | 开关、简单键 |
| volume / projected | kubelet 同步文件（常见秒到分钟级） | 配置文件、证书 |
| imagePullSecrets | — | 拉私有镜像，不是业务 env |
| subPath 挂单文件 | 不少版本 **不会** 热更新 | 必须滚动 |

```mermaid
flowchart TB
    A[apply 新 CM]
    A --> B[etcd 里对象已经是新的]
    B --> C[envFrom：进程环境仍旧 · 必须新建 Pod]
    B --> D[volume：kubelet 过几秒到一分钟同步文件]
    D --> E[进程要自己 reload，否则行为仍旧]
    B --> F[subPath：不少版本文件也不变]

    classDef a fill:#2563EB,stroke:#1E3A8A,stroke-width:2px,color:#fff
    classDef b fill:#D97706,stroke:#92400E,stroke-width:2px,color:#fff
    class B a
    class C,E,F b
```

Helm checksum annotation / Reloader 可把「CM 变了」自动变成「滚 Pod」。checksum 在 Pod template 里，CM 一变就滚动——误改也会自动推全集群，活动中要冻结或只盯非生产。

**projected volume** 可把 CM、Secret、DownwardAPI 合成一个目录。SA token 也是 projected，1.24+ Bound Token 自动轮换。老的永不过期 Secret Token 少用：泄漏后无法靠过期自愈。

Secret 类型：Opaque、`kubernetes.io/tls`（`tls.crt` / `tls.key`）、`kubernetes.io/dockerconfigjson`。volume 建议 `defaultMode: 0400`。0400 只有属主读——容器 `runAsUser` 对不上文件属主就会 Permission denied，Events 不一定点名 Secret。

**面试怎么说**：「env 改 CM 必须 rollout restart；volume 文件会变但进程要 reload；subPath 我默认当不热更。不说凡是 volume 都会热更。」

> **记住**：env 必须滚动；volume 文件会变但进程要 reload；subPath 常常不热更。

### 3.2 Secret 不是保险箱

**一句话：** Secret 默认只是 base64，能 get 就能看明文。

**打个比方**：Secret 像把密码写在信封里——信封（base64）不等于保险柜，有权限拆信（get secret）的人一眼就能读。

**现场走一遍**（新人问「密码放 Secret 安全吗」）：

1. 直接 decode：

```bash
kubectl get secret db-pass -n game -o jsonpath='{.data.password}' | base64 -d; echo
```

2. 屏幕出现：

```text
Pr0d_MySQL_2026!
```

3. **说明什么**：能 `get secret` 就能看明文。base64 不是加密，只是编码。

4. 看谁有权限：

```bash
kubectl auth can-i get secrets --as=system:serviceaccount:game:default -n game
kubectl auth can-i list secrets --as=dev-user -n game
```

5. 生产补丁叠在一起：
   - RBAC **少给 get / list secrets**（13）
   - etcd Encryption at Rest（下一节）
   - Git 里只放 ExternalSecret / Sealed 引用
   - 审计 `get secret`
   - immutable Secret / CM：禁止原地改

**业务密码轮换四步**（顺序不能反）：

```
1. 新建 Secret（新密码）
2. 改引用（Deploy / STS）
3. 滚动完成，验证连接
4. 删旧 Secret
```

反序会有一批 Pod 仍拿旧密码打新库。反例：生产 `kubectl edit secret` 改密码，Git 仍是旧的，下次 Argo CD 同步把新密码盖回去。

1MiB 打满：etcd 写失败，apply 报 too large。拆文件或改对象存储。

**面试怎么说**：「Secret 是 base64 不是加密，能 get 就能 decode。生产叠 RBAC + EncryptionConfiguration + 值放 KMS + 审计。密码轮换走新对象，不要原地 edit。」

> **记住**：Secret = base64；生产叠 RBAC + etcd 加密 + 值放 KMS + 审计。

### 3.3 EncryptionConfiguration：静止加密

**一句话：** 静止加密在 API Server 写 etcd 前，不在 etcd 进程里。

**打个比方**：API Server 像银行柜台——钱（Secret）进金库（etcd）前先装密封袋（加密）；etcd 只管存袋子，不管袋子里面是什么。

**现场走一遍**（合规问「etcd 里 Secret 是不是明文」）：

1. 看 API Server 是否配了加密：

```bash
# Stacked 控制面，在控制面节点
grep encryption-provider-config /etc/kubernetes/manifests/kube-apiserver.yaml
```

2. 屏幕出现：

```yaml
- --encryption-provider-config=/etc/kubernetes/enc.yaml
```

3. 维护窗口，只读抽查 etcd（02 禁止生产 put）：

```bash
ETCDCTL_API=3 etcdctl get /registry/secrets/game/db-pass --print-value-only | head -c 200
```

4. **开了加密**应看到乱码 / 前缀标识（常见 `k8s:enc:aescbc:v1:`），**不是** `"apiVersion": "v1"` 明文 JSON。

5. **但** `kubectl get secret` 仍能看明文——API 读时解密后给你。开了加密 ≠ get 不到。

**配置示意**：

```yaml
apiVersion: apiserver.config.k8s.io/v1
kind: EncryptionConfiguration
resources:
  - resources:
    - secrets
    providers:
    - aescbc:
        keys:
        - name: key2          # 新密钥必须排第一：新写用它
          secret: <base64-32-bytes>
        - name: key1          # 旧密钥留下：还要能解密存量
          secret: <base64-old>
    - identity: {}            # 最后：解密尚未加密的旧明文
```

**输出解读** — provider 链规则：

| provider | 干什么 | 生产 |
|----------|--------|------|
| `identity` | 明文 | 必须留在链尾；**放第一 = 新写又变明文** |
| `aescbc` | AES-CBC | 常见；密钥 **32 字节** |
| `aesgcm` | AES-GCM | 比 CBC 更现代 |
| `kms` / `kmsv2` | 信封加密，DEK 由 KMS 管 | 托管 / 要轮换密钥不碰静态文件时优先 |

链的规则：

1. **第一个能加密的 provider 负责新写。**
2. **整条链都能用来解密**——旧 key 必须留到存量都改写完。
3. **不要删仍需要解密的 provider。**
4. 轮换：新 key 插到列表第一 → 重启全部 API → rewrite 存量 → 确认后再撤旧 key。
5. 改完必须 **重启所有 API Server**。只改文件不重启，进程还拿着旧链。

**现场走一遍**（轮换 encryption key）：

```bash
# 1. 新 key 放 providers 第一
# 2. 滚动重启全部 API（一台 ready 再下一台）
# 3. 维护窗口 rewrite
kubectl get secrets --all-namespaces -o json | kubectl replace -f -
# 4. etcdctl 抽查已是密文
# 5. 确认存量都改写完，再撤旧 key
```

**面试怎么说**：「静止加密配在 kube-apiserver 的 EncryptionConfiguration，不在 etcd.yaml。新 key 放第一，identity 放最后，改完重启全部 API。轮换不能立刻删旧 key，存量还要靠它解密。」

> **记住**：静止加密在 API Server，不在 etcd 进程。新 key 放第一；identity 放最后；重启全部 API；旧 key 留到 rewrite 完成。

etcd 快照一旦带着明文 Secret 流出，事后再开加密也救不回那份旧快照——旧备份仍按当时的明文 / 旧 key 理解。

### 3.4 游戏热更：CM 只适合当指针

**一句话：** 大表不进 CM；热更指针和 Deploy 滚动是两条通道。

**打个比方**：ConfigMap 像菜单上的「今日特供编号」——编号（指针）可以热改，但整本菜谱（技能表）塞不进菜单牌（1MiB 上限），得去仓库（对象存储）取。

**现场走一遍**（运营要把整张技能表塞进 ConfigMap）：

1. apply 失败：

```bash
kubectl apply -f skill-table-cm.yaml
```

2. 屏幕出现：

```text
Error from server: ConfigMap "skill-table" is invalid: data: Too long: must have at most 1048576 bytes
```

3. **说明什么**：单对象 **1MiB** 上限（对齐 etcd `--max-request-bytes`）。

4. 正确做法：对象存储放表 + 进程内 watch / reload，CM 只存版本号：

```yaml
data:
  SKILL_TABLE_VERSION: "v20260828"
  SKILL_TABLE_URL: "s3://game-config/skill/v20260828.json"
```

5. 热更成功、Deploy 滚动失败可以并存——脚本热更了，进程没换，下一次 Deploy 仍会杀进程。两套通道要写在同一张发布单上（15）。

**面试怎么说**：「大表放对象存储，CM 放指针。热更改指针不等于 Deploy 滚动，两套通道写一张单。逻辑服表结构变了仍要按热更方案走，不是 K8s 自动完成。」

> **记住**：大表放对象存储，CM 放指针；热更 ≠ 滚动，两套通道写一张单。

---

## 4. 安装部署

CM / Secret 是 API 内置资源，不用单独装。要装的是静止加密、ExternalSecret、以及你选的 Reloader。

| 项 | 选 | 不选 |
|----|----|------|
| 静止加密 | 所有 API Server 同一份 EncryptionConfiguration | 只改一台、不重启 |
| 外部值 | ExternalSecret / Sealed + KMS | Git 明文 stringData |
| 验收 | `get secret` 仍可读（API 解密）；etcd 里对应 key 不是明文 JSON | 以为 get 不到就是加密成功 |

托管：ssh 不进 API 机器时，问清 Encryption at Rest 是否默认开、谁轮换 key。

rewrite 存量不是「再 apply 一遍 YAML 就一定重写」。常用迫写：`kubectl get secrets --all-namespaces -o json | kubectl replace -f -`（维护窗口，会打 API）。只加密 `secrets` 资源即可；把 Pod 也加密收益小、密钥轮换面更大。

---

## 5. 核心配置

生产清单，缺一项就当还没上完密钥：etcd 加密、ExternalSecret、审计 get secret、volume 0400、immutable 或 checksum 滚动、大文件不进 CM、禁止 Git 明文。

| 项 | 选 | 不选 |
|----|----|------|
| env 变更 | rollout restart | 以为 apply CM 进程就变了 |
| volume | 确认进程 reload；subPath 当不热更 | 「凡是 volume 都会热更」 |
| Secret 权限 | 0400 + 对上 runAsUser | 0777 |
| 跨 NS | 复制或 ExternalSecret | 改引用路径 |
| 活动开关 | 不要每秒 apply | 高基数打 etcd |
| Reloader | 活动中冻结或只盯非生产 | 误改自动推全集群 |
| identity | 链尾 | 链头（等于关加密） |

Helm 值渲染出来仍是 CM / Secret，包管理和 GitOps 在 21、22，这里不展开。

> **记住**：起不来先看引用；改了不生效先分 env / volume / subPath。

---

## 6. 命令与操作

值班先跑这几条：

```bash
kubectl describe pod <p> -n <ns>            # CreateContainerConfigError 看 Events
kubectl exec <p> -n <ns> -- env | grep KEY
kubectl exec <p> -n <ns> -- cat /path/to/file
kubectl rollout restart deploy/<d> -n <ns>  # env 型变更后必做
kubectl get secret db-pass -n <ns> -o jsonpath='{.data.password}' | base64 -d
```

| 目的 | 命令 | 正常 | 异常 |
|------|------|------|------|
| 引用失败 | `describe pod` Events | 容器 Creating | Secret/CM 不存在、key 错、错 NS |
| env 是否新 | `exec env` | 与 CM 一致 | apply 成功但 env 旧 = 没滚动 |
| 文件是否新 | `exec cat` | 与 CM 一致 | 立刻 cat 可能还旧；等一轮 |
| 滚动 | `rollout restart` / `rollout undo` | 新 Pod Ready | checksum 在 template 里 undo 会带回旧引用 |
| 静止加密 | 维护窗口 etcdctl get `/registry/secrets/...` | 密文前缀 | 明文 JSON = 没开或 identity 在第一 |
| API 配置 | apiserver yaml 里 `--encryption-provider-config` | 三台都有、路径一致 | 缺一台 = 读写不一致 |

### 开静止加密

1. 生成 32 字节 key，写成 EncryptionConfiguration；`aescbc`（或 kms）在前，`identity` 在最后。
2. 分发到 **每台** 控制面，apiserver 加 `--encryption-provider-config`。
3. 滚动重启全部 API Server（一台 ready 再下一台，对齐 03）。
4. rewrite 存量 Secret：读出再 replace，迫使用新 provider 写回。
5. 用 etcdctl 只读抽查不再是明文 JSON。

### 轮换 encryption key

新 key 插入 providers **第一** → 重启全部 API → rewrite 存量 → 确认 → 再撤旧 key。永远不要先删旧 key。

### 业务密码轮换

新建 Secret → 改 Deploy/STS 引用 → 滚动验证 → 删旧。不要原地 edit 当日常。应急 edit 当天补 Git。

### 配置变更标准动作

1. Git 改 CM/Secret（或 ExternalSecret）。
2. `kubectl diff` / 看 checksum 是否变。
3. env 型：rollout restart；volume 型：确认进程 reload 或同样滚动。
4. exec 核对容器内值和 Git 一致。
5. 失败：undo Deploy + 回上一版 CM。

先 apply 配置再发工作负载。CreateContainerConfigError 时业务容器还没开始，不要先看应用日志。

---

## 7. 生产排障

```mermaid
%%{init: {"flowchart": {"padding": 16, "nodeSpacing": 28, "rankSpacing": 48}, "theme": "base"}}%%
flowchart TB
    subgraph gate ["先分叉"]
        A[配置 / 密钥出问题] --> B{现象}
    end

    subgraph start ["起不来"]
        B -->|CreateContainerConfigError| C[对象不存在 / key 错 / 错 NS]
    end

    subgraph hot ["改了不生效"]
        B -->|行为没变| D{注入方式}
        D -->|env| E[必须滚动]
        D -->|volume| F[等 sync + 进程 reload]
        D -->|subPath| G[当不热更，滚动]
    end

    subgraph sec ["密钥"]
        B -->|以为加密| H[base64 ≠ 加密；查 EncryptionConfiguration]
        B -->|GitOps 盖回旧密码| I[当天补 Git / immutable]
        B -->|too large| J[拆 CM 或改对象存储]
    end

    classDef bad fill:#DC2626,stroke:#7F1D1D,stroke-width:2px,color:#fff
    classDef warn fill:#D97706,stroke:#92400E,stroke-width:2px,color:#fff
    class C,H bad
    class E,F,G,I warn

    style gate fill:#DBEAFE,stroke:#1D4ED8,stroke-width:4px,color:#1E3A8A
    style start fill:#FEF2F2,stroke:#DC2626,stroke-width:4px,color:#7F1D1D
    style hot fill:#FEF3C7,stroke:#D97706,stroke-width:4px,color:#92400E
    style sec fill:#EDE9FE,stroke:#6D28D9,stroke-width:4px,color:#4C1D95
```

| 现象 | 先做什么 | 不要做什么 |
|------|----------|------------|
| CreateContainerConfigError | describe Events；先 apply 配置再发工作负载 | 先看应用日志 |
| 改了不生效 | 是否 env；volume 是否等了 sync；进程是否 reload；是不是 subPath | 怪 kubelet 坏了 |
| 权限太死 | 0400 + 非 root 读不了，对 runAsUser | 改 0777 |
| 跨 NS | 复制对象或 ExternalSecret | 改引用路径 |
| ImagePullBackOff | 先分清 runtime 还是 imagePullSecrets（16） | 先改业务 env 里的仓库密码 |
| apply too large | 拆 CM 或改对象存储 | 加大 etcd 单次写入硬顶活动表 |
| 密码放 Secret 就安全 | 叠 RBAC + EncryptionConfiguration + KMS + 审计 | 单靠 Vault 章或单靠 base64 |
| checksum 把坏开关推全集群 | undo Deploy + 回上一版 CM | 只回其中一个通道 |
| 开了加密 get secret 仍能看 | 正常，API 解密后给你 | 以为没开成功就关 identity 放到第一 |

**真实现象举例**：支付 Deploy envFrom 型，运营 apply 了新 CM 说「开关开了但还在收单」——exec env 仍是 `false`，rollout restart 后恢复。不要怪 kubelet。

---

## 8. 必会 · 常考

白板和追问高频，能一口回：

| 说法 | 对错 | 口径 |
|------|:----:|------|
| Secret 进 etcd 默认加密 | 错 | 默认明文 JSON；静止加密在 API Server |
| base64 就是加密 | 错 | 能 get 就能 decode |
| etcd TLS 等于静止加密 | 错 | TLS 管谁能连；EncryptionConfiguration 管盘上是不是明文 |
| EncryptionConfiguration 配在 etcd.yaml | 错 | kube-apiserver `--encryption-provider-config` |
| identity 放第一更安全 | 错 | 新写又变明文 |
| 新 key 追加在列表最后 | 错 | **第一**才负责新写 |
| 只改 pem/yaml 不重启 API 就加密了 | 错 | 必须重启全部 API Server |
| 轮换时立刻删旧 key | 错 | 存量还要靠旧 key 解密 |
| env 改 CM 立刻变 | 错 | 必须滚动 |
| 凡是 volume 都会热更 | 错 | 进程要 reload；subPath 常常不热更 |
| 跨 NS 改个路径就能引用 Secret | 错 | 复制或 ExternalSecret |
| Git 可以放 stringData 图省事 | 错 | API 仍存 data；明文进了密码本 |
| imagePullSecrets 就是业务 env 里的仓库密码 | 错 | 给 kubelet 拉镜像 |
| 大表塞进 CM | 错 | 1MiB 上限；放对象存储，CM 放指针 |
| 单靠 Vault 章就不用懂这章 | 错 | 值不进 Git 仍要懂注入、滚动、静止加密 |
| 生产长期 edit secret 不补 Git | 错 | 下次同步盖回去 |

数字：单对象 **1MiB**；kubelet 同步常见 **秒到分钟**；aescbc key **32 字节**；SA Bound Token 自 **1.24+**。

易混：env vs volume vs subPath；etcd TLS vs EncryptionConfiguration；get secret 能看 vs 盘上明文；热更指针 vs Deploy 滚动；imagePullSecrets vs 业务密码；identity 链头 vs 链尾。

---

## 合上书，问自己

1. Secret 默认加密吗？谁能看明文？静止加密配在哪个进程？
2. EncryptionConfiguration 链：新 key 放哪、identity 放哪、为什么要重启全部 API？
3. env / volume / subPath 改了各怎样才生效？
4. 密码轮换四步？生产 edit secret 第二天会怎样？
5. 1MiB 限制为什么存在？游戏热更大表放哪？
6. CreateContainerConfigError 先看什么？跨 NS 为什么不行？

六题都能口述，这一章就过了。下一章把 RBAC 和安全边界剖开：少给 get secret，PSS 怎么管文件权限。

...
**一句话**：Secret 默认是 etcd 明文 JSON；静止加密在 API Server EncryptionConfiguration。
...
**一句话**：env 改 CM 必须滚动；volume 文件会变但进程要自己 reload；subPath 常不同步。
...
**一句话**：CreateContainerConfigError = 引用的 CM/Secret 不存在或 NS 不对；不能跨 NS 引用。
...
**一句话**：轮换顺序：新 Secret → 改引用 → 滚动 → 删旧；生产 edit 当天必须补 Git。
...
**一句话**：CM 单对象约 1MiB；大表放对象存储，CM 只放指针，不要每秒 apply。
...
**一句话**：Secret volume `0400` 要对上容器 runAsUser/fsGroup，否则非 root 读不了。
...
**一句话**：imagePullSecrets 给 kubelet 拉镜像，和业务 env 里的仓库密码不是一回事。
...
**一句话**：1.24+ 用 Bound Token 自动轮换；projected 可合成 CM/Secret/SA token 目录。
...
**一句话**：stringData 进 Git 仍是明文密码本；用 Sealed/ExternalSecret，不要图省事。
...
**一句话**：配置改坏：undo Deploy 与回 CM/reload 是两条通道，发布单要写清回滚路径。
...
**覆盖**：必会 · 常考
**对应**：知识点 §3.3 EncryptionConfiguration
...
**一句话**：加密链：新 key 放第一、identity 放最后、重启全部 API、rewrite 后再撤旧 key。

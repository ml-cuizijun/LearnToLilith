...
**一句话**：MySQL/InnoDB 是游戏权威账本，Redis 只是缓存；写成功 = 页 + redo（双 1）+ binlog 两阶段对齐。
...
**一句话**：B+ 树叶存数据、范围 O(log n + k)；聚簇存整行，二级索引要回表，覆盖索引可消随机 IO。
...
**一句话**：联合索引 (a,b,c) 必须从 a 连续用；跳列、范围后面的列、函数和左模糊都会让索引失效。
...
**一句话**：RR 靠 ReadView + undo 保快照读一致；当前读靠 Next-Key 挡插入，库存扣减不能用普通 SELECT。
...
**一句话**：redo 扛崩溃恢复物理页，binlog 管复制和 PITR，两阶段对齐；支付库必须双 1，Buffer Pool 太小加从库写不会更快。
...
**一句话**：主从延迟看 GTID 两 Set 和大事务，不是 Seconds_Behind_Master；半同步会退异步，该读主的接口必须读主。
...
**一句话**：备份 = 异地全量 + binlog 归档 + restore 演练；PITR 停写重放到误操作前，游戏回档还要对齐 Redis。
...
**一句话**：大表 DDL 禁止高峰锁表，优先 INSTANT/gh-ost；升级先升从再切主，数据目录升过不能降二进制。
...
**一句话**：慢 SQL 先 EXPLAIN/锁/盘，延迟查 GTID 和大事务，MDL 找长事务和 DDL，不要先重启 mysqld。
...
**一句话**：死锁看 INNODB STATUS；无索引 UPDATE 在 RR 下 Next-Key 锁间隙，等于锁表。
...
**一句话**：写后立即读的接口必须读主；Seconds_Behind_Master=0 不等于这条事务已回放到从库。
...
**一句话**：百万行单事务发奖会让从库回放更久，KILL 整事务回滚可能更惨；应拆批可重入，加从消化不了事务内部。
...
- [ ] 能说出谁是权威账本，以及 Buffer Pool / redo / binlog 写路径
- [ ] 能讲聚簇、回表、覆盖、最左前缀；EXPLAIN 先看哪四列
- [ ] 能把 RR 讲到快照读 vs 当前读；库存不能普通 SELECT
- [ ] 能说出双 1 掉电丢什么、GTID 两个 Set、半同步会退异步
- [ ] 能区分 3 丢主 vs 误 DELETE；备份 = 异地 + 演练；游戏回档对齐 Redis
- [ ] 能说两千万行走 INSTANT/gh-ost；支付库不能 flush_log=2

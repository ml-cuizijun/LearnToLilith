...
- [ ] 能说出 Mongo vs MySQL 怎么选，以及应用连 URI / mongos
- [ ] 能画出副本集 vs 分片，并说明每片仍是副本集
- [ ] 能讲 w:1 / majority、读 secondary 的坑、Arbiter 不算数据副本
- [ ] 能选 shard key，并说明单调键和难改
- [ ] 能拆玩家文档：邮件独立 + TTL；数组不推向 16MB
- [ ] 能先看 COLLSCAN 和 WT cache，而不是加 CPU
- [ ] 能说明备份 = 快照/dump/oplog + 与 Redis/MySQL 对齐；升级滚动、回滚靠快照

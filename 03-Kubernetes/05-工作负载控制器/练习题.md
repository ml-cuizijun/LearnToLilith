...
开始做题前：合上知识点，对着**第一节的主图**，把「一次发布的一生」口述一遍——apply → 新 RS 出生 → 新 Pod 逐个 Ready → 旧 Pod 退场 → 旧 RS 缩 0，再标出 pause、deadline、undo、PDB 四个岔口。讲得出来再做题。
...
题目顺序：Q1～Q2 选型与调和；Q3～Q6 滚动与零停机；Q7～Q9 pause/undo/PDB；Q10～Q14 归属与综合定责。
...
卡在 Ready：ProgressDeadlineExceeded 不会自动 undo。
...
- [ ] **默画发布主图与四个岔口**。（Q1）
- [ ] **口述选型与为何不手建 RS**。（Q1、Q2）
- [ ] **口述 level-triggered 与删 Pod**。（Q2）
- [ ] **口述滚动参数与零停机**。（Q3～Q6）
- [ ] **口述 pause / undo / 回滚路**。（Q7、Q8）
- [ ] **口述 PDB 拦与不拦**。（Q9）
- [ ] **口述归属 Helm/HPA/Git**。（Q10、Q11）
- [ ] **定责三现场**。（Q12～Q14）

能拦住「删 Pod / 默认滚战斗服 / 删 PDB」三类误操作，这章过关。

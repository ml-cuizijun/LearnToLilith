...
> 发版当天，网关滚动卡在 50%，有人 `kubectl delete pod` 想「让它重建一下」；战斗服写成了 Deployment 默认滚，三百个对局同时被 SIGTERM；另一头 drain 卡了四十分钟，有人上来就要删 PDB。
> **本章回答**：四类控制器怎么选，一次滚动发布从 apply 到新 Pod 全量顶替旧 Pod 会经历哪几站，卡住、暂停、回滚各走哪条路，PDB 拦的是什么。
> **不讲**：三探针、preStop、优雅退出 → [04](../04-Pod生命周期/)；HPA 指标与公式 → [07](../07-弹性伸缩/)；蓝绿/金丝雀策略 → [16](../16-灰度与回滚/)；Chart 结构与模板渲染 → [22](../22-Helm与Kustomize/)；GitOps 流水线 → [23](../23-GitOps-ArgoCD与Tekton/)；PVC Retain 与丢盘 → [11](../11-存储/)；污点打分与 Pending → [06](../06-调度机制/)。
...
两者**不能同时为 0**（新旧都不动，滚动原地踏步），也不能让副本数掉到 0 以下或超出约束。这两个参数怎么组合出「零停机」，是整章最容易被追问的细节 🔥。
...
- [ ] **默画**一节的主图：一次发布的一生——apply → 新 RS → 新 Pod 逐个 Ready → 旧 Pod 退场 → 旧 RS 缩 0 保留，标出 pause、deadline、undo、PDB 四个岔口。
- [ ] 口述选型：四类期望状态各对应哪个控制器；登录服/网关/战斗服/日志采集各选什么、为什么；为什么不能手建 ReplicaSet。
- [ ] 口述调和：level-triggered 是什么意思；为什么删 Pod 没用；Ready 和 Running 差在哪，滚动和 PDB 各按哪个计数。
- [ ] 口述滚动：maxSurge/maxUnavailable 两种组合的差别；为什么等 Ready；minReadySeconds 防什么；ProgressDeadlineExceeded 之后会发生什么、不会发生什么。
- [ ] **默画**「零停机发布凭什么」四组清单：新 Pod 靠得住 / 旧 Pod 退得干净 / 留好回头路 / 外面有人护。
- [ ] 口述停与退：pause 的两个用途；undo 的机制（放大旧 RS）；latest 和 `revisionHistoryLimit=0` 怎么把回滚路走死；只回滚 Deployment 什么时候不够。
- [ ] 口述保护：PDB 拦哪三类、不拦哪三类；drain 卡死的处理；单副本 `minAvailable=1` 卡死为什么是保护；和 HPA 的 minReplicas 什么关系。
- [ ] 口述归属：kubectl edit Helm 对象会发生什么；`helm rollback` 和 `rollout undo` 为什么不能混用；HPA 和 Git 抢 replicas 的三种解法。
- [ ] 定责演练：滚动卡 50%、drain 卡死、战斗对局成片掉，各走决策树哪条分支、第一刀砍在哪。

九条都能不看稿讲下来，这一章就过了。下一章拆调度：Pending 到底是资源、污点还是硬反亲和。

...
> 战斗服扩容，新节点上的 Pod 全部卡在 ContainerCreating，Events 里刷着 `FailedCreatePodSandBox`，有人翻了半小时应用镜像日志——业务容器根本还没启动；另一边有人给 pay 命名空间上了 default-deny，全集群 `nslookup` 秒挂、ping IP 却通，有人已经在准备重启 CoreDNS。
> **本章回答**：一个 Pod 网络的一生——CNI 在 Pod 生命周期的哪一步被调、做了哪几件事，Flannel/Calico/Cilium 各修什么路，三张网段（Pod CIDR/Service CIDR/Node CIDR）各归谁管，NetworkPolicy 怎么放行与拦截；网络出问题时按生命周期定责。
> **不讲**：Service VIP 与 kube-proxy → [08](../08-Service与kube-proxy/)；CoreDNS 解析与 ndots → [09](../09-CoreDNS/)；Ingress 南北向七层 → [15](../15-Ingress与流量管理/)；Cilium eBPF 深机制、Hubble、替代 kube-proxy → [25](../25-eBPF与Cilium/)；抓包与 PMTUD 深排障 → [01-16](../../01-基础底座/16-网络排障与抓包/)。
...
**CNI 是规范不是程序**：运行时按环境变量调用 `/opt/cni/bin` 下的插件二进制，配置读自 `/etc/cni/net.d`，插件干完活就退出。三个动词：**ADD**（给 Pod 配网络）、**DEL**（回收）、**VERSION**（能力协商）。
...
三个易错点：「拒绝」从来不用显式写，**选中即拒**；方向相互独立，被 ingress 策略选中不影响 egress；多条 NP 永远取并集，**没有交集**——想「叠一条例外减掉」是做不到的。
...
1. **默画一节主图**：Pod 网络一生五阶段（DNA → 出生 → 上路 → 立规 → 死亡），每个阶段说出一个值班现象和第一刀。
2. **口述 DNA**：三原则；Pod/Service/Node 三张 CIDR 各归谁管；为什么 NS 不是安全边界；hostNetwork 的四个代价。
3. **口述出生**：CNI 在哪一步被调（sandbox 后、业务前）；ADD 四件事；FailedCreatePodSandBox 的三种定责。
4. **口述上路**：同节点怎么走；Overlay 封装 vs BGP 真路由；安全组三口（4789/协议4/179）；MTU 减多少、大包丢长什么样。
5. **口述选路**：Flannel/Calico/Cilium 差异；为什么裸 Flannel 的 NP 是空转；Calico 三种数据面。
6. **口述立规**：无策略全通/选中默认拒/并集；三种选择器；为什么先放 53；改 NP 要不要滚动。
7. **口述死亡**：DEL 回收什么；agent 挂了存量与新各怎样；为什么不能重启业务。
8. **口述定责**：60 秒剧本；没 IP / 同节点 / 跨节点三个入口；名字挂 IP 通去哪查。
9. **口述数字**：4789 / 协议4 / 179 / 1450 / 110 / 80%。

下一章：[11](../11-存储/) —— PV/PVC/SC 与丢盘。

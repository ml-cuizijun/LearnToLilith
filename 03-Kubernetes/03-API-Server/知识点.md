...
> **本章回答**：一次 API 请求的一生——APF 限流 → 认证 → 鉴权 → 准入 → 校验 → 写 etcd，每一段会坏成什么样、看到一个状态码怎么一刀定责。
> **不讲**：etcd 的 Raft / 备份恢复 / defrag → [02](../02-etcd/)；RBAC 细则 / PSS / 配额 → [13](../13-RBAC与安全/)；证书体系与 kubeconfig → [14](../14-证书与kubeconfig/)；各组件挂掉的总影响面 → [01](../01-集群架构/)。
...
- [ ] **默画一节的主图**：一次请求的一生——APF → 认证 → 鉴权 → Mutating → 校验 → Validating → 写 etcd → watch 推送，并说出每关的死法（429 / 401 / 403 / 拒绝）。

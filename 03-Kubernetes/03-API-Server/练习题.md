...
开始做题前：合上知识点，对着**第一节的主图**，把「一次 API 请求的一生」口述一遍——APF → 认证 → 鉴权 → Mutating → 校验 → Validating → 写 etcd。能顺下来，状态码定责题就有了地图。

每道题先看场景自己答，再对「完整解答」。直接看答案容易产生「我会了」的错觉。
...
题目顺序：Q1～Q2 钉主线和影响面；Q3～Q6 把 401/403/准入/Fail 焊死拆开；Q7～Q10 是 APF、watch、聚合、审计；Q11～Q14 综合定责。
...
Q1 画完门，这道题问门全倒时什么还在——和 01/02 章同一套三问。
...
和 02 章盘问题分家——读慢常在 cache，不在 etcd。
...
扩展入口：别把 metrics-server 挂了说成 API 挂了。
...
14 道题做完，按知识点「合上书」逐条自查（题号已标）：

- [ ] **默画主图**：七道关 + 每关死法。（Q1）
- [ ] **口述进门与影响面**：唯一入口、readyz、全挂时什么坏什么不坏。（Q1、Q2、Q12）
- [ ] **口述 401 / 403 口诀与排查**。（Q3、Q4）
- [ ] **口述准入 Fail 焊死与部署纪律**。（Q5、Q6）
- [ ] **口述 APF 429 三步处置**。（Q7）
- [ ] **口述 watch cache 与 too old**。（Q8）
- [ ] **口述聚合 API / 审计要点**。（Q9、Q10）
- [ ] **定责树 + 60 秒剧本**。（Q11～Q14）

能拦住「给 API 加核 / 重启 etcd」的误操作，这章才算过。

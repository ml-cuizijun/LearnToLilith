...
> 开服秒进高峰，大厅登录 P99 从 80ms 跳到 2s，报错全是「业务域名解析超时」；值班看 CoreDNS CPU 飙满，第一反应是加 10 个副本——越加越慢；群里又有人喊「重启逻辑服」，一重启，全场域名解析抖成一片。
> **本章回答**：一次 `svc.cluster.local` 解析的一生——resolv.conf 从哪来、查询怎么到 CoreDNS、Corefile 插件链怎么答、外部域名怎么出门；NodeLocal DNSCache 为什么必要；CoreDNS 挂了/慢了的影响面，扩缩容与监控怎么做。
> **不讲**：VIP 怎么转到 Pod、iptables vs IPVS → [08](../08-Service与kube-proxy/)；Pod IP 从哪来、NetworkPolicy 写法 → [10](../10-CNI与网络策略/)；公网权威 DNS / CDN → [01-15](../../01-基础底座/15-DNS与CDN/)；conntrack 深机制 → [01-17](../../01-基础底座/17-iptables与conntrack/)；Cilium 替 kube-proxy 后的 DNS → [25](../25-eBPF与Cilium/)。
...
1. **默画一节主图**：resolv.conf → kube-dns VIP → 插件链（内问 kubernetes / 外走 forward）→ 答案，NodeLocal 这条捷径画在哪。
2. **口述出生**：resolv.conf 谁写的、三个字段各从哪来、ndots 默认 5 的用意与副作用。
3. **口述出门**：Service 为什么叫 kube-dns、Endpoints 摘空会发生什么、查询走 UDP 为什么更要命。
4. **默画插件链收束图**：cache / kubernetes / loop / forward 的位置与分工，背出四句清单；说清插件顺序为什么与书写无关。
5. **口述外域与放大**：`api.pay.com` 在 ndots:5 下放大成几次、公式是什么、两个治本修法。
6. **口述本地化**：NodeLocal DNSCache 治什么（表满 / insert race / 热查询）、不治什么（ndots 垃圾）、凭什么地址监听。
7. **口述生死**：CoreDNS 挂了哪些会断哪些不断、为什么别重启逻辑服、慢了为什么表现为 5 秒级超时。
8. **口述供养**：打满四层各看什么签名、HPA 为什么要按有效 QPS、监控五条指标。
9. **定责**：全断 / 偏科（内坏、外坏、Headless 空）/ 间歇超时，各走决策树哪条分支、第一刀敲什么。

下一章：[10 CNI与网络策略](../10-CNI与网络策略/)——Pod IP 从哪来，以及为什么上 NP 第一天要先放行 53。

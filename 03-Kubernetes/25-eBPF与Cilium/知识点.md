# eBPF 与 Cilium

> 大家好。上一章讲完 Mesh sidecar；这一章换到**内核数据面**——eBPF 在 hook 上转发，Cilium agent 只下发 map，不经手每一包。
> 开讲之前，先带大家回到几个现场：集群已经 8000 个 Service，有人把 Cilium 讲成「在用户态拦包」；另一人担心 eBPF 会像 ko 一样把机器打挂；某节点 cilium-agent CrashLoop，战斗服还在，新登录失败，有人要重启战斗进程；NetworkPolicy 上线后部分匹配失败，tcpdump 看不清是不是策略 drop。手都别动。
> 这一章讲完，你会知道这几个人各自错在哪——转发在内核不在用户态、verifier 过不了上不了机、agent 挂了存量长连接往往还在别杀业务。
> **本章回答的问题**：为什么 iptables 会慢；Cilium 怎么替代 kube-proxy；Hubble 看什么；L7 策略开在哪；关 kube-proxy 之前确认什么。
> **本章不讲**：kube-proxy 四种模式和 iptables 链 → [08](../08-Service与kube-proxy/)；Calico/Flannel、原生 NetworkPolicy → [10](../10-CNI与网络策略/)；排障分层、Endpoints 空 → [18](../18-典型故障剧本/)；主机 iptables/conntrack → [01-17](../../01-基础底座/17-iptables与conntrack/)；Istio 金丝雀细节 → [15](../15-Ingress与流量管理/)、[16](../16-灰度与回滚/)。不考写 eBPF C 代码。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：先认主线三步——① 认转发（内核 hook + map）；② 挂了（存量还在、规则不更新）；③ Hubble / L7 分批切。口诀先埋一句：⭐ **agent 挂了和 kube-proxy 挂了同类——旧通新不通**。下面按节展开。

```mermaid
flowchart LR
    a[1 概念] --> b[2 架构]
    b --> c[3 原理]
    c --> d[4 部署]
    d --> e[5 配置]
    e --> f[6 命令与操作]
    f --> g[7 排障]
    g --> h[8 常考]
```


**本章主线（数据面就按这三步想）：**

1. **认转发**：eBPF 在内核 hook 上跑；agent 只负责下发 map，不经手每一包。
2. **挂了**：存量长连接往往还在，规则不再更新 — 和 kube-proxy 挂了同类。
3. **观测 / 策略**：Hubble 看 drop 和流；L7 策略和原生 NP 分开讲，游戏池分批切 replacement。

**读完你能：** 白板上画出 hook 与 map，不经手每一包；判断 iptables / IPVS / Cilium 谁该上；会 Hubble observe；能把可观测和安全分开讲；游戏池分批切 replacement。

## 本章目录

- [1. 基本概念](#1-基本概念)
- [2. 架构图](#2-架构图)
- [3. 原理](#3-原理)
- [4. 安装部署](#4-安装部署)
- [5. 核心配置](#5-核心配置)
- [6. 命令与操作](#6-命令与操作)
- [7. 生产排障](#7-生产排障)
- [8. 必会 · 常考](#8-必会--常考)
- [合上书](#合上书问自己)

---

## 1. 基本概念

🎯 Cilium 用 eBPF 在内核 hook 转发，agent 只更新 map 不经手每一包；verifier 过不了上不了机。

01 说过：数据包不经过 kube-proxy 用户态，它只负责 **更新** 本机转发规则。Cilium 用 eBPF 把「更新规则」和「转发」都放进内核 hook，不再走漫长的 iptables 链。agent 挂了，和 kube-proxy 挂了同类：**存量往往还在，规则不再更新。** 口诀：⭐ **旧通新不通**——别为了「对齐状态」去杀还活着的战斗服。

**值班里什么时候碰到**：Service 上万规则 CPU 涨；NetworkPolicy 上线后部分不通；某节点 agent CrashLoop 新登录失败；关 kube-proxy 后 NodePort 不通。

eBPF 是内核里的可编程虚拟机：在 hook 点跑经过 **verifier** 的程序，不改内核源码、不加载任意 ko。verifier 拒绝越界、死循环、乱调辅助函数，这是能上生产的前提。过不了验证就上不了机，不会当后门 ko 用。不是「内核 JavaScript 随便写」。

口诀：⭐ **verifier 过不了 = 上不了机**——这是和乱加载 ko 最大的差别。

| hook（运维相关） | 典型用途 |
|------------------|----------|
| XDP / TC / socket / cgroup | 网络：更早拦包、DNAT、策略 |
| kprobe / uprobe / tracepoint | 观测：系统调用、延迟 |
| LSM | 安全：进程/文件/网络强制 |

Cilium 还是 CNI：配 Pod IP、连通、NetworkPolicy。和 Calico/Flannel 的差别不只「另一种插件」，而是数据面从「路由 + iptables」换成 eBPF。附带 Hubble 和 L7，传统 CNI 要另装。

**打个比方**：iptables 像门卫按名单一个个核对（O(n)）；eBPF map 像刷卡闸机——ClusterIP 直接查表跳到后端 Pod。

> **记住**：转发在内核 hook 上跑，agent 只负责下发；verifier 过不了上不了机。

**面试怎么说**：「Cilium 用 eBPF 在内核 hook 转发，agent 只更新 map 不经手每一包；agent 挂了和 kube-proxy 挂了同类，存量长连接往往还在。」

---

本节对应练习题 Q1、Q2。

## 2. 架构图

好，概念有了。接下来看架构——后面的内容都对着这张图。

🎯 转发在内核 hook 上跑不经用户态，agent 只下发 map，关 kube-proxy 前必须覆盖三种流量。

先把整张图钉住。后面 Hubble、replacement、排障都对着这张图说话。

```mermaid
%%{init: {"flowchart": {"padding": 18, "nodeSpacing": 28, "rankSpacing": 48}, "theme": "base"}}%%
flowchart TB
    subgraph pkt ["① 每一包 · 不经用户态"]
        nic[网卡] --> hook[XDP / TC / sock]
        hook --> map[eBPF map ClusterIP→后端]
        map --> pod[容器]
    end

    subgraph user ["② 用户态 · 只在变的时候干活"]
        agent[Cilium agent]
        kp[kube-proxy 可关]
        api[API Watch EndpointSlice]
        api --> agent
        agent -->|编译/下发 map| map
        kp -.->|传统写 iptables| ipt[iptables 链]
    end

    subgraph obs ["③ 观测与策略"]
        hub[Hubble]
        np[CiliumNetworkPolicy / 原生 NP]
        hook --> hub
        np --> agent
    end

    class nic,hook,map,pod data
    class agent,kp,api,ipt userN
    class hub,np obsN

```

**读图三句话：**

1. **没有箭头从每一包指向 agent**——agent 挂了不等于立刻断光；先拆「已有连接 / 新连接 / Endpoints 已变之后」。
2. **Hubble 依赖同一套 hook**——agent 不健康时观测先瞎，不要说网络全死。
3. **关 kube-proxy 之前** replacement 必须覆盖 ClusterIP / NodePort / 外部流量三种。

三种数据面（装的时候选，挂的时候排障工具不一样）：

```mermaid
%%{init: {"flowchart": {"padding": 16, "nodeSpacing": 36}, "theme": "base"}}%%
flowchart TB
    subgraph ipt ["kube-proxy iptables"]
        i1["链 O(n) · 中小集群"]
    end
    subgraph ipvs ["kube-proxy IPVS"]
        v1["哈希 O(1) · 仍多次跳转"]
    end
    subgraph cil ["Cilium replacement"]
        c1["map · 更早 hook"]
    end

    class i1 a
    class v1 b
    class c1 c
```

小集群 `iptables -L` 还能看。Cilium 这边改看 `hubble observe` 和 agent 日志。为了简历换 CNI 不值得。

---

本节对应练习题 Q2、Q3。

## 3. 原理

带着这张图，咱们进原理——难点放慢讲。

### 3.1 为什么 iptables 会慢

🎯 Service×Pod 规则到几万条时 iptables 查找 O(n)；Cilium 用 map 在更早 hook 直接 DNAT。

**一句话：** Service×Pod 规则到几万条，iptables 查找 O(n)，Cilium 用 map 在更早 hook 直接 DNAT。

**打个比方**：iptables 像电话簿从 A 翻到 Z 找号码；eBPF map 像通讯录存联系人直接点名字。

**现场走一遍**（大集群 CPU `%si` 涨、尾延迟抖）：

1. 未切 Cilium 的节点：`iptables -t nat -L -n | wc -l`

```text
48231
```

2. 四万多行规则 → kube-proxy 每次 Endpoints 变化整表刷新，软中断涨
3. 对比 Cilium 节点：没有对应长度的 iptables 链，转发在 eBPF map
4. 玩家打大厅 ClusterIP，包到 Worker 网卡后在 sock/TC hook 查 map → 后端 Pod，**不经用户态**

| | 传统 iptables | Cilium eBPF |
|--|---------------|-------------|
| 拦包 | PREROUTING / KUBE-SERVICES | 更早的 sock/TC/XDP |
| 查找 | 按 Service 一条条匹配 | map：ClusterIP → 后端 Pod |
| 谁更新 | kube-proxy Watch 后改链 | agent Watch 后更新 map |
| 这一包经不经用户态 | 不经 | 不经 |

**输出解读**：IPVS 已经把查找变成哈希（08），「已经上了 IPVS 还要不要 Cilium」要另算 Hubble / L7 / 更短路径。老内核程序装不上——选 Cilium 的硬前提（16）。

**面试怎么说**：「iptables 规则随 Service×Pod 线性涨；Cilium 用 eBPF map O(1) 查找；小集群不必为性能换，大集群再评估。」

### 3.2 Hubble：无 sidecar 的流量图

🎯 Hubble 在内核看连接和 drop 原因，比 tcpdump 更能指到是哪条策略拒的。

**一句话：** Hubble 在内核看连接和 drop 原因，比 tcpdump 更能指到是哪条策略拒的。

**打个比方**：tcpdump 像路口监控录像——看见车停了不知道谁拦的；Hubble 像带原因码的闸机日志——「policy pay-api-only 拒绝」。

**现场走一遍**（NetworkPolicy 上线后网关连支付超时）：

1. `hubble status` 确认 Relay/UI 正常
2. `hubble observe --type drop -n game --from-pod game/gateway`

```text
TIMESTAMP   SOURCE                    DESTINATION           TYPE   VERDICT   POLICY
14:32:01    game/gateway:45678        game/pay:8080         TCP    DROPPED   pay-api-only
```

3. **drop + policy 名** → 就是这条 CiliumNetworkPolicy 拒的，改策略不要先关 CNI
4. 若 `forwarded` 但应用仍超时 → 网络层过了，回到 17 分层

```mermaid
flowchart LR
    a[hubble observe] --> b[src / dst / port]
    b --> c[verdict forwarded / dropped]
    c --> d[drop 带 policy 名]
    d --> e[可选 L7 path / method / topic]

    class a,b,c,d,e a
```

**输出解读**：

| 你看到的 | 含义 | 下一步 |
|----------|------|--------|
| drop + policy 名 | 就是这条策略拒的 | 改 CNP / 原生 NP |
| forwarded 但应用超时 | 网络层过了 | 走 17 |
| Hubble 没数据 | agent 不健康，观测先瞎 | 先救 agent |

和 Istio sidecar 比：省资源、少一跳。复杂熔断/重试仍不如 Istio。大规模「每个战斗服再加 Envoy」的 CPU 账，Hubble 更对游戏场景。

**面试怎么说**：「Hubble 无 sidecar 看拓扑和 policy drop；drop 带策略名就改 NP，forwarded 仍超时走 17 分层。」

> **记住**：Hubble 无 sidecar 看拓扑和 policy drop；没 Hubble 就退回 17 的分层。

### 3.3 eBPF：可观测 vs 安全

🎯 网络数据面、Hubble 观测、LSM 安全是三条产品线，「上了 eBPF」不等于「安全了」。

**一句话：** 网络数据面、Hubble 观测、LSM 安全是三条产品线，「上了 eBPF」不等于「安全了」。

**打个比方**：Cilium 像高速公路（转发）；Hubble 像路侧摄像头（看见发生了什么）；Tetragon 像交警（拦住违章）——摄像头不等于交警。

**现场走一遍**（老板问「上了 eBPF 还要 RBAC 吗」）：

1. **网络**：Cilium 做 DNAT + NetworkPolicy → 管包往哪走
2. **可观测**：Hubble 记录连接、延迟、drop → **默认不拦截**
3. **安全**：Tetragon/Falco 用 LSM 拦 exec、读密钥 → 和 13 RBAC、03 准入是两层
4. Hubble 看到 drop 多半是 NetworkPolicy；Tetragon 拦住的是「容器不该 exec」

| 方向 | 做什么 | 典型 | 不是什么 |
|------|--------|------|----------|
| **网络数据面** | LB、DNAT、NetworkPolicy | Cilium | 不是 WAF |
| **可观测** | 连接、延迟、drop 原因 | Hubble | 不是 SLO 本身（18） |
| **运行时安全** | 进程执行、文件、syscall | Tetragon / Falco | 不是替代 RBAC/准入（13） |

**输出解读**：verifier 保证程序不把内核打挂，**不保证**策略写对了。战斗服全量 syscall 跟踪是税；支付/GM 节点更值得。

**面试怎么说**：「eBPF 分网络、观测、安全三条线；Hubble 是看见，LSM 是拦截，不能代替 RBAC。」

### 3.4 L7 策略：比原生 NP 多一层

🎯 原生 NP 只到端口；Cilium L7 能在端口上再限 HTTP method/path，只给敏感路径开。

**一句话：** 原生 NP 只到端口；Cilium L7 能在端口上再限 HTTP method/path，只给敏感路径开。

**打个比方**：原生 NP 像「这扇门只能进」；L7 像「进了门只能走收银台那条过道」。

**现场走一遍**（支付网关只该收 POST /v1/charge）：

1. 应用 CiliumNetworkPolicy `pay-api-only`（见 §5 示例）
2. 网关误发 GET：`hubble observe --from-pod game/gateway --to-pod game/pay`

```text
VERDICT   L7 INFO
DROPPED   HTTP GET /v1/charge denied by pay-api-only
```

3. 改网关只发 POST → forwarded
4. 战斗服 UDP 心跳 **不要** 开 L7 解析——CPU 不划算

```yaml
apiVersion: cilium.io/v2
kind: CiliumNetworkPolicy
metadata:
  name: pay-api-only
spec:
  endpointSelector:
    matchLabels: { app: game-pay }
  ingress:
    - fromEndpoints:
        - matchLabels: { app: game-gateway }
      toPorts:
        - ports: [{ port: "8080", protocol: TCP }]
          rules:
            http:
              - method: POST
                path: "/v1/charge"
```

**输出解读**：sidecarless Mesh 做基础 L7 可见与策略；完整 Service Mesh 治理仍选 Istio（14/15）。Cilium Gateway API 是南北向，和东西向 eBPF 策略是两层。不要把 SQL SELECT 写成 http method。

**面试怎么说**：「L7 策略按需开在支付/GM；战斗心跳只到端口；Hubble 能打出 policy 名和 HTTP path。」

> **记住**：L7 策略按需开在敏感路径；战斗服心跳全量解析通常不划算。

---

本节对应练习题 Q1、Q3、Q4、Q5、Q6。

## 4. 安装部署

🎯 内核够新是硬前提；replacement 必须覆盖 ClusterIP/NodePort/外部流量三种再关 kube-proxy。

生产怎么选：

| 项 | 选 | 不选 |
|----|----|------|
| 节点少、NP 只到端口、内核老 | Calico/Flannel + kube-proxy | 为简历全面 eBPF |
| 规则爆炸、要 Hubble、要 L7、延迟敏感 | Cilium eBPF | 小集群交调试税 |
| 已有 Istio 重治理 | 并存评估，职责写清 | 为了 Mesh 再换 CNI |
| 已有 IPVS | 按 Hubble/L7/更短路径加税 | 「IPVS 不能用」 |

落地硬前提：内核够新，对 Cilium 兼容矩阵（16）。eBPF 程序比 iptables 难一眼看懂；升级要匹配内核能力。

验收：Hubble 能 observe；ClusterIP / NodePort / 外部流量在 replacement 下测过再关 kube-proxy。不要只测 ClusterIP。很多登录走 Ingress/LB（14），NodePort 仍在的节点上要单独测。

---

本节对应练习题 Q7、Q11。

## 5. 核心配置

🎯 kube-proxy replacement 覆盖三种流量再关；Hubble 开但 L7 按需；agent OOM 先加 agent 不加战斗服。

| 项 | 生产怎么用 | 改错会怎样 |
|----|------------|------------|
| kube-proxy-replacement | 覆盖三种流量再关 kube-proxy | NodePort 不通 |
| Hubble | 开；L7 可见性按需 | 全开吃 CPU |
| L7 policy | 支付/GM/管理 Kafka | 心跳全量解析 |
| agent 内存 | OOM 先给 agent 加，不给战斗服加 | CrashLoop，map 不更新 |
| Host 防火墙 | 不要和 eBPF 抢 | 外部流量黑盒 |
| 兼容矩阵 | kubelet / 内核 / Cilium 一张表 | hook 装不上 |

> **记住**：回滚 replacement 是重新拉起 kube-proxy，不是重启业务。

---

本节对应练习题 Q7、Q11。

## 6. 命令与操作

好，配置记住了。接下来是值班真敲的命令。

### 6.1 命令速查

🎯 cilium status 看 agent 健康，hubble observe 看 drop 和流，bpftool prog show 确认程序真挂载。

```bash
kubectl -n kube-system get ds,po -l k8s-app=cilium
kubectl -n kube-system describe po cilium-<node>
hubble status
hubble observe -f
hubble observe --type drop -n battle
hubble observe --from-pod game/hall
kubectl get ciliumnetworkpolicy -A
kubectl get endpointslice -n game
iptables -t nat -L -n | wc -l    # 小集群对照；replacement 后几乎帮不上
```

| 目的 | 看什么 |
|------|--------|
| agent | CrashLoop / OOMKilled |
| Hubble drop | policy 名、L7 path |
| 后端是否追上 | EndpointSlice vs observe 的目的 IP |
| 规则规模 | iptables 行数（未切 Cilium 时） |

指标：agent ready；转发 P99；drop 率；Hubble 有无数据。agent 不健康时观测先瞎。

没有 Hubble 时仍走 17 的分层：localhost → EP → ClusterIP → 跨节点。

跨节点不通先怀疑 CNI（10），不要一上来重装 Cilium。小集群 `iptables -L` 还能排；切了 replacement 后这条链路几乎帮不上忙。

### 6.2 kube-proxy-replacement

🎯 先确认三种流量全覆盖，分批关 kube-proxy；回滚是重新拉起 kube-proxy 不是重启业务。

确认 Cilium 已覆盖原 kube-proxy 的 ClusterIP/NodePort/外部流量模式，内核与配置支持 replacement，Host 防火墙没有和 eBPF 抢。分批：先观察 Hubble 和延迟，再关 DaemonSet。回滚是重新拉起 kube-proxy 并关 replacement，不是重启业务。关了之后 NodePort 不通——replacement 没覆盖全。先核对流模式，再分批回滚 kube-proxy，不要先重启游戏进程。

### 6.3 游戏集群落地

🎯 非战斗池先切，大厅次之，战斗服最后空服窗口；L7 不打心跳。

```mermaid
flowchart LR
    a[对内核兼容矩阵] --> b[非战斗池开 replacement]
    b --> c[看 Hubble / P99 / NodePort]
    c --> d[大厅 / 网关池]
    d --> e[战斗服单独窗口空服]
    e --> f[三种流量都覆盖再关 kube-proxy]

    class a,b,c a
    class d,e,f b
```

注意点：

1. **agent 挂了 ≈ 规则不更新。** 存量长连接往往还在；新登录、重连、滚动后旧后端会出问题。拉起 agent，核对 EndpointSlice。不要先重启战斗服。
2. **战斗服最后切。** 先非战斗池、再大厅、再战斗窗口。
3. **L7 不要打在心跳上。** 支付/GM 开；UDP/心跳只到端口。
4. **Hubble 代替每 Pod Envoy** 做东西向看见。完整金丝雀仍走 15。
5. **maxPods 和每节点 CIDR 没一起改**，FailedCreatePodSandBox 看起来像 CNI 坏了，其实是 16。

活动扩容滚动后，某节点 cilium-agent OOM。已在对局的长连接还在，新匹配进这节点失败。拉起 agent 后 Hubble 看到后端 map 追上 EndpointSlice。若当时重启战斗服，等于自己踢完还活着的玩家。

---

本节对应练习题 Q8、Q11、Q12。

## 7. 生产排障

好，命令会了。接下来按现象定责——先做什么、别做什么。

🎯 先拆三问：已有连接、新连接、Endpoints 已变之后——不要答「Cilium 挂了网络全死」。

不要答「Cilium 挂了网络全死」。先拆三问：已有连接、新连接、Endpoints 已变之后。userspace 转发早已废弃（08）；谁当「规则下发器」，谁挂了都是「不再更新」。

```mermaid
%%{init: {"flowchart": {"padding": 16, "nodeSpacing": 28, "rankSpacing": 48}, "theme": "base"}}%%
flowchart TB
    subgraph gate ["先分叉"]
        A[网络 / 登录异常] --> B{agent / 策略 / 跨节点?}
    end

    subgraph agent ["下发器"]
        B -->|CrashLoop / OOM| C[拉起 agent · 加内存]
        B -->|map 旧 IP| D[等 Ready · 对 EndpointSlice]
    end

    subgraph pol ["策略"]
        B -->|Hubble drop + policy| E[改 NP / CNP]
        B -->|forwarded 仍超时| F[走 17]
    end

    subgraph cni ["连通"]
        B -->|同节点通跨节点不通| G[隧道 / MTU / 防火墙 · 10]
        B -->|sandbox 失败| H[maxPods / CIDR · 16]
        B -->|hook 装不上| I[内核矩阵 · 回滚 kubelet/Cilium]
    end

    class C bad
    class E,F ok
    class D,G,H,I warn

```

现场顺序：

```bash
kubectl -n kube-system describe po cilium-<node>
kubectl get endpointslice -n game
hubble observe --type drop -n battle
hubble observe --from-pod game/hall
```

| 现象 | 先做什么 | 不要做什么 |
|------|----------|------------|
| agent CrashLoop，长连接还在 | 拉起 agent | 重启战斗进程 |
| 新登录失败、observe 旧 IP | 等 map 追上 | 先删 Pod「刷新」 |
| drop + policy | 改策略 | 先关 CNI |
| forwarded 仍超时 | 17 分层 | 重装 Cilium |
| 关 kube-proxy 后 NodePort 死 | 核对流模式，回滚 kube-proxy | 重启游戏 |
| sandbox 失败 | maxPods / CIDR | 当 eBPF 程序挂了 |
| 升级 kubelet 后 hook 装不上 | 按矩阵回滚 | 先卸 CNI |
| Hubble 没数据 | 先救 agent | 说网络全死 |

和 etcd 挂了不能发版、但长连接还在，是同一套拆法（01）。

🔧 **排障工具对照**：未切 Cilium 用 `iptables -t nat -L -n | wc -l` 看规则规模；切了 replacement 后改用 `hubble observe` 和 `cilium status`。

🔧 **易错对照**：
- `错讲法 → Hubble 没数据是网络全死` → `对讲法 → agent 不健康时观测先瞎，先救 agent 再说网络`
- `错讲法 → forwarded 仍超时就重装 Cilium` → `对讲法 → 网络层过了，走 17 分层查应用层`
- `错讲法 → Pod IP 分配失败是 eBPF 程序挂了` → `对讲法 → 先看 maxPods 和 cluster-pool CIDR，不是 Cilium 数据面问题`

---

本节对应练习题 Q8、Q10、Q14。

现在再看开篇那几句——「用户态拦包」「eBPF 像后门 ko」「agent 挂了重启战斗服」——你知道该怎么拦住他们了：转发在内核、verifier 挡坏程序、agent 挂了先救 agent 别杀业务。

## 8. 必会 · 常考

遮住「口径」列自问自答，卡壳的行回去重读对应小节——能讲出来才算会，看着答案点头不算。

🎯 Cilium 在用户态拦包是错的；agent 挂了存量长连接还在；verifier 过不了上不了机。

白板和追问高频，能一口回：

| 说法 | 对错 | 口径 |
|------|:----:|------|
| Cilium 在用户态拦每一包 | 错 | 转发在 hook；agent 只下发 |
| eBPF 等于乱加载 ko | 错 | verifier 过不了上不了机 |
| 小集群必须上 Cilium | 错 | 规则不多不必交调试税 |
| 上了 IPVS 就不能用 Cilium | 错 | 理由变成 Hubble/L7/更短路径 |
| agent 挂了游戏长连接立刻死 | 错 | 存量往往还在；规则不再更新 |
| Hubble 必须插 sidecar | 错 | 内核看流，不改应用 |
| 有 Hubble 就不用 17 | 错 | forwarded 仍超时要分层 |
| 有 eBPF 就安全了 | 错 | 观测 ≠ LSM 拦截 ≠ RBAC |
| L7 对战斗心跳全开 | 错 | 只给敏感路径；按真实协议 |
| sidecarless 能替代 Istio 金丝雀 | 错 | header 切 5% 仍是 14/15 |
| 关 kube-proxy 只测 ClusterIP | 错 | NodePort / 外部流量也要覆盖 |
| 回滚 replacement 靠重启业务 | 错 | 重新拉起 kube-proxy |
| FailedCreatePodSandBox 一定是 Cilium | 错 | 先看 maxPods / CIDR |
| Gateway API 和东西向策略是一回事 | 错 | 南北 vs 东西两层 |
| 为简历换 CNI | 错 | 内核版本和调试是税 |

数字：iptables 规则随 Service×Pod 涨；verifier 是硬门槛；L7 按需；战斗池最后切。

⭐ **面试锚点速记**：
- ⭐ eBPF 为什么比乱加载 ko 安全？——verifier 检查越界/死循环/非法调用，过不了不上机，不改内核源码。
- ⭐ cilium-agent CrashLoop 时存量长连接还在吗？——在，map 还在内核里；新连接和 Endpoints 变化后的路由会出问题。
- ⭐ 关 kube-proxy 前必须确认什么？——replacement 覆盖 ClusterIP / NodePort / 外部流量三种，不只是 ClusterIP。
- ⭐ Hubble drop 带 policy 名和不带的区别？——带 = NetworkPolicy 拒的，改 CNP/原生 NP；不带 = 系统层 drop（防火墙/MTU/conntrack）。
- ⭐ FailedCreatePodSandBox 一定是 Cilium 坏吗？——不一定，先看 maxPods 和 cluster-pool CIDR 是否耗尽。

🔧 **易错对照**：
- `错讲法 → cilium status Ready 就等于数据面生效` → `对讲法 → verifier 可能静默拒载，必须 bpftool prog show 确认程序真挂载`
- `错讲法 → agent 挂了立刻全断` → `对讲法 → 存量长连接还在，规则不再更新，先拉起 agent 不要重启业务`
- `错讲法 → 上了 Cilium 就不用分层排障` → `对讲法 → forwarded 仍超时要走 17 分层，Hubble 只帮看 drop 不替你查应用`

🔥 **高频翻车点**：
- 🔥 升级 Cilium 后 Pod 全 Ready 但包不通——新版本要求更高内核 BTF，verifier 静默拒载，hook 没挂上，必须跑 `bpftool prog show | grep cilium`。
- 🔥 关 kube-proxy 只测 ClusterIP 就说 OK——NodePort / 外部流量没覆盖，生产登录走 NodePort 全死。
- 🔥 L7 policy 全量打在战斗服 UDP 心跳上——CPU 税爆，只该给支付/GM 的 HTTP 路径开。
- 🔥 IPv4 allocation 接近 65535——不是 Cilium bug，是 cluster-pool CIDR 太小，扩 `--cluster-pool-ipv4-cidr`。

易混：agent 挂 vs 数据面死；iptables vs IPVS vs eBPF map；Hubble drop vs 应用超时；可观测 vs 运行时安全；原生 NP vs Cilium L7；kube-proxy 下发 vs Cilium 下发（影响面同一张图）。

> 🔍 **现场**：Cilium 出问题先分三层——agent 健康、数据面 map、verifier 有没有拒载程序。

```bash
# ① agent 健康度（Cilium 自己的视角，比看 Pod Ready 更准）
cilium status --verbose
# DaemonSet         40/40 (40 ready)   # ← 全 ready 才算下发面齐
# KVStore: Disabled (using k8s-api)    # ← 状态源
# ContainerRuntime: docker             # ← 容器运行时识别
# Hubble: OK (3 flows/s)               # ← Hubble 在收流

# ② 单节点 deep dive：cilium-operator 没起来或 maxPods 不够
kubectl -n kube-system exec ds/cilium -- cilium status | grep -E 'Health|Controller|Proxy'
# Controller Status: 252 running / 0 failing  # ← failing>0 = 下发卡了
# IPv4 address allocation: 251/65535 used      # ← 接近 65535 = IP 池快满

# ③ verifier 拒载 eBPF 程序（内核版本/特性不匹配，最隐蔽的故障）
kubectl -n kube-system logs ds/cilium | grep -iE 'verifier|rejected|load.*fail'
# "Error loading exporter: program load: invalid argument"  # ← 内核缺 BTF 或版本不够
bpftool prog show | grep -i cilium   # 宿主上直接看挂了哪些 cilium 程序
# 123: kprobe  name cilium_connect  # ← 没看到 = 程序根本没挂上 hook

# ④ Hubble 看 drop 带不带 policy 名（带=策略拒，不带=系统 drop）
hubble observe --type drop -n battle --since 5m | head -20
# DROP (Network Policy) 10.x→10.y SYN  # ← 带 policy = 改 CNP
# DROP (Policy Denied)                 # ← 也要看策略名
hubble observe --type forwarded -n battle --since 5m | grep -c DENIED
# 0   # ← forwarded 都通过；有 DENIED 说明策略写漏了

# ⑤ kube-proxy replacement 模式确认（关了 kube-proxy 用什么转发）
kubectl -n kube-system exec ds/cilium -- cilium status | grep -i 'kube-proxy'
# Kube-Proxy Replacement:  Disabled   # ← Disabled=还在用 kube-proxy；Enabled=Cilium 接管
# 启用 replacement 前必须覆盖 NodePort + 外部流量，否则 NodePort 死
```

eBPF 排障最大的坑是「Pod 全 Ready、agent 健康，但包就是不通」——这时 verifier 那一层最容易被忽略：内核 BTF 缺失或版本不够，cilium 程序根本没挂上 hook，`cilium status` 显示健康但数据面是空的。`bpftool prog show` 看到挂载的 cilium 程序才算真生效。另一个常见坑是 `IPv4 allocation` 接近 65535——不是 Cilium 坏了，是 IP 池配置小，扩 `--cluster-pool-ipv4-cidr`。

### 数字墙

> kube-proxy replacement 关闭时 Cilium 不接管 Service 转发 · Cilium agent 默认 **daemonset** 每节点一个 · Hubble 默认存储流 **1h**（可调 `--flowlog-flows`) · `cilium status` Controller failing=**0** 才健康 · verifier 是 BPF 程序上机的**硬门槛**（不过不上机）· eBPF map 内核版本 **5.4+** 基本可用 / **5.10+** 推荐 · L7 policy 只给**敏感路径**（支付/GM），战斗心跳 UDP 不开 · Hubble drop 带 **policy 名** = 策略拒 / 不带 = 系统层 drop · `--cluster-pool-ipv4-cidr` 默认 **/24**（251 可用，大规模要扩）· Tetragon 拦截靠 **LSM hook**（5.7+ 内核）· CiliumNetworkPolicy CRD apiVersion `cilium.io/v2`

> 🔍 **现场**：Cilium 升级 / 回滚是高风险操作——数据面在跑，改错全集群断网，必须分批且能回退。

```bash
# 升级前：记录当前版本和 kube-proxy 模式（回滚基准线）
kubectl -n kube-system get ds cilium -o jsonpath='{.spec.template.spec.containers[0].image}'
# quay.io/cilium/cilium:v1.14.0   # ← 记住 tag，回滚靠它
kubectl -n kube-system exec ds/cilium -- cilium config | grep -i 'kube-proxy'
# enable-k8s-proxy-replacement: false   # ← 记住当前模式

# 升级：先单节点 canary，再 maxUnavailable=1 一台一台滚
kubectl -n kube-system rollout status ds/cilium --watch

# 升级中盯三件事：agent 健康 / Hubble 有流 / 业务 SLI
kubectl -n kube-system exec ds/cilium -- cilium status | grep -E 'DaemonSet|Hubble'
hubble observe --type drop --since 2m | wc -l   # drop 数不应突增
curl -s prom:9090/api/v1/query?query=battle_join_5m   # 业务 SLI 不能掉

# 回滚（verifier 拒载或业务掉）：直接回 image tag
kubectl -n kube-system set image ds/cilium cilium=quay.io/cilium/cilium:v1.14.0
kubectl -n kube-system rollout status ds/cilium
# 关 kube-proxy replacement 的回滚不能只改 image——要重新拉起 kube-proxy
systemctl start kube-proxy   # 或 kubectl -n kube-system rollout restart ds/kube-proxy
```

升级最常翻车的是「新版本要求更高内核 BTF，verifier 静默拒载，cilium status 显示 Ready 但 hook 没挂上」——升级后必跑 `bpftool prog show | grep cilium` 确认程序真挂载，不能只看 Pod Ready。关 kube-proxy replacement 的回滚要记得重新拉起 kube-proxy，否则 NodePort 全死。

### 易错一句话对照（补）

- cilium status 健康 = 数据面生效 → 错，verifier 可能静默拒载，看 bpftool prog show
- Hubble drop 不带 policy 名 = 策略拒 → 错，带 policy 名才是策略拒，不带是系统层
- Cilium agent 挂了立刻全断 → 错，存量连接还在，规则不再更新
- 上了 Cilium 就不用 17 分层排障 → 错，forwarded 仍超时要走 17
- L7 policy 开在战斗心跳 → 错，UDP 心跳开 L7 解析 CPU 不划算，只给支付/GM
- 关 kube-proxy 只测 ClusterIP → 错，NodePort / 外部流量也要覆盖
- Pod IP 分配失败 = Cilium 坏 → 错，先看 maxPods / cluster-pool CIDR
- 内核 5.4 一定能跑 Cilium 全特性 → 错，BTF/LSM 要 5.10+ / 5.7+，老内核功能缺
- CiliumNetworkPolicy 和原生 NetworkPolicy 完全等价 → 错，Cilium 支持 L7 / namespace selector 更强
- Hubble 默认永久存流 → 错，默认约 1h，要长期留存接外部存储
- Cilium 升级只看 Pod Ready 就算成功 → 错，要跑 bpftool 确认程序挂载 + 业务 SLI 不掉
- IP 池接近 65535 = Cilium bug → 错，是 cluster-pool CIDR 太小，扩配置

---

本节对应练习题 Q1～Q14。

---

## 合上书，问自己

合上书自检（题号对齐练习题）：

- [ ] eBPF 是什么？为什么比乱加载 ko 安全？（Q1）
- [ ] Cilium 和 Calico/IPVS 差在哪？agent 挂了长连接怎样？（Q2、Q3、Q10）
- [ ] Hubble 能看到什么？和 sidecar 比？（Q4）
- [ ] 可观测和安全是一回事吗？L7 策略全开好不好？（Q5、Q6、Q13）
- [ ] 关 kube-proxy 之前确认什么？什么时候不要上？（Q7、Q11）
- [ ] 跨节点不通怎么定责？60 秒分流？（Q8、Q12、Q14）

六题都能口述，这一章就过了。终极测试：拦住开篇那位要重启战斗进程「对齐 Cilium」的人。

...
先合上知识点，对着 **一节的主图** 把「一条服务间请求经 Mesh 的一生」口述一遍：截获 → mTLS → 路由 → 入站 → 后端 → 回程。讲得出来再做题。
...
- [ ] **默画主图**：业务 → 出站 sidecar → mTLS → 路由 → 入站 sidecar → 目标；istiod xDS。
- [ ] **口述 sidecar 注入**与 kube-proxy 叠加关系。
- [ ] **口述 mTLS**：SPIFFE、三模式、tls-check。
- [ ] **口述金丝雀**：subset + weight；副本≠流量。
- [ ] **口述边界**：north-south Ingress vs east-west Mesh vs Service。
- [ ] **口述成本**与何时不上 Mesh。
- [ ] **默画收束五件套**：身份/路由/韧性/观测/策略。
- [ ] **定责决策树**：STALE、mTLS、subset、Endpoints。
- [ ] **60 秒剧本**命令顺序。
- [ ] **口述 AuthorizationPolicy** 与 NP 分工。
- [ ] **口述** holdApplicationUntilProxyStarts。
- [ ] **口述** Ambient vs sidecar。

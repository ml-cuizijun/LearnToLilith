...
**本章不讲：** 声明式为什么能自愈、HPA 抢 replicas → [01](../01-集群架构/)；Chart、values、helm rollback → [21](../21-Helm与Kustomize/)；滚动/金丝雀/PDB → [15](../15-灰度与回滚/)；节点升级窗口 → [16](../16-节点运行时与升级/)；Jenkins/GitHub Actions 语法细节 → [04 工程化](../../04-工程化与自动化/)。

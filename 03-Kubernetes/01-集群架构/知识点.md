...
> 📌 **Preview 自检**：大纲里应有 **10 个** `##` 标题（一～九 + 合上书）。若只有 3 个 → `Revert File`，关标签重开，**不要 Save**。
...
大门里面还有三块，本章只钉位置，细节在 [03-API-Server/知识点.md](../03-API-Server/知识点.md)：
...
- **APF**：按 FlowSchema 分队列，队列满返回 429（细节 → [03-API-Server/知识点.md](../03-API-Server/知识点.md)）。
...
# 看哪个 flow_schema 在拒、拒了多少（与 API Server 章命令速查相同）
```

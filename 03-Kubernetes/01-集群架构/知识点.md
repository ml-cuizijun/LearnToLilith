...
- **etcd 键路径**：前缀 `/registry/`，后面依次是 ``{resource}``、``{namespace}``、``{name}`` 三段（写法与 [02](../02-etcd/) 一致，用花括号不用尖括号）。
...
# etcd failed: reason withheld      ← API 进程活着，但 etcd 健康检查失败：先查盘和 quorum
# poststarthook failed ...          ← 第一个不 ok 的检查项就是死因；全 ok 才是健康
```
...
- **认证 / 鉴权 / 准入（authentication / authorization / admission）**：写 etcd 之前的三道门，对应症状 401 / 403 / apply 报 webhook timeout。**准入 Webhook** 先 Mutating 改、再 Validating 拒；若配成 `failurePolicy=Fail` 而 Webhook 服务又挂了，**写路径会一起焊死**——所有 apply 全失败。`failurePolicy=Ignore` 是紧急窗口，不是长期方案；Webhook 超时应小于 3 秒、多副本 + PDB。
- **Watch Cache**：API 用内存挡掉大部分 List/Watch/Get；**写仍然过 etcd**，cache miss 才打盘。
- **APF（API Priority and Fairness，API 优先级与公平性）**：按 FlowSchema 把请求分队列，队列满返回 **429**；别把控制器选主和狂 List 的客户端挤在同一条道里。

> 🔍 **现场**：控制器日志刷 429，先认是哪条队列满的。

```bash
kubectl get flowschemas
# system-leader-election   ...   exempt      ← 选主类流量有豁免队列，业务 429 一般不在这
# bulk-list-clients        ...   100         ← 看哪条在抢配额
kubectl get --raw /metrics | grep apiserver_flowcontrol_rejected_requests_total
# ...flow_schema="bulk-list-clients"... 348  ← 拒绝集中在哪条 FlowSchema，就去治哪个狂 List 的客户端
```

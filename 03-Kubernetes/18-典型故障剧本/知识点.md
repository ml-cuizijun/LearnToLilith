...
> 大家好。前面各章把控制面、工作负载、网络、存储、节点都拆过了；这一章把它们**串成值班能用的决策树**——接到报障先问什么、进哪棵树、第一刀敲什么。
> 开讲之前，先带大家回到两个现场：夜班大厅 CrashLoop，群里有人已经准备 `kubectl delete pod --all`，Events 还没人看过；另一边节点 NotReady，有人要杀该节点逻辑服「对齐状态」——重启会丢掉 `logs --previous`，有状态还会踢玩家。手都别动。
> 这一章讲完，你会知道这两个人各自错在哪——以及正确的第一刀为什么永远是 Events 落盘，不是重启。
> **本章回答的问题**：三问怎么定影响面；Pending / CrashLoop / OOM / Evicted / 不通 五棵树怎么分流；三宝怎么取证；有状态怎么止血。
...
**怎么听这场分享**：先认主线三步——① 三问定影响面；② 五棵树分流；③ 留证再动手。口诀先埋一句：⭐ **重启是最后一刀，不是第一刀**。下面按节展开。
...
任何 Pod 异常，80% 的线索在 **Events**。定域顺序是 **Events → 对象 spec/status → 节点 / CNI / CSI → 应用日志**。大家想想：为什么不能一上来重启？——重启会丢掉 `logs --previous`，有状态还会踢玩家。
...
好，概念有了。接下来看架构——后面三问、五棵树、三宝都对着这张图。
...
口诀记一下：⭐ **137 先看 Reason 有没有 OOMKilled**——有才是超 limit，没有可能是 grace 强杀。
...
🔥 这是全章最容易混的一对。大家想想：Pod 没了，Reason 写着 Evicted，重建一个就好了吗？——**不会**。节点还满着，新 Pod 还会被赶。
...
→ readiness 没过或 selector 对不上（01）。⭐ **空 EP 不要重启 kube-proxy**——proxy 挂了是「旧通新不通」，空名单是业务没进名单。
...
现在再看开篇那两句——「`delete pod --all`」「杀该节点逻辑服对齐状态」——你知道该怎么拦住他们了：先三问、选树、Events 落盘；有状态先停匹配。

本节对应练习题 Q6、Q7、Q9、Q12、Q13、Q14。
...
- [ ] 三问是哪三问？为什么不能一上来重启？（Q1、Q14）
- [ ] Pending 有没有 nodeName 分别查谁？（Q2）
- [ ] CrashLoop 第一刀？137 怎么分？（Q3）
- [ ] OOMKilled 和 Evicted 差在哪？（Q4）
- [ ] 不通六层顺序？空 EP 能不能重启 proxy？（Q5、Q13）
- [ ] distroless 怎么进？debug 和 run 怎么选？有状态先做什么？（Q8、Q11）

六题都能口述，这一章就过了。终极测试：拦住开篇那两个误操作——`delete pod --all`、杀该节点逻辑服「对齐状态」。下一章把指标从哪来、谁该被电话叫醒、SLO 怎么管发版节奏焊死。

...
开始做题前，先给大家一个小建议：合上知识点，对着**第一节的主图**，把「一份状态数据的一生」口述一遍——出生（API 唯一入口）→ 共识（Raft 选主与多数派复制）→ 落盘（WAL fsync）→ 日常（compact/备份）→ 换命（换节点/升级）→ 生病定责。能顺下来，下面这些题大半都会了；卡壳的地方，就是该回去重读的那一节。
...
题目顺序是有意排的：Q1、Q2 先钉「谁碰 etcd」和「挂了什么还在」；Q3～Q6 把 Raft / quorum / fsync / 强一致五件套钉死；Q7～Q12 是备份、restore、换命、部署；Q13、Q14 收尾到值班定责。
...
这是 Q1「谁碰 etcd」的影响面版——账本挂了，现场还在不在？
...
Q3 的 quorum 建立在选举之上——这道题把选主过程拆开。
...
Q3「3 丢 1 换节点」的保险侧——什么时候才轮到 restore。
...
Q7 里 `too old` 的追问扩开，别和时钟/证书搅在一起。
...
- [ ] **2（出生）**：谁唯一碰 etcd、对象路径、2379/2380 各给谁、为什么禁直连、Secret 静止加密在哪层。（Q1、Q12）
...
全部打勾之后，回头看 01 章那个活动高峰现场：根因常在 etcd 盘，第一刀永远不是重启战斗服——你现在能拦住他了。

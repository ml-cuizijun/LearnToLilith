...
开始做题前：合上知识点，对着**第一节的主图**，把「一次扩容决策的一生」口述一遍——采指标 → 算副本 → 改 replicas → 调度+就绪 → 接流量，缩容和 CA 两条分支挂在哪。讲得出来再做题。
...
题目顺序：Q1～Q3 公式与指标；Q4～Q5 防抖与开服；Q6～Q8 四件套；Q9～Q12 战斗服边界与 Git；Q13～Q14 综合定责。
...
- [ ] **口述边界**：HPA 的三个假设、战斗服为什么一条都不满足、战斗服该怎么扩缩。（Q9、Q10、Q12）
...
能拦住「加大 max 救 unknown / 战斗服挂 CPU 自动缩」两类误操作，这章过关。

# 20｜日志管理 · 练习题

开始做题前：合上知识点，对着**本章主线**口述一遍——认链 → 分对象 → 止血。讲得出来再做题。

每道题先看场景自己答，再对完整解答。

题目顺序：Q1～Q3 链路与选型；Q4～Q9 盘满与后端；Q10～Q14 关联与定责。

| 标记 | 含义 |
|------|------|
| **核心** | 不懂整章就没建立起来 |
| **必会** | 值班和面试都要能讲 |
| **常用** | 线上会反复碰到 |
| **常考** | 白板和追问的高频口径 |

## 本题目录

- [Q1 从进程到能搜到](#q1)
- [Q2 DaemonSet vs Sidecar](#q2)
- [Q3 EFK vs Loki](#q3)
- [Q4 盘满第一刀](#q4)
- [Q5 JSON 与 traceId](#q5)
- [Q6 保留 / ES 运维税](#q6)
- [Q7 采集器全挂](#q7)
- [Q8 审计不能混 index](#q8)
- [Q9 ES red / Loki 超时](#q9)
- [Q10 三支柱怎么串](#q10)
- [Q11 战斗服日志与合规](#q11)
- [Q12 采集器升级](#q12)
- [Q13 ES/Loki 容量](#q13)
- [Q14 60 秒日志定责](#q14)
- [合上书](#合上书)

---

## Q1

**★★★★★｜容器日志从进程到能搜到，经过哪几步？**

**覆盖**：核心 · 必会 · 常考
**对应**：知识点 [三、原理](知识点.md)


### 场景

白板：「你们日志怎么采的」。有人说「装了 ELK」。说不清文件在哪、谁轮转、谁读。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：容器日志路径 stdout→containerd→/var/log/containers→kubelet 轮转→DaemonSet→ES/Loki。

能搜到之前，日志先是节点文件。`kubectl logs` 读的就是这份，不是魔法通道。

```mermaid
flowchart LR
    a[stdout/stderr] --> b[containerd]
    b --> c["/var/log/containers<br/>/var/log/pods"]
    c --> d[kubelet 轮转]
    d --> e[Fluent Bit DS 只读]
    e --> f[ES 热 / Loki 温]

    class a,b,c a
    class d,e,f b
```

应用出 JSON，带 traceId。本机 `/etc/logrotate.d` 管主机文件；容器这段是 kubelet（02-22）。

### 再追问

- `--previous` 空？——上一轮文件被轮转清了，或从未写过 stdout。
- Events 算这类吗？——不算。易过期，P0 先落盘（17）。

---

## Q2

**★★★★★｜什么时候才上 Sidecar？Fluent Bit / Fluentd 怎么分工？**

**覆盖**：核心 · 必会 · 常考
**对应**：知识点 [三、原理](知识点.md)


### 场景

Java 战斗服写 `game.log` 不写 stdout。有人给每个 Pod 都塞 Fluent Bit sidecar，集群多了几千个容器。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：默认 DaemonSet 采 stdout；Sidecar 只在只写文件/多行/parser 例外，1000 Pod=1000 agent 很贵。

默认 DaemonSet。Sidecar 只在：只写文件改不动、采集端拼多行、按文件打不同 tag。多行 Java stack 优先试 DaemonSet 的 multiline parser。能改 stdout 就改。

```mermaid
flowchart TB
    subgraph ds ["默认"]
        d[stdout → 节点文件 → DaemonSet]
    end
    subgraph sc ["例外"]
        s[只写文件 → Sidecar 共享 volume]
    end
    class d ok
    class s warn
    style ds fill:#ECFDF5,stroke:#16A34A,stroke-width:4px
    style sc fill:#FFF7ED,stroke:#D97706,stroke-width:4px
```

坑：1000 Pod = 1000 agent；sidecar OOM 拖垮业务（同 cgroup）；启动顺序。故障面：DS 挂影响整节点采集（文件还在）；Sidecar 挂只影响该 Pod。

Bit 用 C、&lt;5Mi，当边缘。Fluentd 插件重，当 Kafka 后面的聚合。不要在每个节点跑满配 Fluentd。

### 再追问

Sidecar 当默认，资源账漏在哪？——只算了业务容器，没算每 Pod 50–100Mi 采集器。

---

## Q3

**★★★★★｜EFK 和 Loki 怎么选？Loki 是不是不能搜正文？**

**覆盖**：核心 · 必会 · 常考
**对应**：知识点 [三、原理](知识点.md)


### 场景

新集群要上日志。搜索同事要 ES，SRE 嫌 JVM 贵。安全要全文翻支付日志。大厅排障只是 grep userId。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：ES 全文强运维重；Loki 只索引 label 轻量，能 grep 正文但范围大就慢。

ES 全文倒排，搜索强、运维重。Loki 只索引 label，轻、先收窄再 grep。Loki **能** grep 正文，范围一大就慢——设计如此。

```mermaid
flowchart TB
    a[小规模 / 和监控同一 UI] --> loki[Loki + Grafana]
    b[已有 ES 团队 / 支付全文] --> efk[EFK]
    c[游戏海量 + 偶发全文] --> dual[Kafka → ES 热 + Loki/OSS]
    class loki a
    class efk b
    class dual c
```

`userId` / `traceId` 当 Loki label = 基数爆炸，放正文。大厅排障用 Loki；充值对账进 ES。

### 再追问

- 云 SLS/CLS？——省人、按量。盯扫描和 `*`，热 7 天云上、冷 OSS。
- 已经有 IPVS 还要不要……等等那是 23。这里：已经有 ES 还要不要 Loki？——要。ES 热搜短留，Loki/OSS 扛量。

---

## Q4

**★★★★★｜节点被容器日志打满，第一刀是什么？**

**覆盖**：核心 · 必会 · 常用
**对应**：知识点 [七、生产排障](知识点.md)


### 场景

战斗节点 DiskPressure，Pod 开始 Evict。Fluent Bit 昨天就 NotReady，没人理。有人去改 `/etc/logrotate.d`。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：DiskPressure 第一刀 truncate 最大容器日志救节点，再查 DaemonSet 和 kubelet 50Mi×5。

先救节点，再修根因。truncate 会丢未采数据，但 Evict 已在踢游戏服时优先恢复磁盘。主机 logrotate 救不了——那是 02-22。

```mermaid
flowchart LR
    a[df /var/log] --> b[du 找最大容器日志]
    b --> c[truncate 止血]
    c --> d[看 DaemonSet]
    d --> e[kubelet 50Mi×5]
    e --> f[应用关 DEBUG]
    class a,b,c bad
```

根治：独立日志盘、50Mi×5、采集器挂 P1、生产限级别。

### 再追问

采集器恢复后会不会重放全部历史？——有位移则从上次读。位移丢了可能重复或从末尾续（缺一块）。不要假设磁盘上的都进了 ES。

---

## Q5

**★★★★☆｜应用日志为什么必须 JSON？敏感信息和 traceId？**

**覆盖**：必会 · 常用 · 常考
**对应**：知识点 [三、原理](知识点.md)


### 场景

登录日志是一行纯文本。要查某个 userId 只能正则。有人把手机号也打进去了。Grafana 上 P99 高了点不到那次请求。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：应用日志必须 JSON 带 traceId；敏感字段应用侧不打，Ingress 生成 traceId 透传下游。

JSON 让采集器直接 parse，ES/Loki 按字段查。必须 timestamp、level、msg；建议 traceId、service、pod。敏感字段应用侧就不打，采集器再滤一层。审计单独存。用语言自带 JSON logger，不要靠 Fluent 正则「大概能拆」。

traceId 在 Ingress/网关生成，header / gRPC metadata 透传。下游漏传，三支柱就串不上（18、14）。

### 再追问

exemplar？——Metrics 挂 traceId，Grafana 能点到 Trace。只堆三套后端没有统一 ID，只是三份账单。

---

## Q6

**★★★★☆｜保留策略怎么定？ES 会红的那些数字？**

**覆盖**：必会 · 常用
**对应**：知识点 [七、生产排障](知识点.md)


### 场景

ES 三个月单索引一直写，master 经常 yellow。财务问为什么日志比业务机器还贵。Nginx access 全量进 ES。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：热 7–14 天可搜，温 30–90，冷对象存储；ES 单 shard 10–50GB，access 先做指标。

热 7–14 天可搜，温 30–90 天，冷 6–12 月对象存储。交易 90 天 + 1 年归档。DEBUG 不进生产。ES 必须 ILM rollover。单 shard 10–50GB。堆 ≤32Gi 且不超过机器一半。flood 85% 拒写。master 专用 3 台，不要兼 data。日志副本通常 1。access **先做指标，只对 5xx 抽日志**。

### 再追问

yellow 是不是搜不到？——多半是副本没分配。green 才能当搜得到。先 `_cat/allocation`，别删主分片。

---

## Q7

**★★★★☆｜采集器 DaemonSet 全挂了，日志丢不丢？**

**覆盖**：必会 · 常用
**对应**：知识点 [八、必会 · 常考](知识点.md)


### 场景

改了 Fluent Bit ConfigMap，全节点 CrashLoop。过了两小时才发现。有人说「日志全没了」。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：采集器全挂文件还在节点上，恢复后靠位移续读；采集器挂是 P1。

文件还在节点上，只要盘没满、轮转没把文件转没。全挂多半是配置解析失败或镜像。单节点挂看 OOM / NotReady。恢复后靠位移续读。长时间不采 → 盘满 → 才真正 Evict 和丢。所以采集器挂是 **P1**。

监控 agent uptime、records_in/out、DaemonSet desired≠ready。

### 再追问

怎么做日志告警才不炸？——FATAL 可在 agent 侧秒级匹配；趋势用存储后查询。WARN 不要全告。频率阈值 + 节点级 inhibit。

---

## Q8

**★★★★☆｜审计日志和业务日志为什么不能一个 index？**

**覆盖**：必会 · 常考
**对应**：知识点 [七、生产排障](知识点.md)


### 场景

安全要查谁 get 过 Secret。业务 Kibana 全员只读，里面混了 audit。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：审计日志与业务日志分 index、分权限；Events 易过期，P0 先落盘。

审计回答「谁在何时对何资源做了什么」，API Server audit policy 产出，合规保留 6–12 月，权限单独收。业务日志是运行时。混在一起：权限过大、ILM 误删、检索噪音。配置在 03：`--audit-log-path` + policy，Secret 可用 RequestResponse，其余 Metadata。

### 再追问

Events 算哪类？——集群事件，默认很快过期。P0 先落盘（17），不要指望一周后还能在 API 里翻到。

---

## Q9

**★★★★☆｜ES 变 red 了怎么办？Loki 全库 grep 超时呢？**

**覆盖**：必会 · 常用
**对应**：知识点 [八、必会 · 常考](知识点.md)


### 场景

活动日写入打满，集群 red，Kibana 搜支付日志超时。有人准备删所有索引。另一边 Grafana Loki 直接 `{namespace="game"} |= "error"` 扫全集群。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：ES red 有主分片丢失不可搜，先快照再查；Loki 全库 grep 超时先收窄 label。

red = 有主分片没了，那部分不可搜。P0：先快照，再 `_cat/health` `_cat/shards`。不要手删可能还有主分片的数据。yellow 多半是副本。429 = write 线程池满或磁盘水位。这就是 ES 的运维税。

Loki 超时：先把 label 收到 `app` / `pod`，再 grep。不是坏了。分片爆炸、堆 >32Gi、动态 mapping 膨胀、master 兼 data，都是会红的坑。

### 再追问

Kafka lag 涨但 ES 还绿，先 truncate 节点？——不要。瓶颈在聚合层，先看 consumer lag。

---

## Q10

**★★★★☆｜三支柱怎么关联？只堆 Loki + Prometheus + Jaeger 算不算可观测？**

**覆盖**：必会 · 常考
**对应**：知识点 [三、原理](知识点.md)


### 场景

三套都装了。排障仍是「先看 CPU，再 grep 时间附近的日志」，点不开同一条请求。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：三支柱靠统一 traceId 串 Metrics→Logs→Traces；只堆三套后端没有 ID 只是三份账单。

```mermaid
flowchart LR
    a[Ingress 生成 Trace ID] --> b[传到每个下游]
    b --> c[每条日志带 traceId]
    c --> d[Metrics exemplar 点到 Trace]
    class a,b,c,d a
```

排查：延迟指标 → 服务 → 日志 traceId → 调用链。没有统一 ID，三套后端只是三份账单。OTel 的价值是换后端不换埋点，不是「有 Collector 就有 SLO」（18）。

### 再追问

游戏大厅活动夜日志和 etcd 同盘会怎样？——fsync 打爆控制面（02）。独立日志盘；先停发版救盘，不要重启战斗服。

---

## Q11

**★★★★☆｜战斗服日志量大、含玩家 ID，采集和保留要注意什么？**

**覆盖**：必会 · 常用 ｜ **对应**：知识点 [3. 原理](知识点.md)

### 场景

单服 500MB/天日志，合规要求 90 天，有人想把 audit 和业务混一个 index。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：结构化 JSON + 脱敏；audit 独立 index；热温冷分层；战斗服用 DS 别 per-Pod sidecar。

链 [12](../12-配置与密钥/)：Secret 不进日志；PII 字段哈希。

### 再追问

stdout 和文件日志选哪个？——12-Factor 推荐 stdout；遗留文件用 tail 插件。

---

## Q12

**★★★★☆｜Fluent Bit DaemonSet 滚动升级，为什么可能丢日志？**

**覆盖**：常用 ｜ **对应**：知识点 [7. 生产排障](知识点.md)

### 场景

升级采集器，活动夜丢 5 分钟日志。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：buffer 未刷盘、position 文件丢、背压丢弃；升级用 onDelete 或维护窗。

```bash
kubectl rollout status ds/fluent-bit -n logging
```

### 再追问

采集器全挂业务会停吗？——不会，但失明；别为此重启游戏服（Q7）。

---

## Q13

**★★★★☆｜Loki `429`、ES `red`，容量告警先救谁？**

**覆盖**：必会 ｜ **对应**：知识点 [7. 生产排障](知识点.md)

### 场景

Loki 拒写；ES 分片红；磁盘 85%。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：先停写入/扩盘/降保留；查 label 基数和索引模板；Kafka 积压看 consumer。

见 Q9；先止血再调优。

### 再追问

日志盘和控制面 etcd 同盘会怎样？——etcd fsync 被打爆（02）。

---

## Q14

**★★★★★｜搜不到日志、盘满、只有某 NS 无日志，60 秒分流**

**覆盖**：核心 · 必会 ｜ **对应**：知识点 [7. 生产排障](知识点.md)

### 场景

On-call：玩家投诉查不到 traceId；节点 disk pressure；payment NS 无流。

先自己试试：不看下面，先口述一遍你的答案，再往下对。

### 完整解答

> 🎯 **一句话**：无日志→采集/label/权限；盘满→轮转+kubelet；单 NS→RBAC/采集配置 scope。

```bash
kubectl logs {pod} -n prod --tail=20
du -sh /var/log/pods/*
kubectl get ds -n logging -o wide
```

### 再追问

audit 日志能进 Loki 和业务混查吗？——**不要**，合规与权限模型不同（Q8）。

---

## 合上书

与知识点「合上书」逐条对齐：

- [ ] stdout 到能搜到经过哪几步？kubectl logs 读的是什么？（知识点 §1–3 / Q1）
- [ ] 什么时候才上 Sidecar？DaemonSet 挂了日志丢不丢？（知识点 §3.2 / Q2、Q7）
- [ ] EFK 和 Loki 怎么选？label 基数能不能乱打？（知识点 §3.3 / Q3、Q11）
- [ ] 节点 DiskPressure 第一刀？改 logrotate.d 行不行？（知识点 §6–7 / Q4）
- [ ] 审计和业务为什么不能一个 index？三支柱怎么串？（Q8、Q10）
- [ ] ES red / Loki 429 怎么止血？（Q6、Q9、Q13、Q14）
- [ ] 滚动升级丢日志、仅某 NS 无日志 60 秒分流（Q12、Q14）

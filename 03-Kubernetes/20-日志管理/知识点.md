...
> 大家好。上一章讲完指标与告警；这一章落到**一条日志的一生**——从 stdout 到能搜到，盘满了怎么办，采集挂了丢不丢。
> 开讲之前，先带大家回到活动夜现场：大厅刚崩，群里有人问「日志呢」，另有人 `kubectl logs` 是空的；节点已经 DiskPressure，匹配服开始 Evict；有人去改 `/etc/logrotate.d`——那是主机日志，救不了容器路径。手都别动。
> 这一章讲完，你会知道这几个人各自错在哪——改错轮转对象救不了 DiskPressure、采集挂了文件往往还在涨、空 logs 先上节点 ls 不是先骂业务。
> **本章回答的问题**：stdout → 节点文件 → 采集器 → 索引的整条链；kubelet 轮转 vs 主机 logrotate；EFK vs Loki；盘满与 ES red 怎么止血。
...
**怎么听这场分享**：先认主线三步——① 认链（stdout → 文件 → 采集 → 索引）；② 分对象（kubelet 管容器，logrotate 管主机）；③ 止血（盘满清节点，ES red 看 shard）。口诀先埋一句：⭐ **改错轮转对象，救不了 DiskPressure**。下面按节展开。
...
口诀：⭐ **容器路径找 kubelet，主机路径找 logrotate**——改错对象，盘照样满。
...
🔥 很多人默认「每个游戏 Pod 挂一个日志 sidecar」——通常不需要。能写 stdout 的，节点级 DaemonSet 读文件就够；Sidecar 留给写不了 stdout 的老应用。
...
本节对应练习题 Q4、Q7、Q9、Q13、Q14。

现在再看开篇那几句——「改 /etc/logrotate.d」「logs 空就骂业务」「采集挂了日志没了」——你知道该怎么拦住他们了：先认链、分对象、盘满清节点文件。
...
- [ ] stdout 到能搜到经过哪几步？kubectl logs 读的是什么？（Q1）
- [ ] 什么时候才上 Sidecar？DaemonSet 挂了日志丢不丢？（Q2、Q7）
- [ ] EFK 和 Loki 怎么选？label 基数能不能乱打？（Q3、Q11）
- [ ] 节点 DiskPressure 第一刀？改 logrotate.d 行不行？（Q4）
- [ ] 审计和业务为什么不能一个 index？三支柱怎么串？（Q8、Q10）
- [ ] ES red / Loki 429 怎么止血？（Q6、Q9、Q13、Q14）

六题都能口述，这一章就过了。终极测试：拦住开篇那位改 `/etc/logrotate.d` 的人。下一章讲 Operator——CRD 只登记，控制器才干活。

...
> 大家好。上一章把排障串成了决策树；这一章讲**指标从哪来、谁该被电话叫醒、SLO 怎么管发版节奏**。
> 开讲之前，先带大家回到活动现场：网关 P99 从 50ms 跳到 800ms，`kubectl top` 没数据，有人去重启 Prometheus；支付 5xx 到 8%，磁盘 82%，测试 NS 一个 Pod 在重启，三条一起响；产品要把可用性写成 99.99%。手都别动。
> 这一章讲完，你会知道这几个人各自错在哪——top 空先修 metrics-server 不是 Prometheus、告警风暴先降噪不是无限 silence、99.99% 先算账再写进合同。
> **本章回答的问题**：两套管线怎么分；四金信号怎么定域；P0–P3 谁半夜响；SLO + Error Budget 怎么管发布；高基数第一刀是什么。
> **本章不讲**：主机层 CPU/磁盘/logrotate → [01-22](../../01-基础底座/22-监控与告警/)、[01-23](../../01-基础底座/23-日志运维/)；日志采集、EFK vs Loki → [20](../20-日志管理/)；HPA 指标细节 → [07](../07-弹性伸缩/)；Hubble / eBPF → [25](../25-eBPF与Cilium/)；游戏 SLO 定级故事 → [07-13](../../07-游戏运维专题/13-游戏SRE稳定性/)。Thanos 全组件 / 错误预算政策点到为止，不单开模块。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：先认主线三步——① 分两套（metrics-server vs Prometheus）；② 四金信号定域；③ SLO 管发布。口诀先埋一句：⭐ **top 空先修 metrics-server，不是重启 Prometheus**。下面按节展开。
...
大家想想：`kubectl top` 空了，第一反应是什么？很多人会答「重启 Prometheus」——⭐ 这是高频错答。两套管线目的不同，别混：
...
🔥 **Error Budget（错误预算）** 第一次见可能懵：意思是「允许坏多久」——SLO 没烧完才能发高风险变更，烧完就冻发版。
...
🔥 大家想想：Prometheus OOM 了，加内存能根治吗？——多半不能。根因往往是 **高基数 label**（比如 user_id 当标签），series 爆炸。第一刀是停错误指标，不是加机器。
...
### 6.2 🔍 现场命令块：从 kubectl top 到 PromQL 全链路

⭐ 值班真敲的命令——从 top 空到定位高基数 label，一条龙：

```bash
# 1) kubectl top 排查 CPU 最高的 Pod
kubectl top pods -n game --sort-by=cpu
# ← --sort-by=cpu 按用量降序；top 空说明 metrics-server 管线挂了，不是 Prometheus

# 2) Prometheus 查 P99 延迟（Histogram 分位数）
# PromQL 不是 SQL，在 Prometheus UI 或 curl 查
...
# ← rate 必须配 [5m] 窗口；histogram_quantile 要 by(le) 分桶聚合

# 3) 查 5xx 错误率
curl -s 'http://prometheus:9090/api/v1/query?query=sum(rate(http_requests_total{service="gateway",status=~"5.."}[5m]))' | jq '.data.result'
# ← status=~"5.." 正则匹配 500-599；rate 把 Counter 转成每秒速率

# 4) 高基数 label 排查——找 series 最多的指标
curl -s 'http://prometheus:9090/api/v1/status/tsdb' | jq '.data.headStats'
# ← numSeries 超过 200 万就要分片或 drop relabel

# 5) 找哪个 label 组合在炸 series
curl -s 'http://prometheus:9090/api/v1/series?match[]={__name__=~"http_requests_total"}' | jq '.data | length'
# ← 返回 series 数量；如果 user_id 或 request_id 在 label 里，数量会指数级膨胀
```

🔧 关键参数记忆：`--sort-by=cpu` 降序看谁吃 CPU；`rate(...[5m])` 必须 5 分钟以上窗口防抖；`histogram_quantile` 要 `by(le)` 分桶；高基数先 `status/tsdb` 看 numSeries。
...
现在再看开篇那几句——「重启 Prometheus」「三条一起响」「写成 99.99%」——你知道该怎么拦住他们了：先分两套管线、按四金信号定域、用 Error Budget 管发版。
...
- 🔥 `kubectl top` 没数据就重启 Prometheus → top 走 metrics-server，不读 TSDB；修 metrics-server 证书和 kubelet 10250
- 🔥 告警用 irate 图省事 → 告警必须用 rate（5m 窗口防抖）；irate 只适合看板看突发
- 🔧 histogram_quantile 对所有指标都能用 → 只对 Histogram/Summary 有效；对 Counter 硬套无意义
- 🔧 user_id 当 label 方便下钻 → 高基数 label 打死采集；user_id 进日志/Trace，不进 Prometheus label
- ⭐ Thanos 加了就能随便打高基数 → Thanos 解决留存和全局查询，不解决 ingest 端高基数爆炸
...
| 锚点 | 回答要点 |
|------|----------|
| `kubectl top pods --sort-by=cpu` 怎么用？ | 按容器 CPU 用量降序排列；top 空说明 metrics-server 挂了，不是 Prometheus 问题 |
| `histogram_quantile` 怎么写才对？ | `histogram_quantile(0.99, sum(rate(http_duration_seconds_bucket[5m])) by (le))`——必须 `by(le)` 分桶聚合，否则算不出分位数 |
| 高基数 label 怎么发现和止血？ | 先 `curl /api/v1/status/tsdb` 看 numSeries，再 `topk(10, count by (__name__)({__name__=~".+"}))` 找罪魁；停错误指标加 relabel drop |
...
- [ ] Prometheus 在图里哪一段？谁叫人？metrics-server 干什么？（Q1、Q2）
- [ ] 四层指标 + 四金信号？延迟为什么不看平均？（Q6、Q9）
- [ ] 什么情况半夜打电话？inhibit 和 silence 差在哪？（Q3、Q12）
- [ ] SLI / SLO / Error Budget？登录和滚动踢玩家怎么记账？（Q4）
- [ ] 高基数第一刀？Thanos / VM 解决什么、不解决什么？（Q7、Q10）
- [ ] top 空但 Grafana 有图走哪条？HPA unknown 先看谁？（Q2、Q11、Q14）

六题都能口述，这一章就过了。终极测试：拦住开篇那几个误操作——重启 Prometheus、无限 silence、拍脑袋写 99.99%。下一章把日志采集和与 Trace 的关联剖开。

...
开始做题前：合上知识点，对着**第一节的主图**，把「一次外部请求的一生」口述一遍——转发线和下发线各走哪几跳、各会坏在哪。讲得出来再做题。

每道题先看场景自己答，再对完整解答。

题目顺序：Q1～Q3 架构与下发；Q4～Q5 匹配与 Gateway；Q6～Q10 流量旋钮；Q11～Q14 长连接与定责。
...
Q1 钉死「规则 ≠ 执行者」；这道题问执行者挂了流量断不断、改注解为什么不生效。
...
规则认领对了，接下来问：host/path 怎么匹配，404 先看哪。
...
匹配会了；这道题落到入口权重金丝雀——和副本比例的区别是高频错答。
...
把 Q4 的 404 和后端无人合在一起：502/503/504 各先看什么。
...
把前面入口排障收成 60 秒剧本——开篇「重启全部 ingress-nginx」的正确解法。
...
- [ ] 能**默画**主图：玩家 → DNS → LB → ingress-nginx（控制面 / 数据面）→ Service → Pod，说出转发线与下发线各坏在哪（对应知识点合上书 1；Q1、Q14）
- [ ] 能口述架构：规则 vs 执行者；endpoint 免 reload / 结构变化才 reload；「改配置不停服」但长连接会被掐（合上书 2；Q1）
- [ ] 能口述三档影响面：API Server 断连 / controller 死 / Pod 没了，为什么说先坏的是变更能力；配置不生效的第一刀（合上书 3～4；Q2）
- [ ] 能说出 IngressClass 认领机制、无 ADDRESS 三查、多 Controller 共存（合上书；Q3）
- [ ] 能口述匹配顺序与 404 vs 502 的第一刀方向（合上书 5；Q4、Q12）
- [ ] 能对比 Ingress 与 Gateway API 四点、说清三层角色与「不急迁」（合上书 8；Q5）
- [ ] 能口述金丝雀三旋钮与优先级、副本比例为什么不准、5% 已坏立即归零（合上书 7；Q6）
- [ ] 能讲 TLS 三种终止、通配符不配裸域、链不全现象、30 天告警（合上书 6；Q7）
- [ ] 能报出限流 / 超时 / body 三个默认值与对应事故码，重定向怎么做（合上书 7；Q8、Q10）
- [ ] 能讲限流熔断降级的边界：支付不降级、不乱重试、入口 429 ≠ APF 429（合上书；Q9）
- [ ] 能口述战斗长连接为什么不走七层、四层怎么暴露、副本 ≥2 + PDB 救的是什么；定责与数字墙（合上书 9～10；Q11、Q13、Q14）

十一条都能不看稿讲下来，这一章就过了。终极测试：拦住开篇那几个误操作。

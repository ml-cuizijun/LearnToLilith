...
> 凌晨：官网、活动页、支付回调挂在同一个 ingress 上，玩家突然登不上——证书过期三天没人收到告警；值班想改条路由注解救场，全站 404；这时有人提议把所有 ingress-nginx 重启一遍「让它生效」。
> **本章回答**：一次外部请求的一生——从 LB 进集群，谁把规则翻译成配置，证书在哪验，每一段会坏在哪。
> **不讲**：Service / Endpoints / kube-proxy → [08](../08-Service与kube-proxy/)；发布期的金丝雀推进与回滚 RTO → [16](../16-灰度与回滚/)；TLS 握手与证书链原理 → [01-14](../../01-基础底座/14-HTTP与TLS/)；Mesh sidecar 治理 → [24](../24-ServiceMesh-Istio与Envoy/)。
...
后面每节就是这条命的一段：进门（二）、规则怎么进来（三）、怎么匹配（四）、证书在哪验（五）、流量怎么调（六）、另一条四层的路（七）、以及下一代入口（八）。
...
1. **默画一张图**：玩家 → DNS → LB → ingress-nginx（控制面 watch/渲染/reload、数据面转发）→ Service → Pod，规则在虚线的下发线上。
2. **口述架构**：Ingress 是规则、ingress-nginx 是执行者；endpoint 变化免 reload、结构变化才 reload；为什么「改配置不停服」但长连接会被掐。
3. **口述挂掉的影响面**：API Server 断连 / controller 进程死 / Pod 整个没了，三档影响各是什么；为什么说先坏的是变更能力。
4. **口述下发链**：apply → watch → 渲染 → 校验 → reload；一条坏规则如何卡住全局、admission webhook 挡在哪；配置不生效的第一刀。
5. **口述匹配**：host → path → backend；Prefix / Exact 的区别；404 查规则、502/503 查 Endpoints 各是为什么。
6. **口述证书**：TLS 三种终止位置、SNI 怎么选证书、通配符为什么不配裸域、链不全的现象、cert-manager 续期与 30 天告警。
7. **口述工具箱**：金丝雀三旋钮与优先级、限流/超时/body 的三个默认值与三个事故码、重定向、XFF 只信最外层；注解为什么是厂商货。
8. **口述对比**：Ingress vs Gateway API 四点差异；什么时候迁、为什么说 Ingress 不会立刻淘汰。
9. **口述分轨**：战斗长连接为什么不走七层；四层怎么暴露；副本 ≥2 + PDB 在 reload 场景下救的是什么。
10. **定责与数字**：无 ADDRESS / 404 / 413 / 429 / 502 / 504 / 证书报错各先看什么；背出数字墙里的默认值。

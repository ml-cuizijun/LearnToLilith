...
> 活动高峰，玩家登录全掉线——`kubectl get endpoints` 一看是空的，群里有人喊「重启 kube-proxy」，有人主张「给每个服务单独开个 LoadBalancer」；另一头滚动发布中，新分区玩家连进了即将退出的旧 Pod，有人已经准备重启网关。
> **本章回答**：一次访问 Service 的请求的一生——名字从哪来、名单从哪来、内核怎么转发、规则谁来写、发布时何时摘流；卡在哪一段，一眼定责。
> **不讲**：DNS 解析、ndots → [09](../09-CoreDNS/)；Pod IP 分配、NetworkPolicy → [10](../10-CNI与网络策略/)；七层入口 Ingress → [15](../15-Ingress与流量管理/)；conntrack 深机制 → [01-17](../../01-基础底座/17-iptables与conntrack/)；探针与优雅退出时序 → [04](../04-Pod生命周期/)。
...
- [ ] **默画**一节主图：一次请求的一生（名字 → VIP → 内核 DNAT → Pod，回程 conntrack），标出 kube-proxy 只写规则的虚线，说出每个排障工具照哪一段。
- [ ] 口述选门：五种 Service 各自用途；为什么生产不留 NodePort；LB Pending 查什么；为什么 STS 连指定序号必须 Headless。
- [ ] 口述名单：Endpoints 从哪来；为空四原因；名单不空还不通再查什么；为什么第一动作不是重启 kube-proxy。
- [ ] **默画**内核转发：`KUBE-SERVICES → SVC（概率）→ SEP（DNAT）` 三层；为什么回程和存量连接总回同一后端；sessionAffinity 管什么、不管什么。
- [ ] 口述三模式：iptables / IPVS / nftables 的查找与更新差异；切 IPVS 失败先查什么（`lsmod`）。
- [ ] 口述 proxy 挂了：存量长连接与新连接分别怎样；为什么先拉 proxy、不重启游戏进程。
- [ ] **默画**六节时序：删 Pod 瞬间的两条并行线；竞态窗口为什么会 502；摘流三件套各自挡什么。
- [ ] 口述定责：第一动作；名单空与 conntrack 满的现象区别；单节点不通与全节点不通怎么劈。
- [ ] 口述数字：30000-32767、10800s、100/片、65536 与 80%、preStop 5-15s。

下一章：[09](../09-CoreDNS/) —— 名字是怎么变成 VIP 的。

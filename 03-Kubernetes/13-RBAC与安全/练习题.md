...
开始做题前：合上知识点，对着**第一节的主图**，把「一次权限判定的一生」口述一遍——找身份 → 配规则 → 准入 → 运行时。讲得出来再做题。

每道题先看场景自己答，再对完整解答。

题目顺序：Q1～Q4 身份与规则；Q5～Q7 发卡与提权；Q8～Q11 着装与分房间；Q12～Q14 审计与定责。
...
就是开篇要把 prod 改 privileged 那位踩的坑。
...
十条都能不看稿讲下来，这一章就过了。终极测试：拦住开篇那几个误操作。

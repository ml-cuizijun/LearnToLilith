...
开始做题前：合上知识点，对着**第一节的主图**，把「一张证书的一生」口述一遍——出生 → 签发 → 出示 → 验票 → 老去 → 续命。讲得出来再做题。

每道题先看场景自己答，再对完整解答。

题目顺序：Q1～Q2 三套 CA 与证书链；Q3～Q5 kubeconfig 与验票；Q6～Q9 过期与续命；Q10～Q14 排障与定责。
...
这是 Q6 影响面表的 etcd 特写——peer 和 client 症状怎么分。
...
Q6/Q7 讲控制面证；这道题落到节点侧——kubelet 证与 NotReady。
...
就是值班头号坑：renew 了还报 expired——对应知识点七节那句「不重启等于白续」。
...
和 Q5 验票四道门呼应：403 到不了 TLS 层，别拿 renew 治 RBAC。
...
把前面过期/续命/排障收成 60 秒剧本——开篇那个凌晨现场的正确解法。
...
- [ ] **默画**一节主图：出生 → 签发 → 出示 → 验票 → 老去 → 续命，每段对应哪类证书、哪个命令。（对应知识点合上书 1；Q1、Q2）
- [ ] 口述 **三套 CA** 各管什么；`sa.key` 为什么不算 X.509 CA。（合上书 2；Q1）
- [ ] **默画证书链**：每张证挂哪个 CA——说清 `apiserver-etcd-client` 为什么在 etcd CA 下。（合上书 3；Q2）
- [ ] **kubeconfig** clusters/users/contexts/current-context；KUBECONFIG 多文件合并；多集群纪律与长期凭据风险。（合上书 4；Q3、Q4）
- [ ] 背 **验票四道门** + CN/O 映射；`system:masters` 例外；认证 401 与鉴权 403 的分界（链 13）。（合上书 5；Q5、Q12）
- [ ] 对表说出 **admin / apiserver / etcd peer / kubelet** 四种过期各自「什么还在、什么停了」。（合上书 6；Q6、Q7、Q8）
- [ ] **时钟骗局**：`check-expiration` 健康但握手 expired，先校时再说话。（合上书 7；Q6）
- [ ] 背 **renew 三步**：`check-expiration` → `renew all` → **重启静态 Pod + 分发 kubeconfig**；只续不启 = 白续。（合上书 8；Q9）
- [ ] **openssl 两条**：`s_client` 看服务端 `notAfter` 与 SAN；`modulus` MD5 验证钥配对。（合上书 9；Q10）
- [ ] 说出 **SAN** 是什么、为什么新增 LB 入口必须重签证书。（合上书 10；Q11）
- [ ] **定责**：活动高峰不动业务；60 秒剧本；不 reset、不拨时钟。（合上书 11；Q13、Q14）

十一条都能不看稿讲下来，这一章就过了。终极测试：拦住开篇那两个误操作。

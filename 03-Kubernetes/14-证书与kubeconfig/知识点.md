...
> 集群跑了一年，凌晨发版窗口 `kubectl` 全场报 `Unable to connect to the server: x509: certificate has expired`——监控里 API 还 Running。群里已有人敲到一半 `kubeadm reset`，还有人出主意「把系统时间往回拨」。
> **本章回答**：一张证书的一生——`kubeadm init` 出生 → 谁签谁 → 装进 kubeconfig → TLS 验票 → 过期谁挂 → `kubeadm certs renew` 续命；值班 60 秒怎么定责。
> **不讲**：RBAC 规则与 403 → [13](../13-RBAC与安全/)；Token/Webhook/OIDC 认证链 → [03](../03-API-Server/)；etcd Raft 与备份 → [02](../02-etcd/)；TLS 握手机制本身 → [01-14](../../01-基础底座/14-HTTP与TLS/)；时间同步 → [01-32](../../01-基础底座/32-NTP与时间同步/)。
...
> 🎯 **一句话**：证书和人一样有生老病死——出生、签发、出示、验票、老去、续命；出了问题先问卡在哪一段。

```mermaid
flowchart LR
    BORN["出生<br/>kubeadm init 造 CA 根<br/>签各组件证书"]
    CHAIN["签发<br/>集群 CA / etcd CA / front-proxy CA"]
...
- 📖 **读图**
  - 🛤️ 主线：出生/签发讲「有哪些证、谁签谁」；出示/干活讲「怎么拿出来用、API 怎么验」；老去/续命讲「过期症状与怎么续」
  - ⚠️ 虚线：到点没人续，**进程不会自己发现**——TLS 握手直接失败，值班报障都从「老去」进来
  - 🔪 定责第一问永远是「哪一段」：连不上 6443（出示/服务端证）、连上了但 401/403（身份与权限，[13](../13-RBAC与安全/)）、节点 NotReady（kubelet 证）

口诀先埋一句：⭐ **renew 只换磁盘，不重启等于白续**。
...
> 🎯 **一句话**：`kubeadm init` 在控制面造出以 `ca.crt` 为根的 PKI 树；组件证默认 1 年、CA 根 10 年，CA 私钥丢了只能重建信任。

开篇有人要 `kubeadm reset`——证书过期和数据损坏不是一种病。先把「证从哪来」讲清。

- 🌳 **PKI（Public Key Infrastructure，公钥基础设施）** = `/etc/kubernetes/pki/` 下那套 CA + 各角色证书
- 🔏 **CA（Certificate Authority）** = 用自己的私钥给别的证签名的「公章」——被签的证归它担保；组件各自信任对应的 `ca.crt`

> 💡 **打个比方**：CA 是游戏公司公章——公章在，员工工牌（组件证）丢多少都能补；公章私钥丢了，全公司工牌作废，只能重刻章重发证。

### 📂 PKI 树上住着谁 ⭐

控制面节点 `/etc/kubernetes/pki/` 核心文件（别一上来背清单，按「谁签、干什么」记）：

- 🏛️ **集群 CA**（`ca.crt` / `ca.key`）
  - 签：`apiserver.crt`（对外 6443 服务端）、`apiserver-kubelet-client.crt`（API→kubelet）、admin / controller-manager / scheduler 的 client 证、kubelet client 证
- 🗄️ **etcd CA**（`etcd/ca.crt` / `ca.key`）——和集群 CA **两套**
  - 签：`etcd/server.crt`（2379）、`etcd/peer.crt`（2380）、`etcd/healthcheck-client.crt`、**`apiserver-etcd-client.crt`**（API→etcd）
- 🔌 **front-proxy CA**（`front-proxy-ca.crt` / `.key`）
  - 签：`front-proxy-client.crt`（聚合层：metrics-server、扩展 API）
- 🎫 **`sa.key` / `sa.pub`**：签 ServiceAccount JWT——**不是 X.509**，只是住同一目录；走令牌验签链，与本章 client 证两条线，在 API 认证入口汇合（[03](../03-API-Server/)）

> ⚠️ **别混**：面试问「K8s 有哪些 CA」，答三套 X.509 CA 之后补一句 `sa.key` 签 JWT——是加分点，别把它说成第四套 X.509 CA。

**CA 根 10 年、组件证 1 年**——「跑了一年整批同时到期」的结构性原因：它们是 init 那天一批签出来的。

> 🔍 **现场**：接手集群第一件事不是 `kubectl get nodes`，是看证还剩几天。
...
# ← 同一批签发 → 同一批到期：整表只剩 23h = 「半夜集体爆炸」现场
# ← CERTIFICATE AUTHORITY 列 =「谁签的」：apiserver-etcd-client 归 etcd-ca
# ← RESIDUAL < 14d 按 P1 排进变更窗口（七节）；EXTERNALLY MANAGED=yes 的项 kubeadm 不管
```

- 🏷️ **SAN（Subject Alternative Name）** = 证书允许被访问的 DNS/IP 清单。`apiserver.crt` 必须覆盖 LB VIP、每台控制面 IP、`kubernetes.default.svc` 等——少一个，从那个地址访问就报 `x509: certificate is valid for ... not ...`（八节）。清单写在 `kubeadm-config` 的 `apiServer.certSANs`，**装集群时写满**，事后改 kubeconfig 救不了。⭐

### 🚪 Worker 入场：TLS bootstrap 🔧

控制面证是 `kubeadm init` 一次生成；Worker 上的 kubelet 证是 `kubeadm join` 时走 **TLS bootstrap（TLS 引导）** 现场申请的。
...
- 📖 **读图**
  - 🎫 从 **CSR（Certificate Signing Request）** 进、从集群 CA 签发的正式证出
  - 🎫 bootstrap 阶段 kubelet 凭 token 敲门，权限只有「给自己申请证」；证到手后走 **client cert auth**，和 `admin.conf` 同一条进门路
...
#### ❓ 为什么 bootstrap token 过期 ≠ kubelet 证过期？

- 🎫 **bootstrap token 过期**：只挡「新人入场」（`kubeadm join`），已在册节点照常报到
- 🪪 **kubelet client 证过期**：已在册节点变 **NotReady**（六节）——症状和救治对象完全不同
- ⭐ 口诀：**token 管新人入场，kubelet 证管老人报到**
...
很多人会答「全集群就一张 `ca.crt`」——⭐ 这是高频错答。

**证书链（certificate chain）** = 「这张证是谁签的」一路追到根。K8s 不是一棵大树，是**三棵树并排**：
...
- 📖 **读图**
  - 🌳 从三个 subgraph 的**根**进——链路断了，先找属于哪个 CA 的地盘
  - 🔀 全图只有一个「跨域打工者」：**`apiserver-etcd-client` 挂在 etcd CA 下**——API 去敲 etcd 的门，必须拿 etcd 家签的证

#### ❓ 为什么不是一套 CA 签完全部？

- 🛡️ **故障域隔离**：etcd 域被攻破，不等于立刻伪造 6443 上的人证；聚合层扩展也不该拿到签发 admin 证的公章
- 🔑 **信任边界清晰**：etcd 只认自己的 CA——拿集群 CA 手工签一张去连 2379 **必然失败**
- 🔌 **front-proxy** 单独一套给聚合层（metrics-server、自定义 APIService）；API 用 `--requestheader-client-ca-file=front-proxy-ca.crt` 验这批证，再透传原用户身份（细节 → [03](../03-API-Server/)）。与集群 CA **不能互相替代**

三条验证链路各走各的：

- 👤 **人/控制器 → API（6443）**：客户端验 `apiserver.crt`；API 反过来验你的 client 证（**mTLS**，五节）
- 🗄️ **API → etcd（2379）**：etcd 只认 etcd CA，集群 CA 在这没有面子（[02](../02-etcd/)）
- 📡 **API ↔ kubelet（10250）**：API 拿 `apiserver-kubelet-client`，kubelet 拿自己的 server 证。`kubectl logs` / `exec` 的数据面就是这条链

三张高频证对照：

| 对比 | **apiserver.crt** | **apiserver-kubelet-client** | **kubelet client** |
|------|-------------------|------------------------------|---------------------|
| 角色 | API 对外服务端证 | API→kubelet 的客户端身份 | kubelet→API 的客户端身份 |
| 谁校验 | 所有连 6443 的客户端 | kubelet | API Server |
| 过期症状 | 全场 6443 TLS 失败 | `logs`/`exec` 失败 | 节点 NotReady |
...
它们的证过期 → 控制器停写状态、调度器停分配，但**已在跑的 Pod 不受影响**（影响面总表 → [01](../01-集群架构/)）。

> 📝 **面试**：「K8s 有几套 CA？」答「至少三套：集群 CA（API/kubelet/人）、etcd CA（2379/2380 全家，含 apiserver-etcd-client）、front-proxy CA（聚合层）；另有 `sa.key` 签 SA JWT，不是 X.509」。

好，知道「谁签谁」。接下来：证签好了，**人怎么把它拿出来用**？
...
**kubeconfig** 是 YAML，不是单张证书。三个顶层键各管一件事：
...
> 💡 **打个比方**：`clusters` 是「大楼地址 + 认准哪家公证处」，`users` 是你的工牌，`contexts` 是「今天去 A 大楼用管理员工牌」这条排班——换楼、换牌、换排班，互相独立。
...
- 📁 **多文件优先级**（从低到高）
  - `~/.kube/config` → 默认
  - `KUBECONFIG=a.yaml:b.yaml` → 多文件用 `:` 拼接（Windows 用 `;`），同名条目后者覆盖
  - `--kubeconfig /etc/kubernetes/admin.conf` → 命令行最高
  - `--context` → 覆盖 `current-context`

- 🧭 **多环境纪律**（误删 prod 几乎都出在这）
  - context 命名统一「环境-角色」（`prod-admin` / `staging-admin`）
  - shell 提示符显示当前集群（kubectx / prompt 插件）
  - 删除类命令前先 `kubectl config current-context`
  - 舰队式多集群 → [26](../26-多集群与联邦/)

- 💣 **长期证书风险** ⭐：`admin.conf` 内嵌 `O=system:masters` 的 cluster-admin 证，**一年有效且权限封顶**——拷给 CI、进 Git = 集群裸奔。1.24 前 SA Token 是永不过期的 Secret；1.24 起默认 `TokenRequest` 短期令牌（~1h）。CI/系统正确姿势：**独立 SA + 最小 Role + 短期令牌或 OIDC**，永不发 admin 证（→ [13](../13-RBAC与安全/)）

> ⚠️ **别混**：**你的 kubeconfig 里的证过期** 只影响「拿这份文件的人」；**`apiserver.crt` 过期** 影响「所有连 6443 的人」。剩余天数常接近（同一批续的），症状范围天差地别——六节。

从 kubeconfig 拆出 PEM 检查（`-data` 字段是 base64）：
...
好，证装进 kubeconfig 了。kubectl 一敲，**API 怎么验这张票**？
...
🔥 全章最难的一块。很多人以为「证书过期所以 403」——⭐ 高频错答，因果链先拆开。

API Server 启动参数里与证书相关的核心项（`/etc/kubernetes/manifests/kube-apiserver.yaml`）：
...
- 📖 **读图**
  - 🔐 从「双向互验」进——不是公网 HTTPS 那种只验服务端；6443/2379/2380/10250 全是 **mTLS（mutual TLS）**
  - 🧭 出错先分清是**服务端证**（你验它不过）还是**客户端证**（它验你不过），再看 401/403 是否已经过了 TLS

- 🪪 **CN/O 映射**：客户端证的 **CN（Common Name）** → user，**O（Organization）** → group。`admin.conf` 的 `O=system:masters` 命中内置超权组，**绕过 RBAC** 等效 cluster-admin。普通用户证要在 RBAC 里有 Binding 才有权（[13](../13-RBAC与安全/)）
...
- 📖 **读图**
  - ①②③ 任一不过 = **TLS 层失败**（x509 报错，请求到不了 RBAC）
  - ④ 之后才把「证书字段」翻译成「身份」交给下一道门；排障按 ①→④：先验 CA，再看过期，再看 SAN/用途，最后才谈权限

- 🔖 **EKU（Extended Key Usage）** = 证书声明的用途：`serverAuth` 还是 `clientAuth`。拿 server 证当 client、拿 etcd 域的证敲 6443，都会在这一道被拒

#### ❓ 为什么证书过期到不了 403？

- 🪪 **认证（Authentication）**：「你是谁」——失败是 401 或 TLS 握手报错
- 🛂 **鉴权（Authorization）**：「你能干什么」——失败是 403
- ⏱️ **过期死在 TLS 层**：连身份都映射不成，RBAC 没机会开口——「证书过期所以 403」是经典错句

> ⚠️ **别混**：认证 vs 鉴权这对易混全章只在此处讲一次。过期 ≠ 403；403 = 证是好的、权限不够。
...
#### ❓ 为什么改访问地址救不了 `valid for ... not`？

- 🏷️ 报错说的是：**你连的那个 DNS/IP 不在证书 SAN 里**
- ✏️ 只改 kubeconfig 的 `server:` 地址，证书上的 SAN 一个字都没动——握手照样拒
- ✅ 把入口写进 `apiServer.certSANs`，**重签** `apiserver.crt`（装时就该写满；事后用 `kubeadm init phase certs apiserver --config=...`）

好，验票四道门走完了。证到了 Not After，**集群里谁跟着挂**？
...
开篇那个现场——API Running、玩家还在、kubectl 全挂——对表就清楚。
...
#### ❓ 为什么 kubectl 挂了 ≠ API 挂了？

- 🪪 只有 `admin.conf` 过期：API、节点、业务可能全好——**只杀拿这份文件的人**
- 🏛️ `apiserver.crt` 过期：所有 6443 访问失败——才是「控制面对外大门关了」
- 🔍 定范围第一刀：只有值班挂，还是 CI/控制器/节点心跳全挂？
...
etcd 侧（peer 证过期时，[02](../02-etcd/)）：etcd 日志刷 `failed to verify certificate: x509: certificate has expired`，`etcdctl endpoint status` 里 **IS LEADER 全 false**、ERRORS 列带 tls 字样。

### ⏰ 时钟骗局：证书没过期，机器时间错了 🔥

TLS 验证「过没过期」用的是**本机系统时钟**。有人为排障改过系统时间、或虚拟机快照恢复后时钟漂移，就会出现诡异的「提前过期」：

- ⏩ 时钟**调快** → 有效证书被判 `certificate has expired`
- ⏪ 时钟**调慢** → 报 `x509: certificate is not yet valid`
...
- 🕰️ 真实过期：`check-expiration` 也到点——拨回去只骗了**这一台**的时钟，别的节点、客户端、监控仍按真时间验
- 💥 拨时钟：证书本身健康，握手与表矛盾——**校时**，不动证书
- 🚫 拨时钟会让证书校验、日志时间线、Lease/心跳全部错乱——把小事故变成大事故

> ⚠️ **别混**：真实过期 =「`check-expiration` 也到点」；时钟骗局 =「表健康、只有握手报错」。治法南辕北辙。
...
⭐ 面试必考，也是值班头号踩坑。很多人会想：「磁盘上的证换了，进程应该自己发现吧？」——错了。
...
#### ❓ 为什么 renew 之后还报 expired？

- 💾 renew **只换磁盘**上的 pem；静态 Pod 内存里还是旧证
- 🔁 谁要重启对号入座：`apiserver` / `controller-manager` / `scheduler` / `etcd` 都是静态 Pod，各重启各的
- 📂 本地 `~/.kube/config` 还是旧的 `admin.conf` 拷贝 → 分发新文件
- ⭐ **kubelet 例外**：证书轮转是进程内热加载，`systemctl restart kubelet` 只在救急时用

> ⚠️ **别混**：**renew 之后不重启** = 磁盘新证、内存旧证，报错原样不动——「续期了还是 expired」的头号原因。
...
- 🏛️ **控制面**：静态证书没人自动续——人工或定时任务跑 `kubeadm certs renew` + 重启，必须排变更窗口
- 🤖 **kubelet**：kubeadm 默认开 `--rotate-certificates=true`，剩余约 10%~30% 生命周期时自动提交 CSR 换新证（二节同一条路）——在册节点的证**通常不用你管**，除非 CSR 审批链自己出了问题
...
群里有人要 `kubeadm reset`、有人要拨时钟——用决策树把他们拦住。
...
- 📖 **读图**
  - 🚪 入口先做 **TCP 与 TLS 分层**——相当一部分「证书问题」其实是 LB 探活配错或 6443 根本没监听，别一上来就 `renew`
  - 🧭 进了 x509 再按报错词分路；走到右下角「renew 后仍报错」，九成是**忘了重启**或**忘了换 kubeconfig**，小概率是时钟骗局（六节）
...
- 拨时钟「续命」→ 校时或真 renew + 重启，别动系统时钟骗握手
...
7. 解释 **时钟骗局**：`check-expiration` 健康但握手 expired，先校时再说话；为什么不能拨时钟「续命」。（Q6）

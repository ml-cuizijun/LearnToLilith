...
> 大家好。上一章讲完 Pod 网络怎么生、怎么走路；这一章讲**一块盘在集群里怎么申请、怎么挂上、怎么回收**——以及 PVC Pending 半小时为什么不该先重启 kube-system。
> 开讲之前，先带大家回到三个现场：PVC Pending 半小时，有人说 CSI 挂了，准备重启 kube-system 里所有 Pod；同一时刻有人「清理无用 PVC」，生产 SC 是 Delete，云控制台的盘一片片消失；还有人已经抄起 `kubectl patch`，准备把 PVC 的 finalizers 抠掉。
> 这一章讲完，你会知道这三个人各自错在哪——`waiting for first consumer` 为什么不是故障，Delete 为什么把「删申请」和「毁数据」做成同一个按键，以及 finalizer 为什么不能先抠。
> **本章回答的问题**：一块盘在 K8s 里的一生——谁申请、谁创建、谁挂载、谁扩容、谁回收，每一段坏了怎么一眼定责。
> **本章不讲**：调度与拓扑 AZ 细节 → [06](../06-调度机制/)；drain 与本地盘节点操作 → [17](../17-节点运行时与升级/)；对象卡 Terminating 的 finalizer 通用机制 → [01](../01-集群架构/)；游戏回档流程与 RPO 运营 → [07-09](../../07-游戏运维专题/09-玩家数据备份与回档/)；配额把 PVC 卡住 → [13](../13-RBAC与安全/)；GitOps 怎么管 YAML → [23](../23-GitOps-ArgoCD与Tekton/)。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：第一节是盘的主图；二到七节顺着申请 → 出生 → 挂载 → 使用 → 回收 → 另一条路；第八节定责。口诀先埋一句：⭐ **Pending 查出生，Bound 查挂载，Terminating 查回收**。
...
咱们先不急着背 CSI sidecar 名字。学存储最怕一上来抠插件，抠完还是不会定责。先看图，把「卡在哪一段、第一刀敲哪」装进脑子里：
...
口诀：⭐ **Pending 查出生，Bound 查挂载，Terminating 查回收**。好，先看订单怎么下。
...
大家想想：应用 YAML 里写云盘 ID，换云厂商要改多少文件？所以解耦点就在「只认 PVC 名」。
...
口诀：⭐ **申请不是盘；SC 不是盘；Bound ≠ 挂上了**。订单下好了，厨房怎么出餐？
...
开篇那位喊「CSI 挂了」——大家想想：Events 里若是 `waiting for first consumer`，重启 kube-system 有用吗？没用。先把出生链画清楚。
...
口诀：⭐ **CSI Controller 挂了——新盘建不出，已挂上的通常还在**。盘建出来了，怎么接到节点、进容器？
...
开篇 Pending 半小时那位——大家想想：Events 写着 `waiting for first consumer`，是 CSI 挂了吗？很多人答「是」——⭐ 高频错答。这是 WFFC 在等消费 Pod。
...
大家想想：Immediate 建盘更早，是不是更好？很多人答「是」——错了。云盘跟 AZ，先建后调度等于赌拓扑。
...
口诀：⭐ **waiting for first consumer 不是故障；FailedAttach 查旧节点/AZ，FailedMount 查本机**。挂上了，谁能写、怎么扩？
...
大家想想：三副本 Deployment 抢一块 RWO 云盘，改 accessMode 成 RWX 就能共享了吗？不能——后端不支持，改了也挂不上。⭐ 这是高频错答。
...
口诀：⭐ **RWO = 单节点不是单 Pod；改 accessMode 变不成 RWX；扩容要对 df**。用完了，删 PVC 盘还在吗？
...
开篇「清理无用 PVC」那位——大家想想：生产 SC 若是 Delete，删 PVC 等于同意毁云盘。口诀：⭐ **生产一律 Retain；改 SC 不回溯已有 PV**。
...
开篇那位已经抄起 patch 要抠 finalizer——大家想想：守门人还没走完，强行销户会怎样？可能留孤儿盘、脏数据。⭐ 先查 CSI 和云侧，抠是最后手段。
...
口诀：⭐ **删 STS ≠ 删盘；缩容高序号 PVC 还在；Terminating 先查再抠**。不走 PV/CSI 的那条短路呢？
...
口诀：⭐ **热状态在内存，冷数据才落盘；战斗服别用网络盘**。主线走完，收作战定责。
...
现在再看开篇三个人——重启 kube-system、随手删 PVC、先抠 finalizer——你知道该怎么拦住他们了。
...
终极测试：拦住开篇那三个误操作——WFFC 不重启 CSI，Delete 不随手删，Terminating 不先抠。收口。
...
1. **默画一节主图**：申请 → 出生 → 挂载 → 使用 → 回收；另一条路 emptyDir / HostPath 不走 PV/CSI 链；每段的第一刀是 Events。（Q1、Q14）
2. **口述申请**：PVC / PV / SC 三角色各管什么、应用为什么只写 PVC 名、绑定匹配哪四项、静态 PV 什么时候用。（Q1、Q2）
3. **口述出生**：动态供给五步链（provisioner watch → CreateVolume → 建 PV → Bound）；sidecar 能点名；CSI Controller 和 Node Plugin 两种「挂了」影响面差在哪。（Q2）
4. **默画挂载**：Attach（ControllerPublish / VolumeAttachment）vs Mount（NodeStage / NodePublish）；FailedAttach vs FailedMount 各查哪；WFFC 解决什么、`waiting for first consumer` 为什么不是故障。（Q3、Q4）
5. **默画收束清单**：六道关卡「模板齐、盘建出、接到机、目录现、写得进、回收兜底」各对应哪个对象，这条链不保证哪三件事。（Q3、Q4）
6. **口述使用中**：RWO / RWX / ROX / RWOP 各是什么、改 accessMode 为什么没用；扩容五步与为什么要对 df；VolumeSnapshot 为什么不能直接当 DB 回档。（Q5、Q9、Q10、Q13）
7. **口述回收**：Retain vs Delete 删 PVC 后各发生什么；误 Delete 复盘三步；改 SC 为什么救不了存量 PV。（Q6）
8. **口述盘债**：STS volumeClaimTemplates 一人一盘；缩容 / 删除为什么 PVC 还在；释放费用的正确顺序。（Q8）
9. **口述 Terminating**：finalizer 在等什么；为什么不能先抠；什么情况下才可以抠。（Q7）
10. **口述另一条路**：emptyDir vs HostPath 定位；战斗服落盘方案（本地盘 / 单挂云盘 / 别用网络盘）；本地盘为什么和节点同生共死。（Q11、Q12）
11. **定责**：决策树 Pending / Bound / Terminating 三大分支各从哪进；60 秒剧本四步各敲什么。（Q14）

十一条都能不看稿讲下来，这一章就过了。终极测试：拦住开篇那三个误操作。下一章把配置和密钥剖开：[12](../12-配置与密钥/)。调度地基：[06](../06-调度机制/) · 节点操作：[17](../17-节点运行时与升级/)。

...
**一句话**：应用只引用 PVC；动态供给走 SC → CSI Controller 建盘 → Node 挂载。
...
**一句话**：RWO 云盘同一时刻只能挂一个节点；Deployment 多副本不能共用一块 RWO 盘。
...
**一句话**：生产库盘用 Retain；Delete 删 PVC 会删云盘——误删先停写再查回收站/快照。
...
**一句话**：Terminating 先看 CSI finalizer 和云侧 detach，不要第一反应手抠 finalizer。
...
**覆盖**：必会 · 常用 · 常考
**对应**：知识点 §3.1 WaitForFirstConsumer
...
**一句话**：`waiting for first consumer` 常是 WFFC 正常等 Pod 调度，不是 CSI 坏了。
...
**一句话**：FailedAttachVolume：RWO 盘还没从旧节点 detach 完，新 Pod 在另一节点等 attach。
...
**一句话**：删 STS 默认不删 PVC；缩容不会自动释放云盘费用，要显式删 PVC。
...
**一句话**：PVC 扩容单向；VolumeSnapshot 是盘级，MySQL 回档要先 flush 或停写。
...
**一句话**：生产默认动态 SC + CSI；静态 PV 适合导入已有盘或误删后的补救。
...
**一句话**：战斗热状态在内存里，emptyDir/NFS 救不了杀进程；回档盘用 Retain + 快照。

...
开始做题前：合上知识点，对着**第一节的主图**，把「一块盘的一生」口述一遍——申请 → 出生 → 挂载 → 使用 → 回收，外加另一条路 emptyDir / HostPath。讲得出来再做题。
...
题目顺序：Q1～Q4 三角色与挂载；Q5～Q8 使用与回收；Q9～Q13 扩容快照与落盘；Q14 定责剧本。
...
十一条都能不看稿讲下来，这一章就过了。终极测试：拦住开篇那三个误操作。

...
> 大家好，欢迎来到这次分享。开讲之前，先带大家回到一个很多同学白板卡壳的现场：
...
> 这一章讲完，你会知道 SRE live coding 考的不是 ACM 金牌——题面窄、时间紧、要能说清 trade-off，卡壳也要会沟通。
> **本章回答**：一道 **live coding**从「审题」到「AC」完整走一遍——限流器、LRU、TopK、生产者消费者四条主线，外加 SRE 面试里怎么沟通、怎么测、怎么收尾。
> **本章不讲**：LeetCode 全集与动态规划 → 算法竞赛范畴；Go 语言语法全集 → [05-03-01](../03-Go/01-基础语法与类型系统/)；channel 细节 → [05-03-06](../03-Go/06-channel与select/)。这些都有专门的章节，本章只在用到时点到为止。

**怎么听这场分享**：主线是「一道 live coding 的一生」——审题四问 → 限流/LRU/TopK/生产者消费者 → 复杂度与生产化收尾；速查模板可背。每一节讲完提示对应练习题。

🔥 大白话：LRU 的 map 存的是**指向链表节点的指针**，不是再扫一遍 list；接反了复杂度直接穿帮。

大家想想：上来就写 `map`+锁，面试官问滑动窗口为啥卡？——因为**没先问「精确还是近似、按秒还是按分钟」**。审题四问比第一行代码值钱。
...
口诀：**「单机还是分布式？精确还是近似？QPS？内存上限？」**。练习题 Q2。审题会了，限流器。
...
⭐ 令牌桶 vs 滑动窗口。练习题 Q3、Q4。限流会了，LRU。
...
🔥 list+map 接反就 O(n)。练习题 Q5、Q6。LRU 会了，TopK。
...
本节对应练习题 Q14。作战会了，四题速查（背诵用，字面别改乱）。
...
**回到开头那个现场**：闷头 map+锁、LRU 接反 list/map。正确剧本是：审题四问 → 选结构说 trade-off → 先写正确再优化 → 边界用例自测 → 收尾说复杂度和生产化。现在再看面试官那道限流器，你知道第一句该问什么了。

...
> 白板 coding **最高权重**：先问约束、守四条红线、选对语言，写出能进生产的关键骨架——考官看失败可见，不看语法大全。
...
| 日志 5xx | 列号、时间窗、10GB 策略 | [05 正则](../05-正则与文本处理/) |
| K8s 巡检 | JSON、P1/P2、exit 1 | [02 Python](../02-Python运维/) |
| 批量 SSH | sem、Timeout、failed 汇总 | [03 Go](../03-Go运维工具/) |
| 限流 | 令牌桶、429、POST 幂等 | [02 Python](../02-Python运维/) |
| 告警/门禁 | 飞书 timeout、Prom canary | [03-K8s 18](../../03-Kubernetes/18-监控与可观测性/) |
...
**本章不讲：** Shell 三开关 → [01](../01-Shell/)；ThreadPool → [02](../02-Python运维/)；pprof → [03](../03-Go运维工具/)；awk/jq → [05](../05-正则与文本处理/)。
...
flowchart LR
    ...
    classDef core fill:#2563EB,stroke:#1E3A8A,stroke-width:2px,color:#fff
    class A,E core
```
...
1. 白板卡住先问哪四个约束？四条红线？
2. 第 50 台 nginx -t 失败，前 49 台怎么办？
3. combined 日志 IP/status 哪列？10GB 怎么查？
4. K8s 为什么 `-o json`？P1 和 exit code？
5. Go SSH sem/Timeout/failed 汇总？
6. 令牌桶 vs 漏桶？429 后 POST 能重试吗？

---

**下一章**：[05 正则与文本处理](../05-正则与文本处理/知识点.md)

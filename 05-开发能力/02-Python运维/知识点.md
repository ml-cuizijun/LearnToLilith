...
**本章不讲：** Shell 三开关、trap → [01 Shell](../01-Shell/)；Exporter / goroutine / pprof → [03 Go 运维工具](../03-Go运维工具/)；批量部署整题 → [04 运维开发实战题](../04-运维开发实战题/)；正则抠字段 → [05](../05-正则与文本处理/)；GitOps 流水线 → [05-工程化](../../04-工程化与自动化/)、[03-K8s 22](../../03-Kubernetes/22-GitOps-ArgoCD与Tekton/)；开服业务门禁 → [08-游戏 02](../../07-游戏运维专题/02-开服合服/)。
...
**打个比方**：像快递显示「已签收」，其实包裹还在路上——`except pass` 就是假签收。

**现场走一遍**（批量开服 Cron，3 台失败被吞）：

1. 脚本里写了 `except Exception: pass`，50 台开服，103/117 超时。

2. Cron 页面：

```text
Job: open_servers  Status: SUCCESS  Exit code: 0
```

3. 游戏目录里缺 103、117 两个服——直到玩家投诉才发现。

4. **输出解读**：没有 `sys.exit(1)`，Python 默认 exit 0；`log.exception` + `return 1` 才是失败可见。
...
**面试怎么说**：「Cron 绿但没干活，先查有没有 `except pass` 和 future 不 `result()`；生产脚本必须 argparse、logging、失败 `sys.exit(1)`。」
...
**打个比方**：像打电话不设响铃上限——对方不接，你的线一直占着，后面 200 个电话都打不出去。

**现场走一遍**（巡检脚本 8 workers 打网关，下游 hang）：

1. `requests.get(url)` **无 timeout**，网关某接口卡住。

2. 屏幕：`top` 里 Python 进程 CPU 很低，脚本跑了 40 分钟还没完——8 个 worker 全占满。

3. 补上 `timeout=(3, 10)` 后，ConnectTimeout / ReadTimeout 抛异常，failed 汇总，2 分钟内结束。
...
**输出解读**：`ConnectTimeout` = 对端没起来；`ReadTimeout` = 对端卡住；Retry `allowed_methods=GET/HEAD` 否则 POST 双写。

**面试怎么说**：「HTTP 必设 `timeout=(3,10)`；Retry 只给 GET，POST 开服重试会出两个 server_id。」
...
**打个比方**：像派 8 个同事同时打电话——你不逐个问结果，只知道「都打完了」，中间谁没打通不知道。

**现场走一遍**（50 服 `ThreadPoolExecutor(max_workers=8)`）：

1. 错误写法：只 `as_completed` 不 `f.result()`，103 号抛异常。

2. 脚本末尾 `exit 0`，日志里有一行 traceback 被忽略。
...
for f in as_completed(futs):
    try:
        ...
        log.exception("server_id=%s", futs[f])
        failed.append(futs[f])
if failed:
    sys.exit(1)
```

**面试怎么说**：「`as_completed` 必须 `result()`；failed 非空 `sys.exit(1)`；workers 5–20，200 会打爆网关。」
...
**打个比方**：像进机房——笔记本带工牌（kubeconfig），机房里 CronJob 用门禁卡（SA），不能把万能钥匙（cluster-admin）挂 Cron 上。

**现场走一遍**（CronJob 等 Deployment ready）：

1. Pod 内：`config.load_incluster_config()` → 读 SA token。
2. 循环 `read_namespaced_deployment`，`ready_replicas >= want` 才 return。
3. 超过 600s → `TimeoutError` → `sys.exit(1)`，CI 变红。

**面试怎么说**：「脚本巡检 get/list 最小 SA；scale/patch 走 GitOps，不给 Cron cluster-admin。」
...
**打个比方**：像试吃——只看新锅里的菜，不能看全食堂平均咸度，否则新锅放多了盐也被平均值盖住。

**现场走一遍**（金丝雀 10% 流量，集群错误率仍 0.1%）：

1. PromQL：`rate(http_errors{canary="true"}[5m])` vs 发布前 5min baseline。
2. 金丝雀实际 8% 错误，集群均值 0.1% → **应拦**，不能放行。
3. Prom 挂了或没数据 → 生产默认 **exit 1 不放行**。

**面试怎么说**：「门禁按 canary label 查 Prom，对比发布前 5min baseline；健康检查打登录/核心 API，不只 `/health`。」
...
每个模板附 **现场走一遍** + **典型输出** + **输出解读**。

### 模板 A：open_server.py（完整骨架）

**场景**：Cron 调开服 API，第一次必须 dry-run 贴变更单。

**现场走一遍**：

1. `export TOKEN=xxx && python open_server.py --server-id 101 --dry-run` → 只打印 POST URL，不调 API。
2. 去掉 `--dry-run` 实跑 → `[INFO] opened server_id=101`。
3. 故意错 token → `[ERROR]` + traceback，`exit=1`。
...
# 实跑成功
2026-08-28 10:05:00 [INFO] opened server_id=101

# 批量部分失败
2026-08-28 10:05:45 [ERROR] failed count=2 ids=[103, 117]
exit=1
```

**输出解读**：token 日志打 `***`；Cron 只看 exit code；Retry 在 session 里只含 GET/HEAD。

虚拟环境：`python3 -m venv .venv && . .venv/bin/activate && pip install -r requirements.txt`

### 模板 B：批量开服 + 失败汇总

**现场走一遍**：50 个 id 进 `batch_open(ids, workers=8)` → 任一失败 → `sys.exit(1)`，已成功部分需 API 幂等可重入。
...
1. CI 里 `python wait_rollout.py --deploy hall-api --ns prod --replicas 10`。
2. 每 5s poll `ready_replicas`；600s 内到 10 → log `rollout ok`。
3. 超时 → `TimeoutError` → exit 1，流水线停。

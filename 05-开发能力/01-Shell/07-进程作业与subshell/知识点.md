...
> 批量巡检脚本在 `( for h in ...; do ssh "$h" check; done )` 里改了 `failed=0`，循环外 `echo $failed` 仍是 0；有人 `nohup ./sync.sh &` 关掉笔记本以为任务在跑，第二天发现 SIGHUP 把进程带走了；还有人 `jobs` 看不到昨晚的后台任务——群里已经有人在说「脚本逻辑写错了」。
> 这一章讲完，你会知道当时那几个人各自错在哪——**不是命令不会写，是不懂子进程从 fork 到收尸的整条命**。
> **本章回答的问题**：一个**子进程**从 subshell/`&` 出生、在后台或前台干活、被 `wait`/`jobs` 认领、到 `nohup`/`disown` 脱离终端或随 shell 死亡——以及为什么 `( )` 里改的变量父 shell 看不见。
> **本章不讲**：三开关与 `trap` → [02](../02-脚本骨架与三开关/)；管道与 `PIPESTATUS` → [06](../06-管道重定向与PIPESTATUS/)；并发上限与 `xargs -P` → [08](../08-并发与限流/)；`getopts` 与 `--dry-run` → [09](../09-参数解析与dry-run/)。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：先把「一个子进程的一生」主图装进脑子——出生（`( )`/`$()`/`&`）→ 干活 → 收尸（`wait`）→ 脱离（`nohup`/`disown`）；放慢「变量为什么丢」和「关终端会不会死」；作战节回到开头那个 failed 计数器。每一节讲完会提示对应练习题，趁热打铁。
...
好，主图装进脑子了。咱们从「出生」开始——大家想想：`( failed=1 )` 外面为什么还是 0？
...
🔥 很多人会答「子进程里有个指针指回父变量」——⭐ 这是高频错答。大白话：**subshell 拿到的是拷贝，不是共享内存**。
...
🔥 开头那个「关掉笔记本任务没了」就在这里。口诀：**关终端发 HUP，要留命就 nohup 或 disown**。
...
- [ ] 默画「子进程一生」：出生 → 前后台 → jobs/`$!` → wait → nohup/disown（对应练习题 Q1、Q14）。
...
全部打勾之后，找一位同事十分钟讲清「开头那个 `( failed=1 )` 外面仍是 0、关终端任务没了——各自错在哪」。讲得下来，这章就过关了。

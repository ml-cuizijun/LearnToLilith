...
> 新人跑 `./purge-cache.sh --env prod` 没加 `--dry-run`，CDN 全量刷新把源站打穿；有人 `getopts` 只认 `-d` 不认 `--dry-run`，运维文档和脚本行为不一致；还有人 `rm -rf "$TARGET"/*` 在 `TARGET` 为空时删光根——**不是不会写 `if`，是参数从 argv 进脚本到真正执行危险动作之间没有闸门**。
> **本章回答**：一个脚本**参数**从 `argv`/`getopts` 解析、`-h`/`usage` 退出、**`--dry-run` 默认真**、危险操作要 **`--force`/`--yes` 显式确认**到 `main` 执行——以及为什么 `usage` 应 `exit 2`。
> **不讲**：三开关与 `trap` → [02](../02-脚本骨架与三开关/)；`failed[]` 批处理 → [02](../02-脚本骨架与三开关/)、[08](../08-并发与限流/)；复杂 CLI 框架 → 用 Python `argparse`（[05-02-Python/06-argparse](../../02-Python/06-argparse与配置管理/)）。
...
- [ ] 默画参数一生：argv → 解析 → 默认 → 校验 → 闸门 → main → exit。
- [ ] `getopts` 和 `case --dry-run)` 怎么配合？
- [ ] `DRY_RUN` 默认 true 时，真执行要满足哪几个条件（按模板）？
- [ ] `usage` 为什么走 stderr、exit 2？
- [ ] `shift $((OPTIND-1))` 在纯 getopts 流程里何时调用？
- [ ] `${dir:?}` 和 `set -u` 如何一起防 `rm -rf /*`？
- [ ] 命令行与环境变量谁覆盖谁？文档怎么写？
- [ ] 什么情况下该放弃 bash 参数解析换 Python？

...
> 发版脚本对 500 台同时 `ssh`，堡垒机 `MaxStartups` 打满、本机 `ulimit -n` 爆掉；有人 `for h; do ssh "$h" & done` 从不 `wait`，CI 假绿；两个 cron 叠跑全量同步把对象存储打穿——群里喊「再跑一遍就行」。
> **本章回答**：一批**并发任务**从入队、受控并行（`wait -n` / `xargs -P` / 信号量）、`flock` 互斥，到失败汇总与退出码——以及为什么「无限 `&`」比串行更危险。
> **不讲**：subshell 与 `$!` 基础 → [07](../07-进程作业与subshell/)；三开关与 `failed[]` → [02](../02-脚本骨架与三开关/)；`getopts` 传 `-j` → [09](../09-参数解析与dry-run/)。
...
| **必会** | `flock` 防 cron 重叠；信号量 / 命名管道限流；失败列表 + `exit 1` |
| **常用** | `PARALLEL=10`；SSH 批量默认并发；`GNU parallel` 与 xargs 选型 |
| **常考** | 为什么「全 `&`」会假绿；`flock -n` 与 `exit 0`；`xargs` 空输入行为 |

> 📝 **面试（30 秒）**：批量默认**串行**最安全；要加速就设**并发上限**——`xargs -P 10` 或 `running` + `wait -n`。每个后台任务必须 `wait` 并记失败 host，最后 `exit 1`。cron 互斥用 `flock -n`，拿不到锁就退出。别一次扔 500 个 `ssh &`。危险变更仍要 `DRY_RUN`（09）。
...
- 🔑 **各机制管哪一段**
  - `xargs -P`：简单并行 + 上限——**不**管复杂依赖图
  - `flock`：单机单实例互斥——**不**是分布式锁（多机要 etcd）
  - 信号量文件：bash 原生限流——**不**能跨机器协调
  - `wait -n`：动态补位——bash **< 4.3** 没有

> ⚠️ **别混**：并发不是「越快越好」，是**在资源边界内尽快且可汇总地失败**。
...
- 🧩 **四种写法一眼比**
  - 串行 `for`：上限 1，失败汇总最简单
  - `xargs -P`：上限 = `-P`，失败汇总中等（要包装）
  - `&` + `wait -n`：自管上限，失败汇总中等
  - 全 `&` 不 wait：上限无限，失败汇总极难（假绿）
...
- 📖 **读图**
  - 🛤️ 主线：入队 → 两道闸（互斥 + 并发）→ 干活 → 收尸 → 交卷；卡在哪一站就查哪一站
  - 🚪 互斥闸 =「同一 job 只跑一份」；并发闸 =「同时最多 N 个」——两道都能过才开工
  - 📟 父 shell 终点必须 `wait` 收齐退出码写 `failed[]`；监控看并发数、失败率、锁占用、SSH 拒连率

本节对应练习题 Q1、Q14。

好，主图装进脑子了。500 个 `ssh &` 一次性扔出去，最先爆的是哪一层？
...
- 🔑 **一行一任务**：文件 / `printf` / `find` 喂流，别 `ssh host1 host2 ...` 一次塞 500 个参数

> ⚠️ **别混**：一次传 500 个参数会碰 `ARG_MAX`——用 `xargs` 或 `while read`。
...
- ✅ 生产更稳：**每个任务一个可执行脚本** `bin/deploy-one.sh "$host"`，xargs 调脚本
- 🍎 避免 `export -f` 在 macOS 默认 bash 3.2 上踩坑

> 🔍 **现场**：

```bash
ulimit -n                    # ← 1024 常见；并发 500 ssh 易炸
grep -c . hosts.txt          # ← 任务总量，对照 PARALLEL
```
...
- `xargs` 默认：**任一子进程非零 → xargs 最终非零**（GNU）
- 但**不知道哪台**——要脚本自己打日志：
...
#### ❓ 为什么 `xargs -P` 只给总退出码还不够？

- 🚨 **总码只告诉「有没有挂」**，不告诉「哪台挂」——值班要修机就瞎猜
- ✅ **失败文件 / 带 host 的 stderr**：每个工人自己写 `failed.$$` 或 `printf '%s\n' "$h" >&2`
...
```mermaid
flowchart LR
    Q["队列 hosts"] --> GATE{"running &lt; PARALLEL?"}
    GATE -->|是| RUN["cmd &amp;<br/>pid_host[$!]=host"]
    GATE -->|否| WN["wait -n<br/>腾一个坑"]
    WN --> RUN
    RUN --> GATE
    RUN --> DONE["收齐 · failed[] · exit"]
```

- 📖 **读图**
  - 🅿️ 车位满了就在出口等（`wait -n`）；谁开走就放下一辆——不是一次性放 500 辆
  - 🗺️ `pid_host` 把 PID 映射回 host，收尸时才能写进 `failed[]`

#### ❓ 为什么不能「全 `&` 再 `wait`」当并发池？

- 💥 **没有上限**：`wait` 只保证收尸，**不**限制同时跑多少——fd / SSH / 对端先爆
- ✅ **池 = 上限 + 补位**：满了 `wait -n` 腾坑再 `&`；不是先全扔出去再等
- ⚠️ **别混**：`wait -n` 在 `set -e` 下子失败会触发退出——循环里 `wait -n || true` 但要**自己记 `rc`**，别吞了不记 host

> 💡 **打个比方**：`wait -n` 像停车场——车位满了就在出口等，谁开走（子进程结束）就放下一辆进来。
...
> 💡 **打个比方**：`flock` 像公共厕所的门锁——进去的人从里面锁上，外面的人拧不动把手（`-n`）就走了（exit 0）。锁随脚本生命周期（`exec 9>`），人出来自动开锁。
...
#### ❓ 为什么拿不到锁常 `exit 0` 而不是 `exit 1`？

- 📅 **cron 重叠**：故意跳过 = 正常业务决策 → 常选 **exit 0**，避免监控把「已有实例在跑」当故障风暴
- 🚧 **CI 门禁 / 互斥必须独占**：重叠 = 异常 → 选 **exit 1**，让流水线红
- 📝 **面试**：「拿不到锁 `exit 0` 还是 `exit 1`？cron 重叠常选 **0** 表示故意跳过；CI 门禁选 **1** 表示异常重叠。」——先问场景再答码。
...
acquire_sem() {
  ...
  # 全满则阻塞等任意（示意；生产优先 xargs -P）
  flock 8< <(ls "$SEM_DIR"/* | head -1)
}
```

- ✅ 更常见的生产写法仍是 **`xargs -P`** 或 **`flock` 单实例** + 合理 `PARALLEL`
- 🚫 不要手写复杂 sem 除非不能装 GNU 工具；面试会问「目录里最多 N 个 `.slot`」原理，生产别当真维护
...
- 👻 全 `&` 不 `wait`——父进程先 exit 0
- 🙈 `wait || true` 不记 host——CI 绿但不知道谁挂
- 🕳️ 子脚本里 `exit 0` 吞 ssh 失败——要 `set -e` + 正确判码
...
- `|| true` **只保证父进程不因 wait 失败而崩**——不保证你记了失败 host
- ✅ 正确姿势：`wait "$pid"; rc=$?` → `(( rc != 0 )) && failed+=(...)` → 最后 `((${#failed[@]})) && exit 1`
- 🔗 与 `DRY_RUN` 联用：并发只改变**时间**，不改变**安全默认**（09 章）
...
- 📖 **读图**
  - 左列四类事故 → 右列四类对策；可控是**映射**，不是「多写几行 `&`」
  - ⚠️ 右列**不保证**远端回滚——第 50 台失败前 49 台可能已 reload
...
8. macOS 注意 bash 3.2 / xargs 差异——`#!/usr/bin/env bash` 指 Homebrew bash 或降功能

> 📝 **面试**：八件套按「上限 / 收尸 / 互斥 / 交卷」四组背，别背成无序 bullet。
...
> 🔍 **现场**：接到「批量 SSH 挂了 / 双 cron」，60 秒走完再改生产。
...
- 📖 **读图**
  - 🌪️ SSH 风暴先问并发数：>50 先降闸，低并发才查单台
  - 🔁 数据重复先问有没有 flock；有锁仍坏 → `lsof` 查僵死持锁
...
| 锁永不释放 | 持锁进程僵死 | trap EXIT；`lsof` 杀僵死 |
...
- `for h; do ssh & done` 不用 wait → 父先 exit 0，CI 假绿；必须收尸 + `failed[]`
- `xargs -P` 会告诉你哪台挂 → 只给总码；失败写文件带 host
- 全 `&` 再 `wait` = 有并发池 → `wait` 不管上限，必须先限流再补位
- `flock` 能锁多机 → 单机 advisory；多机用分布式锁
- cron 拿不到锁必须 exit 1 → 常 skip 用 0；CI 独占才用 1
- 并发越高越快 → 过拐点更慢；SSH 批量常用 10～20
- `wait || true` 就安全了 → 不记 host 仍假绿；记 `rc` 再汇总
- `PARALLEL=500` 合理 → 先查 `ulimit -n` / MaxStartups
- `wait -n` 到处都能用 → 要 bash 4.3+；老环境降 `xargs -P`
- DRY_RUN 与并发无关 → 先 dry 再提并发；安全默认不变

### 数字墙

> SSH 批量 `PARALLEL` 常用 **10～20**；`ulimit -n` 默认 **1024** 量级要心里有数。`wait -n` 要 bash **4.3+**。并发拐点：打满 fd/SSH 后更慢，不是线性加速。
...
- [ ] **默画**「一批任务并发一生」：入队 → flock → -P/wait-n → wait → failed → exit（Q1、Q14）

...
> 活动前有人把 `deploy.sh` 拷到跳板机直接 `sh deploy.sh`，`[[` 语法报错；CI 里 `kubectl apply` 明明失败，流水线却绿了——根子往往不在业务，而在**一条 shell 命令从敲下到进程退出**这段路你没看清。
...
**读图**：从左上「输入」到「退出码」是**单条简单命令**的路径；管道是多个简单命令串起来（见 [06](../06-管道重定向与PIPESTATUS/)）。监控和 CI 只看最后 `$?` 是否为 0——中间日志再漂亮，非 0 就是失败。
...
**shebang** 是文件第一行 `#!` 开头的解释器路径。内核 `execve` 脚本时，会读 shebang，再用指定解释器打开本文件。
...
> ⚠️ **别混**：「命令打了 ERROR 日志」≠「退出码非 0」。很多工具失败仍 exit 0——**CI 要看 `$?`，不是 grep ERROR**。
...
- [ ] 画出「输入一行 → 退出码」主路径，标出展开、分词、重定向、fork/exec 四站。
- [ ] `#!/usr/bin/env bash` 比 `#!/bin/bash` 好在哪里？CI 写 `sh deploy.sh` 会怎样？
- [ ] `source env.sh` 和 `./env.sh` 对父 shell 环境有什么不同？
- [ ] `cmd >f 2>&1` 和 `cmd 2>&1 >f` 差在哪？各举一个线上后果。
- [ ] 为什么 `cd` 必须是内置？子 shell `()` 里 `cd` 外面能看见吗？
- [ ] kubectl 失败但 CI 绿——列三个执行模型层面的原因和查法。
- [ ] `exec` 适合什么场景？exec 之后还能不能 `trap` 清理？
- [ ] 退出码 127、130、137 各常表示什么？

...
> 凌晨发版脚本跑到一半被 SIGTERM 掐断，临时文件堆在 `/tmp`；有人把 `set -e` 去掉「好调试」，第 50 台 `nginx -t` 失败后面 49 台仍 reload——**不是命令不会写，是脚本没有「从启动到安全退出」的骨架**。
> **本章回答**：生产 Bash 脚本的标准生命周期——三开关筑基、`trap` 善后、`main` 收口、退出码交给 CI。
...
**读图**：`trap EXIT` 在**任何**离开路径都会跑（正常结束、`exit`、信号）——临时文件和锁在这里收。CI 只认最后 `$?`；Jenkins `set -e` 不等价于脚本里的三开关，**脚本自己要写**。
...
若 `cleanup` 里最后裸 `exit` 无参数，会把失败**洗成 0**。
...
- [ ] 默写脚本头 15 行：shebang、三开关、trap、cleanup 保留 `$?`、`main "$@"`。
- [ ] 没有 `pipefail` 时，`false | true` 的 `$?` 是多少？对 CI 意味着什么？
- [ ] `set -e` 下写 `for h in ...; deploy "$h"; done` 和 `deploy "$h" || failed+=` 差在哪？
- [ ] SIGTERM 和 EXIT trap 执行顺序？为什么要单独 trap TERM？
- [ ] 画批量发布数据流：hosts 文件 → dry-run → scp → nginx -t → reload → failed 列表。
- [ ] 说出三个让 CI「假绿」的 Shell 写法及修法。
- [ ] `main "$@"` 比直接在全局写逻辑好在哪里？
- [ ] 第 50 台失败时，三开关能保证前 49 台回滚吗？应该怎么做？

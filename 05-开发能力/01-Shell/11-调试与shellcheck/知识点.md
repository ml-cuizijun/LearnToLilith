...
> CI 里发版脚本红了，本地 `bash deploy.sh` 却又绿；同事说「加 `set -x` 看看」，一打日志把数据库密码全进 Jenkins 归档——群里已经有人在说「先去掉 `set -e` 好」。
> 这一章讲完，你会知道当时那两个人各自错在哪——**不是不会改 bug，是排错手段没有按阶段用**。
> **本章回答的问题**：一次脚本排错从「复现 → 静态检查 → 跟踪执行 → 收敛修复 → 进 CI 门禁」的完整路径——`bash -n`、`shellcheck`、`set -x`、`PS4`、ERR trap 各在什么阶段上场。
> **本章不讲**：三开关语义 → [02 脚本骨架与三开关](../02-脚本骨架与三开关/)；管道与 `PIPESTATUS` → [06](../06-管道重定向与PIPESTATUS/)；完整生产模板 → [12](../12-生产脚本规范与模板/)。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：先把「一次排错的一生」主图装进脑子——复现 → `bash -n` → shellcheck → `set -x` → 修复 → CI 门禁；放慢泄密与 SC 规则；作战节专门讲开头那个「本地绿 CI 红 / 密码进日志」。每一节讲完会提示对应练习题，趁热打铁。
...
好，主图装进脑子了。咱们先把排错顺序口诀钉死：**先静态，再跟踪，秘密不进日志**。
...
🔥 `bash -n` 只抓语法；逻辑坑、未引用变量靠 shellcheck。大家别一上来就 `set -x`——日志一炸，秘密可能已经进归档了。

...
> CI 里发版脚本红了，本地 `bash deploy.sh` 却绿；同事喊「加 `set -x` 看看」，一打日志把数据库密码全进 Jenkins 归档——群里已经有人说「先去掉 `set -e` 好」。
> **本章回答**：一次脚本排错从「复现 → 静态检查 → 跟踪执行 → 收敛修复 → 进 CI 门禁」的完整路径——`bash -n`、`shellcheck`、`set -x`、`PS4`、ERR trap 各在什么阶段上场。
> **不讲**：三开关语义 → [02 脚本骨架与三开关](../02-脚本骨架与三开关/)；管道与 `PIPESTATUS` → [06](../06-管道重定向与PIPESTATUS/)；完整生产模板 → [12](../12-生产脚本规范与模板/)。
...
> 🎯 **一句话**：先静态后跟踪；`-x` 会展开秘密；shellcheck 进 MR——三句过关。

| 标记 | 考点 |
|------|------|
| **核心** | `bash -n` 语法；`shellcheck` 静态；`set -x` / `PS4` 跟踪；**禁止**生产日志打秘密 |
| **必会** | `-x` 展开变量；常见 SC 编号；CI 跑 `shellcheck -e SC1091` 等 |
| **常用** | `trap '...' ERR` 打行号；`BASH_XTRACEFD`；调试完 `set +x`；`bash -x script 2>trace.log` |
| **常考** | 为什么 `-n` 抓不到逻辑错；`-x` 与 `-e` 同开注意什么；如何把 shellcheck 进 MR |

**30 秒怎么说**：排错分阶段——先 **`bash -n`** 抓语法，再 **`shellcheck`** 抓引号/未引用变量，还看不懂才 **`set -x`** 看展开后的真实命令。`-x` 会打秘密，用 `BASH_XTRACEFD` 重定向、脱敏或只在 staging 开。`PS4` 自定义跟踪前缀（行号、函数名）。CI 门禁：`bash -n` + `shellcheck -S warning` + dry-run。别用「去掉 `set -e`」当调试手段。

---

## 能力边界

> 🎯 **一句话**：静态先行、跟踪兜底、秘密不进日志——值班排障和 CI 门禁共用这套顺序。
...
> 🎯 **一句话**：调试三件套按顺序上场——`-n` → shellcheck → 局部 `-x`。
...
- 📖 **读图**
  - 🛡️ 多数 bug 在 **C/D** 就死掉，不该一上来 `-x`
  - 🔭 只有「展开后到底跑了啥」才进 **F**；最后 **I** 把教训写进流水线
  - 📊 监控侧：CI 失败看**日志末尾退出码**和 **shellcheck 报告**，不是 grep ERROR 行数
...
- 🗺️ **常见差异源**
  - 🐚 `sh` vs `bash`（查 CI 是否 `sh script`）
  - 📁 工作目录（`pwd`、是否依赖 `SCRIPT_DIR`）
  - 🌍 环境变量（`env | grep -E 'KUBE|DRY'`）
  - 📄 空输入（hosts 文件在 CI 是否存在）

#### ❓ 为什么本地绿、CI 红，先别怪脚本逻辑？

- 🎯 **80% 是解释器 / env / cwd 不一致**，不是「同一份代码两种命运」
- ✅ 先复现：**显式 `bash`** + 同参 + 同 env 片段，再开 `-n` / shellcheck
- ⚠️ **别混**：本地加 `bash -x` 对不上 CI，若 CI 用 `sh`——跟踪再准也白费

> 📝 **面试**：「本地复现要带 CI 同款 `-n` 和 shellcheck；差异 80% 是解释器和 env。」⭐

> ⭐ **常考**：「本地绿 CI 红」三排查点——解释器、环境变量、工作目录。
...
```bash
bash -n broken.sh
# broken.sh: line 42: syntax error near unexpected token `fi'   # ← 语法树坏了
...
- 🧩 `-n` **不跑命令**——只管语法树
- 💥 抓不到：`[[ -f /missing ]]` 分支错、算术错、`set -u` 未定义变量
- ✅ 下一步：shellcheck（引号/SC）→ 最小复现 + 局部 `-x`

> ⚠️ **别混**：`-n` ≠「脚本正确」；它只证明「能解析」。
...
🔥 `bash -n` 只抓语法；逻辑坑、未引用变量靠 shellcheck。别一上来就 `set -x`——日志一炸，秘密可能已经进归档了。
...
- 🧭 shellcheck 会提示 `set -e` 被 `|| true` 糊掉、`cd` 失败未检查——和 [02](../02-脚本骨架与三开关/) 骨架一致；别为了绿关规则，要改脚本
- 💾 本地越早失败越便宜：扩展 `onSave`；pre-commit 跑 `shellcheck -x -S warning`
- 🔍 **现场**：`shellcheck --version` 确认 CI 与本地同主版本
...
#### ❓ 为什么要写 `shell=bash` / `# shellcheck shell=bash`？

- 🐚 shebang 是 `#!/usr/bin/env bash` 时，shellcheck **仍可能按 sh 语法判**
- 💥 后果：`[[`、bashisms 被误报；团队规则漂移
- ✅ 配置文件或文件头显式声明；`external-sources=true` 配合 `-x` 跟 source
...
- 🤝 **纪律**：`-S warning` = MR 门槛；disable 是**借据**，不是静音键
...
🔥 开头那个「密码进 Jenkins」就在这里。`set -x` 会把展开后的真实 argv 打出来——生产长期开等于泄密。
...
#### ❓ 为什么 `-x` 会泄密，而「只 echo 命令」看起来没事？

- 🔎 `-x` 打印的是**展开后**的 argv——`$password` 已变成明文
- 🧾 **`set -v`** 打印原始行（未展开）；排错几乎只用 **`-x`**
- ✅ 缓解：局部开闭 · `BASH_XTRACEFD` · staging only · 别把 secret 写进被跟踪的命令

> ⚠️ **别混**：`set -v` ≠ `set -x`——v 看源码行，x 看真实执行。
...
set -x
main "$@"
# +(deploy.sh:88:deploy_host): ssh -n game-01 ...   # ← 对齐源码行
```

```bash
( set -x; sub_func )    # 仅子 shell 跟踪；外层不继承 -x
```

> 🔍 **现场**：Jenkins 日志搜 `^+` 即 xtrace；发现 secret → 立刻 `set +x` 再重跑。

> 🔥 **难点**：`BASH_XTRACEFD` 把 trace 写独立文件——不污染 CI 默认日志，也不把秘密冲进 stdout/stderr。
...
- 🟢 去掉 `-e` → 失败继续跑 → **CI 假绿**
- ✅ 用 `DRY_RUN`、局部 `-x`、单步 `bash -x -c 'fn'`——**别动三开关**（见 [02](../02-脚本骨架与三开关/)）
...
> 🎯 **一句话**：修 bug 后跑 **`bash -n` + `shellcheck` + 原失败用例 + dry-run**；把检查写进 CI / pre-commit。
...
> 📝 **面试**：「门禁顺序：`-n` 快筛 → shellcheck 抓风格/引号 → dry-run。xtrace 不进 CI 默认日志。」⭐

### 🧩 收束：排错凭什么不泄密、不假绿

```mermaid
flowchart TB
    subgraph Discipline[排错纪律]
        A[静态先于 -x]
        B[-x 局部 + 关秘密]
        C[不修 -e 来调试]
        D[CI 固化 shellcheck]
...
- 📖 **读图**
  - 🧱 四纪律缺一会重演：`-x` 打密码、去掉 `-e` CI 绿、没 shellcheck 引号 bug 反复入 main
  - 🚫 **不保证**：找到所有逻辑 bug（要测试）；替代 code review
...
> 🎯 **一句话**：先静态分流，再最小复现 + 局部 `-x`；全程 `-x` 上生产 = 事故剧本。
...
- 📖 **读图**：先问语法过不过 → 再问 SC 干不干净 → 才开局部 `-x`；最后把检查固化进 CI。
...
- **45–60s**：修完 dry-run；MR 加 shellcheck job
...
| `set +x` 忘关 | 后续全跟踪 | 函数内开闭 |
| 去掉 `-e` 调试 | 误学坏习惯 | DRY_RUN + -x |
| `local` 在全局 | SC2329 | 挪进函数 |
| xtrace 与子 shell | `( set -x; cmd )` 仅子层 | 外层先 set -x |
| shellcheck 误判 sh | shebang 是 bash | `# shellcheck shell=bash` |
...
> shellcheck 默认 **SC####** 编号；`bash -n` 退出码非 0 = 语法错；xtrace 默认 fd **2**（stderr）；`BASH_XTRACEFD` 常用 **19**；CI 门槛常用 `-S warning`。
...
- [ ] 画出排错主线：复现 → -n → shellcheck → -x → 修 → CI（对应练习题 Q1、Q14）。
- [ ] `bash -n`、`shellcheck`、`set -x` 各看见什么、看不见什么（Q2、Q4）？
- [ ] 写一条带行号的 `PS4`，说明输出长什么样（Q5）。
- [ ] 为什么 `set -x` 可能泄露 `curl -u user:$pass`？怎么缓解（Q4）？
- [ ] SC2086 和 SC2181 各是什么、怎么修（Q3、Q9）？
- [ ] ERR trap 和 `set -x` 如何分工？生产常驻哪个（Q11）？
- [ ] 本地绿 CI 红，列三个环境差异排查点（Q10）。
- [ ] 如何把 shellcheck 写进 GitLab CI / pre-commit？（口述步骤）（Q8）

全部打勾之后，找一位同事十分钟讲清「开头那个本地绿 CI 红、`set -x` 泄密、去掉 `set -e` 当调试——各自错在哪」。讲得下来，这章就过关了。

下一章把巡检/发布完整模板与 runbook、退出码契约写齐。

...
> 大家好，欢迎来到这次分享。开讲之前，先带大家回到一个发版脚本翻车的现场：
> 有人写 `[ $status = 200 ]`，变量一为空就变成 `[ = 200 ]` 语法错误；有人用 `[ $count -gt 10 ]` 比较 `"08"` 和 `"9"`，活动高峰误判「连接数正常」——群里已经有人在说「脚本逻辑写错了，重发」。
> 这一章讲完，你会知道当时那两个人各自错在哪——分支写错往往不是业务不懂，是**测试命令**和**流程控制**没按 Shell 语义写。
> **本章回答的问题**：一个分支判断从「测什么」（数值/字符串/文件）→「用什么测」（`[[ ]]` / `[ ]` / `test`）→「怎么走」（`if` / `case` / `&&` `||`）的完整路径。
> **本章不讲**：三开关与 `errexit` 交互 → [02 脚本骨架](../02-脚本骨架与三开关/)；`PIPESTATUS` 管道成败 → [06](../06-管道重定向与PIPESTATUS/)；复杂参数解析 → [09](../09-参数解析与dry-run/)。

**怎么听这场分享**：先把「一个分支判断的一生」主图装进脑子——选测试器 → 真/假 → if/case；第二到第八节顺着测什么、用什么测、怎么走往下走；原理讲透放慢三个炸点；生产模板可直接复制。每一节讲完会提示对应练习题，趁热打铁。
...
好，边界清楚了。先上主图——一个分支从条件到路径，中间过哪几站。
...
**读图**：咱们先把这张图当地图——菱形「选测试器」是面试高频——字符串、数值、文件三类**不能混操作符**。`$?` 是**上一条命令**的退出码，要在下一条命令跑之前立刻读。`set -e` 下未捕获的假分支可能导致脚本直接退出，见 [02](../02-脚本骨架与三开关/)。

好，主图装进脑子了。从「出生」开始——`[` 到底是什么？很多人会答「语法糖」——⭐ 这是高频错答。
...
🔥 大白话：`[` 像往邮局投表格——空一栏就「格式错误」退回，不是「条件为假」。
...
`[` 的坑认清了，生产 bash 脚本默认升级到 `[[ ]]`——为什么？下一节带着推。
...
测试器选好了，三类测什么不能混——数值、字符串、文件，操作符各走各的。
...
🔥 开头那个「08 和 9 误判」就在这儿。大家想想：为什么 `(( a > 7 ))` 里 `a=08` 会报错？
```bash
count=42
[[ $count -eq 42 ]]    # ← 真（整数相等）
...
三类测试会了，怎么把真/假接到路径上？——`if` 测的是**命令退出码**。
...
二选一用 `if`；多枚举值用 `case`——退出码、服务名、HTTP 前缀，都适合。
...
口诀：**`case` 多枚举，漏 `;;` 会穿透**。简单二选一还可以用短路——但别把 `&& ||` 当 if-else。
...
🔥 很多人把 `A && B || C` 当成 if-else——⭐ 这是高频错答。看完比方你就知道为什么。
...
短路能写短，复杂逻辑仍回 `if`。最后一站：退出码怎么交给 CI。
...
退出码讲完，咱们放慢三个炸点——空变量、数值字符串混比、模式 vs glob。
...
现在再看群里那句「脚本逻辑写错了，重发」，你知道该怎么拦住他了——先查测试器和比较操作符，别急着重发。下面是可复制的生产模板（代码块一字不改）。
...
- [ ] 画分支判断主线：`[[`/`[`/`((` 怎么选？（对应练习题 Q1、Q2）
- [ ] 空变量时 `[ $x = y ]` 为什么语法错误？三种安全写法？（Q1）
- [ ] 比较 `count` 是否大于 10，写两种正确写法；为什么别用 `"9" -gt "10"`？（Q4）
- [ ] 文件检查 `-e` `-f` `-s` 组合表示什么？日志轮转前为什么要 `-s`？（Q6）
- [ ] `case` 里 `1|2|3)` 和 `5*)` 分别匹配什么？漏 `;;` 会怎样？（Q8）
- [ ] `$?` 应该在什么时候立刻读取？和 `set -e` 怎么配合？（Q10、Q11）

全部打勾之后，找一位同事十分钟讲清「开头那个 `[ $status = 200 ]` 为什么炸、活动高峰为什么别用 `-gt` 比字符串」。讲得下来，你就是下一个能拦住「重发」冲动的人。

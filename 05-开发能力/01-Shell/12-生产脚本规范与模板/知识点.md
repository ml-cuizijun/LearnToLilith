...
> 凌晨三点 oncall 翻 Git 找发版脚本，三个目录各有一份 `deploy.sh`，没有 README、退出码不统一、失败只 `echo ERROR`——Jenkins 绿着，游戏服却半套配置；有人想回滚，发现 runbook 链接是 404。
> **本章回答**：一份生产脚本从「规范立项 → 骨架 → 日志与退出码 → 巡检/发布完整模板 → 评审上线 → runbook 联动」的全生命周期——能入库、能交接、能让 CI 和值班都信。
> **不讲**：三开关细节 → [02 脚本骨架与三开关](../02-脚本骨架与三开关/)；参数解析 → [09 参数解析与dry-run](../09-参数解析与dry-run/)；SSH 批量 → [10 SSH批量与远程执行](../10-SSH批量与远程执行/)；静态 lint → [11 调试与shellcheck](../11-调试与shellcheck/)。
...
- [ ] 画出脚本从立项到 runbook 回滚的组织生命周期。
- [ ] README 最少写哪六项？
- [ ] 退出码 0/1/2/127 各表示什么？Jenkins 认什么？
- [ ] 巡检与发布模板核心差异（是否改状态、DRY_RUN、备份）？
- [ ] 为什么日志要 stderr、指标要 stdout？
- [ ] `cleanup` 里为什么要 `exit "$rc"`？
- [ ] MR 进 main 前 CI 应跑哪三步？
- [ ] runbook rollback 节至少写哪三条命令级步骤？

...
> 活动前有人写 `for h in $(cat hosts); do ssh $h "systemctl reload nginx"; done`，三台里一台密钥过期——脚本在 `Are you sure you want to continue connecting` 卡住，后面 200 台全堵在 stdin；还有人远程命令里嵌 `$()`，本地展开把生产密码打进 `ps`。
> **本章回答**：一条远程命令从「本地拼串」到「远端执行、退出码回传、批量收敛」的完整链路——引号地狱、BatchMode、超时、并发与失败列表怎么写才入库。
> **不讲**：OpenSSH 协议与密钥生成 → [01-26 SSH与远程访问](../../../01-基础底座/26-SSH与远程访问/)；引号与 `${var:-}` → [03 变量引号与参数展开](../03-变量引号与参数展开/)；`xargs -P` 原理 → [08 并发与限流](../08-并发与限流/)；完整发布模板 → [12 生产脚本规范与模板](../12-生产脚本规范与模板/)。
...
- [ ] 画出「本地拼串 → ssh 鉴权 → 远端执行 → 退出码 → failed 列表」主路径。
- [ ] `ssh host 'echo $USER'` 和 `ssh host "echo $USER"` 在哪边展开 `$USER`？
- [ ] 批量脚本为什么必须 `BatchMode=yes` 和 `ssh -n`？各解决什么卡死？
- [ ] `ConnectTimeout` 和 `timeout 60 ssh` 分别管哪一段？
- [ ] `set -e` 下如何既跑完全部主机又在最后让 CI 红？写伪代码。
- [ ] 并发 500 台可能触发哪些基础设施限制？你怎么设上限？
- [ ] 复杂远程逻辑，你会选 `ssh cmd`、双引号、还是 `bash -s` heredoc？为什么？
- [ ] 说出 ssh/scp 退出码 255 与远端命令退出码 1 在排障上的区别。

...
> 巡检脚本里 `check_disk` 明明 echo 了 `OK`，上层 `if check_disk; then` 却走了失败分支；有人把 `local` 写在函数外，整脚本变量全空；还有人 `result=$(get_ip)` 却只收到空行——stderr 里明明有报错。群里已经有人在说「探活函数写坏了，先注释掉」。
> 这一章讲完，你会知道当时那几个人各自错在哪——根子往往不在业务探活，而在**一次函数调用的三条通道**没分清。
> **本章回答的问题**：一次**函数调用**从定义到返回，参数怎么进、变量在哪活、`stdout`/`stderr`/`exit code` 三条通道各传什么、调用方怎么接。
> **本章不讲**：subshell 与 `( )` 的差异、后台 `&`/`wait` → [07](../07-进程作业与subshell/)；`set -euo pipefail` 三开关 → [02](../02-脚本骨架与三开关/)；管道中间失败 → [06](../06-管道重定向与PIPESTATUS/)。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：先把「一次函数调用的一生」主图装进脑子——定义 → 参数 → 作用域 → 执行 → 双通道 → 退出码；第二到第八节顺着这条线走；第九节生产模板可直接复制（代码块一字不改）；第十节专门讲开头那个「`if` 失败 / `$()` 空 / local 写穿」怎么定责。每一节讲完会提示对应练习题，趁热打铁。
...
好，边界清楚了。咱们先上主图——一次函数调用从进门到回程，中间过哪几站。
...
大家先把这张图当地图：开头那个「echo 了 OK 却 `if` 失败」——多半是 stdout 和退出码两条线拧了。好，从「定义」出发。
...
⭐ 这是面试必考：透传永远 `"$@"`。大家想想，路径里有空格时，未加引号的 `$*` 会怎样？
...
🔥 很多人会答「函数里的变量当然是局部的」——⭐ 这是高频错答。大白话先说清：**Bash 函数默认跟外面共用一张变量表**；不写 `local`，你改的就是全局。
...
好，到这儿咱们已经知道变量在哪活。接下来自然会问：函数体里命令失败了，会不会像外面一样立刻停？
...
🔥 这是开头那个「`$()` 空、终端却有输出」的根因。大家先别急着看修法——先问自己：终端「看得见」的，一定是 stdout 吗？
...
口诀记一句：**数据进筐（stdout），喇叭给人听（stderr）**——`$()` 只收筐里的。
...
🔥 很多人会答「函数能 `return "ok"`，跟 Python 一样」——⭐ 这是高频错答。先抛问题：`return` 传的到底是什么？
...
好，到这儿三条通道各自讲清了。接下来把它们收成一套能上线的写法。
...
回到开头那个现场：`if check_disk` 失败、`$()` 空、`local` 写穿——按这棵树定责。
...
- [ ] **默画**「一次函数调用的一生」：定义 → 参数 → 作用域 → 执行 → stdout/stderr → 退出码 → 调用方接收。（对应练习题 Q1、Q14）
...
全部打勾之后，找一位同事十分钟讲清「开头那个 `if check_disk` 失败、`$()` 空、`local` 写穿——各自错在哪条通道」。讲得下来，你就是下一个能拦住「先注释掉探活」冲动的人。

...
> 这一章讲完，你会知道当时那几个人各自错在哪——根子往往不在业务 YAML，而在**一条管道的退出码契约**和 **`2>&1` 的顺序**。
> **本章回答的问题**：一条**管道/重定向**从命令产生字节到落盘或进下一节，fd 0/1/2 怎么分叉合并、`2>&1` 为什么顺序致命、`|` 只把谁的退出码交给 `$?`、`PIPESTATUS` 和 `pipefail` 怎么补洞。
> **本章不讲**：subshell 与 `( )` → [07](../07-进程作业与subshell/)；三开关总览 → [02](../02-脚本骨架与三开关/)；函数内通道分工 → [05](../05-函数与作用域/)。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：先把「一条管道的一生」主图装进脑子——出生写 fd → 重定向分叉 → 管道接力 → 退出码交卷；中间放慢 `2>&1` 顺序和 `pipefail`；作战节专门讲开头那个「CI 假绿 / 日志丢 stdout」。每一节讲完会提示对应练习题，趁热打铁。
...
好，主图装进脑子了。咱们继续往下走——大家想想：一条命令往屏幕打字，到底走的是哪条 fd？
...
🔥 这是开头那个「日志只有 stderr、stdout 丢了」的根因。很多人会答「`2>&1` 写哪都行」——⭐ 这是高频错答。先抛问题：`>` 和 `2>&1` 谁先执行，fd2 会指到哪？
...
🔥 大白话：`PIPESTATUS` = **管道每一节的退出码数组**，像黑匣子把每一段的成败都录下来。大家想想：`$?` 只告诉你最后一节，中间挂了你怎么查？
...
🔥 这是开头「CI 假绿」的保险丝。口诀：**管道要问责，`pipefail` 一起开**。
...
- [ ] 写出正确日志合并一行，并解释错误顺序 `2>&1 >file` 发生了什么（Q2）。
- [ ] `false | true` 默认 `$?` 是多少？开 `pipefail` 后呢（Q1、Q3）？
- [ ] `kubectl apply | grep -q .` 失败时，如何用 `PIPESTATUS` 判断是 apply 还是 grep（Q4、Q5）？
- [ ] 为什么管道不传 stderr？要让错误也进 `grep` 怎么写（Q6）？
- [ ] `set -e` 为什么管不住管道中间失败？缺哪一块（Q7）？
- [ ] `PIPESTATUS` 为什么必须「立刻读」？插一个 `echo` 会怎样（Q4）？
- [ ] 说出管道可靠性三件套，各解决什么问题（Q14）。

全部打勾之后，找一位同事十分钟讲清「开头那个 `kubectl apply | grep` 假绿、`2>&1` 写反丢 stdout——各自错在哪」。讲得下来，你就是下一个能拦住「流水线绿了就发」冲动的人。

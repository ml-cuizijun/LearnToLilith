...
> **本章回答**：模块的一生——`go mod init`、module path、`require`/`go.sum`、目录布局（`cmd`/`internal`/`pkg`）、`go build`/`go test`、版本与 `replace`/`vendor`。
...
- [ ] 画出模块一生：init → tidy → test → build。
- [ ] `go.mod` 与 `go.sum` 各解决什么？
- [ ] 为什么 main 放 `cmd/` 而不是根目录？
- [ ] `internal` 和 `pkg` 选型一句话？
- [ ] `checksum mismatch` 你怎么处理？
- [ ] `replace` 和 `go.work` 区别？
- [ ] CI 最少跑哪三条 go 命令？
- [ ] `GOOS=linux go build` 与 module 有关系吗？

八题能口述，工程化交付就过关。下一章进 **零值与 slice/map 陷阱**，避免「能编译但数据串了」。

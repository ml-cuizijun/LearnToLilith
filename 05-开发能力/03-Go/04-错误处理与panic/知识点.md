...
> 这一章讲完，你会知道当时那几个人各自错在哪——error 要 wrap 能 Is/As，panic 有边界，假成功是运维 CLI 头号元凶。
> **本章回答的问题**：`error` 接口、`errors.New`/`fmt.Errorf`、`%w` 与 `errors.Is`/`errors.As`、`defer`/`recover` 边界、何时 return error vs panic。
...
**怎么听这场分享**：先把「一个 error 的一生」主图装进脑子——产生 → wrap → Is/As → 退出；放慢 %w 与 recover 边界；作战节回到开头吞错 exit 0 现场。每一节讲完会提示对应练习题，趁热打铁。
...
好，主图装进脑子了。接下来从 error 接口与多返回值讲起。
...
多返回值讲完。口诀：**失败用 error，别靠 panic 当业务分支**。
...
大家想想：为什么 `errors.Is` 突然失败了？很多人会答「哨兵写错」——也可能中间层用了 `%v`。🔥 大白话：**%w 是拉链，%v 是剪断**。
...
Is/As 讲完。大家想想：能和哨兵直接 `==` 吗？中间有 wrap 时往往不行。
...
哨兵与类型讲完。下一节 defer：资源与 panic 清理。
...
defer 讲完。🔥 LIFO；循环里 defer 会攒到函数结束。
...
⭐ 这是面试必考。口诀：**配置错/用户输入 → return error；编程错误才 panic；recover 只救本 G**。
...
边界讲完。口诀：**预期失败 return；真不行才 panic；recover 救不了别的 G**。
...
现在再看群里那句「日志打了但 CI 绿了」，你知道该怎么拦住他了——先查有没有 return err / 非 0 退出，再查 %w 链。下面是可复制的生产模板（代码块一字不改，直接拿走）。
...
全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「开头吞错 exit 0 / %w 断链现场怎么查」。讲得下来，你就是下一个能拦住「假成功」的人。

八题能口述，错误链就过关。下一章 [05 goroutine与调度模型](../05-goroutine与调度模型/)。

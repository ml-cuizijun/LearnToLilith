...
> 这一章讲完，你会知道当时那个人错在哪——容器里默认 GOMAXPROCS=NumCPU() 是误读，要对齐 cgroup limit，否则 P99 被 CFS throttle 打毛。
> **本章回答**：`GOMAXPROCS` 从默认值、P-M-G 关系、容器 cgroup 误读、CFS throttling 到 automaxprocs 与手动设置的一生——该设多少、谁误导、怎么验。
> **不讲**：GMP 调度细节 → [05](../05-goroutine与调度模型/)；GC 与逃逸 → [11](../11-GC内存与逃逸分析/)；errgroup 与并发限流 → [09](../09-errgroup与并发限流/)；pprof 排障 → [13](../13-pprof与性能排查/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：从面试口径和语言最小集进场；核心三节是 P-M-G、NumCPU 与 cgroup 错位、CFS throttling；生产模板代码一字不改。每一节讲完提示对应练习题。
...
大家想想：limit 2 核，为啥利用率不到 2、P99 却炸？很多人会答「CPU 不够再加 limit」——**先错了**。先查是不是 32 个 P 在抢 2 核被 CFS 掐。
...
> ⚠️ **别混**：`limits.cpu: 2` 是 **CPU 时间片上限**（100ms 周期内最多用 200ms），不是「NumCPU 变成 2」。

🔥 大白话：**quota 是账本上的时间配额，不是把 /proc/cpuinfo 改成 2 行**。`requests.cpu: 500m` 是调度权重（Guaranteed/Burstable），不影响 GOMAXPROCS——GOMAXPROCS 应对齐 **limit**，不是 request。
...
口诀：**「P 多核少 → throttle → P99 毛刺」**。练习题 Q9、Q10。throttle 懂了——生产模板（代码一字不改）。

---

**回到开头那个现场**：limit 2、NumCPU 32、P99 毛刺。正确剧本是：automaxprocs 或显式 GOMAXPROCS=limit → 改 limit 要重启/热更新对齐 → 用 cpu.throttled 指标验证。现在再看群里那句「再加核」，你知道该先查什么了。

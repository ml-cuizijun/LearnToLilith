...
> 这一章讲完，你会知道当时那几个人各自错在哪——一个 Go 值从声明、零值、类型转换到导出，站站都可能把巡检工具打挂。
> **本章回答的问题**：一个 Go 值的一生——声明与初始化、静态类型、零值、值/指针、基础类型转换、结构体字段可见性，到第一次被 `fmt` 打印或传给函数。
...
**怎么听这场分享**：先把「一个 Go 值的一生」主图装进脑子——声明 → 零值 → 转换 → 传值/指针 → 导出；放慢零值与导出规则；作战节回到开头那些编译/JSON 空字段现场。每一节讲完会提示对应练习题，趁热打铁。
...
好，主图装进脑子了。接下来从 package / import / main 入口讲起。
...
入口讲完。未使用 import 直接编译失败——运维 CI 最常见噪音之一。
...
大家想想：`var n int` 是多少？很多人会答「未初始化」——错了。🔥 大白话：**Go 没有「脏值」，声明即零值**；但 `0` 到底是「端口没配」还是「就是 0」要业务自己分。
...
零值讲完。⭐ 面试必考：每个类型都有零值，但业务上要区分「零」和「没配」。
...
转换讲完。🔥 `int` 与 `int64` 不能直接加——显式转换是纪律不是啰嗦。
...
指针讲完。大家想想：传 struct 改字段，调用方变吗？很多人会答「变」——错了。
...
大家想想：字段写成 `name` 为啥 JSON 是空的？很多人会答「tag 写错了」——也可能只是**没导出**。🔥 **小写出不了包，也常出不了 JSON**。
...
导出讲完。口诀：**首字母大写才出门；小写字段 JSON 常变空**。
...
现在再看群里那句「undefined: fmt / JSON 全空」，你知道该怎么拦住他了——先查 import 与导出，再查类型转换。下面是可复制的生产模板（代码块一字不改，直接拿走）。
...
全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「开头那些编译错与 JSON 空字段，各自卡在值的一生哪一站」。讲得下来，你就是下一个能拦住「乱改类型」的人。

八题能口述，类型系统地基就过关。下一章 [02 工程结构与go-mod](../02-工程结构与go-mod/)。

...
> 这一章讲完，你会知道当时错在哪——「同一个 tag」不等于同一份二进制；要用 ldflags 钉版本、`-trimpath` 去本机路径、校验 checksum。
> **本章回答**：一个 Go 二进制从「源码 `go build`」到「镜像进生产」再到「线上验版本、读 K8s 源码」完整走一遍——go build flags、交叉编译、可复现构建、读官方库套路。
> **不讲**：Dockerfile 多阶段全集 → [04-02](../../../04-工程化与自动化/02-Docker深度/)；CI 流水线编排 → [04-04](../../../04-工程化与自动化/04-CI-CD/)；pprof 与 metrics → [13](../13-pprof与性能排查/)、[14](../14-CLI与Prometheus指标/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：从面试口径进场；重点是 go build flags、可复现构建、生产 Makefile/Dockerfile 模板（代码一字不改）、以及读 client-go/cobra 的套路。每一节讲完提示对应练习题。

大家想想：为啥审计会红？很多人会答「再打一次 tag」——**不够**。要 `-trimpath` + 可复现构建 + 版本 ldflags，让二进制可核对。
...
⭐ `-ldflags -X` 钉版本；`-trimpath` 去本机路径。练习题 Q3、Q4、Q5。flags 懂了，看原理。
...
本节对应练习题 Q6、Q7。原理懂了——生产模板（代码一字不改）。

---

**回到开头那个现场**：同 tag 行为不同、strings 露出 /Users/alice。正确剧本是：CI 统一 `go build -trimpath -ldflags` → 镜像里只放产物 → 启动打版本日志 → 对不上就比 checksum。现在再看安全审计那张红牌，你知道该怎么改流水线了。
...
口诀：**「tag 相同 ≠ 二进制相同」**。练习题 Q12、Q13。作战完，必会常考。

...
> 这一章讲完，你会知道当时那几个人各自错在哪——slice/map 从零值、共享底层到并发写，站站都能让数据「莫名被改」。
> **本章回答的问题**：零值语义、nil vs 空、make/new、slice 头与扩容、底层数组共享、map 写入与并发、安全拷贝与传参习惯。
...
**怎么听这场分享**：先把「slice/map 的一生」主图装进脑子——声明 → make → append/共享 → 传参 → 并发坑；放慢子切片与 nil map；作战节回到开头共享乱数据 / concurrent map 现场。每一节讲完会提示对应练习题，趁热打铁。
...
好，主图装进脑子了。接下来从零值、make 与 new 讲起。
...
零值讲完。口诀：**nil slice 可读可 append；nil map 读可以、写 panic**。
...
大家想想：`b := a[1:3]; b[0]=99`，a 会变吗？很多人会答「不会，切片是拷贝」——错了。🔥 大白话：slice 头是遥控器，底层数组才是电视机；两个遥控可以对同一台电视。
...
内幕讲完。🔥 子切片改元素 = 改共享底层；append 超 cap 才换房。
...
⭐ 高频错答：「slice 是引用传递所以 append 外面一定变」。头拷贝了——len/cap 变了要接返回值。
...
传参讲完。大家想想：函数里 append 为啥外面 len 不变？
...
🔥 再钉一次：`var m map[string]int` 后写入 = panic。并发写 = `concurrent map writes`。口诀：**先 make，再写；要并发就加锁或分 shard**。
...
JSON 语义讲完。口诀：**要 [] 就 make，要 null 才 nil**。
...
现在再看群里那句「数据乱了 / nil map panic」，你知道该怎么拦住他了——先查子切片共享与 map 是否 make。下面是可复制的生产模板（代码块一字不改，直接拿走）。
...
全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「开头共享 slice 乱数据 / nil map panic，根因各在哪一站」。讲得下来，你就是下一个能拦住「先 append 再说」的人。

十一题能口述，slice/map 陷阱就过关。下一章 [04 错误处理与panic](../04-错误处理与panic/)。并发 slice 与 05/07 章连读。

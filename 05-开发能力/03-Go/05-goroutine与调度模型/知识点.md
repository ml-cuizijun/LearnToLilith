...
> 这一章讲完，你会知道当时那几个人各自错在哪——goroutine 不是免费线程：GMP 怎么转、泄漏长什么样、GOMAXPROCS 限不了「个数」。
> **本章回答的问题**：一个 **goroutine** 从 `go` 诞生到正常结束（或泄漏）的一生——谁创建、谁调度、在哪阻塞、怎么收尾、泄漏长什么样。
> **本章不讲**：channel 收发细节 → [06](../06-channel与select/)；`sync.Mutex` / `-race` → [07](../07-sync原子与竞态检测/)；`context` 取消链 → [08](../08-context取消与超时传播/)；`errgroup` 编排 → [09](../09-errgroup与并发限流/)；容器里 `GOMAXPROCS` 与 cgroup → [12](../12-GOMAXPROCS与容器cgroup/)。

**怎么听这场分享**：先把「一个 goroutine 的一生」主图装进脑子——go → 调度 → 阻塞/结束 → 泄漏岔路；放慢 GMP 与泄漏；作战节回到开头 goroutine 暴涨现场。每一节讲完会提示对应练习题，趁热打铁。
...
诞生讲完。口诀：**go 很轻，但不是免费；main 退了后台也没了**。
...
大家想想：M 是 OS 线程，为啥还要 P？很多人会答「P 就是 CPU」——不精确。🔥 大白话：**P 是工位**（队列+工具），M 是工人，G 是工单；工人没工位就干不了 Go 代码。
...
阻塞讲完。pprof 里大量 `chan receive` 常常是信号。
...
收尾讲完。WaitGroup：Add 别塞进 goroutine 里抢。
...
⭐ 这是面试必考。口诀：**曲线只升不降先 pprof，栈上 chan/select 找谁没关门**。
...
大家想想：设 `GOMAXPROCS=1` 能挡住 goroutine 暴涨吗？很多人会答「能，限住了」——错了。🔥 **GOMAXPROCS 限的是并行度，不是 G 个数**；无界 `go` 照样把内存打爆。
...
生产写法讲完。口诀：**无界 go 等于无界炸弹**；用 worker/semaphore。
...
现在再看群里那句「再 go 一点 / 把 GOMAXPROCS 设 1」，你知道该怎么拦住他了——先看 pprof 栈，再谈限流与 worker 池。下面是可复制的生产模板（代码块一字不改，直接拿走）。
...
全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「开头 goroutine 暴涨 / 无限 go 现场，60 秒怎么查、怎么改」。讲得下来，你就是下一个能拦住「再 go 一点」的人。

十题能口述，goroutine 与调度模型就过关。下一章 [06 channel与select](../06-channel与select/)。

...
**本章不讲：** 脚本胶水、HTTP 重试 → [02 Python](../02-Python运维/)；并发 SSH 整题 → [04 实战题](../04-运维开发实战题/)；Prometheus 告警规则 → [03-K8s 18](../../03-Kubernetes/18-监控与可观测性/)、[01-22 监控](../../01-基础底座/22-监控与告警/)；Operator/CRD 展开 → [03-K8s 20](../../03-Kubernetes/20-Operator与CRD/)；RSS、cgroup 底层 → [01-07 内存](../../01-基础底座/07-内存与OOM/)、[01-10 cgroup](../../01-基础底座/10-cgroup与容器/)。
...
若简历写了「熟悉 Go」却只改过小工具 → 应写「脚本级」。写了「熟悉」被问语法是自己挖的坑。

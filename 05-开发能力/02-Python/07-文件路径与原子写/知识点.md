...
> 巡检脚本写 `status.json` 写到一半被 kill，下游读到半截 JSON 全站告警；有人 `open("data/../etc/passwd")` 没校验路径逃逸；cron 两个实例同时写同一文件，内容交错损坏——群里已经有人在喊「下游坏了先回滚」。
> 这一章讲完，你会知道当时那几个人各自错在哪——运维脚本的落盘契约是：路径用 pathlib 管、写入要原子、编码显式 utf-8。
> **本章回答的问题**：一次写入从创建临时文件、刷盘到 `rename` 替换目标文件的一生。
...
**怎么听这场分享**：先把「原子写一生」主图装进脑子——pathlib → 写 tmp → fsync → rename；放慢原子写与路径逃逸；作战节回到开头那个半截 JSON 现场。每一节讲完会提示对应练习题，趁热打铁。
...
🔥 **原子写（atomic write）** = 读者要么看到旧完整文件，要么看到新完整文件，绝不半截。大白话：换招牌时先在后院写好新牌，再一下子换上——路人不会看见半块牌子。⭐ 这是面试必考。

很多人会答「直接 `open(path,"w")` 写完就行」——错了。进程中途被杀，下游读到的就是半截。
...
现在再看群里那句「下游坏了先回滚」，你知道该怎么拦住他了——先查是不是半截写，再查路径逃逸与权限。下面是可复制的生产模板（代码块一字不改，直接拿走）。
...
全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「开头那个半截 JSON 现场，为什么 rename 能救、直接 w 不行」。讲得下来，你就是下一个能拦住「先回滚下游」的人。

八题能口述，文件落盘就过关了。下一章用 **subprocess** 安全调外部命令。

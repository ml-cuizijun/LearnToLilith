...
> **本章回答**：一次「脚本变慢 / 内存涨」从复现、采样 profiling、定位热点、验证修复到回归门禁的完整链路。
...
- [ ] 画「cProfile/tracemalloc 排查一生」主图，标 CPU/I/O 分叉。
- [ ] `cumtime` 与 `tottime` 区别？
- [ ] tracemalloc 标准三步？
- [ ] 为何不信 `getsizeof`？
- [ ] 全量 list API 结果会怎样？怎么改？
- [ ] cProfile 与 py-spy 适用场景？
- [ ] 修复后如何回归？
- [ ] 与 09/10/11 如何分工？

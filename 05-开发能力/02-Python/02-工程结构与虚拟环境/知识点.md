...
> 活动前在跳板机 `pip install requests` 装到系统 Python，第二天另一个脚本 import 版本冲突；同事把仓库 zip 到生产机直接 `python check.py`，缺 `kubernetes` 模块；CI 绿但本地跑不起来——**不是代码写错，是工程没按「可复现环境」搭**。
> **本章回答**：一个运维 Python 工程从目录布局、虚拟环境、依赖锁定到可执行入口的完整生命周期。
...
- [ ] 画出工程一生主图：clone → venv → install → run → exit code。
- [ ] 为什么禁止 `sudo pip install`？跳板机正确做法？
- [ ] `pip freeze` 和手写 `>=,<` 各适合什么场景？
- [ ] `if __name__ == "__main__"` 去掉会怎样？举例。
- [ ] 平铺 `scripts/` 和 `src/` 布局各何时用？
- [ ] README 必须包含哪三段命令？
- [ ] `which python` 在排障里第一步说明什么？
- [ ] `.gitignore` 至少忽略哪四类路径？

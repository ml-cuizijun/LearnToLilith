...
> 巡检脚本 `os.system("kubectl get pods")` 把 shell 注入面全打开；有人 `subprocess.run(..., shell=True)` 拼用户输入导致 RCE；`Popen` 只读 stdout 不读 stderr，子进程写满管道死锁挂死——**运维脚本调外部命令的契约是：列表形式传参、显式超时、检查退出码、分清 stdout/stderr**。
...
- [ ] 画外部命令一生主图，标 returncode 与 check。
- [ ] 列表 argv 与 shell 字符串区别？
- [ ] `check=True` 发生什么？
- [ ] 何时用 Popen 而非 run？
- [ ] PIPE 死锁如何避免？
- [ ] timeout 超时异常叫什么？
...
八题能口述，subprocess 就过关了。下一章 [09 HTTP客户端与重试](../09-HTTP客户端与重试/) 管网络调用。

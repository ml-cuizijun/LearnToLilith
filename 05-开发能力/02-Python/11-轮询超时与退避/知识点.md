...
> 这一章讲完，你会知道当时那几个人各自错在哪——等待不是 sleep 循环：deadline、间隔策略、成功条件与可观测日志缺一不可。
> **本章回答的问题**：一次「等到某条件成立」从设定 timeout、轮询间隔、退避增长到放弃或成功的完整链路。
> **本章不讲**：单次 HTTP 重试 → [09 HTTP客户端与重试](../09-HTTP客户端与重试/)；K8s SDK 细节 → [12 K8s与云SDK调用](../12-K8s与云SDK调用/)。

**怎么听这场分享**：先把「轮询等待一生」主图装进脑子——设 deadline → check → sleep → 退避/超时；放慢成功条件与 09 分层；作战节回到开头那个 CI 挂死 / API 风暴现场。每一节讲完会提示对应练习题，趁热打铁。
...
deadline 钉死了。大家想想：计时用墙钟还是单调钟？下一节再谈间隔怎么选。
...
大家想想：1000 个脚本都 `sleep(1)` 等 Ready，恢复瞬间会发生什么？很多人会答「多等一会儿没事」——错了。🔥 大白话：**固定 1s = 同步打 API**；长等待要退避，还要 jitter 打散峰值。
...
大家想想：为什么「每个脚本自己退避」还是会一起打满？很多人会答「退避了就不会」——错了。🔥 **thundering herd（惊群）** = 一群脚本同一时刻醒来，同步撞门。大白话：像下课铃一响全班冲食堂——每人「等一会儿」但闹钟对齐了，还是挤爆。
...
间隔策略讲完。记住口诀：**短等固定，长等退避+jitter**。下一节钉死「什么叫成功」。
...
⭐ 这是面试必考。很多人会答「Pod Running 就算成功」——高频错答。🔥 成功 = **replicas 对齐 + Ready +（可选）generation**，不是状态字符串碰巧是 Running。
...
成功条件讲完。很多人只看 Running——⭐ 这是高频错答。下一节看跟 09 HTTP 重试怎么分层。
...
分层讲完。⭐ 面试必考：09 管单次瞬态，11 管「条件何时为真」——两层都 aggressive 就会风暴。
...
库与手写选型讲完。下一节专讲 timeout 到了怎么退出、日志打什么。
...
超时出口讲完。大家想想：只 raise 不打 last_reason，值班怎么排障？下一节把可观测收束。
...
可观测清单齐了。下一节回到开头那个 CI / API 风暴现场定责。
...
现在再看群里那句「CI 挂了六小时 / API 被打满」，你知道该怎么拦住他了——先查有没有 deadline，再看 interval 与 success 条件。下面是可复制的生产模板（代码块一字不改，直接拿走）。
...
全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「开头那个无 deadline / 1s 轮询风暴现场，该怎么改」。讲得下来，你就是下一个能拦住「再 sleep 紧一点」的人。

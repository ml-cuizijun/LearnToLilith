...
> 巡检脚本用 list 存主机又去重，O(n²) 扫到超时；用 dict 当有序队列结果顺序乱跳；函数默认参数 `tags=[]` 导致第二次调用带着第一次的 tag——群里已经有人在喊「Python 太慢换 Go」。
> 这一章讲完，你会知道当时那几个人各自错在哪——根子往往不是语言慢，而是数据结构选型没按场景来。
> **本章回答的问题**：一次数据结构选型的一生——`list` / `dict` / `set` / `tuple` 何时用、怎么遍历、可变默认参数为什么是坑。
...
**怎么听这场分享**：先把「选型一生」主图装进脑子——场景 → list/dict/set/tuple → 遍历 → 可变默认陷阱；放慢 dict 与可变默认参数；作战节回到开头那个超时与脏数据现场。每一节讲完会提示对应练习题，趁热打铁。
...
🔥 **dict** = 用键直接跳到值的查表，平均 O(1)。大白话：花名册按工号翻，不是从头翻到尾。⭐ 这是面试必考。

很多人会答「一律用 list 最简单」——错了。先问：你是「按名字找」还是「按顺序扫」？
...
🔥 **可变默认参数** = 函数定义时创建一次默认对象，之后每次调用共享。大白话：默认参数不是「每次发一份新试卷」，而是「全班共用一张草稿纸」。⭐ 高频错答集中地。
...
现在再看群里那句「Python 太慢换 Go」，你知道该怎么拦住他了——先画访问模式，再选型，别用 list 扫一切。下面是可复制的生产模板（代码块一字不改，直接拿走）。
...
全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「开头那个巡检超时现场，为什么 list 去重会慢、可变默认怎么脏数据」。讲得下来，你就是下一个能拦住「换 Go」的人。

八题能口述，数据结构就过关了。下一章把失败用**异常和退出码**交到调度器手里。

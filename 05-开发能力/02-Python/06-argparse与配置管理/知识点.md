...
> 同一脚本在笔记本上 `--cluster staging` 能跑，上 Jenkins 忘了传参却用了代码里写死的 `prod`；有人把密码写在 `config.yaml` 提交 Git，又在 shell 里 `export TOKEN=xxx` 覆盖——**运维脚本的配置契约是：来源可溯、优先级明确、敏感项只走环境变量**。
> **本章回答**：配置从默认值、配置文件、环境变量到 CLI 参数合并成一份 `Config` 对象的一生。
...
- [ ] 画配置合并主图，标优先级箭头。
- [ ] CLI、env、file、default 谁覆盖谁？
- [ ] `action='store_true'` 和 `type=bool` 为何不同？
- [ ] 为什么合并后用 `Config` dataclass？
- [ ] `parse_args(argv)` 如何方便单元测试？
- [ ] subparsers 典型场景？
- [ ] 三条配置安全红线？
- [ ] 用法错误与运行时失败退出码如何区分？

八题能口述，配置管理就过关了。下一章 [07 文件路径与原子写](../07-文件路径与原子写/) 管落盘安全。

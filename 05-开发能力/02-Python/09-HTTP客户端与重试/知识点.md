...
> 巡检脚本调 Prometheus API，`requests.get(url)` 没设 timeout——挂 30 分钟 cron 重叠；502 立刻连打 100 次把网关打挂；POST 创建工单失败重试导致重复单——**HTTP 不是「发出去就完」：超时、幂等、退避、状态码分层处理才是运维脚本的契约**。
...
- [ ] 画「HTTP 一生」主图，标 4xx 不重试岔路。
- [ ] `timeout=(3.05, 27)` 两层各防什么？
- [ ] `raise_for_status` 与 `resp.ok` 怎么用？
- [ ] 手写重试五要素：次数、条件、退避、jitter、POST 边界？
- [ ] urllib3 `Retry` 的 `allowed_methods` 为什么默认不含 POST？
- [ ] Session 与裸 `get` 在百 URL 巡检差在哪？
- [ ] 429 除了退避还应看什么 Header？
- [ ] HTTP 重试与 11 章轮询如何分工？

八题能口述，HTTP 客户端与重试就过关了。下一章 [10 并发与 GIL](../10-并发与GIL/)。

...
> 这一章讲完，你会知道当时那几个人各自错在哪——kubernetes-client 不是 curl：配置加载、ApiException、分页与 watch 各有一层契约。
> **本章回答的问题**：一次 K8s API 调用从加载 kubeconfig、构造 client、发请求、读响应/异常到脚本退出的完整链路；云 SDK（boto3）遵循同一心态。
> **本章不讲**：HTTP 通用重试 → [09 HTTP客户端与重试](../09-HTTP客户端与重试/)；等到 Ready 的 poll → [11 轮询超时与退避](../11-轮询超时与退避/)。

**怎么听这场分享**：先把「一次 K8s API 调用一生」主图装进脑子——load config → client → 调用 → ApiException 决策；放慢分页与 403 不重试；作战节回到开头 Unauthorized / OOM 现场。每一节讲完会提示对应练习题，趁热打铁。
...
好，主图装进脑子了。接下来从「加载配置」开始——配置错了，后面全白搭。
...
配置讲完。⭐ CronJob 里默认想 in-cluster，别死读本地 kubeconfig。下一节看 API 类与 namespace。
...
发出侧讲完。记住口诀：**typed API + namespace 别写死 prod**。下一节专攻 ApiException。
...
⭐ 这是面试必考。大家想想：403 能不能按 09 式连打重试？很多人会答「重试几次就好」——高频错答。🔥 **401/403 = 我不认你或你不能干；503/429 才谈退避**。
...
异常分层讲完。⭐ 403/404 不重试，503/429 才谈退避——这是高频错答高发区。
...
大家想想：`list_pod_for_all_namespaces()` 一次拉回，为什么会 OOM？很多人会答「Python 慢」——错了。🔥 大白话：**没分页 = 把整个集群状态一次性塞进进程**。口诀：**limit + continue，页页清**。
...
分页讲完。大家想想：一次 list 全命名空间会发生什么？很多人会答「内存够就行」——错了。
...
watch 与 11 章 poll 选型讲完。下一节看云 SDK 是否同一心态。
...
🔥 别把云 SDK 当成「另一套语言」——和 K8s client 同一心态：显式配置、错误码分层、分页。
...
boto3 对照讲完。🔥 心态三件套：显式区域/配置、错误码分层、分页。
...
串联收束了。下一节回到开头 Unauthorized / OOM / 403 重试现场。
...
现在再看群里那句「Unauthorized / list 把机器打爆了」，你知道该怎么拦住他了——先分 in-cluster 还是 kubeconfig，再查分页与 403 是否在瞎重试。下面是可复制的生产模板（代码块一字不改，直接拿走）。
...
全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「开头 Unauthorized / 无分页 OOM 现场该怎么查」。讲得下来，你就是下一个能拦住「全量 list + 403 重试」的人。

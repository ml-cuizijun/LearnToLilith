...
> 这一章讲完，你会知道当时那几个人各自错在哪——运维脚本也要 pytest 锁行为、type hints 锁形状；CI 门禁不是可选装饰。
> **本章回答的问题**：一个运维脚本从写第一个测试、加类型、mock 外部依赖、进 CI 到重构不怕炸的完整链路。
> **本章不讲**：logging 规范 → [05 logging日志规范](../05-logging日志规范/)；argparse → [06 argparse与配置管理](../06-argparse与配置管理/)；性能回归阈值 → [13 性能与内存排查](../13-性能与内存排查/)。

**怎么听这场分享**：先把「pytest 与类型在脚本里一生」主图装进脑子——抽纯函数 → 类型 → 测试 → CI；放慢 fixture/mock 与 TypedDict；作战节回到开头那个改默认值打到 prod 的现场。每一节讲完会提示对应练习题，趁热打铁。
...
好，主图装进脑子了。接下来从「可测结构」开始——200 行全塞 main，测都测不了。
...
⭐ 高频错答：「脚本就跑一次，不用测」。运维脚本改默认值能打到 prod——测的是**行为契约**，不是「仪式感」。
...
结构讲完。口诀：**main 编排，纯函数干活**。下一节写第一个 pytest。
...
基础断言讲完。下一节用 fixture 与 mock 把外部依赖隔开。
...
大家想想：不连 K8s 怎么测 main 返回 1？很多人会答「测不了」——错了。🔥 **monkeypatch** = 把真依赖换成假手；fixture = 桌上摆好假数据。
...
mock 讲完。大家想想：测 main 返回 1 一定要连真集群吗？很多人会答「要」——错了。
...
⭐ 这是面试必考。口诀：**TypedDict 锁形状，Any 等于没写**；`# type: ignore` 要当成债务，不是通行证。
...
类型讲完。⭐ TypedDict 锁行记录形状；满屏 Any 等于没写。
...
CI 讲完。🔥 PR 必跑 pytest+mypy；打真集群的测试默认别开。
...
现在再看群里那句「我本地跑过了 / 全是 Any」，你知道该怎么拦住他了——先要可测结构，再把门禁写进 CI。下面是可复制的生产模板（代码块一字不改，直接拿走）。
...
全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「开头那个改 namespace 默认值打到 prod，测试与类型本可以拦住哪一层」。讲得下来，你就是下一个能拦住「我本地跑过了」的人。

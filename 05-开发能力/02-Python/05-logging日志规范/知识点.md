...
> 凌晨 cron 邮件里混着 `print("checking...")` 和半截 traceback；ELK 里搜 `ERROR` 却全是 `print` 打的信息级噪音；有人 `logging.basicConfig()` 在 import 时执行两遍，handler 重复、同一条日志打三行——群里已经有人在喊「日志没用关掉算了」。
> 这一章讲完，你会知道当时那几个人各自错在哪——运维脚本的观测契约是：结构化、分级、可检索，且别把密钥打进日志。
> **本章回答的问题**：一条日志从 `logger.info()` 产生、经 Handler/Formatter 格式化到 stdout/文件/采集器落盘的完整链路。
> **本章不讲**：异常与退出码 → [04 异常与退出码](../04-异常与退出码/)；CLI 参数 → [06 argparse与配置管理](../06-argparse与配置管理/)；子进程输出 → [08 subprocess执行外部命令](../08-subprocess执行外部命令/)。

**怎么听这场分享**：先把「一条日志一生」主图装进脑子——Logger → Handler/Formatter → 落盘/采集；放慢 basicConfig vs dictConfig；作战节回到开头那个搜不到 ERROR 的现场。每一节讲完会提示对应练习题，趁热打铁。
...
🔥 **basicConfig** = 给 root logger 一次性装默认 Handler；**dictConfig** = 用字典描述整棵日志树。大白话：前者像「一键开灯」，后者像「配电箱按房间布线」。⭐ 重复配置会双份日志。

很多人会答「每个模块都 basicConfig 一下」——错了。只在入口配置一次。
...
现在再看群里那句「日志没用关掉算了」，你知道该怎么拦住他了——先查级别与 Handler，再查有没有密钥和双份。下面是可复制的生产模板（代码块一字不改，直接拿走）。
...
全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「开头那个 ELK 搜不到现场，为什么 print 成噪音、basicConfig 为何双份」。讲得下来，你就是下一个能拦住「关掉日志」的人。

八题能口述，logging 就过关了。下一章用 **argparse** 把配置来源钉死。

...
> 巡检 500 台主机用单线程 HTTP 要 25 分钟；改 `ThreadPoolExecutor(64)` 缩到 3 分钟——但对 500 个 JSON 做 SHA256 仍慢，加线程反而更慢；有人 `multiprocessing` 在 cron 里 fork 爆内存——**并发不是「线程越多越好」：I/O 与 CPU 边界、GIL、进程开销要先画清楚**。
...
- [ ] 画「并发任务一生」主图，标 GIL 串行岔路。
- [ ] GIL 释放的典型场景？
- [ ] 线程池 vs 进程池选型表背三条？
- [ ] 为何 `as_completed` + `result()`？
- [ ] 每线程 Session 怎么写？
- [ ] ProcessPool 在 cron 的两风险？
- [ ] Semaphore 解决什么？
- [ ] 09 章 HTTP 与 10 章并发如何组合？

八题能口述，并发与 GIL 就过关了。下一章 [11 轮询超时与退避](../11-轮询超时与退避/)。

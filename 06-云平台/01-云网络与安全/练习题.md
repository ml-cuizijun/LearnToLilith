...
**一句话**：数据面必须多 AZ，有状态战斗服常单 AZ，跨 AZ 要算 RTT 和 AWS 流量费。
...
**一句话**：私网出公网偶发超时先查 NAT 带宽 / SNAT / 单 AZ，不要先改安全组。
...
**一句话**：安全组有状态、NACL 无状态；生产以 SG 精细控，NACL 只做粗禁。
...
**一句话**：玩家走高防 / NLB，源站藏私网，逻辑服禁 EIP + 0.0.0.0/0 裸奔。
...
**一句话**：TCP / UDP 长连接用 CLB / NLB，心跳必须 < idle timeout 的 1/3。
...
**一句话**：专线给云 ↔ IDC 数据面，玩家不走专线；UDP 丢先测 MTU。
...
**一句话**：WAF 只解析 HTTP，游戏 TCP / UDP 走高防 / Shield，源站 SG 只放回源段。
...
**一句话**：通断先画路径再改规则：LB 红查探测源，出网查 NAT，UDP 查 MTU，CCU 断崖查黑洞。

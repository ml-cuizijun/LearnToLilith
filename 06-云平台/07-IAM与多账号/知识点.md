...
这一章只做一件事——**泄漏怎么禁 Key、最小权限怎么写到前缀、多账号 SCP 怎么当天花板、破窗怎么演练。** 前面的概念和原理，是为了让你动手时不把长期 AK 当角色、不把 SCP 当日常权限清单。

**读完你能：** 人走 SSO + MFA 扮角色、程序走实例角色 / IRSA / RRSA / OIDC；泄漏先禁 Key 再查 Trail；画出 prod / nonprod / audit / shared；能说出 SCP 最小集。

**本章主线（权限就按这三步想）：**

1. **身份**：人 SSO+MFA 扮角色；程序实例角色 / IRSA / RRSA；永久 AK = 泄漏。
2. **半径**：多账号 prod/nonprod/audit；SCP 当天花板不是日常清单。
3. **留证**：泄漏先禁 Key 再开会；Trail 查影响面，不加 `*` 验证。
...
**本章不讲：** AWS IAM 求值细节、IRSA Trust `sub` 展开 → [03](../03-AWS生产/)；ACK RRSA、RAM 开服用法 → [02](../02-阿里云生产/)；火山 IAM 与办公身份 → [08](../08-火山引擎生产/)；跨云策略不能粘贴 → [04](../04-多云对照/)；账单按账号拆 → [06](../06-成本治理/)；K8s RBAC / SA → [03-Kubernetes / 13-RBAC](../../03-Kubernetes/13-RBAC与安全/)；堡垒机、跳板 → [01-基础底座 / 25](../../01-基础底座/28-堡垒机与跳板机/)。
...
**一句话：** 节点角色只拉镜像；应用角色绑 SA；GM 发奖不能和逻辑服同一条。

**打个比方：** IAM 像「不同工牌进不同门」——节点角色只够拉镜像，GM 发奖和逻辑服共用一条 policy 等于万能钥匙。

**面试怎么说**：「AK 泄漏先禁 Key 查 Trail；SCP 是组织天花板；跨账号 403 查桶策略，模拟器不是真相。」
日常权限按 **职责** 拆，不按人名堆：
...
**一句话：** AK 泄漏先禁 Key、查 Trail，再开会；不要 force-push 当修复。
...
**一句话：** SCP 是组织对账号的天花板；prod/nonprod/audit 分账号。
...
**一句话：** 跨账号缺桶策略一边就 403；以 Trail 为准，不加 `*` 验证。
与 03 同一套，这里落到操作：

...
**打个比方：** 50 节点同时拉 4GB 镜像穿 NAT，像早高峰全员挤一个出口——ImagePullBackOff 和 NAT 账单一起炸。

**面试怎么说**：「ImagePullBackOff 先加 S3/ECR VPC Endpoint，不要先扩 NAT；本 AZ NAT，跨 AZ 流量要钱。」
...
**面试怎么说**：「Spot 只给无状态可中断；T3/T4g 和阿里 t 系列一样禁用于游戏；中国区 AWS 和 Global 账号隔离。」
| 模式 | 何时用 | 何时不用 |
|------|--------|----------|
...
**打个比方：** preserve client IP 像「快递直接送到房间」——后端 SG 既要放 LB 探测源，也要能认玩家 IP，两套规则缺一不可。

**面试怎么说**：「自定义 TCP 接 NLB 不接 ALB；Healthy=0 查两套 SG；探测源不是 100.64。」
| | ALB | NLB |
|--|-----|-----|

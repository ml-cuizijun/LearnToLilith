...
这一章只做一件事——**怎么选盘和库、怎么验 RPO、切主后连接池怎么收、Redis 触顶怎么止血。** 前面的概念和原理，是为了让你动手时不把块、对象、库、缓存放错层。

**读完你能：** 白板上画出块 / 对象 / RDS / Redis 各放什么；多 AZ 和只读分得清；能说出「现在能回到过去哪一分钟」；活动 Redis 超时先看硬顶，而不是先改重试。

**本章主线（数据面就按这三步想）：**

1. **分层**：块给库 / 对象给热更 / Redis 给热状态；跨服大文件不进 RDS blob。
2. **归类**：`gone away` → 切主 + 连接池；Redis 超时 → 硬顶不是重试；IOPS 爆 → 规格不是容量。
3. **留证**：RPO 要真 restore 过；只读 ≠ 多 AZ 写路径。
...
**本章不讲：** MySQL 复制、锁、慢 SQL → [03-中间件 / 01-MySQL](../../02-中间件与数据库/01-MySQL/)；Redis 大 key 内核 → [03-中间件 / 02-Redis](../../02-中间件与数据库/02-Redis/)；OSS / S3 产品细节 → [02](../02-阿里云生产/)、[03](../03-AWS生产/)；TOS 生命周期、内网 endpoint → [08](../08-火山引擎生产/)；游戏回档流程、选点、公告 → [08-游戏运维 / 07-玩家数据备份与回档](../../07-游戏运维专题/07-玩家数据备份与回档/)；云盘挂到 K8s PVC → [03-Kubernetes / 11-存储](../../03-Kubernetes/11-存储/)；对象存储公共读账单 → [06](../06-成本治理/)；VPC 安全组 → [01](../01-云网络与安全/)。
...
**一句话：** 云盘 IOPS 按规格锁死，不是盘空就快；热更不走 ECS 网卡。

**打个比方：** 云盘 IOPS 像「套餐里的流量包」——盘再大，PL0 的上限就那么多，热更走 OSS+CDN 别用 ECS 网卡扛下载。

**面试怎么说**：「IOPS 看规格不是容量；RDS 多 AZ 保写路径，只读不给登录写路径；RPO 要真 restore 过。」
...
**一句话：** 多 AZ 保写路径高可用；只读给查询，登录写路径不要指只读。
两者经常被说成「都是高可用」，不是。
...
**一句话：** RPO 要真 restore 过；备份成功要告警，删除保护要开。
...
**一句话：** 云 Redis 连接/带宽是硬顶；活动超时先看水位，不是先改重试。
...
**一句话：** 块给库、对象给热更、Redis 给热状态；跨服大文件不进 RDS blob。

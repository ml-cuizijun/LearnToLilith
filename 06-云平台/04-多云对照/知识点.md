...
这一章只做一件事——**对照表怎么当验收、账号怎么隔离、切云三件事怎么回滚 DNS。** 前面的概念和原理，是为了让你动手时不把名字 1:1。

**读完你能：** 按能力翻译五朵云；说出游戏为什么不做流量双活；切云当天探测源 / idle / CDN 预热能一口回。

**本章主线（迁移就按这三步想）：**

1. **翻译**：按能力对照，不按产品名 1:1；名字像 ≠ 默认值像。
2. **隔离**：国内 / 海外账号分开；CIDR 启动前文档，不对等后改。
3. **切云**：探测源 / idle / CDN 预热三件必验；DNS 回滚写进预案。
...
**本章不讲：** VPC / SG / NAT 通用排障 → [01](../01-云网络与安全/)；阿里组合、100.64、Terway → [02](../02-阿里云生产/)；AWS NLB / IRSA / IAM 求值 → [03](../03-AWS生产/)；火山 TOS / VKE / 沐瞳现场 → [08](../08-火山引擎生产/)；RDS / 对象存储 / Redis → [05](../05-云上存储与数据库/)；跨 AZ / NAT / CDN 账单 → [06](../06-成本治理/)；跨账号 IAM、RAM 不能粘贴细节 → [07](../07-IAM与多账号/)；出海区服、数据驻留专题 → [07-游戏运维专题](../../07-游戏运维专题/) / [10-出海与多区域](../../07-游戏运维专题/10-出海与多区域/)。
...
**一句话：** 按能力对照，不按产品名 1:1；名字像 ≠ 默认值像。

**打个比方：** 像跨国搬家——都叫「负载均衡」，但健康检查源、idle 超时、跨 AZ 计费默认值各不同，不能照搬国内配置。

**面试怎么说**：「多云按能力翻译：CLB≈NLB、OSS≈S3、RAM≈IAM；切云必须单独验探测源和 idle。」
...
**一句话：** 健康检查源、idle、跨 AZ 计费每家默认值不同，切云必须单独验。
...
**一句话：** 托管模型相同：你 ssh 不进 etcd，节点池/身份/RPO 仍是你的。
...
**一句话：** 国内/海外账号分开；CIDR 启动前文档，禁止同会话漂跨市场。

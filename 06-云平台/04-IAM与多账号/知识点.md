...
> 这一章讲完，你会知道当时那两拨人各自错在哪——泄漏先停钥不先开会，临时权限必须带到期。
> **本章回答的问题**：一把 AK 从签发到出事的一生——谁该拿永久钥匙、谁该拿临时票、出了事先做什么。
> **本章不讲**：AWS IAM 求值细节、IRSA Trust `sub` 展开 → [07](../07-AWS生产/)；ACK RRSA 用法 → [06](../06-阿里云生产/)；火山 IAM 与办公身份 → [08](../08-火山引擎生产/)；跨云策略不能粘贴 → [09](../09-多云对照/)；账单按账号拆 → [05](../05-成本治理/)；K8s RBAC / SA → [03-13](../../03-Kubernetes/13-RBAC与安全/)。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：先把第一节「一把钥匙的旅程」看熟——后面每一节都是这张图上的一段；第二到第六节顺着签发 → 授权 → 跨账号 → 落地 → 出事往下走；第七节专门讲开头那个现场怎么定责。每一节讲完会提示对应练习题，趁热打铁。
...
好，主图装进脑子了。接下来咱们从「签发」开始——永久 AK 和临时 STS，谁该拿哪把。
...
大家先别急着背策略 JSON。很多人以为「有一把 AK 最省事」——⭐ 这是高频错答。🔥 大白话先钉死：**永久 AK = 长期有效的大门钥匙；STS = 有过期时间的临时通行证**——能发临时票就别发永久钥。
...
好，到这儿咱们已经知道：人用联邦、机用角色、永久 AK 是例外。接下来自然会问——权限怎么写才叫最小？
...
授权讲完。大家想想：测试账号能不能 Assume 生产角色？下一节讲跨账号与 SCP 天花板。
...
跨账号讲完。⭐ 面试必考：实例上的钥匙不该是明文 AK。下一节讲实例角色与 IRSA。
...
大家想想：热更包里的 AK 被玩家抓到了，第一件事是开会还是停钥？很多人会答「先对齐情况」——错了。⭐ 泄漏第一刀是**禁用/轮换**，会议纪要写不进攻击窗口。
...
现在再看群里那句「先开会」和「force-push 删 Git」，你知道该怎么拦住他了。收口把命令、面试口径和数字墙收齐。
...
- [ ] **默画**一把钥匙的旅程（签发→授权→跨账号→落地→出事），标出永久 AK 和 STS 各在哪段、排障从哪进（Q1）。
- [ ] **口述签发**：AK vs 角色 vs STS 三种凭证各什么特性；STS 写环境变量跑 12h 为什么等于长期票；人走 SSO + MFA 扮角色 vs 程序走实例角色 / IRSA / OIDC（Q1）。
- [ ] **默画策略语法**：`Effect / Action / Resource / Condition` 四要素；求值三步（Deny → Allow → 默认拒绝）；最小权限落到一条 OSS 策略长什么样（Q3、Q4、Q10）。
- [ ] **口述授权**：权限边界是什么、和 SCP 什么关系、三层天花板怎么叠；临时 `*` 最低限度；GM 发奖和逻辑服为什么拆开（Q5、Q6、Q7、Q13）。
- [ ] **默画账号树**：管理账号 → prod / shared / nonprod / audit / sandbox 五格；SCP 最小集 Deny 哪几条；落地顺序为什么先 nonprod 再审计最后生产（Q8、Q9、Q14）。
- [ ] **口述跨账号**：AssumeRole 怎么跨账号；跨账号 403 缺哪一边；KMS 为什么也要 Decrypt；模拟器为什么撒谎；不加什么验证（Q2、Q11）。
- [ ] **口述落地**：节点角色 vs 应用角色为什么拆开；IRSA Trust sub 不限等于什么；节点 S3 Full 为什么不算做了 IRSA；CI 该拿什么权限（Q12）。
- [ ] **默画泄漏应急**：禁 Key → Trail → 断公网 → 轮换 → 通报五步；为什么先开会不及格；改密码为什么不等于禁 AK；force-push 为什么不能当应急（Q12）。
- [ ] **口述出事**：没有 Trail 怎么办；告警项有哪些；破窗演练怎么做；为什么不能把 AWS JSON 粘到 RAM（Q12）。
- [ ] **定责**：作战决策树三叉（泄漏 / 403 / 过宽）各走哪条；60 秒剧本 0–60s 各敲什么、什么绝不碰（Q12）。
- [ ] **口述数字墙**：STS 小时级过期 · AK 90 天未用告警 · SCP 最小集 5~7 条 · 账号树 5 格 · 三层天花板 · 跨账号三道门 · 破窗密封包双人各持一半（Q12）。
- [ ] **口述根账号保护**：不开日常 AK、开 MFA、不跑业务、SCP 禁止根用户创建 AccessKey；根 AK 泄漏按 P0 轮换根 + 查第二把（Q12）。
- [ ] **口述 STS 凭证链**：SDK 怎么从 metadata 自动拿 STS？为什么手写 STS 进环境变量等于退化成永久 AK？（Q12）

全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「热更里的 AK 泄漏——为什么先停钥不先开会，临时 *:* 为什么必须带到期」。讲得下来，你就是下一个能拦住 force-push 假装没事的人。

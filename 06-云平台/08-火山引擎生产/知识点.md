...
这一章只做一件事——**探测源怎么验、TOS 内网怎么配、VKE IP 怎么留、办公 SSO 怎么扮到角色。** 前面的概念和对照表，是为了让你动手时不把阿里的网段和 JSON 粘过来。

**读完你能：** 画出沐瞳在火山上的入口；对照阿里说出健康检查源、内网域名、IAM 三条差在哪；开服当天 5 分钟清单里多出来的那一条不会漏。

**本章主线（字节系就按这三步想）：**

1. **对照**：和阿里同构图；禁止照抄 `100.64` 和内网域名。
2. **验收**：探测源查控制台 / 文档；TOS 回源走内网 endpoint。
3. **留证**：VKE IP 按 Pod × 1.5 留；办公 SSO 扮角色，不粘 RAM JSON。
...
**本章不讲：** VPC / SG / NAT / 高防通用路径 → [01](../01-云网络与安全/)；阿里 `100.64`、Terway、OSS 细节 → [02](../02-阿里云生产/)；AWS NLB / IRSA 对照 → [03](../03-AWS生产/)；五朵云翻译、腾讯对照 → [04](../04-多云对照/)；多 AZ vs 只读、RPO、云 Redis 上限 → [05](../05-云上存储与数据库/)；包年 / 流量 / 闲置 → [06](../06-成本治理/)；泄漏应急、多账号树 → [07](../07-IAM与多账号/)；开服合服、热更流程、出海区服 → [07-游戏运维专题](../../07-游戏运维专题/)。不背 veDB 全家桶。
...
**一句话：** 产品模型同构；探测源、内网域名、IAM JSON 不能从阿里粘过来。

**打个比方：** 火山像「同户型的另一栋楼」——VPC/NAT/LB 结构一样，但探测源 CIDR、TOS 内网域名、IAM 字段名不能复制粘贴。

**面试怎么说**：「火山禁止照抄 100.64 探测源；TOS+CDN 热更；VKE 节点池 IP 按 Pod×1.5。」
...
**一句话：** 禁止照抄 `100.64.0.0/10`；以火山控制台/文档为准。
...
**一句话：** TOS 私有桶+CDN 内网回源；版本放对象键，禁止公共读。
...
**一句话：** VKE 和 ACK 同构；节点池 IP 按 Pod × 1.5 留；LB 用域名。
VKE 对标 ACK / EKS：控制面托管，你管节点池、CNI、插件、升级窗口。升级不要和开服日重叠。ssh 不进 etcd，影响面照样问（见 [03-01](../../03-Kubernetes/01-集群架构/)）。

...
> **不讲**：主机 HA → [01-33](../../01-基础底座/33-高可用与容灾/)；监控底座 → [01-22](../../01-基础底座/22-监控与告警/)；摘流发布 → [01](../01-游戏服务器架构/)、[05](../05-热更新与版本发布/)；回档 → [09](../09-玩家数据备份与回档/)。
...
- 📖 **读图**
  - 🛤️ 主线：定标 → 告警 → 定位 → 止血 → 复盘；复盘虚线回灌定标与变更规范
  - ⏱️ 告警挂在 **5min SLI 窗口**，不是月平均；CCU 腰斩是独立哨兵——CPU/网关全绿也可能全球静默
...
> 🎯 **一句话**：**SLI** 量玩家废了什么；**SLO** 是对内刹车线；**SLA** 是对外合同——三张表分开，不要混成一句 99.9%。

- 🔥 **大白话**：SLO 看玩家废了什么（登录/进战斗/支付），不是看网关 200 率
  - 登录绿、战斗破 → 查 battle SLI，别拿网关 99.9% 挡枪
  - 网关 200 =「门能推开」；空货架、收银台坏了，玩家照样废
...
- 📖 **读图**
  - ⬅️ 左：渠道鉴权挂、排队满、空 room、客户端崩溃时仍可能全绿
  - ➡️ 右：六条才是玩家体验；破 SLO ≠ 自动破 SLA；没破 SLA ≠ 可以不升 P0

> 💡 **打个比方**：网关 200 是「门能推开」，玩家要的是「货架有货、收银台能结账」——六条 SLI 量的才是体验。

> ⚠️ **别混**：SLI = 指标，SLO = 内部目标，SLA = 对外承诺。开服 10 分钟登录 90%，5 分钟 SLI 已破 = P0，月历网关 99.9% 救不了。

#### ❓ 为什么网关 200 不能当登录 SLI？

- ❌ **网关 200**：探活、空响应、鉴权前通过都算成功
- ✅ **登录 SLI**：鉴权 → 大厅成功 / 尝试登录——渠道挂、排队满、目录错都会掉
- 🚨 **现场**：网关 99.9% + 登录 90% → 按登录升 P0，别被网关绿骗了

### 🛑 SLO vs SLA 与错误预算 ⭐🔥

- **SLO 破了** → 升 P0、吃预算
- **SLA 破了** → 才谈公告补偿
- **错误预算（Error Budget）** = 本月还能「浪费」多少失败窗口
  - 预算打光 → 冻**新玩法**，不冻回滚和安全修复
...
- 🔑 **预算三板斧**：故障扣 · 计划内不扣 · 打光冻新不冻回滚
  - 合服窗口停服 = 计划内维护，单独记账，**不扣**故障预算
  - 把合服算进故障预算 → 团队会作假关维护窗

#### ❓ 为什么开服 10 分钟 90% 必须 P0，月历 99.9% 却可能仍绿？

- ⏱️ **值班看 5min 滑动窗口**：玩家这十分钟登不上就是废了
- 📅 **月历平均**：后面 29 天全绿能把尖刺「平均掉」
- ✅ **口径**：5min SLI 破 = P0；SLA/月历另算——别用月平均挡枪
...
- 🔒 **硬冻结**：活动整点 ±30min 无关变更
- 📦 **无状态**（登录、目录、排队）→ 可 HPA、活动中可扩
- 🏠 **有状态**（战斗 room、合服、回档）→ 必须摘流、停写、等存量
- 三条 SLI 破了止血完全不同：
  - 登录无状态 → 可扩
  - 战斗有状态 → 不能 HPA 迁已开 room
  - 支付 → 不扩机器能好，冻写、对账
...
- ❌ **仍禁止**：活动整点 ±30min 发无关变更；周五晚发战斗/协议；合服 + 战斗热更 + 强更叠做
- ✅ **仍允许**：登录/目录无状态扩容；已演练降级开关（回执齐）；回滚（要广播，避免双人反向）
...
- 📖 **读图**
  - 先说哪条 SLI 破了，再决定摘登录还是摘 room——不要同时改三处
  - SLO 破 → P0；SLA 破 → 公告补偿；预算紧冻新玩法，不冻回滚/安全修复

> ⚠️ **别混**：预算打光**冻新玩法，不冻回滚和安全修复**；合服停服**不扣**故障预算。扩 Gate 救不了空 room——登录可扩，战斗不能 HPA 迁已开 room。CCU 腰斩先分全球 vs 单区，不是先扩机器。
...
- 🖥️ **Dashboard 分行**
  ...
  - SLO：login_5m · battle_join_5m · pay_deliver_5m · ccu_vs_predict
- 📐 **RED**（见 [21](../../01-基础底座/21-性能方法论-USE与RED/)）：Rate / Errors / Duration —— 比「CPU 不高」更能解释卡顿

> 🔍 **现场**：网关 99.99%，渠道鉴权挂了，登录已掉
...
- 长连接健康检查恒 200；登录失败在渠道或排队，网关这一跳仍 200

### 🏷️ 高基数与 server_id ⭐

- ✅ `server_id` **必需**——腰斩和产出异常经常是单服，全局聚合会漏
- ❌ 不要 `server_id × 全量接口`——Prometheus 先死，SLO 看板空白
- 做法：
  - 保留 server_id 按服聚合 CCU / 产出
  - 接口只留核心 + recording rule
  - 原始指标采样，SLI 用 recording 聚合

> ⚠️ **别混**：丢掉 server_id 只留全局 = 单服漏报；每个 debug 接口都当标签 = Prometheus 爆。

### ☎️ 叫醒标准与维护静默 🔧

- **叫醒**：支付、数据错、多服登录、CCU 腰斩、安全
- **不叫醒**：单渠道文案、已降级排行、已知渠道故障且有公告
- **维护静默**按集群 / 大区 / server_id，**不全集群**——单服维护误静默全球 = 漏报
...
- P0 即使「最近没发版」，仍先看变更：
  - 配置中心 · CDN 指针 · 云侧 · 渠道 · 灰度开关 · 证书 · DNS
  - 很多事故是「没发版但改了配置」

> 📝 **面试**：「最近没发版为什么还查变更？」——「配置、CDN、渠道、灰度、证书、DNS 都是变更。」

- 📋 **变更单最小字段**
  - 变更 ID / 类型 / 影响范围（server_id / region / 全球）
  - 观察指标（login_5m / battle_join / ccu / pay_deliver）
  - 回滚步骤（具体命令或 pipeline id）
  - 超时（N 分钟 SLI 恶化则自动回滚）
  - 负责人 + 复核人

- 💥 **双人反向操作**：A 回滚 CDN 指针，B 同时 push 新配置 → 版本分裂，「能进不能打」

> 💡 **打个比方**：变更像开车——同时踩油门和刹车，不是保守，是不知道车往哪走。

> ⚠️ **别混**：扩 Gate 救不了空 room；回滚 CDN 指针和回滚战斗是两件变更，要指定唯一指挥官顺序。
...
- 📖 **读图**
  - CCU 对**预测/环比**，不对绝对值——凌晨低谷用同时段基线
  - 全球 → 账号/登录/客户端/CDN/版本；单区 → 区服逻辑/摘服；登录绿 CCU 掉 → 集体掉线或客户端崩溃
...
#### ❓ 为什么 CPU 全绿、CCU 腰斩仍是我们的事？

- 🟢 CPU 绿只说明机器不忙，不说明玩家在玩
- 📉 CCU 腰斩 = 人走了或连不上——客户端崩溃、静默踢线、CDN/版本错都会这样
- ✅ 第一刀：环比 + 分服 + 查变更，**不是**「机器没事就关单」
...
- 📖 **读图**：告警进 → 确认 SLI/范围 → **一人指挥** → 一件止血 + 对外口径并行 → 复盘回灌
...
- 🔀 并行切主 + 回指针 + 重启 → 不知道哪件有效，也无法回滚「有效的那一刀」
- ✅ 唯一指挥官排顺序：先停变更 → 一件止血 → 观察 SLI → 再下一件
- 🚫 **止血阶段禁止顺便优化**
...
- ❌ **不在生产演**：随机杀战斗、对生产主库写压测、无公告全服踢人、说不清隔离方式就硬杀 Pod

> ⚠️ **别混**：预案演练 ≠ 混沌工程——答不出隔离方式，就说预案演练。主机切主见 [01-33](../../01-基础底座/33-高可用与容灾/)；这里要的是切完背包和订单对得上。
...
- 📋 **复盘六栏**
  - 时间线（发现 → 确认 → 止血 → 恢复，UTC + 本地）
  - SLI（哪条、何时破、持续多久）
  - 根因（技术 + 变更 + 流程，5 Whys 一层）
  - 玩家口径（公告 / 补偿说了什么）
  - 经济（是否对平、有无资损、对账方法）
  - 动作项（负责人 + 截止日期 + 验收标准）

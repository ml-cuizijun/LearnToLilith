...
> 美西玩家打日服卡成幻灯片，有人说专线能把 200ms 变 20ms。欧区备份 S3 跨 region 复制一勾，PII 出了发行地。北京 05:00 cron 给美西合服，美西还在黄金时段。群里有人要 `UPDATE region='us'` 给玩家换区。
> **本章回答**：一个游戏从国内到出海会经历什么——规划定边界、建设搭骨架、运行守延迟和合规、故障分三域止血；换区是合服不是改字段。
> **不讲**：CDN 预热细节 → [08](../08-CDN与资源更新/)；迁角色步骤 → [06](../06-开服合服/)；主机双活理论 → [01-33](../../01-基础底座/33-高可用与容灾/)；加速器失真 → [12](../12-游戏网络与对抗/)；云平台 region/VPC → [07-AWS生产](../../06-云平台/07-AWS生产/)、[09-多云对照](../../06-云平台/09-多云对照/)。
...
> 🎯 **一句话**：出海不是国内架构多买几个 region——规划定边界，建设定骨架，运行守延迟和合规，故障分三域；换区是合服不是 `UPDATE`。
...
- 📖 **读图**
  - 🛤️ 主线：规划 → 建设 → 运行 → 故障；**目录**是运行/故障总闸——先查目录指哪，再猜 DNS
  - 🔀 **迁移**挂在运行期：换区是作业，不是改字段；**复盘**回规划，不是打完就散

> 📝 **面试**：「没做过真出海怎么答？」——讲国内多 AZ 与出海边界差别，标明未执刀；不编 region 切流故事。（→ Q7）
...
- 🌐 **物理下限**（常见量级，非协议规定）
  - 同城 10～30ms · 同大陆跨城 30～80ms · 跨大陆/跨洋 **150～250ms+**
  - **专线**修平坑（降抖动、稳丢包），不能把洛杉矶和东京拉近
...
- 📖 **读图**
  - Gate 就近只改善握手和大包下载；**战斗 Tick 往返**仍受 logic/room 所在 region 约束
  - 专线降抖动、稳丢包，**不缩光速距离**
...
- ❌ **Gate 在美西、room 在东京**：登录 HTTPS 可能很快，**Tick 仍跨洋**
- ✅ **实时玩法**：就近匹配 + **就近 room**；就近 Gate 只是入口优化
- 🚫 产品卖「全球同服无延迟」= 架构谎言；专线把 200ms 变 20ms = 高频错答⭐

> ⚠️ **别混**：**就近 Gate** vs **就近 room**——前者管入口，后者管 Tick。
...
- 📒 **目录**：**server_list**——告诉客户端去哪个 region 的哪条 `server_id`
...
- 📖 **读图**
  - **没有** APAC↔EU 角色库双写箭头——跨区同步只发生在账号层（单一真相源）
  - 写路径（扣费、发货、结算）必须落在**角色所在 region**；远 region 只读副本可给排行/展示，**不能写背包**

#### ❓ 为什么不做两个 region 双活写同一角色库？

- 💥 有状态 + **区服经济对账**——冲突、回档、刷物资都无法收场
- ✅ 账号层单一真相源；角色数据 **region 独立**，坏了维护该区，不跨洋双写
- ⚠️ **分服（server_id）** ≠ **玩法 region**——一 region 可有多 `server_id`；换区跨 region = 迁移，不是改目录显示名

> 💡 **打个比方**：账号是护照——全球唯一；角色是银行账户——钱在哪国就只能在那国取。
...
> 🎯 **一句话**：数据驻留按发行地，不是按机房在哪；GDPR 给删除权，数据本地化给物理隔离；跨区不同步角色；答「在 S3」不够，要说账号 + region + 有无复制。⭐🔥

### 📜 GDPR

- **GDPR（General Data Protection Regulation）** = 欧盟通用数据保护条例；适用处理欧盟居民个人数据的服务，不限于欧盟公司
- 核心义务
  - **删除权（right to erasure）**：删号要彻底删，不是软删标记
  - **数据可携带权（data portability）**：可要求机器可读导出
  - **跨境传输**：出欧盟需 **SCC（Standard Contractual Clauses）** 或充分性认定
  - **72 小时违规通知**：发现泄露 72h 内报告监管机构

> ⚠️ **别混**：GDPR 管「跨境传输」和「处理目的」，**不强制**数据必须存在欧盟境内——但**数据本地化法**才是真强制物理存。

### 🏠 数据本地化（data localization）

- **数据本地化** = 部分法域要求个人数据必须存本国境内服务器
  - **俄罗斯**：个人数据首次录入须在俄境内库
  - **中国 PIPL（个人信息保护法）**：个人信息出境需安全评估
  - **印度 DPDP**：数据受托者须在印度存储
- 本地化 = **物理隔离**；不是 GDPR 的「限制出境」口径

### 🔁 跨区数据同步

- 出海 ≠ 把国内数据同步到海外——每个玩法 region 独立，全局的只有账号
- 数据回流（analytics、对账、排行汇总）走**批处理脱敏**，不走实时双写

| 数据类型 | 跨区策略 | 理由 |
|----------|----------|------|
| 账号 / 封禁 | 全局，账号 region 单一真相源 | 非双写 |
| 角色数据 | 不同步，region 独立 | 有状态 + 经济 |
| 排行 / 展示 | 只读副本可跨区 | 只读不写 |
| 支付对账 | 批处理聚合，非实时 | 离线结算 |

> 💡 **打个比方**：总行管身份（账号），分行管本地账本（角色）；月底要报表——脱敏汇总，不是实时翻对方账本。

#### ❓ 为什么「备份开跨 region 复制」可能踩合规？

- **PII（Personally Identifiable Information）** = 可识别个人身份的信息
- 复制一开，PII 可能**出发行地**——驻留不是口号，是路径上每一跳
- 答「数据在 S3」不够 → 要说**哪个云账号、哪个 region、有没有跨账号/跨 region 复制**
...
> ⚠️ **别混**：**数据驻留** vs **多 AZ**——国内三 AZ 不是真出海；多 AZ 保机房，跨洋保不了 RTT，也保不了合规。
...
- 📖 **读图**
  - 上层账号全局唯一、非双写；下层各玩法 region 独立经济、数据不出域
  - 两层之间只有**目录指向**，没有角色数据同步箭头；回流走批处理脱敏
...
④ 数据本地化：俄/中/印有物理存储要求
⑤ 跨区：账号单一真相源；角色不同步；排行只读；回流批处理
⑥ 备份说清账号 + region + 有无复制，默认关 replication
⑦ 云账号按发行主体拆，一个 Administrator 打全球 = 合规 + 误操作双爆
```

> ⚠️ **别混**：这套机制保证「数据不出域」「合规可审计」，**不保证**跨区实时一致——角色本来就允许各 region 各自为政。
...
3. 目录
   - 聚合各 region gate / cdn / maintenance
...
> 📝 **面试**：「多 region 最小骨架？」——账号 region 多 AZ + 每玩法 region 独立经济 + 目录 + 按 region CDN。先画游戏约束，再填云产品。
...
- 📖 **读图**
  - `maintenance` 按 region 独立——摘 EU 不影响 US 登录（账号层仍要活）
  - 客户端永远先拉目录再连 Gate / 下资源；写死 IP 或只改 DNS = 绕过总闸
...
- 📖 **读图**
  - 顺序硬约束：**预热 → 灰度 → 改目录**；客户端永远先拉目录再下资源
  - DNS 切完登录通、更新全挂 = 目标没独立源/没预热

#### ❓ 为什么不能「国内预热完就切全球指针」？

- 海外有**独立源、独立带宽、独立 `min_version` 链路**
- 国内命中率绿 ≠ 海外边缘已热；硬切 = 回源打爆 + 热更失败
...
> 🎯 **一句话**：活动整点、合服窗口、变更冻结（±30min）都用发行当地时区——一份北京 cron 打全球 = 美西黄金时段停服。⭐
...
- 🕐 **现场走一遍**（北京 05:00 给美西合服）
  1. 北京 05:00 ≈ 美西 13:00～16:00 黄金时段（视 DST）
  2. 合服停写 → 美西玩家进不去 → 地区 P0 / 舆情
  3. 正确：`TZ=America/Los_Angeles` 当地低峰；多 region 配置按时区拆
...
> ⚠️ **别混**：**翻译** vs **本地化**——翻译只换文字；本地化包括支付通道、活动日历、法务评级、客服语言。

### 🛠️ GM 权限

- 美西 oncall 只写 us region 角色库
- 欧区 audit 存 eu-west-1，不打国内 SaaS
- 跨 region 查号：只读 + 审计 + 工单
- 临时全球写 ≤24h + 双人 + 过期回收

见 [14](../14-GM工具与运营支撑/)；地区支付 P0 定级见 [13](../13-游戏SRE稳定性/)。
...
- 📖 **读图**
  - 第一刀分类，不「全球同服」乱切；分支落在**真实现象**
  - 目录 / 账号 / 玩法 / 支付四条线止血互斥（十二节）
...
- 🛂 **账号挂**：全球登不上——修账号多 AZ / 鉴权依赖；**摘玩法目录没用**（门在账号）
- 🎮 **玩法挂**：仅该区不可玩——`maintenance.<region>=true`；**关全球登录 / 把玩家指到别区** = 空号
- 💳 **支付挂**：登录正常、渠道发货失败——渠道回调 / 对账补发；**扩 Gate、让玩家重复点支付** = 加重

### 账号 region 挂

```text
现象：全球各渠道登录失败，玩法 region SLI 无意义
...
#### ❓ 为什么不能 `UPDATE region='us'`？

- 改字段后：目录可能指向 US，**角色数据仍在 APAC 库** → 进 US Gate = 空号或脏数据
- ✅ 与合服同一套作业（[06](../06-开服合服/)）：
...
- 跨法域（欧↔美、欧↔国内）**默认拒绝**，除非产品 + 法务书面允许（GDPR 删除权 / 数据本地化）
- 失败恢复：**两份备份都在**，禁止反向再迁
...
对着第一节主图口述一遍，卡住就回对应小节。开头那个现场——专线、UPDATE、北京 cron——各自错在哪，现在该能说清了。

- [ ] **默画**出海的一生：国内单区 → 规划 → 建设 → 运行 → 故障，标出目录总闸和迁移挂在运行期（→ Q1 · Q14）
- [ ] 跨洋 200ms 专线消得掉吗？实时玩法 room 必须在哪？就近 Gate 救得了 Tick 吗？（→ Q1 · Q7）
- [ ] **默画**账号全局、角色落 region：为什么没有双写箭头？写路径落在哪？（→ Q2 · Q9）
- [ ] 账号挂和玩法 region 挂分别做什么？为什么不能双活写角色库？（→ Q2 · Q9 · Q11）
- [ ] GDPR 给了什么权？数据本地化和 GDPR 限制出境有什么区别？（→ Q3 · Q5）
- [ ] 跨区同步：什么同步、什么不同步？「数据在 S3」缺哪三句话？（→ Q3 · Q5）
- [ ] 多 region 最小骨架？云账号怎么拆？（→ Q4 · Q5 · Q9）
- [ ] 切 region 先切目录还是 DNS？目标未预热会怎样？（→ Q4 · Q12）
- [ ] 活动整点和合服用哪个时区？一份 cron 打全球为什么错？（→ Q6）
- [ ] 本地化包括哪些维度？翻译和本地化有什么区别？拨测为何不能用办公网？（→ Q8 · Q10）
- [ ] 三故障域怎么止血？地区支付失败为什么不扩 Gate？（→ Q8 · Q11 · Q13）
- [ ] 换区和合服什么关系？`UPDATE region` 为什么禁止？跨法域怎么办？（→ Q2 · Q9）

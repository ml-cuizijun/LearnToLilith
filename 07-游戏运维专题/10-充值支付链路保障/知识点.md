...
> 这一章讲完，你会知道当时「渠道说发了 / 我们没收到」该查哪段，以及为什么禁止裸改背包补发。
> **本章回答的问题**：一笔充值从创单到回调到发货到对账到补偿，每段唯一真相在哪、幂等键是什么、掉单/重复怎么防、为什么不能裸改背包。
> **本章不讲**：MySQL 事务机制 → [02-01 MySQL](../../02-中间件与数据库/01-MySQL/)；玩家回档 → [09](../09-玩家数据备份与回档/)；登录鉴权 → [02](../02-登录排队与选服/)；支付 SLO 定级 → [13](../13-游戏SRE稳定性/)。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：先把第一节「充值订单的一生」看熟——创单→扣款→回调→发货→对账；第二到第六节顺着这条钱链走，第七节专门讲开头那个 648 掉单现场怎么定责。每一节讲完会提示对应练习题，趁热打铁。
...
好，主图装进脑子了。接下来从「下单」开始——为什么必须服务端先生成 order_id？
...
大家想想：回调处理失败，要不要返回 5xx 让渠道重试？很多人会答「当然要」——半对。⭐ 验签失败拒，**业务已处理后重复回调必须仍 200**，否则渠道狂重试把你打爆；幂等靠 order_id。
...
回调讲完。记住口诀：**重复回调也要 200，幂等键防重复发货**。下一节发货一次且仅一次。
...
🔥 大白话：**paid→delivered 只走发货状态机**，禁止旁路 UPDATE 背包。重复发货 = 资损，和掉单一样严重。
...
⭐ 这是高频错答点：运营要补发，DBA 打开背包准备 UPDATE——错了。补偿走**工单 + 发货接口/状态机**，回档也要带着支付库规则。
...
现在再看群里「人工 UPDATE 背包」，你知道该怎么拦住他了。

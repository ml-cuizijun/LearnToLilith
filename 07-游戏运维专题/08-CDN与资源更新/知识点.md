...
> **本章回答**：一个资源包从打包贴 hash、上传源站、预热铺货、切指针换清单、客户端差分取页到回滚打回，每个阶段怎么验、炸了怎么止血。
> **不讲**：HTTP 缓存语义与 CNAME 基础 → [01-15 DNS与CDN](../../01-基础底座/15-DNS与CDN/)；热更协议与 `min_client_version` → [05](../05-热更新与版本发布/)；登录排队 → [02](../02-登录排队与选服/)；加速器与 CC 对抗 → [12](../12-游戏网络与对抗/)；海外多 region → [15](../15-出海与多区域/)。
...
- 📖 **读图**
  - 🛤️ 主线六段：打包 → 上传 → 预热 → 切指针 → 客户端 → 回滚；没有箭头从玩家指向源站
  - 🚪 下载和登录是两条路——人卡在更新页，扩逻辑服没用；登录绿、下载红 = CDN 事故
  - 🔭 监控分阶段：上传看源站同步，预热看 HIT/回源带宽，切指针后看下载成功率与版本分布，回滚先看指针是否打回
...
- 📋 **manifest**（资源清单）= 逻辑路径 → 物理 URL → sha256 → 大小；客户端对比本地 hash，只拉**差集**
- 📌 **指针** `version.json` 很小，告诉客户端当前 manifest 版本号
- 🪪 **content hash URL** = 内容身份证：改一个字节就换文件名，边缘不会新旧并存
...
- 📖 **读图**
  - manifest 是「目录」；每个文件是不可变对象（immutable）
  - 差分 patch 校验失败必须能改走整包——关整包入口 = 差分失败的人永远进不了服

#### ❓ 为什么必须带 hash，不能同名覆盖？

- ❌ **同名覆盖**：边缘 A 已有旧对象、边缘 B 拉到新对象 → 版本分裂、随机闪退
- ✅ **换 hash = 换 URL**：新内容新缓存键，旧 URL 继续服务未切指针的客户端
- 🧹 因此几乎**不用**对文件层做 purge——换 URL 即可
...
- 🏷️ 源站 **Cache-Control** 正确：文件 `public, max-age=31536000, immutable`；指针短 TTL（第五节）
- 🔐 源站鉴权：预热和玩家拉包用**相同鉴权方式**抽检——鉴权过严 → miss 时 403 → 客户端当损坏重下
- 🌐 下载域名独立：`res.example.com` → CDN → OSS

#### ❓ 为什么不能把包挂到逻辑服 Nginx「顶一下」？

- 🚫 逻辑服公网 IP 一暴露大文件，版本日高防和带宽账单爆炸
- 🧭 正确拓扑：玩家 → CDN 边缘 → OSS；逻辑入口在高防后且**不提供大文件**
- 🔧 运维调试走内网或带鉴权管理入口，不进客户端配置
...
- 📖 **读图**：左图源站隐藏 behind CDN；右图把源站暴露给全球——版本日临时顶一下，第二天账单爆炸。
...
- 📦 预热范围至少：指针、manifest 索引、体积最大的 N 个包、各渠道主包
- 🔗 **URL 必须和客户端完全一致**（scheme、大小写、path；query 部分 CDN 忽略 → 缓存键分裂）
- 🌍 海外 region 要**独立预热**——国内 HIT ≠ 全球可切（[15](../15-出海与多区域/)）
- ⏰ 大版本提前**数小时**预热，盯厂商配额排队
...
- 📖 **读图**：三关全过才切；预热 URL 和客户端真实 URL 差一个 query = 白预热。

> 💡 **打个比方**：预热像把货铺到各城市门店——工单说「已发货」但货架空着，开门营业（切指针）后顾客（客户端）全会涌向总仓（源站）。

#### ❓ 为什么工单绿勾不够？

- 🎫 厂商工单只保证「任务受理/下发」，不保证**你客户端用的那条 URL**在各地已 HIT
- 🧪 必须对**客户端真实 URL**多地抽检 HIT + hash
- 📉 切前回源带宽已降 = 货真的铺到了；切完才降 = 玩家在帮你预热
...
- 📖 **读图**（分组记忆）
  - **文件层**管不变——① hash URL 换内容换 URL；③ immutable 让 CDN/浏览器放心长缓存
  - **指针层**管可变——② 短 TTL 让切指针分钟级全球生效
  - **操作层**管可靠——④ 预热保证切指针即 HIT；⑤ Vary 不乱设，否则缓存键爆炸

- 🏷️ **immutable**：CDN/浏览器可假设对象永不变——换内容必须换 URL；勿对 hash URL 做 purge「刷新」
- 🔀 **Vary**：`Vary: User-Agent` + 客户端 UA 分散 → 缓存键爆炸 → 命中率崩；游戏静态资源通常不设 Vary
- 📡 **HTTP/2 / HTTP/3**：多路复用减连接数，**不减**回源字节数——大版本日仍靠预热

> ⚠️ **别混**：**Vary 头**不是越多越安全——`Vary: User-Agent` 在游戏场景会分裂缓存键；静态资源不设 Vary。

> 📝 **面试**：「CDN 命中五件套：hash 不可变、指针短 TTL、immutable、预热验证、Vary 不乱设。记忆分组：文件层（hash+immutable）管不变，指针层（短TTL）管可变，操作层（预热+Vary）管可靠。」
...
⭐ 这是面试必考。🔥 大白话：**指针短 TTL，文件长 TTL + 内容 hash**——回滚打指针，不要靠 purge 全世界。
...
- 📖 **读图**
  - ①②③ 做完才切；客户端先拿小指针，再按差集拉文件（应 HIT）
  - 指针被边缘长 TTL 缓存 → 你以为切了，部分边缘仍吐旧清单 → 版本分布**双峰**、闪退
  - 灰度指针用**独立文件名**（`version-canary.json`），勿与全量共用长 TTL URL

#### ❓ 为什么指针必须短、文件必须长？

- 📌 **指针短（~30s）**：改「今日菜单」要全球快速生效；长 TTL → 切了边缘还旧清单
- 📦 **文件长（31536000 + immutable）**：hash URL 内容永不改，放心长缓存、几乎免 purge
- 🔄 两者策略**必须相反**——同 TTL 要么切不动，要么回源爆炸
...
- ❌ 错配 `no-cache` on 文件层 → 每次 miss 回源
- ❌ 错配 pointer 长 TTL → 切了边缘不更新
...
客户端流程：拉指针 → 比 hash → 尝试 patch → 校验 → 失败则**整包 URL**。
...
- 📖 **读图**：差分失败必须能落到整包；运营关整包入口「省带宽」→ 差分失败玩家永远卡在更新页。灰度时盯**兜底比例**，不只报平均下载体积。

#### ❓ 为什么不能关整包入口「省带宽」？

- 📉 差分失败 30% 时，容量上就是**整包洪峰**——关入口 = 那 30% 永远进不了服
- 💰 关整包是假省钱：客诉与渠道差评成本远高于带宽
- 📊 灰度盯 `patch_fallback_ratio`，容量按最坏整包 × 峰值启动预留
...
- 📖 **读图**：第一刀回指针；purge 会更糟——全员 miss 再打源站。CDN 炸时抬 `min_client_version` = 客诉不可分；排队人数假性偏低，不要加买量。

#### ❓ 为什么第一刀不是全网 purge？

- 🧹 purge = 主动让全球边缘变 MISS → 下一波启动全打源站，把回源打爆放大
- ✅ 指针打回 → 客户端改拉旧 manifest → 旧 hash 文件仍在边缘 HIT
- 🗑️ **不删**源站新文件——修好预热后再切回；删了就真回不去
...
- 🔐 证书未覆盖下载域名 → TLS 全失败，像「更新挂了」
- 🎫 防盗链/签名在备上回源不通 → miss 时 403 → 客户端当损坏重下 → 重试放大
- 📦 备上未预热同一批 hash → 切流 = 换一家回源打爆
- ✅ 主备都 HIT 验收后再切；主备都 miss 时先回指针，别瞎切 CNAME

- 🛡️ **回源盾**（Origin Shield）：全球 miss 收成少数节点回源——盾 miss 时源站仍吃一波，不能替代预热
- 📐 **Range**：客户端分片下载若 Range 无限制，回源连接数可能翻倍——源站或 CDN 侧设合理 Range 策略
...
- 📖 **读图**
  - CDN 未就绪禁止抬 min；`min` 抬起后大量老端被挡在登录——若 CDN 同时 miss，新端也下不完包，客诉不可分
  - 投放（买量）在下载成功率恢复前**不加**——排队人数假性偏低

#### ❓ 为什么 CDN 炸时不能抬 min？

- 🚧 抬 min = 强迫所有人走新资源路径；CDN miss 时新端也下不完 → **登录挡 + 下载挂**叠在一起
- 🔍 客诉不可分：分不清是强更问题还是 CDN 问题
- ✅ CDN 炸：回指针 / 暂停强更；**不叠抬 min、不加投放、不扩逻辑服**
...
- 📌 全量指针仍为 `version.json`（短 TTL）；canary 用户拉 canary manifest——两套 URL 不可共用同一长 TTL 缓存键
...
- 🛡️ **CC 下载攻击**（[12](../12-游戏网络与对抗/)）：更新页 QPS 异常高但带宽正常 → 可能是恶意拉小 manifest；源站限 QPS + WAF
- 💰 **带宽计费 vs 请求数计费**：大包体看**字节出账**；小 manifest 高频看**请求数**——两个预算都要版本日前检查
...
- 📖 **读图**：先问卡更新页？→ HIT/回源；闪退 → hash/双峰；进服失败 → min/协议；海外慢单独验 region。

...
> 版本日群里喊「热更一下」——有人重载配置，有人滚战斗镜像，有人切 CDN 指针；活动掉落多写一个零，另有人要再热更「×0.01」当没发生。
...
> 🎯 **一句话**：版本发布像一列火车——打包装货、CDN 铺差异包、强更/灰度检票、热更途中换规则、回滚是倒车，每站职责不同。
...
- 📖 **读图**
  - 🛤️ 从左到右是**一次发布**时间线：产物钉死 → CDN 预热 → 强更/灰度 → 热更 → 按可逆性回滚
  - 🔭 监控分站看：打包看 artifact hash；CDN 看 HIT；强更看端大版本；热更看 `loaded_version`；回滚看 SLI
  - 🚨 出事故先问「火车停在哪一站」——配置 reload 和滚战斗镜像是两列不同的车
...
- 🪪 **镜像 digest**（`sha256:…`）= 二进制层唯一身份证，CI 打出后不可再改
- 📋 **配置 version** = 服务端 reload 的加载号，和端大版本无关
- 📌 **资源指针** = CDN 上 `version.json` 指向哪一版 manifest
- 🚧 **`min_client_version`** = 登录/目录挡不兼容老端的门槛——四者必须在同一变更单里自洽
...
- 📖 **读图**
  - 四元组各管一层：改活动数值只动配置 version；改战斗逻辑动 digest；改客户端资源动指针；改协议才动 min
  - 揉成一个「版本号 2.5.1」无法分层回滚
...
- 🧩 **端侧也要拆开**：端大版本（强更）· 端资源号（manifest）· 服配置 version（reload）· 镜像 digest（二进制）
- 面试常问「怎么回滚」——先判断坏在哪一层，再选对应按钮
...
- 📎 客户端启动拉小指针 `version.json`（短 TTL），对比本地 manifest 只拉**差异包**——hash 变了的文件才下载
- 🧭 **指针**告诉边缘「现在该哪一版」；**文件** URL 带 content hash、长 TTL，内容变 = 新 URL，几乎不用 purge
- 差异包靠的是 manifest **逐文件 hash 比对**，不是整包版本号递减
...
- 📖 **读图**
  - 切指针是**最后一跳**——之前必须文件齐、预热 HIT、服务端能解析新资源协议
  - 指针切了但大包还在上传 40% → 边缘全球 miss、源站带宽打满、下载成功率掉而登录仍绿

> 💡 **打个比方**：指针像餐厅今日菜单（天天换、短 TTL）；文件像 sealed 罐头——换内容就换 SKU（新 hash URL）。差异包 = 只拿菜单上新增的菜。

#### ❓ 为什么不能先切指针再传包？

- ❌ 指针先切 → 全球边缘去拉还不存在的 hash URL → **回源风暴**
- ✅ 正确顺序：上传齐 → 预热多地 HIT → 服务端兼容 → **最后**原子切指针
- 🔁 失败时**指针打回**上一份已预热 manifest，**不删**源站新文件——便于修好后再切
...
- 📖 **读图**
  - `min` 是最后一道闸——之前服务端必须同时认老协议和新协议
  - 只降 `min` 不够：服务端若已删老协议，降 min 后老端能登录、进战斗握手仍失败

> 💡 **打个比方**：检票应在大厅门口（登录/目录）做——**不要在进房间时才查身份证**，那时局已开，踢人 = 判负。

#### ❓ 为什么 min 不能放战斗？

- 🚪 放登录/目录 = 老端**进不了服**，体验是「请更新」
- 💣 放战斗 = 老端能登录、匹配成功，进战斗握手炸 → **局内判负 / 投诉风暴**
- 🔄 回滚强更要两步：先恢复双协议，再降 min——顺序反了仍握手失败
...
> ⚠️ **别混**：**降 min**（客户端门槛）≠ **服务端双协议**（服务端还认不认老包）——回滚强更要两步都做。
...
- 删老协议前必须确认端大版本分布 **v1 < 0.1%** 且支付/进战斗 SLI 稳定
...
- 🛤️ **灰度路径**：内网 → 指定服 → uid 哈希尾号 → 渠道包
- 🌐 加速器和 NAT 把源 IP 灰度打成「某出口 100% 新包」
- 🏛️ 全服组件（登录配置、支付开关）没有「只灰 S1」——要么全服一致，要么设计独立灰度指针
...
闸条件写进变更单：崩溃翻倍 / 登录掉阈值 / 支付异常 → **立刻停灰度**。
...
- 📖 **读图**
  - 灰度是**漏斗**不是开关——每档通过才放大
  - 资源灰度最好用**独立指针文件名**（如 `version-canary.json`），勿与全量共用会被长 TTL 缓存的 URL
  - 只用新注册号测兼容 = 没有老存档，测不出协议问题

#### ❓ 为什么不按源 IP 灰度？

- 🔌 加速器 / NAT / 运营商出口 → 大量玩家共享同一源 IP
- 📉 结果：某出口 100% 新包、别的出口 0%——比例完全失真
- ✅ 按 **uid / server_id / 渠道** 哈希，玩家身份稳定、可复现
...
群里喊「热更一下」——三个人可能做了三件完全不同的事。⭐ 先问**改的是哪一类**，再问**可不可逆**——掉落写错多一个零，不是再乘 0.01 能抹平的。
...
- 配置中心发布 version=X，后台显示「已发布」≠ 全服生效——要对账 **`loaded_version` 回执**
- 收不齐当失败：重推或摘未回执实例的新进，**不要开放活动**（规则分裂比晚开代价大）
- 重复「加载 X」必须**幂等**——不能叠加成两份活动
- 热更通道要鉴权、审计、限频，不要暴露公网或共用玩家 GM 令牌

### 📜 脚本热更

- 脚本能改分支逻辑，**改不了结构体布局和协议字段含义**
- 进行中的局：按房间切版本，或强制结算；一半新公式一半旧公式 → 结算争议
- 战斗**二进制**不能 RollingUpdate（抄登录 Deployment = 砍在线局）
...
- 📖 **读图**
  - ① 只影响**新进**房间；已开房间仍钉旧进程直到局结束
  - ③ flush 未完成就 kill = 背包丢数据
  - 金丝雀只能让**新匹配**进新 digest 的 Pod，不能「切 5% 流量」到已开房间

大厅 / 登录可 RollingUpdate 或蓝绿——Session 可重建。Gate 滚动会断长连接，需重连快通道（[02](../02-登录排队与选服/)）。

#### ❓ 为什么战斗不能 RollingUpdate？

- 🔪 RollingUpdate = 旧 Pod 被 Terminating → **进行中的局直接判负**
- ✅ 摘流四步：新匹配不进旧 Pod → 等空/强制结算 → flush → 换 digest
- 🐤 金丝雀只对新**开**房间分流，不能对已开房间切 5% 流量
...
- 客户端资源变更**不经过**服务端 reload——靠 CDN 指针 + 客户端下载器拉差异包
- 服务端须**兼容**新旧资源协议（双 manifest 解析或字段 optional）
- 已装新包的设备：
  - 改回 CDN 指针**管不到**已写入本地存储的文件
  - 卸载/清缓存不可依赖
  - 正确路径：**再发修复包**（新 hash URL），客户端版本号向前递增
- 资源灰度用**独立指针文件**（如 `version-canary.json`），与全量 `version.json` 分离

### 🔐 热更通道：鉴权、审计、幂等 🔧

- **鉴权**：独立令牌，不与玩家 GM 共用；IP 白名单 + mTLS
- **审计**：谁、何时、对哪些实例、加载哪个 version
- **幂等**：重复「加载 version X」结果与一次相同
- **限频**：防误点 / 脚本狂发
- 回滚 = 再发「加载 version X-1」，不是 SSH 改文件
...
- 📖 **读图**
  - 越往下越难撤——配置最快；已装客户端包和已产出经济不能靠「再热更 ×0.01」假装没发生
  - 经济配错顺序：**回配置停产出 → 评估回档（07/09）**，不是再乘 0.01

> 💡 **打个比方**：五类回滚不是同一个「撤销键」——像 Word 里撤销文字、撤销图片、撤销保存完全是三个菜单。

#### ❓ 为什么经济错了不能热更 ×0.01？

- 🎒 已经发到玩家背包的道具，配置回滚**抹不掉**
- ❌ 「再乘 0.01」= 假装没发生，账实不符、审计不过
- ✅ 顺序：回配置停产出 → 评估回档/补偿（[07](../07-停服维护与不停机更新/)/[09](../09-玩家数据备份与回档/)）

> ⚠️ **别混**：**发版回滚**（换 version/指针/digest）≠ **经济回档**（改玩家背包数据）——掉落多写一个零走后者。

> 📝 **面试**：「开口分五类。配置看 loaded_version 回执。战斗二进制按 01 摘流四步。经济错了先停产出，不是热更 ×0.01。」

本节对应练习题 Q1、Q6、Q7、Q11。
...
- 📖 **读图**
  - 决策树第一条分支是**分类**，不是「全员回滚」
  - 灰度 5% 崩溃 → 停灰度 + 回滚该类，不是观察一小时再全量
  - 战斗 RollingUpdate 进行中 → **先停滚动**，再按摘流 undo
...
- 预发必须跑完：五类更新路径各至少演练一次；回滚演练（配置 version-1、指针打回、rollout pause + digest-1）；灰度闸模拟崩溃×2 是否自动停
- 合服前 [06](../06-开服合服/) 版本对齐应比较四元组，不是只比 Git branch 名
...
- 📖 **读图**：分支按**现象**进门，不是按「谁喊得响」——卡更新页先查 CDN，进服规则错先查回执，CCU 掉先问有没有滚 battle。
...
| min 放哪？ | 登录/目录；不放战斗。回滚要双协议+降 min。 |
...
- [ ] **默画主图**：打包 → CDN → 强更/灰度 → 热更 → 回滚，标出四元组钉在哪（Q1）
- [ ] 能分**五类更新**并用三句讲热更 vs 停服更（进程在不在、端听不听、回滚能否加载旧版）（Q2）
- [ ] 能口述**变更单四元组**：各管哪一层、禁 latest 的原因（Q3、Q8）
- [ ] 能口述 **CDN 发布顺序**与「先切指针」为何全球回源；能讲差异包靠逐文件 hash 比对（Q4、Q12）
- [ ] 能口述**强更两段抬闸**：min 放哪、双协议与降 min 的关系（Q5）
- [ ] 能口述**灰度**：按什么切、观察什么、何时立刻停；灰度比例 ≠ 覆盖率（Q1、Q6、Q7、Q11）
- [ ] 能口述**配置回执**：`loaded_version` 不齐为何不能开活动（Q9、Q10）
- [ ] 能口述**战斗发布**：为何禁 RollingUpdate、摘流四步、金丝雀只对新房间（Q14）
- [ ] 能口述**回滚决策树**：五类各怎么撤、经济为何不能 ×0.01（Q13）
- [ ] 能**定责**：更新页 / 进不去 / 规则错 / 战斗不一致各查什么（Q13）
- [ ] 能口述**协议演进五阶段**与删 v1 前看什么指标（Q13）
- [ ] 能口述**预发 promote 纪律**：为何禁止线上改 digest、回滚演练跑什么（Q13）

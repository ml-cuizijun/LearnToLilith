...
> **不讲**：Gate/战斗拓扑与有状态发布 → [01](../01-游戏服务器架构/)；活动整点登录峰容量 → [03](../03-活动高峰与压测/)；全链路压测建模 → [04](../04-全链路压测与容量模型/)；开服写目录 → [06](../06-开服合服/)；TCP 队列与 `ss` → [12-网络栈与Socket](../../01-基础底座/12-网络栈与Socket/)、[13-TCP与UDP](../../01-基础底座/13-TCP与UDP/)。
...
- [ ] **默画**六跳主图：渠道→账号→目录→排队→Gate→大厅，标出 ticket 在哪发、长连接在哪建。
- [ ] 口述 **排队消化率 R** 怎么从压测来，为什么不是 Login 副本数×QPS。
- [ ] 说清 **ticket 一次性 + 短 TTL** 防什么；**game_token** 和 **ticket** 各管什么。
- [ ] 解释 **重连快通道**——什么情况下重连误进登录队，监控上长什么样（queue 深、Gate 空）。
- [ ] 背 **目录两条铁律**：不可 500；爆满只拦新号。
- [ ] 「全渠道登录失败」**第一刀**查什么，**不要**做什么。
- [ ] 「有 ticket 连不上 Gate」沿 **fd / SYN-RECV / ticket 日志** 定责。
- [ ] 「Gate 已连、卡加载角色」为什么往往是 **大厅/创角/MySQL**，不是排队。
- [ ] 口述 **登录漏斗** 监控比单跳 SLA 多的原因。
- [ ] **默画**排队时序：enqueue → 出队 → ticket → Gate 校验 → session。
- [ ] 口述 **全服踢人四步顺序**，解释跳过任一步会怎样。

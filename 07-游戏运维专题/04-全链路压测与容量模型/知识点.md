# 全链路压测与容量模型

> 运营喊「撑 50 万 CCU」——有人只压 Login HTTP，报告写 Gate 还能再扛 3 倍；活动当晚 fd 打满、创角锁死。另一次对着生产主库跑发奖脚本，回档用了两天。
> **本章回答**：从零建 **容量模型**、跑 **全链路压测**、验收瓶颈、把数字写进活动/开服 runbook。
> **不讲**：登录六跳与 ticket → [02](../02-登录排队与选服/)；活动三峰降级 → [03](../03-活动高峰与压测/)；HPA/PDB 语法 → [03-05](../../03-Kubernetes/05-工作负载控制器/)；MySQL 调优 → [02-01](../../02-中间件与数据库/01-MySQL/)；通用 RED/USE → [21](../../01-基础底座/21-性能方法论-USE与RED/)。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

---

## 一、一张图：一次压测的一生

> 🎯 **一句话**：容量建设是 **立项 → 建模 → 构造 → 执行 → 验收 → 投产** 六段——跳过任一段，活动容量就是猜。

```mermaid
flowchart LR
    S1["立项<br/>目标·口径"] --> S2["建模<br/>分层账本"]
    S2 --> S3["构造<br/>数据·机器人"]
    S3 --> S4["执行<br/>全链路压"]
    S4 --> S5["验收<br/>SLO·瓶颈"]
    S5 --> S6["投产<br/>HPA·预算"]
    S5 -.->|×0.7| RB["runbook·R·副本数"]
```

- 📖 **读图**
  - 🛤️ 从左到右是项目时间线：压测结果 → 安全系数 → 日常/活动配置，不是「压一次就完」
  - ⭐ 建模（三节）与执行（五节）最常考——「要多少 Gate」必须能追溯到报告某一页；验收照 USE/RED（[21](../../01-基础底座/21-性能方法论-USE与RED/)）

本节对应练习题 Q1。

---

## 二、立项：容量目标与口径统一 ⭐

> 🎯 **一句话**：先写清 **CCU、登录 QPS、DAU** 三个口径和 **活动系数**，再谈 Pod 数——口径混了，50 万 CCU 可以算出 50 种 Gate 数。

### 📐 三个口径

- **CCU（Concurrent Users）** = 同时在线、已登录在大厅或战斗中的角色数（去重规则、是否扣机器人要书面写死）
- **DAU（Daily Active Users）** = 日活；`CCU ≈ DAU × 同时在线率`（手游峰值常见 8%～15%）
- **登录 QPS** = 每秒完成 **账号→…→大厅** 的新登录/重连数，与 CCU **非线性**——重连潮、整点推送会拉高（见 [03](../03-活动高峰与压测/)）

```mermaid
flowchart TD
    BIZ["业务目标<br/>周年 80万 CCU"] --> CCU["CCU 定义去重"]
    CCU --> LOGIN["登录 QPS = CCU×系数×活动峰"]
    CCU --> PLAY["玩法 QPS / room 数"]
    CCU --> WRITE["发奖 TPS · 单独估"]
```

- 📖 **读图**：一个 CCU 目标必须拆 **登录 / 玩法 / 写** 三条线——登录卡 L4 Gate、玩法卡 L6/L7、发奖卡 L8 MySQL，不能只交一个数字给运维。

### 📝 立项文档最小字段 🔧

```text
目标 CCU / 分服 CCU 上限
活动登录峰系数（相对日常）
核心玩法并发 room 数
结算写 TPS 峰值 · 是否全服同一秒
SLO：登录成功率 / P99 延迟 / 零资损
环境：预发全链路 · 数据规模 ≈ 生产 30%+
```

- 🗣️ **口径会议必问**（书面确认，否则报告无法签字）
  - 「80 万在线」= CCU 峰值还是 DAU？
  - 活动是否全服同一秒结算？
  - 是否含机器人/工作室账号？
  - 分服峰值还是全服加总？

> ⚠️ **别混**：**压测 CCU**（机器人在线）≠ **压测登录 QPS**——10 万机器人慢慢进是低 QPS（测稳态 fd/内存）；1 万机器人秒进是高 QPS（测登录 funnel 吞吐）。

> 📝 **面试**：「压测 CCU 怎么来的？」——「先统一 CCU 口径，再拆登录/玩法/写三峰，每峰单独目标+SLO，不是 Pod 拍脑袋。」

> 🔍 **现场**：口径文档存在性检查

```bash
test -f /docs/capacity/ccu_definition.md && grep -E 'CCU|DAU|bot' /docs/capacity/ccu_definition.md
# CCU=concurrent  DAU=daily  bot=included   ← 口径已书面定义
# 若文件不存在则立项未闭环，拒签压测计划
```

本节对应练习题 Q10。

---

## 三、建模：分层容量账本 🔥⭐

> 🎯 **一句话**：**容量模型 = 分层账本**——每层写 **容量 C、需求 D、利用率 U、安全系数 α**，最窄层决定全链路 R。

> 💡 **打个比方**：容量像水桶链——最窄的那截决定流速，加粗别的桶不增加出水。扩非瓶颈层就是「给粗桶换了个更粗的，出水没变」。

### 🪣 标准分层（游戏全链路）

```text
L0 CDN/版本        （带宽 · QPS）
L1 账号/Login HTTP （无状态 · DB 池）
L2 目录            （读缓存 · 不可 500）
L3 排队            （按服 R 发票）
L4 Gate            （长连接 fd · 内存）
L5 大厅/匹配       （CPU · DB 读）
L6 战斗/跨服       （空 room · CPU tick）
L7 Redis           （QPS · 热 key · 内存）
L8 MySQL 主库      （写 TPS · 行锁）
```

```mermaid
flowchart TB
    subgraph chain["需求 D 传递"]
        D1["登录 QPS"] --> L4
        D1 --> L5
        D2["在线 CCU"] --> L4
        D3["玩法 API"] --> L6
        D4["发奖 TPS"] --> L8
    end
    L4["L4 Gate C4"] --> BOT["瓶颈 min(Ci/Di)"]
    L8["L8 MySQL C8"] --> BOT
    BOT --> R["对外 R · 副本预算"]
```

- 📖 **读图**
  - 一条登录请求的 D 同时压到 L1/L3/L4/L5，但只有**最先饱和**的那层是瓶颈
  - 瓶颈层决定排队 **drain_rate**、Gate **副本数**、MySQL **batch**——扩非瓶颈层是假容量

### 🧮 单层公式

```text
可用容量 C_layer = 压测饱和点 × α（α 典型 0.7）
需求 D_layer     = 业务目标 × 活动系数
余量 headroom    = C_layer - D_layer  （应 >0）
全链路 R          = min(C_i / D_i) × 满足SLO的最大负载
```

- 🔑 **Gate 示例**
  - 压测：单 Gate 4 万 ESTAB @ CPU 70% · 内存 60%；集群 10 台 → C_raw = 40 万连接
  - `C_gate = 40万 × 0.7 = 28万`；目标 CCU 30 万 → headroom = −2 万 → 加 Gate 或降 per-process 内存

- 🔑 **MySQL 写示例**
  - 压测：主库 8000 write TPS @ Threads_running 80；发奖需求 5000 TPS × 10min
  - `C = 8000 × 0.7 = 5600`；headroom = 600 → 余量不足 → 分批/延迟结算，不能硬顶

#### ❓ 为什么不能用「CCU ÷ Pod 数」定 Gate？

- ❌ **一个除法**：忽略登录峰、创角锁、战斗 room、主库写——四条瓶颈线完全不同
- ✅ **分层账本**：每层各自 C/D，`min(C_i/D_i)` 定全链路 R；CCU 是**结果**，不是模型输入唯一值
- 🧾 **可复盘**：每层 C 有压测报告页码，D 有口径文档；偏差 >15% 触发补压

> ⚠️ **别混**：**Kubernetes requests/limits** 是调度单位，**不是**业务容量 C——C 来自压测饱和点。Gate Pod request 4C/8G ≠ 能扛 4 万连接。见 [03-05](../../03-Kubernetes/05-工作负载控制器/)。

> ⚠️ **别混**：**压测最大值** ≠ **投产目标**——投产 = 最大值 × α（见六节验收）。最大值是实验边界，投产是 SLA 承诺。

> 🔍 **现场**：容量表在不在 runbook

```bash
test -f /ops/runbook/capacity_ledger.yaml && yq '.layers[] | .name + " " + .C + " " + .D' /ops/runbook/capacity_ledger.yaml | head
# gate_connections 280000 300000   ← D>C 活动前必须动作
# login_drain_rate 1200 1000        ← headroom 充足
# 若文件不存在 → 容量未建模，活动靠猜
```

> 📝 **面试**：「容量模型长什么样？」——「分层账本 L0～L8，每层 C/D/U，min 层定 R 和副本，α=0.7。」

### 收束：容量模型凭什么可复盘

```mermaid
flowchart TD
    subgraph in["输入"]
        I1["业务 CCU/三峰"]
        I2["真协议压测"]
    end
    subgraph model["模型"]
        M1["分层 C/D"]
        M2["min 瓶颈"]
        M3["×0.7"]
    end
    subgraph out["输出"]
        O1["R · 副本 · batch"]
        O2["runbook"]
        O3["活动前对表"]
    end
    in --> model --> out
```

- 📖 **读图**（分组背）
  - **输入**：口径统一 + 影子库压测——输入错了，整个模型白建
  - **模型**：分层账本，min 层定旋钮——不是每层都扩
  - **输出**：可执行数字写 runbook；数字不在 runbook 里等于没做

> ⚠️ **别混**：容量模型 **不保证**「永不故障」——保证 **知道先坏哪层、旋钮在哪**。

> 📝 **面试**：背三句——「分层 C/D · min×0.7 · 写进 R 和副本表」。追问「凭什么复盘」——「每层 C 有报告页码，D 有口径文档，偏差 >15% 补压。」

口诀：**CCU 是结果，分层账本才是模型**。本节对应练习题 Q2、Q12。

---

## 四、构造：流量机器人与数据工厂 🔥⭐

> 🎯 **一句话**：**全链路压测** 必须用 **真协议机器人** + **规模相当的数据**——Mock HTTP 只配测 Login，测不出 Gate 粘包、战斗 room、MySQL 行锁。

> 💡 **打个比方**：HTTP 压 Login 像只试了大门弹簧，没试电梯、走廊、房间——真链路要让人走进去坐下打仗才算压过。

### 🤖 机器人能力矩阵

| 能力 | 测哪层 | 没有则漏测 |
|------|--------|------------|
| 渠道/账号登录 | L1 | 账号 DB 池 |
| 选服+排队+ticket | L2-L3 | drain_rate |
| Gate 长连接+心跳 | L4 | fd · 半连接 |
| 进大厅+创角 | L5-L8 | 行锁 |
| 匹配+进战斗 tick | L6 | CPU · room |
| 活动 API 循环 | L6-L7 | 热 key |
| 结算发奖脚本 | L8 | 写尖刺 |

```mermaid
flowchart LR
    BOT["压测机器人"] -->|真 TCP/KCP| GATE["Gate"]
    BOT -->|非 HTTP only| HALL["大厅/战斗"]
    FACT["数据工厂"] --> DB["影子 MySQL"]
    FACT --> REDIS["影子 Redis"]
```

- 📖 **读图**：机器人必须 **走 Gate 长连接**——只 curl Login 是假压测；bot 造流量，数据工厂造存量，两家独立工厂。

#### ❓ 为什么录制回放 HTTP 不算全链路？

- 📼 **录制回放** = 无状态请求序列，不维护 Gate session，进不了战斗 tick
- 🔌 **漏测**：有状态 room、长连接心跳、粘包、行锁、热 key
- ✅ **要 stateful bot**：真协议走 Gate → 大厅 → room → 主库写

### 🏭 数据工厂纪律

- 账号/角色/背包 **量级** ≥ 生产 30%（索引选择性才真实）
- **影子库**：生产脱敏 dump + 定期刷新；**禁止**压测写生产主库
- 公会/排行 **预置** 热数据，否则排行表只有 100 行时 Redis「假快」

> ⚠️ **别混**：**录制回放 HTTP** ≠ **stateful bot**——前者测不出有状态 room 与长连接心跳。

> 📝 **面试**：「压测数据哪来？」——「影子库脱敏+工厂造数，量级够；写永不打生产主库。」

> 🔍 **现场**：压测环境隔离检查

```bash
mysql -h mysql-shadow -e "SELECT COUNT(*) FROM role;"
# 8500000   ← 量级够；若只有 1万行，创角索引压不准

grep -r 'prod.mysql' /opt/loadtest/config/ && echo 'LEAK!' || echo 'shadow ok'
# shadow ok   ← 配置干净，未连生产
# 若输出 LEAK! → 立即停压测，检查是否已写生产
```

本节对应练习题 Q3、Q4。

---

## 五、执行：全链路压测怎么跑 🔧🔥⭐

> 🎯 **一句话**：压测 **分场景 ramp**——先 **soak（浸泡）** 找泄漏，再 **spike** 找尖刺，每阶段 **固定时长 + 固定指标采样**。

### 📋 场景模板

```text
S1 基线 soak：70% 目标 CCU · 2h · 看内存泄漏/连接泄漏
S2 登录 spike：0→目标登录 QPS · 5min · 看排队/Gate SYN
S3 玩法 spike：活动 API · 目标 QPS · 10min · CPU/热 key
S4 发奖 spike：结算 batch · 目标 TPS · 30min · 主库锁
S5 混合：三峰叠加 · 20min · 定 min 瓶颈层
```

```mermaid
flowchart TD
    RAMP["ramp-up 梯度<br/>10%→50%→100%"] --> HOLD["hold 稳定段"]
    HOLD --> OBS["采样 RED/USE"]
    OBS --> STOP{"SLO 破?"}
    STOP -->|是| BI["记录瓶颈层"]
    STOP -->|否| NEXT["下一档负载"]
```

- 📖 **读图**：**不要秒级从 0 拉到 100%**——中间档定位「从哪层开始非线性恶化」。10% 正常、50% L4 队列堆积、100% L8 先挂——恶化曲线比一个「最大 QPS」更值钱。

#### ❓ 为什么必须先 soak 再 spike？

- 🛁 **soak**：70% 目标 · ≥2h → 找连接/session/内存泄漏（spike 几分钟看不出来）
- ⚡ **spike**：找饱和点与尖刺瓶颈
- ❌ **无 soak 直接 spike**：活动当晚才发现 2h 后 fd 泄漏——报告「最大值」骗人

### 🛑 执行纪律

- 变更 **冻结**（压测期无发布）——发布会改变 C，数据作废
- 监控 **双写**：压测平台 + 生产同构 Prometheus（预发）
- **abort 条件**：主库 Threads_running>200、错误率>1%、资损苗头——任一立即停

### 🧩 预发与生产同构 checklist ⭐

```text
内核：somaxconn · ip_local_port_range · ulimit 与线上一致
Gate：buffer 大小 · 心跳间隔 · ticket 校验逻辑同版本
MySQL：参数 template 同生产 · 行数 30%+
Redis：cluster 分片数一致 · 热 key 预置
K8s：requests/limits 比例同生产 · 网络插件同 CNI
```

- 📊 **阶梯加压记录**（交付模板）：每档记目标 QPS/CCU · hold · gate_ok% · 瓶颈层
  - 例：10% / 8000 / 10min / 99.8% / —；50% / 40000 / 10min / 99.6% / L4；100% / 80000 / 10min / 99.1% / L4·L8

> ⚠️ **别混**：**预发 CPU 比生产强** 会高估 C——同构含 **单核性能与磁盘 IOPS**，不是只同 K8s yaml。预发 c5.4xlarge、生产 c5.2xlarge，同一份结果可差 40%。

> 📝 **面试**：「全链路压测和接口压测区别？」——「真协议走 Gate+room+主库写；分场景 spike/soak；找 min 瓶颈层写进 R。」

> 🔍 **现场**：压测中控 + kernel 对齐

```bash
curl -sS 'http://loadtest:9090/api/v1/status' | jq '.phase, .target_qps, .actual_qps, .abort'
# "login_spike"   ← 当前阶段
# 8000            ← 目标 QPS
# 7920            ← 实际 QPS，达标
# false           ← 未触发 abort

curl -sS 'http://prom:9090/api/v1/query?query=login_funnel_ratio{stage="gate"}' | jq '.data.result[0].value[1]'
# "0.996"   ← gate 段 SLO 达标（≥99.5%）

sysctl net.core.somaxconn net.ipv4.ip_local_port_range
# net.core.somaxconn = 4096       ← 与生产一致
# net.ipv4.ip_local_port_range = 32768 60999
# 与生产 sysctl 导出 diff，差异项压测前修齐
```

本节对应练习题 Q5、Q6、Q14。

---

## 六、验收：SLO、瓶颈与报告 🔥⭐

> 🎯 **一句话**：压测交付物不是「QPS 最大值」，而是 **每层 C、瓶颈层、0.7 后 R、副本预算表**——能直接写进 [02](../02-登录排队与选服/) 的 drain_rate。

### 🎯 瓶颈定责

| 场景 | 瓶颈层 | 现象 | 建议 |
|------|--------|------|------|
| 登录 spike | L4 Gate | ESTAB 满、SYN-RECV 涨 | +Gate 或降 R |
| 登录 spike | L8 MySQL | 创角锁等待 | 创角限流/拆索引 |
| 玩法 | L7 Redis | 单 key QPS 顶 | 拆 key |
| 发奖 | L8 MySQL | Threads_running 200 | 降 batch |

```mermaid
flowchart TD
    MET["指标恶化"] --> U{"Utilization>70%?"}
    U -->|是| SAT{"队列/锁?"}
    SAT -->|ListenOverflows| L4["L4 Gate"]
    SAT -->|queue_depth涨 gate空闲| L3["L3 排队配错"]
    SAT -->|Innodb row lock| L8["L8 MySQL"]
    U -->|否| ERR["错误率·逻辑 bug"]
```

- 📖 **读图**：先看 Utilization 是否到 70%——到了再看 Saturation 类型。**queue_depth 涨但 Gate 空闲** 是经典误判：看起来像 L4，实则 L3 drain_rate 配低了，Gate 根本没收到请求。见 [02](../02-登录排队与选服/)。

> 💡 **打个比方**：瓶颈定责像水管找堵点——不是哪里水多就是堵，要看哪里 **该来水没来**（L3 没发给 L4）和哪里 **水溢出来了**（L4 fd 满）。

#### ❓ 为什么「Gate 空闲 + queue 涨」不是加 Gate？

- 🚪 **Gate 空闲** = L4 没吃饱，加副本没用
- 🎟️ **queue_depth 涨** = L3 排队没按 R 放行（drain_rate 配低 / 发票卡住）
- ✅ **先修 L3**，再看 L4 USE——否则全家桶加副本是假容量

### ✅ SLO 验收示例

```text
登录：hall_ok/login_total ≥ 99.5% @ 目标 QPS
Gate：ticket 校验失败 < 0.1%
玩法：P99 < 500ms @ 目标 API QPS
发奖：0 重复 · 主库 TPS 可持续 30min
```

> ⚠️ **别混**：**压测最大值** ≠ **投产目标**——投产 = 最大值 × 0.7。压到 10 万连接不挂，投产只能按 7 万算（三节公式，此处验收签字用）。

> 🔍 **现场**：报告是否可执行

```bash
grep -E 'drain_rate|gate_replicas|mysql_batch' /docs/loadtest/2026_anniversary_report.md
# drain_rate: 840/min   ← 有具体数字
# gate_replicas: 22     ← 有副本建议
# mysql_batch: 200      ← 有 batch 建议
# 若 grep 无结果 → 报告不可执行，拒签上线
```

本节对应练习题 Q7。

---

## 七、投产：弹性、预算与复盘 ⭐

> 🎯 **一句话**：压测结论 **投产** = 写进 **HPA max、Gate 副本基线、排队 R、MySQL batch**——活动前按表执行，不是重新猜。

```text
Gate：replicas_min/max · 连接告警阈值
排队：drain_rate per server_id
战斗：empty_room 预开公式 = 匹配QPS × P99等待 × 1.5
HPA：仅对 L1/L4/L6 无状态或空 room 层；战斗按 room 指标自定义
MySQL：发奖 worker 并发 · batch · sleep
```

- 💰 **成本**：日常按 70% C 买资源；活动临时扩容 +7d 保留 · 或云按量
- 🔄 **复盘**：未覆盖路径（新玩法/渠道）补进下轮；生产偏差 >15% → 补压或调 α

### 📦 报告最小交付清单 🔧

```text
① 口径文档 CCU/三峰目标
② 每场景 spike 曲线 + 瓶颈层 Lx
③ 饱和点 raw 值 + ×0.7 后 C
④ drain_rate / gate_replicas / mysql_batch 建议值
⑤ 已知缺口与活动前 action item
```

> 🔍 **现场**：HPA 与容量表一致吗 · 评审报告

```bash
kubectl get hpa gate-prod -o jsonpath='{.spec.maxReplicas}'
# 30   ← HPA max=30

grep 'gate_max' /ops/runbook/capacity_ledger.yaml
# gate_max: 30   ← 两处一致；不一致 → git blame

grep -cE '瓶颈层|drain_rate|alpha.*0\.7' /docs/loadtest/latest_report.md
# 4   ← 至少各出现 1 次；若 0 → 退回补写
```

本节对应练习题 Q8。

---

## 八、作战：压测事故与容量偏差定责 🔧

> 🎯 **一句话**：**打生产、口径错、只压 HTTP** 三类人为事故；**活动实际偏差** 先对表哪层 **D>C**。

### 决策树

```mermaid
flowchart TD
    START["容量/压测问题"] --> Q1{"是否写了生产?"}
    Q1 -->|是| P0["P0 停写 · 回档 · 09章"]
    Q1 -->|否| Q2{"活动爆 vs 报告"}
    Q2 -->|报告有余量| GAP["实际 D 或口径漂移"]
    Q2 -->|报告已预警| IGNORE["未执行 runbook"]
    GAP --> LAYER["对账本哪层 C<D"]
    IGNORE --> EXEC["补扩/降R/降级"]
```

- 📖 **读图**：第一步永远排除「写了生产」——那是 P0 数据安全，不是容量问题。排除后：报告有余量却挂 → 实际 D/口径漂移；报告已预警却没动作 → runbook 未执行。

| 现象 | 先做 | 别做 |
|------|------|------|
| 压测打生产主库 | 停脚本、查 binlog | 继续「测完再说」 |
| Login 压测漂亮活动 Gate 挂 | 查是否真走 Gate | 再加 Login |
| 报告无 drain_rate | 拒签上线 | 凭 CCU 估 R |
| soak 2h 内存涨 | 查连接/session 泄漏 | 直接上生产 |
| 瓶颈层判错 | 重看 spike + USE | 全家桶加副本 |

### 60 秒剧本 🔧

```text
0–10s  问：预发还是生产 · 写库还是只读 · 当前 D 层指标
10–30s 对 capacity_ledger：哪层 C<D · 最近是否改口径
30–45s 压测报告瓶颈层 vs 现网 Prometheus 同层 USE
45–60s 出动作：扩/降R/降级/停错误压测；生产写事故升级
```

> 🔍 **现场**：活动容量偏差 60 秒排查

```bash
# 0-10s: 看当前 Gate 连接
ss -ant state established '( sport = :4103 )' | wc -l
# 285000   ← 接近 C=280000，headroom 几乎为零

# 10-30s: 对账本
yq '.layers[] | select(.D > .C) | .name + " OVER"' /ops/runbook/capacity_ledger.yaml
# gate_connections OVER   ← 这层先挂

# 30-45s: 看报告瓶颈层 vs 现网
curl -s 'http://prom:9090/api/v1/query?query=sum(rate(login_errors[5m]))' | jq '.data.result[0].value[1]'
# "0.012"   ← 错误率 1.2%，超 abort 线

# 45-60s: 止血
kubectl scale deploy gate-prod --replicas=30   # ← 扩瓶颈层
```

本节对应练习题 Q9、Q13。

---

## 九、收口

### 命令速查 🔧

```bash
# 压测中控
curl -sS 'http://loadtest:9090/api/v1/status' | jq .

# Gate 容量
ss -ant state established '( sport = :4103 )' | wc -l   # ← ESTAB 连接数
ss -s | awk '/TCP:/'                                     # ← TCP 总览

# MySQL 写容量
mysql -e "SHOW GLOBAL STATUS LIKE 'Threads_running';"   # ← 写压力
mysql -e "SHOW GLOBAL STATUS LIKE 'Com_commit';"         # ← 提交频率

# 排队 R 验证
redis-cli HGETALL queue:config:SERVER_ID                 # ← drain_rate 配置

# 容量表
yq '.layers[] | [.name,.C,.D,.alpha] | @tsv' /ops/runbook/capacity_ledger.yaml

# 误连生产检查
grep -r 'prod' /opt/loadtest/config/                     # ← 应无输出
```

### 面试锚点

遮住右列自问自答，卡壳的行回去重读对应小节。

| 问 | 30 秒答 |
|----|---------|
| 容量模型是什么？ | 分层账本 L0-L8，每层 C/D，min 瓶颈×0.7 定 R |
| 全链路压测要点？ | 真协议·Gate·room·主库写；分登录/玩法/发奖场景 |
| 为何不用 HTTP 压测代表全链路？ | 测不出 fd、room、行锁、热 key |
| 安全系数为何 0.7？ | 留 headroom 给抖动、发布、弱网重连 |
| 压测交付物？ | 瓶颈层+drain_rate+副本+batch，不是一句 QPS |
| 影子库作用？ | 量级真实·不写生产 |
| soak 测什么？ | 泄漏、长时间 degradation |
| 活动前怎么用报告？ | 对 capacity_ledger，D 不得大于 C |

### 易错对照

- 错：CCU 目标直接除 Pod → 对：分层 min 瓶颈
- 错：只压 Login HTTP → 对：Gate 长连接+大厅+战斗
- 错：压测最大值当投产 → 对：×0.7
- 错：压测写生产 → 对：影子库
- 错：从库参与写压测 → 对：写只打主库
- 错：无 soak 直接 spike → 对：先浸泡找泄漏
- 错：报告不写 drain_rate → 对：必须可执行
- 错：requests/limits 当容量 C → 对：C 来自压测饱和点
- 错：queue 涨就加 Gate → 对：先看 Gate 是否空闲（可能是 L3）

### 数字墙

> α **0.7**（常用）· **0.6** 更保守 · **0.8** 需强监控 · soak ≥**2h** @ **70%** 目标 · spike ramp **10%→50%→100%** 每档 ≥**10min** · 影子库 ≥生产 **30%** 行数 · Gate 看 ESTAB + SYN-RECV + 内存 · MySQL 写饱和 Threads_running **70～80%** · 登录压测必须含排队+ticket · bot 心跳与线上一致 **30～90s** · abort：错误率>**1%** 或 Threads>**200** 或资损 · 生产超报告 **15%** 触发补压

### capacity_ledger.yaml 最小示例 🔧

```yaml
layers:
  - name: gate_connections
    C: 280000
    D: 300000
    alpha: 0.7
    source: loadtest_2026_q1_spike_login
  - name: login_drain_rate_s1001
    C: 1200
    D: 1000
    unit: per_minute
    source: loadtest_2026_q1_spike_login
  - name: mysql_write_tps
    C: 5600
    D: 5000
    alpha: 0.7
    source: loadtest_2026_q1_spike_settle
```

```bash
yq '.layers[] | select(.D > .C) | .name + " OVER"' /ops/runbook/capacity_ledger.yaml
# gate_connections OVER   ← 活动前必须处理
```

### USE/RED 映射到分层

| 层 | Utilization | Saturation | Errors |
|----|-------------|------------|--------|
| L4 Gate | fd/CPU | SYN-RECV | ticket 失败 |
| L3 排队 | drain 使用率 | queue_depth | 出队超时 |
| L8 MySQL | Threads_running | lock wait | 死锁 |

---

## 合上书

对着第一节主图口述一遍，卡住就回对应小节。开头那两个人——只压 Login、打生产主库——各自错在哪，现在该能说清了。

- [ ] **默画**一次压测的一生：立项→建模→构造→执行→验收→投产，标出 0.7 在哪段乘（Q1）。
- [ ] 口述 **分层账本 L0-L8** 和 **min 瓶颈 ×0.7** 怎么得到 **drain_rate**；说清为何不能 CCU÷Pod（Q2、Q10、Q12）。
- [ ] 说清 **真协议压测** 与 **HTTP/录制回放** 漏测什么（fd、room、行锁、热 key）（Q3、Q4）。
- [ ] 背 **三场景 spike**：登录、玩法、发奖各盯哪层 USE；**soak 先于 spike**（Q5、Q6、Q14）。
- [ ] **影子库** 三条纪律（量级、脱敏、不写生产）（Q7）。
- [ ] 压测 **交付物清单**——报告里必须有的五个字段（Q8）。
- [ ] 「活动 Gate 挂了但 Login 压测过」**定责**路径；**queue 涨但 Gate 空闲** 先查哪层（Q9、Q13）。
- [ ] **60 秒**对 capacity_ledger 判断 C 与 D 的剧本（Q9、Q13）。
- [ ] 口述 **投产** 如何把报告写进 HPA max 和排队 R（Q8）。
- [ ] **口径会议** 四条必问（CCU vs DAU、是否同时结算、机器人、分服加总）（Q10）。

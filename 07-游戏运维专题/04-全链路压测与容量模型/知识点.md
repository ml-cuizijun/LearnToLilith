...
> 这一章讲完，你会知道当时那两个人各自错在哪——只压 Login 不算全链路，对着生产主库压是事故，以及容量模型怎么建才不翻车。
> **本章回答的问题**：从零建立 **容量模型** 怎么做、**全链路压测** 怎么执行与验收、瓶颈怎么定责、结果怎么写进活动/开服 runbook。
> **本章不讲**：登录六跳与 ticket 细节 → [02](../02-登录排队与选服/)；活动三峰降级 → [03](../03-活动高峰与压测/)；K8s HPA/PDB 语法 → [03-05 HPA](../../03-Kubernetes/05-工作负载控制器/)；MySQL 索引调优 → [02-01 MySQL](../../02-中间件与数据库/01-MySQL/)；通用 RED/USE → [21-性能方法论](../../01-基础底座/21-性能方法论-USE与RED/)。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：先把第一节「一次压测的一生」看熟——后面每一节都是这张图上的一段；第二到第七节顺着立项 → 建模 → 构造 → 执行 → 验收 → 投产往下走；第八节专门讲开头那两个事故怎么定责。每一节讲完会提示对应练习题，趁热打铁。
...
好，主图装进脑子了。接下来咱们从「立项」开始——容量目标与口径统一。
...
立项讲完。大家想想：50 万 CCU 是一个数还是一张分层账本？下一节建模。
...
大家想想：运营说「撑 50 万 CCU」，你回「好」还是回一张表？很多人直接回「好」——⭐ 这是高频错答。🔥 大白话先钉死：**容量模型 = 分层账本**（Gate fd、登录 QPS、创角锁、战斗房间、主库写），不是一个 CCU 口号。
...
记住口诀：**CCU 是结果，分层账本才是模型**。下一节构造流量机器人与数据工厂。
...
大家想想：只压 Login HTTP，报告写「Gate 还能扛 3 倍」——靠谱吗？很多人会答「Login 过了就行」——错了。全链路必须覆盖到 Gate fd、创角、战斗、发奖写路径；对着生产主库跑发奖脚本，那是事故不是压测。
...
投产讲完，咱们回到开头——只压 Login、打生产主库，怎么定责。
...
现在再看群里那句「Gate 还能再扛 3 倍」，你知道该怎么拦住他了。收口把命令、面试口径和数字墙收齐。
...
- [ ] 口述 **分层账本 L0-L8** 和 **min 瓶颈 ×0.7** 怎么得到 **drain_rate**（Q10）。
- [ ] 说清 **真协议压测** 与 **HTTP 压 Login** 漏测什么（fd、room、行锁、热 key）（Q2、Q12）。
- [ ] 背 **三场景 spike**：登录、玩法、发奖各盯哪层 USE（Q3、Q4）。
- [ ] 解释 **soak 2h** 找什么（泄漏），**spike** 找什么（饱和点）（Q5、Q6、Q14）。
- [ ] **影子库** 三条纪律（量级、脱敏、不写生产）（Q7）。
- [ ] 压测 **交付物清单**——报告里必须有的五个字段（Q8）。
- [ ] 「活动 Gate 挂了但 Login 压测过」**定责**路径（Q9、Q13）。
- [ ] **60 秒**对 capacity_ledger 判断 C 与 D 的剧本（Q9、Q13）。
- [ ] 口述 **投产** 如何把报告写进 HPA max 和排队 R（Q9、Q13）。
- [ ] 解释 **USE** 在 Gate 层看哪两个 S（Saturation 队列、Utilization fd/CPU）（Q9、Q13）。
- [ ] **口径会议** 四条必问（CCU vs DAU、是否同时结算、机器人、分服加总）（Q9、Q13）。

全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「50 万 CCU 怎么拆成分层账本、为什么只压 Login 的报告不能信」。讲得下来，你就是下一个能拦住「对着生产主库压」的人。

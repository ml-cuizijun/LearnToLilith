...
> **不讲**：登录排队与 ticket → [02](../02-登录排队与选服/)；活动三峰与压测 → [03](../03-活动高峰与压测/)；全链路容量模型 → [04](../04-全链路压测与容量模型/)；开服合服 → [06](../06-开服合服/)；TCP 握手/长连接队列 → [12-网络栈与Socket](../../01-基础底座/12-网络栈与Socket/)、[13-TCP与UDP](../../01-基础底座/13-TCP与UDP/)。
...
- [ ] **默画**主图：客户端 → 目录 → Gate → 大厅/战斗 → Redis/MySQL，标出长连接钉在哪。
- [ ] 口述 **CCU、Gate 连接数、登录 QPS** 三个数为什么不能混用一个扩容策略。
- [ ] 说清 **有状态战斗** 为什么不能用 Web 的 `maxUnavailable: 25%` 滚动。
- [ ] 背 **战斗发布摘流四步**，并说出跳过某一步的事故症状（全员判负）。
- [ ] 解释 **MySQL 权威、Redis 热状态**——道具纠纷先查谁。
- [ ] 分服 / 跨服 / 全局层 各挂一个时的**影响面**和**第一刀**。
- [ ] 「能进大厅进不了房间」沿主图**定责路径**（匹配 → 空 room → 战斗进程）。
- [ ] 「全服进不去」**不要做什么**（不要先重启战斗集群）。
- [ ] 说出 **Gate 像前台、战斗像棋局** 的扩容比喻，并对应到 HPA 边界。
- [ ] 口述 **目录不可 500、爆满只拦新号** 两条运维铁律。
- [ ] **默画** K8s 三层 Deployment 边界：Gate 可滚、战斗摘流。
- [ ] 说清 **TCP ESTABLISHED 多但 CCU 低** 的两种原因（未进大厅、心跳/session）。

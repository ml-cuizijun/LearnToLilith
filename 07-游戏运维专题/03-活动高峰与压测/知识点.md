...
> 周年庆 20:00 整点——推送、活动开启、排行结算叠在同一分钟。HPA 把战斗副本加了一倍，新 Pod 全是空房间；主库已被全服发奖 UPDATE 锁死，邮件队列积压 400 万。有人喊「再扩战斗」，有人喊「加从库」——两刀都打错层。
...
> 🎯 **一句话**：活动不是「CCU 高一点」，而是 **酝酿 → 登录峰 → 玩法峰 → 发奖峰 → 散场**——每段瓶颈在不同层，扩容和降级手段完全不同。
...
- 📖 **读图**
  - 🛤️ 横轴是时间：20:00 登录 → 20:05 玩法 → 20:30 发奖——不要用一条 HPA 规则盖全程
  - 🏗️ 纵轴是层：登录打 Gate/排队；玩法打战斗/跨服；发奖打 **主库写**——从库、缓存都救不了写尖刺
  - 值班先问 **哪座峰、哪一层**——CCU 高但发奖慢是 DB，不是战斗 Pod 数

好，主图装进脑子了。从「酝酿」开始——预热、开关与时间窗。
...
> 🎯 **一句话**：**登录峰** = 短时间 **登录 QPS / 重连** 相对 CCU 的尖刺——打 **账号、排队、Gate**，放大常见 **日常 10×～30×**（整点+推送叠加更高）。
...
- 📖 **读图**：登录峰 **不等于 CCU 线性涨**——大量是 **短连重连** 或 **排队等待**；监控看 `login_qps`、`queue_depth`、`gate_new_conn`，不是只看 CCU。
...
3. **重连快通道**——已有有效 session 跳过账号验证直进大厅，避免假排队挤占新登录
4. **目录推荐打散**——勿推单一新服
5. **创角限流**——新服创角 QPS 上限，防 DB 写瓶颈

#### ❓ 为什么登录峰扩战斗零作用？

- 🚪 登录打的是 **账号 / 排队 / Gate 连接**——战斗空 room **不接** 登录 HTTP
- 🏨 HPA 加战斗 = 酒店多开一层空房；进站闸机堵了，加客房没用
- ✅ 第一刀永远是 **R + Gate + 重连分类**，不是 `kubectl scale battle`
...
登录峰讲完。HPA 把战斗副本加一倍，已开的房间会搬过去吗？下一节玩法峰。
...
🔥 大白话先钉死：⭐ **新 Pod 全是空房间**；已开房间钉在旧进程里，HPA 救不了玩法峰的「房间满」。
...
- 📌 **房间钉进程**：匹配成功后 room 钉在某个战斗进程；K8s 调度的是 **新 Pod**，不会把已 Fighting 的局搬过去
- 🆕 HPA 只增加 **可承接新匹配** 的空 room 容量
- 🔥 若 `empty_rooms=0` 且旧 Pod CPU 99%——瓶颈是 **room 内逻辑 / 匹配堆**，不是副本数不够
- ✅ 对策：活动前 **预开空 room**；峰中先查热 key / 跨服 RPC，别盲目顶 maxReplicas
...
记住口诀：**HPA 加的是空房间，救不了已开局**。下一节发奖峰——写尖刺才是主库杀手。
...
🔥 大白话：⭐ **发奖峰打的是写路径，不是 CCU**——全服 UPDATE、邮件队列、排行榜结算，主库锁和队列积压才是刀口。别用登录峰的扩容剧本套发奖峰。
...
- ✍️ 结算写路径只打 **主库**——读写分离只管读副本，**写仍进主库**
- 📚 加从库 = 多加复印机，**账本还是一本**
- ❌ HPA 战斗更帮不上——战斗 Pod 不分担 MySQL 行锁
- ✅ 正解：**分片 worker + 控 batch + 幂等 + 邮件异步**；锁死先 **降 batch / 停加 worker**
...
> ⚠️ **别混**：**重复发奖** 不是「玩家多领一次道具」——涉及 **充值法币、稀有掉落、排行公平**，回滚成本 >> 慢 10 分钟。
...
- 📖 **读图**（分组背）
  - **登录峰**：保护连接与进服——排队、Gate、创角限流
  - **玩法峰**：保护算力与读——空 room、跨服池、Redis 拆 key
  - **发奖峰**：保护账本——主库写速率、幂等、异步邮件
...
- 🧯 峰中 **先止血**：关特效 / 限流 API / 停加结算 worker——立刻减伤
- 📈 扩容有 **冷启动**（拉镜像、预热、空 room 就绪），来不及救当前分钟的主库锁
- 🎯 扩错层更糟：发奖锁死时 HPA 战斗 = 浪费副本配额，主库更没空
- ✅ 顺序：**定峰 → 降级止血 → 对账 → 再扩该层**（登录扩 Gate，玩法预开 room，发奖控 batch）
...
- 🏭 **数据工厂**：压测账号需 **真协议 handshake**——不是 HTTP 压测替代品。虚拟角色要有背包/等级/帮派等业务上下文，否则压不到真实 SQL 路径。
- 🪞 **影子库**：压测流量打影子 MySQL，与生产主库物理隔离。
...
- 📖 **读图**：三条主分支对应三峰；**奖励错/重复** 走发奖，**禁止**先加战斗副本。
...
现在再看群里那句「HPA 加战斗」和「再扩主库」，你知道该怎么拦住——先分清是哪一峰。
...
遮住右列自问自答，卡壳的行回去重读对应章节——能讲出来才算会，看着答案点头不算。
...
- [ ] **默画**活动生命周期主图：酝酿→登录峰→玩法峰→发奖峰→散场，标三峰各打哪层（Q1、Q14）。
- [ ] 口述 **为什么 CCU 不能买三峰容量**——各举一条瓶颈（Q9、Q11）。
- [ ] 说清 **HPA 战斗** 只能增加什么、不能救什么（Q2、Q3）。
- [ ] 背 **发奖幂等三件套** 和 **分批写主库** 理由（Q4、Q10）。
- [ ] 解释 **从库为什么不救发奖写尖刺**（Q5、Q6、Q7、Q12）。
- [ ] 「整点登录排队爆炸」**第一刀**（R、Gate、重连）——**不要**扩战斗（Q8）。
- [ ] 「活动 API 502 但 CCU 正常」查 **热 key / 空 room / 跨服** 路径（Q11、Q12）。
- [ ] 「主库 Threads_running 200+」**止血**动作（降 batch、停 worker）——**不要**加从库（Q13）。
- [ ] 口述 **降级顺序** 和 **不可降级的 P0**（支付、核心结算幂等）（Q13）。
- [ ] **默画** 世界 BOSS 伤害分桶聚合路径，说清为何不能单 key INCR（Q13）。
- [ ] 口述 **配置热更 ack** 不一致的症状与第一刀（Q13）。
- [ ] 口述 **活动分钟表** T+0/T+5/T+30 各盯哪条曲线（Q13）。

压测与容量章 [04](../04-全链路压测与容量模型/) 负责把活动系数写进分层账本；本章只讲活动当晚三峰怎么拆、怎么降。

全部打勾之后，给自己出一个终极测试：找一位同事，十分钟讲清「周年庆三峰叠在一起——为什么 HPA 加战斗救不了，先降级再扩怎么做」。讲得下来，你就是下一个能拦住「用一个 CCU 买全部容量」的人。

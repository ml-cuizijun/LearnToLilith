...
> 这一章讲完，你会知道当时「HPA 加战斗」错在哪——三峰各打哪层，以及为什么先降级再扩。
> **本章回答的问题**：一场活动从预热到散场，**登录峰 / 玩法峰 / 发奖峰** 各打哪层、各怎么扩、各怎么降，以及为什么 HPA 救不了已开房间和主库写尖刺。
> **本章不讲**：登录六跳与 ticket → [02](../02-登录排队与选服/)；容量模型与压测执行 → [04](../04-全链路压测与容量模型/)；配置热更开关 → [05](../05-热更新与版本发布/)；CDN 资源 → [08](../08-CDN与资源更新/)；回档 → [09](../09-玩家数据备份与回档/)；SLO 体系 → [13](../13-游戏SRE稳定性/)、[09-SRE](../../09-可观测性与SRE方法论/)。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：先把第一节「活动高峰的一生」看熟——后面每一节都是这张图上的一段；第二到第七节顺着酝酿 → 登录峰 → 玩法峰 → 发奖峰 → 散场 → 压测往下走；第八节专门讲开头那个周年庆现场怎么定责。每一节讲完会提示对应练习题，趁热打铁。
...
好，主图装进脑子了。接下来咱们从「酝酿」开始——预热、开关与时间窗。
...
登录峰讲完。大家想想：HPA 把战斗副本加一倍，已开的房间会搬过去吗？下一节讲玩法峰。
...
大家想想：CCU 涨了，HPA 把战斗副本加一倍——已开的公会战房间会自动均衡过去吗？很多人会答「会，K8s 会调度」——⭐ 这是高频错答。🔥 大白话先钉死：**新 Pod 全是空房间**；已开房间钉在旧进程里，HPA 救不了玩法峰的「房间满」。
...
现在再看群里那句「HPA 加战斗」和「再扩主库」，你知道该怎么拦住他了——先分清是哪一峰。收口把命令、面试口径和数字墙收齐。

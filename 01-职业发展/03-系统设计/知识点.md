...
白板题：「设计一个每天能开 N 个新区的开服系统。」45 分钟。不问约束就画「接入-逻辑-DB」、有状态战斗服按 Web 乱扩、失败没有回滚——按中级评。
...
**一句话：** 数据流先于盒子；有状态逻辑服不能套 Web 弹性；目录写入是开服最后一步。
...
容量公式（允许对方改数字）：目标 CCU × 单玩家连接 / 单网关连接上限 = 网关台数。逻辑服按人数上限算，不按 CPU 利用率。压测两条曲线分开打：登录脉冲 vs 平稳 CCU，打到 **1.2×** 找第一瓶颈。

错误预算：烧完停 **非关键变更 / 功能发布**，不停 hotfix 和回滚。金丝雀门禁看金丝雀自己的错误率相对发布前 5 分钟 baseline，不用集群均值。

> **记住**：有状态逻辑服不能按 HPA 乱扩；多活看数据层，不是两个 AZ 框。
...
游戏 SLI 不要只套「HTTP 99.9%」：登录成功率、进战斗 / 核心协议成功率、掉线率、支付成功率、CCU 异常（10 分钟腰斩）。

...
**覆盖**：核心 · 必会 · 常考
**对应**：知识点 [1. 基本概念](知识点.md)、[3.1 超时链：谁先放弃](知识点.md)
...
**覆盖**：核心 · 必会 · 常考
**对应**：知识点 [3.2 状态码：499 / 502 / 504 / 503](知识点.md)
...
**覆盖**：必会 · 常用 · 常考
**对应**：知识点 [5. 核心配置](知识点.md)、[2. 架构图](知识点.md)
...
**覆盖**：必会 · 常用 · 常考
**对应**：知识点 [3.3 鉴权：网关验票，后端验权](知识点.md)
...
**覆盖**：必会 · 常用 · 常考
**对应**：知识点 [3.4 限流：漏桶、令牌桶、按谁限](知识点.md)
...
**覆盖**：必会 · 常用 · 常考
**对应**：知识点 [3.5 熔断：失败才停送，不是超时本身](知识点.md)、[3.6 灰度：权重 / uid hash / header](知识点.md)
...
**覆盖**：核心 · 必会 · 常考
**对应**：知识点 [3. 原理](知识点.md)、[4. 落地](知识点.md)
...
**★★★★☆｜全局限流 1 万 QPS 和按账号限流怎么配？**

**覆盖**：必会 · 常用 ｜ **对应**：知识点 [3. 原理](知识点.md)

### 场景

活动 API 被脚本刷，`limit_req zone=global` 10k/s 仍把正常玩家挡外面。

### 完整解答

> 🎯 **一句话**：**全局兜底 + 按 key 细限**——`$binary_remote_addr` 或 `$arg_account_id`；登录/发奖/查询分桶，**不要**一个桶挡全服。

```nginx
limit_req_zone $account_id zone=per_account:10m rate=10r/s;
limit_req zone=per_account burst=20 nodelay;
limit_req zone=global burst=5000;
```

```text
脚本刷：单账号低频但账号多 → 设备指纹 / 验证码 / WAF
正常玩家：per_account 够，global 只防总量 DDoS
```

> ⚠️ **别混**：503 是限流，不是 502——**不要**当后端挂了重启。

### 再追问

**NAT 后全服同 IP？**——用账号/Token 限流，别只限 IP。
...
支付下游偶发超时，Envoy `outlier_detection` 把整组后端 eject，全服支付 503。

### 完整解答

> 🎯 **一句话**：熔断看 **连续失败率 + 最小主机数**——阈值太松误杀，太紧不保护；要有 **half-open 探测**。
...
outlier_detection:
  ...
  max_ejection_percent: 30   # ← 不能 eject 100%
```

```text
调参：提高 consecutive 阈值 / 缩短 ejection 时间
架构：支付独立集群，不要和查询共熔断域
止血：临时关 outlier，先修下游 P99
```
...
**和超时什么关系？**——超时触发失败计数；超时链仍要 60→50→40 对齐。
...
大厅 HTTP 灰度 10% 新版本 OK。战斗也想 `weight=10` 切 10% 房间到新逻辑服。

### 完整解答

> 🎯 **一句话**：**HTTP 无状态可权重**；**战斗 room 有状态**——灰度按 **房间/区服整池切**，不要中途按连接权重拆房间。

```text
大厅：LB 权重 / Header 路由 / 金丝雀 Ingress
战斗：新服池 s2001 只开新玩家；老 room 不迁移
      或 matchmaking 规则指定版本池
```

> ⚠️ **别混**：mid-fight 切后端 = 全员掉线。灰度单位是 **池/区服**，不是 **单连接 %**。

### 再追问

**APISIX `traffic-split` 能用于 TCP 吗？**——看插件；自定义 TCP 战斗一般四层 + 独立入口。

---

## Q14

**★★★★★｜499/502/504/503 同时涨，60 秒怎么分？**

**覆盖**：核心 · 必会 · 🔧 ｜ **对应**：知识点 [7. 生产排障](知识点.md)

### 场景

活动整点网关告警四种状态码齐涨。有人要重启全部逻辑服。

### 完整解答

> 🎯 **一句话**：**先 error.log 关键字分类**——`refused→502`、`timed out→504`、`499→客户端更短 deadline`、`limit_req→503`；**不要**重启逻辑服。

```text
0–10s   access/error 按 status 聚合
10–30s  499 涨：查 SDK timeout 是否 < 网关 read_timeout
30–45s  502：curl 直连 upstream；504：后端 P99
45–60s  503：限流桶；止血限流/扩容网关，不杀后端
```

```bash
tail -f /var/log/nginx/error.log | grep -E 'refused|timed out|limiting'
```

> ⚠️ **别混**：活动期四种码同时有 → 先 **停滚动发布 + 限流**，再分类。重启网关会再踢长连接。

### 再追问

**etcd 抖动能重启 APISIX 吗？**——已有路由仍转发；先修 etcd  quorum，见 Q8。

...
开始做题前，先给大家一个小建议：合上知识点，对着 **本章主线**（分层 → 归类 → 留证）和第二节架构图，把「副本集 vs 分片、刚写立刻读走哪」口述一遍。能顺下来，下面这些题你会发现大半都会了；卡壳的地方，就是你该回去重读的那一节。
...
题目的顺序也是有意排的：Q1、Q2 钉选型与架构；Q3～Q8 顺着写读、shard key、文档、TTL、索引、WT；Q9～Q14 备份、分流、选举、CDC、balancer 与 60 秒定责。
...
白板第一题：副本集和分片差在哪——别把 Secondary 当分片。
...
开篇「只还 Mongo」那位——回档要对齐 Redis/MySQL。
...
oplog / change stream——CDC 边界别和 Kafka 混。
...
- [ ] 能说出 Mongo vs MySQL 怎么选，应用连 URI / mongos（Q1）
- [ ] 能画出副本集 vs 分片，每片仍是副本集（Q2）
- [ ] 能讲 w:1 / majority、读 secondary 的坑、Arbiter 不算数据副本（Q3、Q11）
- [ ] 能选 shard key，说明单调键和难改（Q4、Q13）
- [ ] 能拆玩家文档：邮件独立 + TTL；数组不推向 16MB（Q5、Q6、Q7）
- [ ] 能先看 COLLSCAN 和 WT cache；备份对齐 Redis/MySQL；升级回滚靠快照——现在再看开篇那三刀，你知道该怎么拦住他们了（Q8、Q9、Q14）

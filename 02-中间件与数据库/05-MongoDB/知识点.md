...
> 这一章讲完，你会知道当时那三个人各自错在哪，以及刚写立刻读、TTL、回档该怎么下手。
> **本章回答的问题**：一份玩家文档从写入到读出、从副本集到分片，中间会在哪卡住；读不到、慢、16MB、切主失败时怎么一眼定责。
> **本章不讲**：强事务、JOIN、Explain 方法论 → [01 MySQL](../01-MySQL/)；合服冲突、映射、窗口 → [07-06](../../07-游戏运维专题/06-开服合服/)；备份 RPO、误操作回档流程 → [07-09](../../07-游戏运维专题/09-玩家数据备份与回档/)；全文检索、倒排 → [04 Elasticsearch](../04-Elasticsearch/)；消息积压、发奖流水 → [03 Kafka](../03-Kafka/)。这些都有专门章节，本章只在用到时点到为止。
...
**怎么听这场分享**：咱们先把「分层 → 归类 → 留证」装进脑子；概念钉 Mongo vs MySQL，架构钉副本集 vs 分片；原理五块放慢讲——开篇读 secondary 那位同学，根因就在 3.1。第七节专门定责。每一节讲完会提示对应练习题，学完就去练。
...
咱们先不急着背 `rs.status()`。学 Mongo 最怕的就是「有事务了，账本也搬过来」——错了。先把边界钉死。
...
⭐ 口诀：**「Mongo 管灵活档，MySQL 管账；连 URI / mongos，别写死 IP」**。

**面试怎么说**：「Mongo 管灵活文档，MySQL 管账；副本集是 HA，分片才是水平扩写；刚写立刻读走 primary。」

好，边界清了。接下来钉死最容易混的一对：副本集 vs 分片。
...
> 🎯 **一句话**：副本集解决「主挂了谁顶」；分片解决「一张集合写不下」——每片仍是副本集。
...
好，架构有了。开篇那位把读偏好改成 secondary——咱们放慢把因果推清楚。
...
大家想想：写成功了，为什么马上读从库可能没有？很多人答「Mongo 丢了」——**错了**，多半是读偏好选错。

🔥 **写关注（write concern）** = 写要等到多少节点确认才回成功；**读偏好（read preference）** = 读打 Primary 还是 Secondary。大白话：**收银台回了「收到」≠ 后厨已经贴单**。
...
⭐ 口诀：**「刚写立刻读 → primary；核心写 → majority」**——别用 secondary 扛读来「分担压力」而不管一致性。
...
⭐ 口诀：**「shard key 定错难改；单调键打热点，哈希键毁范围查」**。
...
⭐ 邮件独立文档 + TTL；背包数组别无限涨撞 16MB。

> 本节对应练习题 **Q5、Q6、Q7**。

### 3.4 WiredTiger 缓存 🔥

开篇有人要加 CPU——先问 WT cache 打满了没有。
...
⭐ 口诀：**「cache 打满先看命中，别先加 CPU」**。

> 本节对应练习题 **Q8**。

### 3.5 索引 / TTL / 16MB ⭐

开篇那位要把 TTL 打到 `players`——红线就在这儿。
...
好，原理走完。落地、配置、命令之后，回到开篇三刀用决策树拦住。
...
> 🎯 **一句话**：三数据节点副本集；分片才上 mongos；URI 带 replicaSet，别写死 IP。
...
> 🎯 **一句话**：只动有证据的项；读偏好、w、TTL、shard key 是最容易改错的四处。
...
> 🎯 **一句话**：值班三刀——`rs.status()`、`explain`、profiler；切主后查 URI 是否写死旧主。
...
好，命令会敲了。回到开篇——读 secondary、乱打 TTL、只还 Mongo——咱们用决策树拦住他们。
...
现在再看开篇那三刀——「读 secondary 分担」「TTL 打到 players」「只还 Mongo」——你知道该怎么拦住他们了。
...
- [ ] 什么数据进 Mongo、什么留 MySQL？应用连谁、为什么不能写死 IP？（对应练习题 Q1）
- [ ] 副本集和分片各解决什么？每片为什么还是副本集？（Q2）
- [ ] w:1 / majority、读 secondary 各会看到什么？Arbiter 的坑？（Q3、Q11）
- [ ] shard key 三条原则？单调键和哈希键各坑什么？explain 里 `shards` 几个 key 算正常？（Q4、Q13）
- [ ] 玩家文档哪些嵌、哪些拆？TTL 和 16MB 怎么踩？（Q5、Q6、Q7）
- [ ] WT cache 打满什么指标？备份 restore 为什么要对齐 Redis/MySQL？——现在再看开篇那三刀，你知道该怎么拦住他们了。（Q8、Q9、Q14）

六题都能口述，这一章就过了。下一章咱们把 CAP、Raft、分布式事务这些「中间件背后的理论」串起来 → [06 分布式理论与一致性](../06-分布式理论与一致性/)。发奖流水回 [03 Kafka](../03-Kafka/)；合服映射去 [07-06](../../07-游戏运维专题/06-开服合服/)。

...
**本章不讲：** 强事务、JOIN、Explain 方法论 → [01 MySQL](../01-MySQL/)；合服冲突、映射、窗口 → [07-02](../../07-游戏运维专题/02-开服合服/)；备份 RPO、误操作回档流程 → [07-07](../../07-游戏运维专题/07-玩家数据备份与回档/)；全文检索、倒排 → [09 Elasticsearch](../09-Elasticsearch/)；消息积压、发奖流水 → [05 Kafka](../05-Kafka/)。
...
`rs.status()` 正常：`PRIMARY` 1 个，`SECONDARY` optime 与主差 <1s（视 SLA）。异常：全 `SECONDARY` = 无主要查多数派；`RECOVERING` = 全量同步中。
...
4. shard key 三条原则？单调键和哈希键各坑什么？explain 里 `shards` 几个 key 算正常？
5. 玩家文档哪些嵌、哪些拆？TTL 和 16MB 怎么踩？
6. WT cache 打满什么指标？备份 restore 为什么要对齐 Redis/MySQL？

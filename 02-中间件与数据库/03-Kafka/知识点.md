...
**本章不讲：** 落库唯一键、大事务拖垮消费 → [01 MySQL](../01-MySQL/)；热 key / 缓存被消费打爆 → [02 Redis](../02-Redis/)、[02-04 缓存架构](../02-Redis/04-缓存架构/)；活动流量模型、限流降级 → [07-03](../../07-游戏运维专题/03-活动高峰与压测/)；ZK 运维、KRaft 迁移方案 → 项目文档，本章只要求知道新集群用 KRaft；日志检索、埋点聚合 → [04 ES](../04-Elasticsearch/)。RabbitMQ / RocketMQ 不单开章，对照在 3.7 末。
...
- [ ] 并行度为什么等于分区数？组间为什么不抢 offset？
- [ ] acks=all 等的是谁？LEO / HW / consumer offset 各是什么？
- [ ] ISR 收缩什么时候拒写？为什么不能降 min.isr？滚动 Broker 等什么？
- [ ] 同 key 如何保序？扩分区为什么要评估？
- [ ] Lag 五步怎么查？平均 Lag 为什么骗人？发奖 offset 何时提交？
- [ ] 「丢消息」四层怎么查？reset 三条红线？活动整点先保什么？

...
> **本章回答的问题**：一条发奖消息从 Producer 进分区、过 ISR、落到消费位点，中间每一站谁在干活；Lag 涨了、写失败了、说「丢了」时，怎么一眼定责。
> **本章不讲**：落库唯一键、大事务拖垮消费 → [01 MySQL](../01-MySQL/)；热 key / 缓存被消费打爆 → [02 Redis](../02-Redis/)、[02-04 缓存架构](../02-Redis/04-缓存架构/)；活动流量模型、限流降级 → [07-03](../../07-游戏运维专题/03-活动高峰与压测/)；ZK 运维、KRaft 迁移方案 → 项目文档，本章只要求知道新集群用 KRaft；日志检索、埋点聚合 → [04 ES](../04-Elasticsearch/)。RabbitMQ / RocketMQ 不单开章，对照在 3.7 末。这些都有专门章节，本章只在用到时点到为止。
...
**怎么听这场分享**：咱们先把「分层 → 归类 → 留证」这条主线装进脑子，再顺着概念、架构、原理一站一站往下走；第七节专门讲定责——也就是开头那个现场的正确解法。每一节讲完会提示对应练习题，学完就去练。
...
值班里什么时候碰到 Kafka？发奖写不进去、Lag 告警、客服说「奖没到」、滚 Broker 前对 ISR——都是这一章的事。咱们先别急着背命令，把三个角色分清。

大家想想：很多人口中的 Kafka「像一个队列，谁抢到谁消费」——**错了**。Kafka 不是「一个队列里抢消息」，而是 **按 Topic 分类的追加日志**。Partition 是真正的并行单位：分区内有序、跨分区无序。Consumer Group 决定「这份日志被谁分摊」：组内一人一块分区，组与组互不影响。
...
⭐ 口诀记一句：**「并行看分区，可靠看 ISR，积压看分区 Lag」**——后面所有排障都从这三句展开。

好，概念钉住了。接下来咱们把整张架构图看熟——后面 ISR、Lag、滚 Broker 都对着它说话。
...
> 🎯 **一句话**：写打 Leader，Follower 进 ISR 后 HW 才前进；组内一分区一消费者，组间各拿一份。

后面 ISR、Lag、滚 Broker 都对着这张图说话。咱们先不急着背组件名，对着图问自己三个问题：
...
3. **没有箭头从业务直接改 Broker 上的日志文件**——`--reset-offsets` 改的是组位点，必须 dry-run + owner 签字。⭐ 这是值班红线，练习题 Q10 会考。

每个分区的副本关系（ISR / HW / LEO 在 3.1 展开）。大家想想：Leader 自己已经写到 100 了，消费者为什么还读不到最后几条？——答案就在这张图里：

> 本节对应练习题 **Q1、Q2**（架构与写入主线；细节在第三节）。
...
好，架构图有了。接下来放慢讲原理——这一节是全章最厚的一块，七个小节顺着「写 → 复制 → 顺序 → 消费 → 积压 → 重平衡 → 丢消息」往下推。
...
很多人会答「acks=all 就是等每台 Broker 都写完」——**错了**。咱们放慢，把三个位点拆开。

🔥 **ISR（In-Sync Replicas）** = 跟上 Leader 的那批副本；**LEO（Log End Offset）** = 该副本本地日志最后一条的下一条；**HW（High Watermark）** = ISR 里最小的 LEO——大白话：**「所有合格分仓都抄到的页码」**。Leader 自己可以多记两页（LEO 更大），对外发货只能发到 HW 那一页。
...
🔥 **幂等（idempotent）** 这个词第一次见：同一个操作执行一次和执行十次，最终结果一样。**幂等 Producer** 防的是网络重试导致 Broker **同一条写两遍**（PID + 序号去重）。它不防消费侧重复，也不跨 Topic、跨会话。口头仍是 **At least once + 消费幂等**；进 MySQL 靠 `order_id + event_type` 唯一键。

> ⭐ **口诀**：acks=all 等 ISR，不是等「全机房」；HW = ISR 最小 LEO，不是 Leader 的 LEO。
...
开篇那个「把 min.isr 改成 1」的同学——错就错在这儿。大家想想：拒写了，是 Kafka 坏了，还是 Kafka 在保你的数据？
...
⭐ 高频错答：**「拒写了先降 min.isr」**——正确是修 Follower，让 ISR 回来。
...
大家想想：offset 什么时候提交？很多人说「拉到就提交」——那就是在制造静默丢。
...
> ⭐ **口诀**：先落库再提交；毒消息进 DLQ，别 reset 往后跳。
...
开篇 Lag 飙到 80 万、有人要加 20 个 consumer——咱们用这一小节拦住他。
...
Lag 锯齿、日志里频繁 join/leave——很多人先怀疑 Kafka 坏了。大家想想：是不是刚发了一版、一次杀了太多 consumer？
...
开篇那句「Kafka 丢消息了」——多半是分层没查清楚。咱们把四层摊开。
...
> ⭐ **口诀**：丢消息四层——生产 / Broker / 消费 / 业务；`--to-latest` = 业务当丢，活动窗口禁用。

好，原理七块走完了。接下来落地：生产怎么选、配置怎么配、命令怎么敲、挂了怎么定责。
...
> 🎯 **一句话**：≥3 Broker、rf=3、KRaft、盘独立；验收看 ISR 齐，不看 TCP 通。
...
> 本节对应练习题 **Q10**（落地与变更红线交叉）。

---

## 5. 核心配置

> 🎯 **一句话**：只动有证据的项；降 min.isr、缩 retention、一次杀光 consumer 都是变更。
...
⭐ 开篇那位改 min.isr 的同学，就是踩了这张表最危险的一行。
...
> 🎯 **一句话**：值班两刀——`describe --topic` 看 ISR，`describe --group` 按分区看 Lag；reset 必须 dry-run。
...
好，命令会敲了。回到开篇那个现场——群里三刀齐下，咱们用决策树拦住他们。
...
> 🎯 **一句话**：先问还能不能生产、还能不能消费、位点还在不在——拒写 ≠ 立刻 reset。
...
现在再看开篇群里那三刀——「Kafka 丢了」「`--to-latest`」「降 min.isr」——你知道该怎么拦住他们了：先分层留证，修 ISR / 下游，reset 必须 dry-run。

> 本节对应练习题 **Q14**（以及前面 Q3、Q5、Q9 的综合）。
...
- [ ] 并行度为什么等于分区数？组间为什么不抢 offset？（对应练习题 Q1）
- [ ] acks=all 等的是谁？LEO / HW / consumer offset 各是什么？（Q2）
- [ ] ISR 收缩什么时候拒写？为什么不能降 min.isr？滚动 Broker 等什么？（Q3）
- [ ] 同 key 如何保序？扩分区为什么要评估？（Q7）
- [ ] Lag 五步怎么查？平均 Lag 为什么骗人？发奖 offset 何时提交？（Q4、Q5）
- [ ] 「丢消息」四层怎么查？reset 三条红线？活动整点先保什么？——现在再看开篇那三刀，你知道该怎么拦住他们了。（Q9、Q10、Q14）

六题都能口述，这一章就过了。落库唯一键回 01；下一章咱们讲日志怎么倒排、黄红怎么定责 → [04 Elasticsearch](../04-Elasticsearch/)。Mongo 的副本集与分片 → [05](../05-MongoDB/)。

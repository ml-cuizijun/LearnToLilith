...
开始做题前，先给大家一个小建议：合上知识点，对着 **本章主线**（分层 → 归类 → 留证）和第二节架构图，把一次检索从 Coordinating 到倒排口述一遍。能顺下来，下面这些题你会发现大半都会了；卡壳的地方，就是你该回去重读的那一节。
...
题目的顺序也是有意排的：Q1～Q3 钉倒排、分片、Heap；Q4～Q6 黄红与脑裂；Q7～Q10 写入、水位、Snapshot、ILM；Q11～Q14 mapping、主分片、慢查询与 60 秒定责。
...
开篇 Kibana 顶栏变黄、有人要重启 Data——正确第一问在这里。
...
写侧吞吐：Bulk 多大、reject 算不算丢——接 Q1 的 refresh 近实时。
...
开篇磁盘 87%、flood stage——水位摘只读就在这道题。
...
mapping 炸了和 Q5 日志/业务混 Index 是一类选型坑。
...
- [ ] 能画出倒排和 Query / Fetch，说明 refresh 后才可搜（Q1、Q13）
- [ ] 能按 30～50GB 估主分片；主分片不能改；routing=player_id（Q2、Q12）
- [ ] 能说出 Heap ≤ 一半且 ≤ 31GB；3 专用 Master 奇数（Q3、Q6）
- [ ] 能区分 Yellow 能搜全 / Red 数据缺，黄了不重启 Data（Q4、Q14）
- [ ] 能把日志检索和业务检索拆开，选出 ES / Loki / CH（Q5）
- [ ] 能处理 85% 只读和 Red（修盘或 Snapshot，不要空 reroute）；脑裂不要合并——现在再看开篇那两刀，你知道该怎么拦住他们了（Q8、Q9、Q6）
- [ ] 能默写 ILM 四阶段（Hot→Warm→Cold→Delete），不设 = 分片爆炸拖死 Master（Q10）
- [ ] 能说明 text vs keyword、动态 mapping 关 strict 防字段爆炸（Q11）

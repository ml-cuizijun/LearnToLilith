...
> 这一章讲完，你会知道当时那两个人错在哪：不该先加 Heap 到 40GB、不该先重启 Data——该先问磁盘水位和黄红能不能搜全。
> **本章回答的问题**：一次检索从 Coordinating 到分片倒排怎么走；黄/红/只读/Heap 飙高时怎么一眼定责。
> **本章不讲**：采集、EFK vs Loki 架构 → [03-20 日志](../../03-Kubernetes/20-日志管理/)；消息管道、积压 → [03 Kafka](../03-Kafka/)；文档库、TTL 邮件 → [05 MongoDB](../05-MongoDB/)；列存 SQL 细节不单开，选型见 3.6。这些都有专门章节，本章只在用到时点到为止。
...
**怎么听这场分享**：咱们先把「分层 → 归类 → 留证」主线装进脑子；概念、架构打底，原理六块放慢讲；第六、七节专门讲黄红、水位、定责——也就是开头那个现场的正确解法。每一节讲完会提示对应练习题，学完就去练。
...
> 🎯 **一句话**：倒排 + 分片并行让 ES 快；它适合搜，不适合当账本。

咱们先不急着背 `_cat` 命令。学 ES 最怕的就是上来加 Heap、重启节点。先把「它是什么、不是什么」钉死。
...
好，概念钉住了。接下来把整张架构图钉住——后面黄红、水位都对着它说话。

> 本节对应练习题 **Q1**（倒排地基；角色细节进第二节）。

---

## 2. 架构图

> 🎯 **一句话**：Master 管元数据不存业务；Data 存倒排；写入打 Primary，搜可打副本。

先把整张图钉住。后面安装、命令、排障都对着这张图说话。咱们对着图问自己：Master 在哪？业务数据在哪？
...
好，架构有了。接下来放慢讲原理——开篇那位要 Heap 加到 40GB 的同学，根因就埋在 3.5。
...
很多人会答「ES 快是因为分布式」——只答对一半。快的核心是倒排，不是「机器多」。

🔥 **倒排索引（inverted index）** = 词 → 文档 ID 列表。大白话：**像字典的「词目 → 页码」**，不是把整本日志从头扫到尾。
...
⭐ Query 找 ID、Fetch 取正文——两阶段别混成「扫一遍就出结果」。
...
大家想想：写入成功了，为什么客服马上搜还可能搜不到？——因为还没 refresh。
...
⭐ 口诀：**「Master 奇数专用；脑裂别手合并分片」**。

> 本节对应练习题 **Q6**。

### 3.5 GC 与堆：为什么不能 40GB 🔥⭐

开篇那位要把 Heap 加到 40GB——错就错在这儿。大家想想：内存越大堆越大越好吗？
...
⭐ 口诀：**「Heap ≤ 一半 RAM，且 ≤ 31GB」**——超了 compressed oops 失效，GC 更疼。
...
好，原理六块走完。接下来落地、配置、命令，最后回到开篇那个「变绿 / 加 Heap」现场。
...
> 🎯 **一句话**：3 专用 Master、Data 分角色、Heap 红线、盘独立；验收看 health 绿和能搜全。
...
> 🎯 **一句话**：只动有证据的项；Heap、水位、主分片数是最容易改错的三处。
...
> 🎯 **一句话**：值班三刀——`_cluster/health`、`_cat/shards`、`allocation/explain`；黄了别先重启 Data。
...
好，命令会敲了。回到开篇——群里要重启 Data、加 Heap 到 40GB，咱们用决策树拦住他们。

> 本节对应练习题 **Q4、Q8、Q9、Q10**。
...
现在再看开篇那两刀——「Heap 加到 40GB」「重启 es-data-2 变绿」——你知道该怎么拦住他们了：先看水位和黄红，Heap 守 31GB 红线。
...
- [ ] 倒排是什么？Query / Fetch 两阶段？为什么不是实时？（对应练习题 Q1、Q13）
- [ ] 500GB 怎么估主分片？主分片数能改吗？routing 干什么？（Q2、Q12）
- [ ] Heap 两条铁律？为什么不能 40GB？Master 为什么专用且奇数？（Q3、Q6）
- [ ] Yellow / Red 能不能搜全？黄了为什么别重启 Data？（Q4、Q14）
- [ ] 85% 只读怎么恢复？ILM 四阶段？Snapshot 的 RPO？（Q8、Q9、Q10）
- [ ] 日志检索和业务检索 Index 怎么分开？什么时候用 Loki / CH？脑裂已经发生怎么办？——现在再看开篇那两刀，你知道该怎么拦住他们了。（Q5、Q6）

六题都能口述，这一章就过了。下一章咱们讲文档层：副本集、分片、刚写立刻读 → [05 MongoDB](../05-MongoDB/)。账本仍在 MySQL，管道在 Kafka，检索在本章。

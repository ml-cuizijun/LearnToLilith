...
先合上知识点，对着 **一节的主图** 把「一个 key 的高可用路径」口述一遍（单点→主从→哨兵→Cluster，每层解决什么故障），再来做题。
...
- [ ] **默画一节的图**：单点→主从→哨兵→Cluster 四步递进，每层解决什么故障，`INFO replication` / `SENTINEL` / `CLUSTER` 各照哪段。（Q1 · Q4 · Q7 · Q9）
- [ ] **口述主从复制**：PSYNC 全量 vs 部分重同步的条件、全量同步步骤、backlog 的作用和调优公式。（Q1 · Q2）
- [ ] **口述读写分离**：为什么会读到旧值、哪些场景该读主库、`WAIT` 命令、和 MySQL 半同步的区别。（Q3）
- [ ] **默画哨兵架构与故障转移**：哨兵集群≥3 奇数、SDOWN→ODOWN、quorum 的含义、leader 选举、选 slave 三步排序、客户端 `get-master-addr-by-name`。（Q4 · Q5 · Q6）
- [ ] **口述 Cluster**：16384 槽为什么是这个数、hash tag 让多 key 落同槽、MOVED vs ASK、gossip 心跳。（Q7 · Q8）
- [ ] **默画 Cluster 故障转移**：slave 发现超时→拉票→master 多数同意→SLAVEOF NO ONE，和哨兵故障转移的区别。（Q9）
- [ ] **口述脑裂**：脑裂怎么产生、为什么丢数据、`min-slaves-to-write` + `min-slaves-max-lag` 怎么防、为什么不等于强一致。（Q10）
- [ ] **口述哨兵 vs Cluster 选型**：哨兵只做单 master HA 不分片，Cluster 分片+内置 HA，数据量超单机才上 Cluster。（Q11）
- [ ] **口述 slot 迁移**：`CLUSTER SETSLOT` 三步、`MIGRATE` 搬 key、迁移中 ASK 是正常的、别删迁移中的 key。（Q12）
- [ ] **定责**：主从断了看 `INFO replication`、哨兵没切查 quorum、Cluster fail 看 `CLUSTER NODES`、脑裂查 `min-slaves` 配置。（Q13）
- [ ] **默画高可用清单**：主从复制 / backlog / 故障转移 / 多数派投票 / min-slaves / 持久化兜底，说清每件保证什么、不保证什么。
- [ ] **边界**：key 选数据结构去 [01](../01-数据结构与使用场景/)；持久化与内存管理去 [02](../02-持久化与内存管理/)；缓存穿透/击穿/雪崩去 [04](../04-缓存架构/)。

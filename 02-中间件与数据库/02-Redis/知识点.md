...
**本章不讲：** 穿透 / 击穿 / 雪崩、Cache Aside、和 DB 谁先写 → [03 缓存架构](../03-缓存架构/)；钱包权威、PITR → [01 MySQL](../01-MySQL/)；延迟队列积压 → [05 Kafka](../05-Kafka/)；网关超时把 Redis 超时当 miss → [04 接入网关](../04-接入网关/)。合服/回档多存储对齐 → [07-07](../../07-游戏运维专题/07-玩家数据备份与回档/)。

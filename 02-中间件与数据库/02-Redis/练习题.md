...
**一句话**：Redis 快在内存 + epoll + 命令单线程；HGETALL/KEYS/大 DEL 会堵住该实例所有请求，不是 CPU 不够。
...
**一句话**：BGSAVE/AOF 重写 fork 复制页表，父进程命令排队；缓存可丢用 RDB，少丢用 AOF everysec，SAVE 生产禁止。
...
**一句话**：Cluster 只按 slot 分片，热 key 跟着走；大 key 先拆再迁槽，热 key 用 L1 或复制分片读。
...
**一句话**：ZSet 适合排行展示，但不能一个 key 扛全服；发奖和扣钻权威在 MySQL，Redis 可重算。
...
**一句话**：多 key 操作必须同 slot；hash tag 按需要原子的一组来，不能全站 `{user}` 否则更热。
...
**一句话**：哨兵管单机 HA，Cluster 管分片容量；切主丢未同步的写，AOF 救重启不救已切到落后从库。
...
**一句话**：Redis 锁用 SET NX PX + Lua 删；支付幂等靠 DB 唯一键，核心交易不靠 Redis 锁当事务。
...
**一句话**：备份 = 异地 RDB/AOF + restore 演练；Cluster 扩容迁 slot 先拆大 key，格式升过不能降二进制。
...
**一句话**：P99 尖 CPU 不高先看 fork_usec/slowlog；单分片 CPU 高是热 key；切主丢缓存回源，权威在 MySQL。
...
**一句话**：noeviction 写满会拒绝写入；碎片率高低峰 activedefrag，禁止高峰直接重启主。
...
**一句话**：K8s 滚动窗口新旧 Pod 叠加会让连接数翻倍打满 maxclients，要限滚动并行和池上限。
...
**一句话**：切主后多从同时 FULLRESYNC 是复制风暴；加大 repl-backlog 走 PSYNC，应用侧要限流回源。
...
- [ ] 能说出为什么快、哪类命令让该实例所有请求排队
- [ ] 能讲 BGSAVE 父进程为什么抖、latest_fork_usec 红线
- [ ] 能区分哨兵 vs Cluster；热 key 为什么加节点没用
- [ ] 能讲排行榜 / 会话 / 道具各怎么存、谁是权威
- [ ] 能区分切主丢哪几秒 vs AOF；备份 = 异地 + 演练；扩容迁 slot
- [ ] 能说出值班先跑哪几条，P99 尖先看 fork 不是加核

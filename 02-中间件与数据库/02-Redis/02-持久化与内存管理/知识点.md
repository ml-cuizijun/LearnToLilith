...
> Redis 重启后有的 key 还在、有的没了，INFO 里 used_memory 和 RSS 差了 2GB——持久化不是「存磁盘」三个字能概括的，内存管理也不是「设个 maxmemory」就完了。
...
- [ ] **默画一节的图**：写入内存 → RDB/AOF 落盘 → 重启恢复 → 过期/淘汰消亡；五道防线。
- [ ] **口述内存模型**：used_memory / RSS / mem_fragmentation_ratio 各是什么、碎片率高怎么办、activedefrag 整理什么。
- [ ] **口述 RDB**：fork + COW 怎么工作、bgsave vs save、自动触发条件、优缺点、fork 为什么慢。
- [ ] **口述 AOF**：追加日志流程、appendfsync 三档各丢多少数据、优缺点。
- [ ] **口述 AOF rewrite**：为什么需要 rewrite、bgrewriteaof 触发条件、rewrite 期间新写入怎么办、fork 开销。
- [ ] **口述过期策略**：惰性删除 + 定期删除各怎么做、过期 key 不被清会怎样、expired_keys 怎么看。
- [ ] **默画淘汰策略**：八种策略分两组（allkeys-* / volatile-*）、noeviction 是默认、LRU vs LFU 区别。
- [ ] **口述大 key**：怎么发现、怎么删（UNLINK）、怎么拆、对 fork 的影响。
- [ ] **口述恢复流程**：重启时先 load RDB 再 replay AOF、为什么同时开两种。
- [ ] **定责**：fork 毛刺 / AOF 恢复慢 / OOM / 碎片各看什么字段、各走哪条。

下一章：[高可用与集群](../03-高可用与集群/)。数据结构 → [01](../01-数据结构与使用场景/)；缓存架构 → [04](../04-缓存架构/)。

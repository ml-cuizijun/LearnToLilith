...
先合上知识点，对着 **一节的主图** 把「一个 key 的存活路径」口述一遍（写入内存 → RDB/AOF 落盘 → 重启恢复 → 过期/淘汰消亡），再来做题。
...
- [ ] **默画一节的图**：写入内存 → RDB/AOF 落盘 → 重启恢复 → 过期/淘汰消亡；五道防线。（Q1）
- [ ] **口述内存模型**：used_memory / RSS / mem_fragmentation_ratio 各是什么、碎片率高怎么办、activedefrag 整理什么。（Q8）
- [ ] **口述 RDB**：fork + COW 怎么工作、bgsave vs save、自动触发条件、优缺点、fork 为什么慢。（Q1 · Q2）
...
- [ ] **默画淘汰策略**：八种策略分两组（allkeys-* / volatile-*）、noeviction 是默认、LRU vs LFU 区别。（Q6 · Q7）
- [ ] **口述大 key**：怎么发现、怎么删（UNLINK）、怎么拆、对 fork 的影响。（Q9）
- [ ] **口述恢复流程**：重启时 AOF 优先 replay、RDB 做 fallback、为什么同时开两种。（Q11）
- [ ] **定责**：fork 毛刺 / AOF 恢复慢 / OOM / 碎片各看什么字段、各走哪条。（Q2 · Q8 · Q10 · Q12）

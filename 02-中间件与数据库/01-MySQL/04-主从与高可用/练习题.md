...
先合上知识点，对着 **一节的主图** 把「一次写入的复制之旅」口述一遍（主库写入 → redo log → binlog 两阶段提交 → 复制线程 → relay log → 从库回放 → 延迟 → 故障切换），再来做题。
...
面试官开场就问「两阶段提交解决什么问题」。有人答「分布式事务一致性」，有人答「crash 恢复」。线上一个 MySQL crash 后重启，有人怀疑从库数据可能和主库不一致。
...
线上从库数据和主库对不上，查发现 binlog_format=STATEMENT，有一条 `UPDATE t SET time=NOW() WHERE id=1` 在主库和从库执行的时间不同。
...
面试官让你画主从复制的流程图。有人只画了「主库 binlog → 从库」，说不出线程。线上复制断了，不知道看 IO Thread 还是 SQL Thread。
...
核心业务要求数据不丢，有人提议改成全同步。也有人开了半同步觉得万无一失，但切主后发现数据还是少了几条。
...
线上用传统 file:position 复制，主库挂了切主时手动算新主的 position 算错了，从库数据少了。有人提议上 GTID。
...
从库延迟告警，`Seconds_Behind_Master=30`。有人要重启从库，有人要加从库 CPU。主库刚做了个大事务删了几百万行。
...
主库突然不可用，群里有人喊「直接把从库提升为主」，有人喊「先等等」。旧主可能只是网络抖了一下还没真挂。
...
新项目要选 MySQL 高可用方案。有人推荐 MHA（老牌），有人推荐 MGR（官方），有人推荐 Orchestrator（有 UI）。
...
用户提交订单后立刻查列表，读走从库，结果查不到刚提交的订单。有人要加从库性能，有人要上全同步。
...
从库设了 `read_only=ON`，但有个有 SUPER 权限的 DBA 不小心在从库上执行了 `UPDATE`，从库数据和主库对不上，复制报错。
...
告警「从库复制延迟 NULL」（=断了）。有人要重启从库 MySQL，有人要抓包查网络。
...
从库开了 `replica_parallel_workers=16` 还是延迟。主库 QPS 不高但偶尔有个大 DDL。
...
凌晨告警「主库不可用」。从库还在但延迟 3 秒。有人喊直接切，有人要先确认旧主真挂了。
...
主库开了半同步 `rpl_semi_sync_master_timeout=1000`（1 秒）。从库机房网络抖 2 秒，支付写入全部超时。
...
- [ ] **定责**：决策树三个出口（断了/延迟了/挂了）；IO 断 vs SQL 断各看什么；60 秒剧本每步敲什么。（Q11 · Q6 · Q13）

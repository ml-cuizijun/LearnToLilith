...
> 这一章讲完，你会知道当时那个人错在哪：没先确认旧主真挂了（防脑裂）、没选数据最新的从库（防丢）——切主六步里，②③ 两步才是命门。
> **本章回答的问题**：一次写入从主库到从库会经历什么（两阶段提交 → 复制 → 回放 → 延迟 → 故障切换），每个阶段什么会坏。
> **本章不讲**：InnoDB 存储引擎内部 → [02-01-01](../../02-中间件与数据库/01-MySQL/01-架构与存储引擎/)；事务隔离与锁 → [02-01-03](../../02-中间件与数据库/01-MySQL/03-事务与锁/)；备份恢复与慢查询 → [02-01-05](../../02-中间件与数据库/01-MySQL/05-备份恢复与慢查询治理/)。这些都有专门章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先把第一节的复制主图看熟——两阶段提交 → 三线程接力 → 延迟 → 切主；第二到第五节顺着这条链展开；第六节专门讲断了/延迟了/挂了怎么定责——也就是开头那个现场的正确解法。每一节讲完会提示对应练习题。
...
带着这张图，咱们从主库内部出发——两本日志怎么对齐，复制才有源头。
...
01 章讲过「不怕崩」，这里换个角度：binlog 还要喂给从库——两本日志撕开了，主从就裂了。⭐ 面试必考。
...
大家想想：为什么不能「redo 写完就算提交」？很多人答「反正崩溃能恢复」——半对。崩溃能保主库，保不住从库也一致。

如果 redo log 写了但 binlog 没写，crash 恢复后主库有这条数据但从库收不到；反之 binlog 写了但 redo log 没 commit，从库有了主库反而没有。🔥 **两阶段提交（2PC, two-phase commit）** 大白话：**两本账先都写到「准备好」，再一起落锤——中间崩了，以 binlog 有没有完整记录来决定提交还是回滚**。
...
高频错答预警：半同步 ≠ 从库已回放完，更 ≠ 防脑裂。下面带着大家推清楚。
...
开讲时那个现场的正确解法，就从这一节开始：⭐ **先 fencing 确认旧主真挂，再选 Executed_Gtid_Set 最大的从库**——别先改 VIP。
...
很多人切主只想着「快点恢复业务」——错了。先问一句：旧主真的死了吗？

🔥 **脑裂（split brain）** 大白话：**两个船长同时上任，船员不知听谁的**——旧主没真挂、新主已提升，两边都在接收写入，数据不一致。这是故障切换最怕的事。
...
- [ ] **默画主图**：主库写入 → redo log prepare → binlog commit（两阶段提交）→ Binlog Dump Thread → IO Thread → relay log → SQL Thread → 从库回放 → 延迟 → 故障切换；`SHOW REPLICA STATUS` 照哪段。（对应练习题 Q1、Q3、Q13）
...
- [ ] **口述切换**：故障切换六步、脑裂怎么防（fencing / MGR 多数派）、半同步为什么 ≠ 防脑裂——现在再看开篇那句「直接提升从库」，你知道该怎么拦住他了。（Q7、Q13）

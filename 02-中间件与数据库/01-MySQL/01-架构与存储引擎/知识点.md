...
> 发版后 DB CPU 飙到 100%，慢查询里全是 `UPDATE`——有人猜是索引丢了，有人要扩内存，群里已经有人在敲「重启 mysqld」。`SHOW PROCESSLIST` 一排 `Updating` 卡着，根因其实是 `innodb_flush_log_at_trx_commit=1` 碰上了机械盘——每次提交都在等 fsync，盘跟不上就把整条写入链路堵死了。
> 这一章讲完，你会知道当时那个人错在哪：不该先动索引、不该先扩内存、更不该先重启——该先看写入卡在哪一层。
> **本章回答的问题**：一条 SQL 从进 MySQL 到落盘走完全程，每一步什么会坏、怎么一眼定位。
> **本章不讲**：索引调优 → [02-01-02](../02-索引与执行计划/)；事务与锁 → [02-01-03](../03-事务与锁/)；复制与高可用 → [02-01-04](../04-主从与高可用/)；备份恢复 → [02-01-05](../05-备份恢复与慢查询治理/)。这些都有专门章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先把第一节的主图看熟——一条 SQL 过连接→解析→优化→执行→存储引擎五道关；第二到第七节顺着这五道关往下走，卡在哪道关症状完全不同；第八节专门讲写入卡住怎么定责——也就是开头那个现场的正确解法。每一节讲完会提示对应练习题，学完就去练。
...
带着这张图，咱们正式出发。先从门口说起——连都连不上，后面的优化器、Buffer Pool 都轮不到上场。
...
开篇那个现场是「写得慢」，但值班里更常见的第一刀其实是「连不上」。大家先把门口搞清楚。
...
> ⭐ **口诀**：`Too many connections` 先查谁占坑，别先调大上限——`PROCESSLIST` 里 `Sleep` 堆成山，多半是池太大或泄漏。

好，门口过了。SQL 文本进来以后，MySQL 先「读懂你要干什么」，还不碰数据——这就是解析。
...
面试官偶尔还会问「MySQL 不是有查询缓存吗」——先别急着答怎么开，咱们先想一句：为什么 8.0 干脆删了？
...
解析完了，MySQL 知道「你要做什么」，下一步才是「怎么做最划算」——优化器上场。
...
大家想想：同一条 SQL，昨天走索引 2ms，今天全表扫 2s——人没改 SQL，改了什么？往往是**统计信息过期**，优化器按过期地图选了错路。这是慢查询最常见的根因之一，也是 ⭐ 面试高频。
...
到这儿，咱们终于碰到 MySQL 架构里最常考的那一刀：Server 层和存储引擎层怎么分。⭐ 面试白板几乎必画。
...
很多人第一次听「B+ 树」会以为是另一种二叉树——先别慌。大白话先记一句：**B+ 树 = 矮胖的目录树，真正的数据只躺在最底层叶子上**；中间层只负责指路，所以能塞更多键、树更矮、磁盘 I/O 更少。叶节点之间还有**双向链表**，范围查询顺着链表扫即可。

InnoDB 用 **B+ 树（B+ Tree）** 组织索引和数据。核心特征就是上面那句：数据只在叶节点，非叶节点只存索引键用于导航。
...
大家想想：二级索引的叶子上如果存完整行，改一个非索引列是不是所有二级索引都要跟着改？所以 InnoDB 选择了另一条路——

**聚簇索引（Clustered Index）**：数据和主键索引在**同一棵 B+ 树**里，叶节点直接存完整行数据。一张表只有一个聚簇索引（主键索引）。口诀：**聚簇索引 = 数据就是索引**。

**二级索引（Secondary Index）**：叶节点不存完整行，而是存**主键值**。查到主键值后要回聚簇索引查完整行——这个动作叫**回表（bookmark lookup）**。口诀：**二级索引 = 索引指向主键，回表 = 拿主键再查一次**。
...
⭐ 面试必背口诀：**undo 保 A，redo 保 D，锁 + MVCC 保 I**——C 不是单靠某个机制，是 AID + 约束的结果。

**ACID** 是事务的四个保证：**A（原子性）**= undo log 回滚；**C（一致性）**= AID + 约束共同保证；**I（隔离性）**= 锁 + MVCC；**D（持久性）**= redo log 恢复。隔离级别细节见 [02-01-03](../03-事务与锁/)。
...
好，执行器会喊引擎了。引擎里真正决定「快不快、崩不崩」的，是 Buffer Pool 和两本日志。
...
回到开头那个现场：CPU 100%、一排 `Updating`——很多人第一反应是「索引呢？内存呢？」其实更该先问：**写入是卡在等锁，还是卡在等盘把 redo log fsync 完？** 这一节把「不怕崩」的机制讲透。
...
很多人会答「提交了数据就落盘了」——错了。提交保证的是 **redo log 落盘**，数据页可能还躺在 Buffer Pool 里当脏页。

**redo log（重做日志）** 记录的是「数据页做了什么修改」，不是修改后的完整数据。它保证 **crash recovery（崩溃恢复）**：MySQL 崩溃后重启，靠 redo log 把已提交但未刷盘的修改重做一遍。

🔥 **WAL（Write-Ahead Logging，先写日志再写数据）** 大白话先拆开：**改账本之前先在便签上记一笔，便签贴牢了（fsync）再慢慢誊到正式账本**。修改数据页之前先把 redo log 写好、刷盘，这样即使数据页还没刷盘就崩了，也能靠日志恢复。
...
引擎层的 redo 能保崩溃恢复了，可还有一本 Server 层的 binlog——两本日志怎么保证不撕裂？下一节。
...
大家想想：如果先写完 redo log 再写 binlog，中间崩了会怎样？很多人答「反正 redo 有了就不丢」——半对。主库靠 redo 恢复了，但从库靠 binlog 同步，少了这条变更 → **主从不一致**。所以需要两阶段提交。
...
⭐ **口诀**：双 1 = 两次 fsync 换「已提交绝不丢」；核心库别为了性能先动这两把闸。

机制讲完了。回到开篇那个群——CPU 100%、有人要重启——正确做法是什么？
...
开讲时那个现场的正确解法，就在这一节。记住：**先定责，再动手**——别先重启、别先改 `flush_log_at_trx_commit=0`。
...
现在再看群里那句「重启 mysqld / 先扩内存」，你知道该怎么拦住他了：先 `PROCESSLIST` → `INNODB STATUS` → `iostat`，定型再开口。
...
- [ ] **默画一节的图**：客户端 → 连接 → 解析 → 优化 → 执行 → 存储引擎 → 磁盘；日志分两层（redo/undo 引擎层 + binlog Server 层）。（对应练习题 Q1、Q2）
...
- [ ] **定责**：写入卡住先看 `PROCESSLIST` State、再看 `INNODB STATUS`；决策树三个出口各走哪条——现在再看开篇那句「重启 mysqld」，你知道该怎么拦住他了。（Q13）

下一章咱们把「优化器到底怎么选路」剖开看：什么时候走索引、什么时候全表扫 → [02-01-02](../02-索引与执行计划/)。

...
> 慢查询告警，DBA 说「加个索引就好了」——加完确实快了，换个条件查又慢了，再换一个索引又快。到底什么时候走索引、什么时候全表扫，凭什么？
> **本章回答**：一条 SELECT 从进来到找到数据，中间走了哪些路、为什么有时快有时慢、怎么用 EXPLAIN 看清它走了哪条。
> **不讲**：存储引擎页结构细节、Buffer Pool → [02-01-01](../../02-中间件与数据库/01-MySQL/01-架构与存储引擎/)；事务隔离与锁 → [02-01-03](../../02-中间件与数据库/01-MySQL/03-事务与锁/)。
...
- [ ] **默画一节的图**：SQL → 优化器（走索引还是全表扫）→ B+树定位 → 聚簇/二级分叉 → 回表/覆盖分叉 → 返回结果。
- [ ] **口述 B+树**：为什么不用红黑树（二叉太高）、B+树非叶子只存 key 有什么好处、叶子链表干什么用、3 层覆盖千万行的原理。
- [ ] **口述页/区/段**：页多大、区多大、段是什么、为什么读一行和读一页是同一次 IO。
- [ ] **默画聚簇 vs 二级**：聚簇叶子存整行、二级叶子存主键值；InnoDB vs MyISAM 索引叶子存什么不同。
- [ ] **口述回表**：什么是回表、为什么二级索引要回表、回表的代价（N 行 = N 次聚簇查找）、怎么用 EXPLAIN 判断有没有回表。
- [ ] **口述覆盖索引**：什么是覆盖索引、Extra 出现什么、为什么 SELECT * 有害、怎么设计联合索引让它覆盖。
- [ ] **默画最左前缀**：联合索引 (a,b,c) 哪些查询能走、哪些不能、范围查询为什么打断后续列。
- [ ] **口述联合索引设计**：等值在前范围在后、区分度高的在前、高频在前。
- [ ] **口述索引下推**：ICP 是什么、Using index condition 和 Using index 区别、ICP 减少了什么。
- [ ] **口述 EXPLAIN**：type 从好到差排列、Extra 三个关键词含义、type=ALL 和 type=index 区别。
- [ ] **口述索引失效**：函数包裹列、隐式类型转换、LIKE %前缀，各给对写法。
- [ ] **定责**：慢查询 EXPLAIN 决策树两个分支（type=ALL / Extra 有额外操作）各走哪条。

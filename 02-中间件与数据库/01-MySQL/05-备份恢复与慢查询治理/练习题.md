...
开始做题前，先给大家一个小建议：合上知识点，对着 **一节的主图**，把「一条慢SQL的诊断 + 一次误操作的恢复」口述一遍（发现→EXPLAIN→治→验 / 备份→误操作→binlog→闪回）。能顺下来，下面这些题你会发现大半都会了；卡壳的地方，就是你该回去重读的那一节。
...
题目的顺序也是有意排的：Q1～Q5 走慢 SQL 线；Q6～Q7 钉死备份与 RPO/RTO；Q8～Q12 专攻 binlog / 闪回 / PITR；Q13、Q14 收尾到综合定责。
...
这道题几乎是慢查询值班的「开场白」。活动高峰接口变慢，DBA 说「CPU 没到瓶颈」，开发说「SQL 没问题之前一直很快」。群里没人知道 slow_query_log 开没开。
...
日志有了，下一刀读 EXPLAIN：面试让你对着一条 SQL 说 EXPLAIN 怎么看。有人只会看 `key` 有没有值，不知道 `type` 和 `Extra` 的含义。
...
这是 Q2 type 列的特写——一条 `SELECT * FROM player_log WHERE create_time BETWEEN ...` 慢了 3 秒，EXPLAIN 发现 `type=ALL`。开发加了索引，但没验证就说「应该快了」。
...
type 会看了，Extra 里的排序坑：EXPLAIN 看到 `Extra: Using filesort`，开发说「名字里有 filesort 说明用了临时文件排序」，但实际结果集只有几千行，不可能落盘。
...
有索引却不走，常见翻车：开发建了索引但 `type=ALL`，怀疑「MySQL 的索引有 bug」。实际是 SQL 写法导致索引失效。
...
慢 SQL 线告一段落，换备份线：新项目要定备份方案。有人提议用 mysqldump 每天跑 cron 就行，有人担心大数据量恢复太慢。
...
备份有了还要会算账：面试问备份策略。有人答「每天 mysqldump 一次就够了」。追问 RPO 和 RTO 各是多少，答不上来。
...
备份和恢复的交叉点是 binlog：面试问 binlog 格式。有人答「生产用 STATEMENT 省空间」，问闪回能不能做就卡住了。再问 binlog 和 redo log 区别也说不清。
...
就是开篇那个 DELETE 现场——凌晨有人执行 `DELETE FROM user WHERE 1=1` 忘了限流。binlog_format=ROW，有全量备份。开发问要不要恢复全量。
...
这是 Q9 的 DROP 对照版——有人执行了 `DROP TABLE player_log`，准备用 binlog2sql 闪回。开发说「binlog 里应该有记录吧」。
...
闪回不够用时走 PITR：面试让你讲 PITR 的原理和步骤。有人说不清从哪个位点开始回放，有人想从 binlog 文件开头开始。
...
把 redo/binlog 口径再钉死：面试高频八股。有人答「都是日志」，追问区别和 crash recovery 顺序就混了。
...
这张表把慢 SQL 线串起来了：`slow.log` 一夜 80GB。有人要逐条 `EXPLAIN`，有人要全加索引。
...
这张表把误操作线串起来了：同时来三条：① P99 变慢 ② 运营说数据没了 ③ DBA 说备份恢复完了。
...
- [ ] **默画一节的图**：两条线——慢SQL线（发现→EXPLAIN→治→验）和误操作恢复线（备份→误操作→binlog→闪回/PITR），说清交叉点是 binlog。（Q1～Q12）

...
> 凌晨有人 `DELETE FROM user WHERE 1=1` 忘了 WHERE 条件外的限流——另一边，白天活动高峰一条全表扫描把库拖死，慢查询堆了两千条没人看。
> **本章回答**：一条慢 SQL 怎么发现、怎么诊断、怎么治；一次误操作怎么用备份 + binlog 恢复到时间点。
> **不讲**：存储引擎与 Buffer Pool → [02-01-01](../../02-中间件与数据库/01-MySQL/01-架构与存储引擎/)；索引数据结构与 B+ 树 → [02-01-02](../../02-中间件与数据库/01-MySQL/02-索引与执行计划/)；事务与锁 → [02-01-03](../../02-中间件与数据库/01-MySQL/03-事务与锁/)；主从复制与高可用 → [02-01-04](../../02-中间件与数据库/01-MySQL/04-主从与高可用/)。
...
- [ ] **默画一节的图**：两条线——慢SQL线（发现→EXPLAIN→治→验）和误操作恢复线（备份→误操作→binlog→闪回/PITR），说清两条线的交叉点是 binlog。
- [ ] **口述发现**：slow_query_log 默认关着、long_query_time 生产设多少、mysqldumpslow / pt-query-digest 怎么聚合排序、先治哪条。
- [ ] **口述诊断**：EXPLAIN 看哪四列、type 从好到坏的顺序、ALL 怎么治、Using filesort / Using temporary 各是什么、Using index 和 type=index 的区别。
- [ ] **口述索引失效**：五种常见场景（函数包裹 / 隐式转换 / 左模糊 / OR / 最左前缀不满足），本质是什么。
- [ ] **默画收束清单**：慢SQL诊断五步（日志→聚合→EXPLAIN→索引→验证），说清为什么改完必须再 EXPLAIN。
- [ ] **口述备份**：mysqldump vs xtrabackup（逻辑 vs 物理）、--single-transaction 不锁表原理、增量备份靠 LSN、三层备份策略。
- [ ] **口述 RPO/RTO**：RPO 丢多少数据、RTO 多久恢复、两者独立、为什么没演练等于没备份。
- [ ] **口述 binlog**：binlog 在 Server 层、三种格式、为什么闪回要 ROW、binlog 和 redo log 的区别。
- [ ] **口述 PITR**：两个位点是什么、恢复步骤（全量恢复 + binlog 回放）、从哪个位点开始回放。
- [ ] **口述闪回**：binlog2sql 生成反向 SQL 原理、DELETE 能闪回 DROP 不能、闪回和 PITR 的区别。
- [ ] **口述误操作分类**：DELETE/UPDATE 能闪回、DROP/TRUNCATE 只能 PITR、发现后第一件事是停写。
- [ ] **定责**：决策树两个入口（慢了/丢了）；60 秒剧本每步敲什么。

下一章：[02-01-01 架构与存储引擎](../../02-中间件与数据库/01-MySQL/01-架构与存储引擎/)。索引原理：[02-01-02](../../02-中间件与数据库/01-MySQL/02-索引与执行计划/) · 事务与锁：[02-01-03](../../02-中间件与数据库/01-MySQL/03-事务与锁/) · 主从与高可用：[02-01-04](../../02-中间件与数据库/01-MySQL/04-主从与高可用/)。

...
> 这一章讲完，你会知道当时那个人错在哪：先分清是死锁还是锁等待，再决定要不要 kill——乱 kill 大事务，回滚可能比等更久。
> **本章回答的问题**：一次事务从 BEGIN 到 COMMIT 经历什么——读为什么不阻塞写、写为什么会死锁、怎么一眼定责。
> **本章不讲**：B+ 树与索引走法 → [02-01-02](../../02-中间件与数据库/01-MySQL/02-索引与执行计划/)；主从复制与 binlog 格式 → [02-01-04](../../02-中间件与数据库/01-MySQL/04-主从与高可用/)；Buffer Pool 与存储引擎内部 → [02-01-01](../../02-中间件与数据库/01-MySQL/01-架构与存储引擎/)。这些都有专门章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先把第一节的「一次事务的一生」看熟——隔离级别 → MVCC 读 → 加锁写 → 提交；第二到第五节顺着四关展开；第六节专门讲死锁与锁等待定责——也就是开头那个现场的正确解法。每一节讲完会提示对应练习题。
...
带着这张图，咱们从「起步」讲起——四个承诺和四档隔离，是后面所有锁问题的坐标系。
...
大家先把坐标系钉死：⭐ 口诀 **undo 保 A，redo 保 D，锁 + MVCC 保 I**——C 是结果不是单独一套机关。
...
隔离级别选完，真正让「读不阻塞写」落地的是 MVCC——放慢讲。

> 本节对应练习题 **Q1、Q9、Q13**（起步口径）。
...
开篇两个 GM 同时改同一玩家——如果只是普通 SELECT 互看，靠的就是这一节；一旦变成 UPDATE，就进下一节的锁战场。

### 快照读 vs 当前读 🔥

很多人会答「读不加锁所以不阻塞」——半对。真正的关键是：**读的是哪个版本**。

🔥 **MVCC（Multi-Version Concurrency Control，多版本并发控制）** 大白话先拆开：**同一行数据同时留着好几个历史版本，读的人看「自己该看的那一版」，写的人改「最新版」——两边不用互相堵着**。它是 InnoDB 实现并发读的核心机制。读分两种：
...
大家想想：RR 和 RC 差在哪？很多人答「一个能防幻读一个不能」——机制上更准的说法是：**ReadView 创建几次**。⭐ 这是面试高频错答点。
...
⭐ 口诀先记：**行锁加在索引上；next-key = 记录锁 + 前间隙；RR 有间隙锁，RC 基本没有**。
...
开讲时那个现场的正确解法，就在这一节：⭐ **先分清死锁（1213，InnoDB 已自动回滚一方）还是锁等待（1205，还在等）**，再决定要不要动手。
...
现在再看群里那句「kill 掉卡住的那条」，你知道该怎么拦住他了。
...
- [ ] **默画一节的图**：BEGIN → 隔离级别 → MVCC 读 → 加锁写 → 死锁判断 → COMMIT/ROLLBACK；读为什么不阻塞写。（对应练习题 Q1、Q2）
...
- [ ] **定责**：死锁（自动回滚 1213）vs 锁等待（超时 1205）怎么分、`SHOW ENGINE INNODB STATUS` 和 `data_locks` 各看什么、60 秒剧本每步敲什么——现在再看开篇那句「kill 卡住的那条」，你知道该怎么拦住他了。（Q10、Q11、Q12、Q14）

下一章咱们把主从复制剖开看：一次写入怎么到从库、主挂了怎么切 → [02-01-04 主从与高可用](../../02-中间件与数据库/01-MySQL/04-主从与高可用/)。地基：[02-01-01](../../02-中间件与数据库/01-MySQL/01-架构与存储引擎/) · 索引：[02-01-02](../../02-中间件与数据库/01-MySQL/02-索引与执行计划/)。

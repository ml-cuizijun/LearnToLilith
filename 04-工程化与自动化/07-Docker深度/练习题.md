...
先对照 [知识点](知识点.md) **本章主线** 自己口述，再对答案。关键字（层缓存、PID 1、overlay2、BuildKit secret）要能点名。每题标了覆盖范围：
...
Harbor / 禁 latest 见 03，K8s 运行时见 04-16，cgroup 文件链见 02-07。
...
每次改一行 Go 业务代码，CI 都重新 `go mod download`，流水线 15 分钟。Dockerfile 先 `COPY . .` 再下载依赖。
...
镜像只有 180MB，节点 root 盘 100%。游戏服把资源包下在容器可写层。有人删了几个停掉的容器，盘没回来多少。另有人在容器里 rm 了一个镜像带来的文件，以为镜像会变小。
...
容器内写大文件进 upper，不进「镜像 180MB」那个数字。删容器不删共享镜像层；还要 prune 悬空镜像和 build cache。CoW 改下层大文件会先拷到 upper。容器里 rm 只在 upper 记 whiteout，下层还在。资源包走 volume / CDN。K8s imagefs 驱逐见 04-16。
...
`free` / `nproc` 经常撒谎，真配额在 cgroup（02-07）。USER namespace 很多生产默认没开，所以镜像非 root 仍要做。kubelet / containerd / pause 是 04-16，不是本章。本章停在镜像层、启动命令、overlay、单机 Docker / 构建。
...
五个工单：① 改一行全重装 ② 滚动 SIGKILL ③ CrashLoop ④ 镜像小盘满 ⑤ history 里有 token。有人开口就是 kubelet。

### 完整解答

**一句话**：本章排障：COPY 顺序→PID 1→inspect/logs→system df→history；kubelet 是 04-16。
...
本章：Dockerfile → 层 → overlay2 → 进程 PID 1。04-16：kubelet → CRI → containerd → 节点升级。值班四条：`inspect` PID 1 → `logs` → `system df` → `history`。
...
- [ ] 能画出只读层 + 可写层，并说明层缓存失效规则
- [ ] 能说出 ENTRYPOINT vs CMD、exec vs shell、PID 1 收信号
- [ ] 能讲多阶段 + distroless/alpine；800MB → 20MB 的步骤
- [ ] 能画出 overlay2 四元组；盘满先看 upper / cache；rm 只是 whiteout
- [ ] 能区分 namespace 视图 vs cgroup 配额；容器不是 VM；USER ns 默认可能没开
- [ ] 能处理密钥进层（先 rotate）和 BuildKit secret / cache mount
- [ ] 能用分流处理 CrashLoop / 盘满 / 杀不掉，而不是先怪 kubelet

...
> 合服窗口，`terraform show plan.out` 里 RDS `will be destroyed`。有人准备 `apply -auto-approve`。另一次 CI 被 kill，下一个人要把 `force-unlock` 写进自动重试——这两刀都能把库删没或把 state 改烂。手就放在回车上。
> 这一章讲完，你会知道当时那个人错在哪，以及正确的做法是什么。
> **本章回答的问题**：plan 读哪三方、盯哪两种 diff；state 丢了会怎样；锁谁拿谁放；漂移怎么治；合服用 for_each 还是 count；和 Helm 怎么分工。
> **本章不讲**：装软件、改配置、滚动 playbook → [06 Ansible](../06-Ansible/)；集群内发布、GitOps → [03-23](../../03-Kubernetes/23-GitOps-ArgoCD与Tekton/)；IAM / 多账号 → [06-04](../../06-云平台/04-IAM与多账号/)；制品、镜像 → [03](../03-制品与镜像/)；变更窗口 → [08](../08-运维平台与Oncall/)。
...
**怎么听这场分享**：咱们先钉三条主线——审 plan、拿锁放锁、对三方；第三节难点放慢；第七节用决策树回收开头现场。每一节讲完会提示对应练习题，学完就去练。
...
本节对应练习题 Q1、Q2——三方对比和 diff 种类就是那道题的地基。


## 2. 架构图


> 🎯 **一句话**：HCL 期望、state 记忆、云 API 真值——plan 是产出物。

好，概念钉住了。接下来咱们看一张图——后面命令、合服、排障都对着它。大家先找一件事：apply 有没有不经过「已审 plan」的箭头？
...
本节对应练习题 Q1——主图就是三方对比的白板答案。


## 3. 原理


> 🎯 **一句话**：state、锁、漂移、count vs for_each——四个难点放慢讲。

好，图看熟了。接下来难点咱们放慢——state 丢了会怎样、锁残留能不能自动 unlock、漂移怎么治、合服用 count 删中间服会发生什么。每个都先抛问题再揭底。

### 3.1 State：地址 ↔ 云 ID 🔥⭐

很多人会答「state 丢了再 apply 就能对齐」——错了。⭐ 会双创；要回滚 state 或 import。
...
开头那个要把 force-unlock 写进自动重试的人——错在哪？大家想想：如果还有人在 apply，你 unlock 会怎样？
...
控制台改完「下次 plan 会认」？——不会。要么拉回、要么改 HCL / import。口诀——**漂移先找谁改了控制台。**
...
合服用 count 删第 2 个，后面索引全 replace——这是 ⭐ 面试必考。口诀——**会删中间的用 for_each，别用 count。**
...
难点讲完了。落地时 backend 怎么摆、prod/staging 为什么分 key——对着第三节的坑逐条焊死。
...
> 🎯 **一句话**：只动有证据的项；prevent_destroy 只给数据库。
...
> 🎯 **一句话**：值班：fmt → validate → plan → 审 destroy/replace → apply 该文件。

合服窗口怎么审 plan、失败怎么回滚，也在这一节。
...
现在再看开头那个现场——RDS will be destroyed、force-unlock 进自动重试——决策树就是正确解法。现在再看群里那句 `apply -auto-approve`，你知道该怎么拦住他了——先 grep destroy，逐条能解释才过。
...
> ⚠️ **别混**：refresh vs destroy vs state rm；count vs for_each；锁残留 vs 漂移；import vs 再 create；HCL revert vs 云已删；workspace vs 目录分离。
...
- [ ] **口述 plan**：读哪三方？apply 盯哪两种 diff？（对应练习题 Q1、Q2）
- [ ] **口述 state**：丢了会怎样？prod / staging 为什么分 key？（Q3、Q6）
- [ ] **口述锁**：残留先问什么？为什么不能自动 unlock？（Q4）
- [ ] **口述漂移**：怎么发现、怎么治？紧急控制台改了事后做什么？（Q8）
- [ ] **口述合服**：count 删中间服会发生什么？for_each + moved 怎么走？（Q5）
- [ ] **口述边界**：TF 和 Helm 谁管什么？force-unlock 前确认什么？（Q7、Q10、Q14）

六题都能口述，这一章就过了——开头那个 `apply -auto-approve` 和自动 unlock 的现场，你现在能拦住他了。下一章把变更、告警、值班收成一套 oncall，而不是个人英雄敲 apply。

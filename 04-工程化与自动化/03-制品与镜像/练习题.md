...
开始做题前：合上知识点，对着**本章主线**（锁内容 → 复制晋升 → 就近拉）口述一遍。关键字（digest、latest、Harbor 复制、Trivy、GC）要能点名。
...
题目顺序：Q1～Q2 钉 digest / latest；Q3～Q5 瘦身 / Harbor / 扫描；Q6～Q10 排障与审计；Q11～Q14 签名 / GC / 综合。
...
先自己试试：同 SHA 再 build，digest 一定一样吗？怎么证明两边是同一份？
...
Deployment 写 `app:latest`，`imagePullPolicy: Always`。回滚时「再发一次 latest」，节点新旧混跑。战斗服扩了 30 个新 Pod。就是知识点开头那个混跑现场。

先自己试试：为什么扩容会混跑？immutable 了还要不要 digest？

### 完整解答

> 🎯 **一句话**：生产禁 latest：不可追溯、不可回滚、Always 可能拉到不同字节。

和 Q1 同一条线：锁不住内容，预发结论和回滚都作废。
...
先自己试试：Harbor 比私有 Hub 多哪几项？开服清单写哪句？

### 完整解答

> 🎯 **一句话**：Harbor 靠 RBAC + 本机房复制 + 预拉扛开服；节点就近 pull 同一 digest。

接 Q3 瘦身——胖镜像 + 打中心仓，开服窗口必炸。
...
先自己试试：Critical 能「先发后补」吗？删 tag 密钥就没了吗？
...
先自己试试：上节点拉同 digest，401 / x509 / timeout 各查什么？
...
先自己试试：这和 Q2 的 latest 混跑是同一类问题吗？

### 完整解答

> 🎯 **一句话**：tag 可变，digest 锁内容；生产引用 @sha256，immutable 仍要以 digest 为准。

就是 Q2 结论的「可变 tag 版」——不只 latest，任何可覆盖 tag 都一样。
...
先自己试试：一次发布要记哪三件？密码能写进 values-prod 吗？

### 完整解答

> 🎯 **一句话**：Chart 当 OCI 制品和镜像同一套 RBAC；values 分层，密码不进 Git。

和 Q1 三联审计同一条线：digest + chart version + values 哈希。
...
先自己试试：复制延迟大能不能切流量？immutable 冲突怎么处理？

### 完整解答

> 🎯 **一句话**：跨机房复制是同一 digest 出现在两个 registry，不是重建；延迟未达标不切流量。

接 Q4 开服清单——本机房没有 digest，不要切。
...
**★★★★☆｜cosign 签名后，节点拉取还要信任什么？**

**覆盖**：必会 ｜ **对应**：知识点 [5. 核心配置](知识点.md)

### 场景

供应链要求镜像签名验证。

先自己试试：签名能替代 digest 晋升吗？

### 完整解答

> 🎯 **一句话**：CI cosign sign；集群 policy/Kyverno 验签；信任根 pin 在集群不随 tag 漂。

签名证明来源，digest 证明内容——和 Q7 互补，不能互相替代。

```bash
cosign verify --key cosign.pub reg/game:battle@sha256:...
```

### 再追问

签名能替代 digest 晋升吗？——不能，签名证明来源，digest 证明内容（Q7）。

---

## Q12

**★★★★☆｜Harbor retention 误删了还在用的 tag，怎么设规则？**

**覆盖**：必会 ｜ **对应**：知识点 [4. Harbor / 开服](知识点.md)

### 场景

清理脚本删了上月 tag，rollback 找不到镜像。

先自己试试：GC 前要对什么？prod 留多少成功 digest？

### 完整解答

> 🎯 **一句话**：保留最近 N + 保护 release tag 模式；删前扫 K8s 引用；immutable tag。

就是 Q10「回档窗口内不 GC」的规则落地。
...
GC 前要对什么？——Running Pod 的 imageID 和 Helm values（Q10）。

---

## Q13

**★★★☆☆｜跨机房复制延迟，开服节点拉到老 digest，怎么办？**

**覆盖**：常用 ｜ **对应**：知识点 [9. 跨机房复制](知识点.md)

### 场景

sea 机房 replication lag 10 分钟。

先自己试试：这和 Q9 是同一类问题吗？

### 完整解答

> 🎯 **一句话**：开服前检查 replication 完成；晋升用 digest；必要时 crane copy 预热。
...
策划表打进镜像为什么错？——热更走 CDN，不是镜像层（Q3）。

---

## Q14

**★★★★★｜latest 漂移、扫描 Critical、Chart 含密钥，60 秒处置**

**覆盖**：核心 · 必会 ｜ **对应**：知识点 [7. 生产排障](知识点.md)

### 场景

prod 用了 latest；扫描 CRITICAL；Chart values 含 AK。60 秒处置，把前面的题串起来。

先自己试试：三件事各先做什么？

### 完整解答

> 🎯 **一句话**：latest→改 digest；Critical→阻断晋升；密钥→ExternalSecret+rotate。

链 Q2/Q5/Q8——开头那些事故现场的急救卡。

链 Q2/Q5/Q8。

### 再追问

一次构建多处部署怎么画？——CI 出一个 digest，各环境只 promote（Q1）。
...
- [ ] **口述贯通**：能画出一次构建、digest 贯通，并能区分复制 vs 再 build（Q1｜知识点 §3.1）
- [ ] **口述禁 latest**：能说出生产禁 latest 的理由，以及 immutable 仍要 digest（Q2、Q7｜§3.2）
- [ ] **口述瘦身**：能用多阶段 + history/dive 砍胖层，策划表不进业务镜像（Q3｜§3.3）
- [ ] **口述 Harbor**：能说出 RBAC / 复制 / Proxy Cache / 扫描 / GC / OCI（Q4｜§4）
- [ ] **口述拉失败**：能在节点 `crictl pull` 同 digest 分流 401 / x509 / timeout / 磁盘（Q6｜§7）
- [ ] **口述门禁**：能处理 Critical、密钥进层（先 rotate）、Chart 三件套记录（Q5、Q8｜§3.4、§6）
- [ ] **口述拆分**：能把热更包、客户端整包和镜像 CI 拆开，合服回档窗口内不 GC（Q10、Q14｜§6）

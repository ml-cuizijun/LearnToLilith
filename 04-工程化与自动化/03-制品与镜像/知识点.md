...
> 大家好，欢迎来到这次分享。开讲之前，先带大家回到几个开服和回滚现场：
...
> 这一章讲完，你会知道当时那个人错在哪：为什么禁 latest、为什么不能每环境 rebuild、开服前本机房 Harbor 必须已有 digest。
> **本章回答的问题**：digest 怎么从 CI 贯通到集群；latest 为什么让扩容和回滚踩空；Harbor 复制预拉、Critical 阻断、`crictl pull` 怎么分流。
> **本章不讲**：流水线阶段、晋升审批、金丝雀 SLI → [04](../04-CI-CD/)；分层缓存细节、ENTRYPOINT vs CMD、overlay2 磁盘、BuildKit secret → [02](../02-Docker深度/)；K8s Secret / External Secrets → [03-12](../../03-Kubernetes/12-配置与密钥/)；云上 RAM / IAM → [06-04](../../06-云平台/04-IAM与多账号/)；GitOps 用哪个 tag 去 sync → [03-23](../../03-Kubernetes/23-GitOps-ArgoCD与Tekton/)；客户端资源包、CDN → [07-05](../../07-游戏运维专题/05-热更新与版本发布/) / [07-08](../../07-游戏运维专题/08-CDN与资源更新/)。不单开 Vault 章，密钥扫描和禁 latest 都在这里。
...
**怎么听这场分享**：先钉三条主线——锁内容、复制晋升、就近拉；第三节四个难点放慢；第七节用 `crictl pull` 把开头现场串回去。
...
> 🎯 **一句话**：制品是不可变快照；测过的是 digest，不是「同一份 Dockerfile」。

制品 = CI 产出的 **不可变快照**（镜像 / Chart / jar）。值班里第一次碰到，多半是 ImagePullBackOff、回滚找不到包、或「预发测过线上却不一样」——大家想想，根因往往在哪？多半不在带宽，在 **digest 没锁** 或 **每环境又 build 了一次**。
...
🔥「构建不确定」第一次听可能有点懵：意思是 **同一次 Git SHA，再 build 一次，打出来的字节也可能不一样**——基础镜像 tag 浮动、apk/apt 当时的包、`go build` 时间戳都会变。所以 staging 测过的和 prod 新 build 的 **不是同一个二进制**。故障时无法回答「预发验的就是线上这份吗」。

生产引用优先 `@sha256:...`。tag 用 `v1.2.3-<sha7>` 给人读。Harbor 复制规则或 CI `crane copy`。口误：`docker tag` 换仓库再 push **同一 digest** 才叫复制；`docker build` 再 push 叫重建。⭐ 这是面试必考的「复制 vs 重建」。

一次发布记录三件：**app digest + chart version + values 哈希**。怎么证明这次线上就是某次 MR：digest ↔ commit ↔ MR 链接，流水线留着。对不上就不要猜。

> ⭐ **记住**：测过的是 digest，不是「同一份 Dockerfile」。开服窗口第一次 pull 的必须已经在本机房 Harbor 里。口诀——**tag 给人读，digest 锁内容；复制不重建。**
...
> 🎯 **一句话**：CI 构建一次 → 同 digest 复制 → 节点就近拉；prod 没有箭头指回 build。

好，对着主图找三件事：有没有从 prod 指回 build 的箭头？复制是不是同一 digest？节点 DNS 指哪？
...
本节对应练习题 Q1、Q2——主图就是「一次构建多处部署」的白板答案。
...
有人说「代码一样不可能有差」——大家想想，Git SHA 相同再 build 一次，digest 一定一样吗？
...
回到开头那个战斗服现场：同一个 `logic:latest`，节点 imageID 两种 digest。有人说「再发一次 latest 回滚」——大家想想，尖上已经换了，上一版还在吗？
...
开服 200 节点超时——大家先问：镜像有多胖？策划表打进去了吗？本机房 Harbor 有没有预拉？
...
有人说「活动先开后补」——大家想想，门禁一旦有 skip 按钮，后面全是人情发布。
...
> 🎯 **一句话**：Harbor 不是私有 Hub——要 RBAC、复制、Proxy Cache、扫描、GC、OCI。
...
> 🎯 **一句话**：immutable tag 不是 digest 的替代；GC 前对照集群仍在用的 image。
...
> 🎯 **一句话**：到节点 `crictl pull` 同 digest，别只看 Deployment 事件。
...
> 🎯 **一句话**：拉不到镜像先 `crictl pull` 分流；已运行容器通常还在，别先重启逻辑服。

回到开头那些现场。三问：进程还在不在？还能不能变更？已有流量还通不通？
...
401 但 Secret 挂了：Secret 在错 ns、过期、CI 用的账号不能 pull 该 project。digest 不对：节点层缓存旧内容，删节点缓存再拉，先确认仓库里 sha256。现在再看群里那句 `logic:latest`、每环境 rebuild、「活动先开」——你知道该怎么拦住了。
...
- [ ] **口述贯通**：一次构建、digest 贯通怎么画？`docker tag` 和 `docker build` 谁叫复制？（对应练习题 Q1）
- [ ] **口述禁 latest**：生产为什么禁 latest？immutable 了还要不要 digest？（Q2、Q7）
- [ ] **口述瘦身**：多阶段后还胖查哪？策划表为什么不能打进业务镜像？（Q3）
- [ ] **口述 Harbor**：比私有 Hub 多哪几项？开服清单写哪句？（Q4）
- [ ] **口述拉失败**：上节点怎么分流？GC 前对什么？（Q6、Q12）
- [ ] **口述门禁与拆分**：Critical / 密钥进层 / Chart 三件怎么处理？热更和镜像怎么拆？（Q5、Q8、Q10、Q14）

六题都能口述，这一章就过了——开头那个 latest 混跑现场，你现在能对症下药了。下一章把门禁、晋升、金丝雀 SLI 剖开。

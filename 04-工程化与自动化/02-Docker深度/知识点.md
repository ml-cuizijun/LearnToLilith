...
> 大家好，欢迎来到这次分享。开讲之前，先带大家回到三个值班现场：
...
> 这一章讲完，你会知道这三件事各自错在哪：缓存为什么失效、PID 1 为什么收不到信号、盘满为什么不在镜像 200MB 里。
> **本章回答的问题**：镜像怎么叠、容器怎么隔离、盘满从哪来、信号为什么到不了进程；以及多阶段瘦身、BuildKit secret、镜像安全最低配。
> **本章不讲**：digest 晋升、Harbor、禁 latest、扫描门禁细节 → [03](../03-制品与镜像/)；流水线何时 build → [04](../04-CI-CD/)；containerd / CRI / pause / 节点升级 → [03-17](../../03-Kubernetes/17-节点运行时与升级/)；cgroup 明细、逃逸链 → [01-10](../../01-基础底座/10-cgroup与容器/)；K8s 探针、preStop、terminationGracePeriod → [03-04](../../03-Kubernetes/04-Pod生命周期/)。
...
**怎么听这场分享**：先钉三条主线——懂分层、看 PID 1、清磁盘；第三节四个难点放慢讲；第七节用决策树把开头三个现场串回去。
...
> 🎯 **一句话**：Docker 深度不是背指令表——是层怎么叠、PID 1 怎么收信号、盘满从哪来。

咱们先把口径对齐：Docker 深度不是背指令表，是**镜像怎么叠、容器怎么隔离、盘满从哪来、信号为什么到不了进程**。K8s 用的是同一套镜像格式，运行时换成 containerd 不改变分层这件事。
...
镜像不是一个大文件，是多层只读 tar 叠在一起。容器启动时在最上面加一层可写层（**Copy-on-Write / CoW** = 改下层文件时先拷到可写层再改，不直接改只读层）。容器**不是**一台小虚拟机——共享宿主机内核；隔离靠 **namespace**（看见什么），限额靠 **cgroup**（能用多少）。口诀先记：**namespace 管看见，cgroup 管能用**。

| 在镜像 / 容器里 | 不在本章、在别处 |
|-----------------|------------------|
| 层、Dockerfile、ENTRYPOINT/CMD、overlay2 upper | digest 晋升、Harbor 复制、禁 latest（03） |
| 单机 `docker run`、PID 1、BuildKit | kubelet / CRI / pause（03-17） |
| namespace 种类、cgroup 配额入口 | cgroup 文件排障链、逃逸（01-10） |
...
本节对应练习题 Q1、Q5——分层和「不是 VM」是整章地基。

---

## 2. 架构图

> 🎯 **一句话**：只读层可共享；upper 可写；PID 1 必须是业务（或 tini）。

好，概念钉住了。对着这张图问自己三个问题——代码层变了后面怎样？镜像 200MB 能解释盘满吗？PID 1 是谁？
...
回到开头第一个现场：改一行代码，构建从 40 秒变 8 分钟。很多人会答「网络慢」——先别急，看 build 日志哪步没 CACHED。
...
大家想想：进容器 `free` 显示 62Gi，你设了 `--memory=512m`——到底信谁？很多人信 free——错了。
...
六个常见 namespace：PID（进程号空间）、NET（网卡/端口）、MNT（挂载）、UTS（hostname）、IPC、USER（**很多生产默认没开**）。cgroup 管 CPU throttle、内存 OOM、pids fork 失败。Go `GOMAXPROCS`、Java GC 按容器内 `nproc` 配会被 throttle 死——详细文件路径去 01-10，K8s request/limit 去 03-04。
...
开头第三个现场：镜像 200MB，节点盘 100%。有人 `docker rm -f` 当清盘——大家想想，删容器就能把盘清回来吗？
...
CoW：改下层大文件会先拷到 upper 再改。容器里 `rm` 镜像带来的文件：只在 upper 记 **whiteout**，下层还在，镜像不会变小。删容器 ≠ 立刻收回镜像层；悬空镜像、build cache 要 prune。资源包走 volume / CDN，K8s imagefs / eviction 在 03-17。
...
开头第二个现场：滚动杀不掉。有人要加长 gracePeriod——大家想想，PID 1 是 sh 时，加长宽限期能根治吗？
...
5. 模拟停止（gracePeriod 30s 后 SIGKILL，见 03-04）：
...
> 📝 **面试怎么说**：⭐「滚动杀不掉我先 inspect Entrypoint/Cmd 看 PID 1 是不是 sh；生产用 JSON exec 形式；宽限期 30s 超时 SIGKILL 是 03-04，根因常在镜像。」
...
> 🎯 **一句话**：多阶段 + distroless/alpine + 非 root + 钉 digest；密钥走 secret mount。

原理四个难点讲完了，落到生产你怎么选镜像、怎么验收。
...
| 运行时 | 生产 K8s 用 containerd（03-17） | 业务 Worker 上 DinD 给不可信 PR |
...
> 🎯 **一句话**：COPY 顺序、exec 形式、secret mount、`.dockerignore`——改错最贵的几项。
...
> ⭐ **记住**：BuildKit 的 cache mount 加速、secret mount 保密钥；token 进层先 rotate。口诀——**token 进层先 rotate，删 tag 救不了 digest**。
...
> 🎯 **一句话**：值班四条——inspect PID 1 → logs → system df → history。
...
本节对应练习题 Q3、Q6、Q8、Q9——瘦身和安全最低配在这儿。
...
回到开头三个现场。三问：进程还在不在？还能不能变更？已有流量还通不通？**CrashLoop 先本地 `docker run` 同一镜像，再把锅甩给 kubelet。**
...
现场：`inspect` PID 1 → `logs` → `system df` → `history`。节点运行时是 03-17，本章停在镜像和单机行为。现在再看群里那句 `docker rm -f` 清盘、加长 gracePeriod、先怪 kubelet——你知道该怎么拦住了。
...
数字：800MB → slim **~150MB** → distroless 多阶段 **10–20MB**；gracePeriod 常见 **30s**（超时 SIGKILL 在 03-04）；开服 200 节点同时 pull 体积就是窗口。
...
学到这里，合上书试试——卡住的那一条，回到对应小节再听一遍：

- [ ] **口述分层**：某一层变了后面怎样？为什么 COPY requirements 要在 COPY . 前面？（对应练习题 Q1）
- [ ] **口述隔离**：六个常见 namespace 各隔离什么？cgroup 和 namespace 差在哪？`free` 可信吗？（Q5）
- [ ] **口述盘满**：overlay2 四元组？盘满先看哪？容器里 rm 镜像文件会怎样？（Q4）
- [ ] **口述信号**：ENTRYPOINT / CMD 谁被 docker run 换掉？PID 1 是 sh 会怎样？（Q2）
- [ ] **口述瘦身 / 密钥**：800MB 怎么瘦到 20MB？token 进层先做什么？（Q3、Q6）
- [ ] **口述安全与分流**：镜像安全最低配哪几条？只读根日志写哪？五个工单先走哪条？（Q9、Q10、Q14）

六题都能口述，这一章就过了——开头三个现场，你现在能对症下药了。下一章把 digest、禁 latest、Harbor 就近拉剖开。

...
现场：`inspect` PID 1 → `logs` → `system df` → `history`。节点运行时是 03-17，本章停在镜像和单机行为。
...
- [ ] 某一层变了后面怎样？为什么 COPY requirements 要在 COPY . 前面？
- [ ] 六个常见 namespace 各隔离什么？cgroup 和 namespace 差在哪？`free` 可信吗？
- [ ] overlay2 四元组？盘满先看哪？容器里 rm 镜像文件会怎样？
- [ ] ENTRYPOINT / CMD 谁被 docker run 换掉？PID 1 是 sh 会怎样？
- [ ] 800MB 怎么瘦到 20MB？token 进层先做什么？
- [ ] 镜像安全最低配哪几条？只读根日志写哪？

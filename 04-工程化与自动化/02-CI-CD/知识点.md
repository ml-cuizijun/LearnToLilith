...
**本章不讲：** workflow YAML、自托管 runner、fork PR 密钥 → [08](../08-GitHub-Actions与GitLab-CI/)；镜像分层、ENTRYPOINT、overlay2 → [07](../07-Docker深度/)；Harbor、扫描细节、禁 latest、digest 复制 → [03](../03-制品与镜像/)；GitOps、Argo selfHeal → [03-22](../../03-Kubernetes/22-GitOps-ArgoCD与Tekton/)；发布策略 YAML、Ingress 权重 → [03-15](../../03-Kubernetes/15-灰度与回滚/)；变更窗口、oncall 冻结 → [06](../06-运维平台与Oncall/)；分支、revert → [01](../01-Git工作流/)。
...
细节 YAML 在 [03-15](../../03-Kubernetes/15-灰度与回滚/)。本章只要求选型理由和回滚动作。

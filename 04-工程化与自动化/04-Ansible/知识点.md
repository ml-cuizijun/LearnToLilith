...
一条 playbook 打了一千台登录服，配错了还在继续。第二次跑仍然一堆 changed，有状态服被无脑 restart。群里有人 `hosts: all`，inventory 混着 prod；另有人把 forks 提到 100，sshd 和玩家抢 fd——活动高峰登录成功率掉成一条线。
...
**本章不讲：** State、锁、plan destroy → [05 Terraform](../05-Terraform/)；镜像 / 制品晋升 → [03](../03-制品与镜像/)；集群内应用发布 → [03-22](../../03-Kubernetes/22-GitOps-ArgoCD与Tekton/) / [02](../02-CI-CD/)；变更窗口、审批、oncall → [06](../06-运维平台与Oncall/)；云上安全组本源 → [06 云平台](../../06-云平台/)；密钥扫描细节 → 03。

---

## 1. 基本概念

Ansible **无 Agent**：控制节点 SSH push，目标机有 Python + SSH 即可。它保证的是「机器处于某状态」，不是「再执行一遍脚本」。值班里第一次碰到，多半是 playbook 打偏 prod、第二次巡检全 changed、或逻辑服被 restart 踢掉长连接。
...
**一句话：** SRE 考的是 `--list-hosts` → limit 1 台看 SLI → `serial 10%` + `max_fail_percentage: 0`——配置本身错，百分比救不了。

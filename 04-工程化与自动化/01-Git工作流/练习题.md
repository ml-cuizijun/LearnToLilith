...
开始做题前，先给大家一个小建议：合上知识点，对着**本章主线**（分历史 → 选对路 → 留审计）口述一遍。关键字（Trunk、revert、hotfix、cherry-pick、bisect、reflog、force push）要能点名。能顺下来，下面这些题你会发现大半都会了；卡壳的地方，就是你该回去重读的那一节。
...
题目的顺序也是有意排的：Q1～Q3 先钉模型与合入/回退主线；Q4～Q6 是值班三板斧；Q7～Q13 展开保护与审计；Q14 收尾 60 秒应急。
...
- [Q10 冲突 / 热更和 CI 拆](#q10)
- [Q11 分支保护与 CODEOWNERS](#q11)
- [Q12 signed commits](#q12)
- [Q13 submodule 陷阱](#q13)
- [Q14 60 秒 Git 应急](#q14)
- [合上书](#合上书)
...
先自己试试：不看下面，你能说出服务端为什么不爱 GitFlow、活动窗口 login 修复从哪拉吗？口述完再对答案。
...
1. **持续交付、服务端热更** → Trunk-based：短分支 + CI 门禁 + feature flag。要求 CI 小于 10 分钟、回滚 小于 5 分钟。
...
就是知识点开头那个现场：活动中登录成功率掉了，坏 commit 已经在 main 上，Jenkins 和 Argo 都按这个 SHA 在跑。有人准备 `reset --hard` 再 force push。你会怎么拦住他？

先自己试试：先问哪一句？revert 之后 GitOps 还要改什么？

### 完整解答

> 🎯 **一句话**：已在 main / CI / Argo 上的 commit 只能 revert 走流水线；GitOps 回滚必须改 Git。

这是 Q1「主干随时可发」的另一面——能发也要能退，而且只能追加退。
...
先自己试试：hotfix 从哪拉？能关 flag 还要不要出新包？

### 完整解答

> 🎯 **一句话**：生产 hotfix 从 origin/main 拉短分支，走同一条 CI 出 digest；能关 flag 就先关。

这是 Q2 结论的「正向修复版」：不能 force，就走同一条流水线出新制品。
...
先自己试试：80 个 commit 大约几次？什么时候 bisect 会指到无辜提交？

### 完整解答

> 🎯 **一句话**：bisect 用可重复脚本 log₂(n) 次定位；根因在配置/下游时 bisect 会误导。

Q4 对齐了拐点之后，根因不清就靠 bisect——别靠群里猜。
...
先自己试试：新 clone 里有没有你的 reflog？已 push 的还能不能靠它救？

### 完整解答

> 🎯 **一句话**：未 push 的误 reset 用本地 reflog 救；已 push 的靠远程/托管，不要 force 擦历史。

注意和 Q2 区分：这里是**未 push** 才 reset；已上 main 走 Q2 的 revert。
...
先自己试试：force 之后 CI 和 Argo 会怎样？验收分支保护要试哪几招？

### 完整解答

> 🎯 **一句话**：force push main 会断 CI/Argo 审计链并制造分叉；主分支保护是运维要求。

就是 Q2 / Q3 那位同学踩的坑的「制度解」——用分支保护把 force 物理挡掉。
...
先自己试试：没有规范时至少强制 PR 模板写什么？digest 怎么反查 MR？

### 完整解答

> 🎯 **一句话**：conventional commits 的价值是让 commit 能当变更审计，驱动 changelog 和 semver。

接上 Q7 的审计链：保护拦住 force，message 让 digest ↔ MR 对得上。
...
先自己试试：有 flag 时谁更快？关开关还要不要走变更单？

### 完整解答

> 🎯 **一句话**：有 flag 时关开关秒级止血；revert 走流水线能拿走代码，但比关开关慢。

和 Q2 的 revert 互补：能关 flag 就先关——比出新制品快。
...
先自己试试：冲突能不能 Accept Incoming？四条变更各走哪条路？

### 完整解答

> 🎯 **一句话**：服务端走 Git/CI 镜像，客户端整包走 release 线，数值/超时走 CDN/Ansible，禁止跳板机 vim。

冲突解法接 Q3；四条路拆分接 Q1「服务端别被客户端拖死」。
...
**★★★★☆｜main 分支保护和 CODEOWNERS 怎么挡误合？**

**覆盖**：必会 · 常考 ｜ **对应**：知识点 [三、原理](知识点.md)

### 场景

周五有人绕过 review 合了破坏性迁移。

先自己试试：主分支保护要开哪几项？hotfix 能不能 break glass？

### 完整解答

> 🎯 **一句话**：required reviews、status checks、CODEOWNERS 路径；hotfix 也要最小两人规则。

这是 Q7 的落地细节——保护规则写进托管，不是写进 wiki。

```text
/prod/**  @sre @dba
require 2 approvals + CI green
```

### 再追问

hotfix 能 break glass 吗？——可以但要审计+事后补 review（Q4）。

---

## Q12

**★★★☆☆｜signed commits 解决什么问题？和 CI 门禁怎么配？**
...
先自己试试：signed commits 解决什么问题？rebase 后签名还有效吗？

### 完整解答

> 🎯 **一句话**：GPG/SSH sign + branch rule 要求 verified；CI 验证 tag 签名。

和 Q8 同属审计链加分项：message 可追溯，签名可验作者。

`git log --show-signature`

### 再追问

rebase 后签名还有效吗？——每个 commit 重签，注意工具链。

---

## Q13

**★★★☆☆｜submodule 指针忘了推，CI 克隆主仓却编不过，怎么防？**

**覆盖**：常用 ｜ **对应**：知识点 [三、原理](知识点.md)

### 场景

主仓 CI green，clone 带 submodule 的 job 红。

先自己试试：指针忘了推会怎样？submodule 适合放什么、不适合放什么？

### 完整解答

> 🎯 **一句话**：CI 显式 `submodule update`；禁止静默 `--recurse-submodules` 漏指针；锁 commit SHA。

和 Q3 冲突解完 CI 还红是同一类——漏文件 / submodule SHA 没更。
...
submodule 适合放什么？——独立版本化的协议/工具仓，**不要**放频繁变的配置。

---

## Q14

**★★★★★｜main 被 force push、误 merge、hotfix 要双合，60 秒你怎么救？**

**覆盖**：核心 · 必会 ｜ **对应**：知识点 [三、原理](知识点.md)

### 场景

凌晨 main 历史被改写；生产要修但 develop 也要跟。这是前面十多道题的综合急救。

先自己试试：60 秒你先救 force、误 merge 还是 hotfix？GitOps 环境能 reset main 吗？

### 完整解答

> 🎯 **一句话**：force→reflog+恢复分支；误 merge→revert；hotfix→main + cherry-pick 回开发线（Q4）。

这张处置把 Q2 / Q4 / Q6 / Q7 串起来了。
...
GitOps 环境能 reset main 吗？——**禁止** force main（Q7）；用 revert 留审计。

---

## 合上书

和知识点「合上书」逐条对齐——能做到，这章才算真的听懂了：

- [ ] **口述模型**：能画出 Trunk-based / GitHub Flow / GitFlow，并说出服务端热更为什么不爱 GitFlow（Q1｜知识点 §3.1）
- [ ] **口述合入**：能说出 rebase / merge / squash 各何时用，已推送公共分支禁止 rebase（Q3｜§3.2）
- [ ] **口述回退**：能区分 revert vs reset，GitOps 回滚必须改 Git（Q2｜§3.3）
- [ ] **口述 hotfix**：能从 `origin/main` 拉 hotfix，cherry-pick 对准合入后 SHA（Q4｜§6.2）
- [ ] **口述定位 / 救人**：能跑 bisect（含 skip / 观测滞后），能用 reflog 救未 push 的 reset（Q5、Q6｜§6.3、§6.4）
- [ ] **口述保护**：能说清禁止 force main 的运维理由（CI 缓存 / Argo / 审计）（Q7｜§4）
- [ ] **口述拆分**：能把服务端热更、客户端整包、数值/超时拆到 Git/CI、release 线、CDN/Ansible，而不是跳板机 vim（Q10、Q14｜§3.4）

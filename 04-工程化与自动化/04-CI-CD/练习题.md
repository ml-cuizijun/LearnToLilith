...
开始做题前：合上知识点，对着**本章主线**（分阶段 → 锁 digest → 卡闸门）口述一遍。关键字（Delivery、digest 晋升、门禁、金丝雀 SLI、GitOps）要能点名。

每道题**先看场景自己口述，再往下看解答**。每题标了覆盖范围：
...
题目顺序：Q1～Q3 钉分段与 digest；Q4～Q8 落地与加速；Q9～Q11 业务红与改造；Q12～Q14 供应链与综合。
...
先自己试试：游戏生产用 Delivery 还是 Deployment？什么情况下才考虑全自动？

### 完整解答

> 🎯 **一句话**：游戏生产默认 Continuous Delivery：CI 测完随时可发，但上 prod 仍要审批和窗口。
...
先自己试试：每段失败怎么停？哪一步最该看业务指标？

### 完整解答

> 🎯 **一句话**：流水线每段失败都有明确停点：测/扫/冒烟/金丝雀 SLI 红则阻断，全量成功才审计 digest。

这是 Q1 分段的「画出来」版——白板题就考这张图。
...
预发测了两天。生产流水线又 `docker build` 了一次，晚上支付挂了。两边 Git SHA 一样。就是知识点开头那个现场。

先自己试试：怎么证明 prod 就是预发那份？Git SHA 相同够不够？

### 完整解答

> 🎯 **一句话**：staging 和 prod 必须是同一 digest；每环境再 build 等于预发结论作废。

和上一章 Q1 同一铁律——本章强调「晋升纪律写进流水线」。
...
先自己试试：Pending 先看哪四样？Master 为什么不能跑 build？

### 完整解答

> 🎯 **一句话**：Jenkins Master 只调度（numExecutors=0），Agent 干活；Pending 先查 label 不是加机器。
...
先自己试试：哪几道是硬阻断？health 200 够不够？

### 完整解答

> 🎯 **一句话**：Critical CVE、业务 SLI 等硬阻断写进流水线且不可 skip；/health 不够当游戏门禁。

接 Q2 的停点——扫描和金丝雀最容易被人情拆掉。
...
先自己试试：回滚前问哪三件事？expand/contract 是什么？

### 完整解答

> 🎯 **一句话**：回滚前对齐时间/digest，问 schema 能否倒、旧 digest 是否还在；GitOps 要 revert Git。

金丝雀红了走 Q5 的自动回滚路径——但 schema 不可逆时这条路走不通。
...
先自己试试：CI 两套可以，CD 为什么只能一条？

### 完整解答

> 🎯 **一句话**：CI 可以两套，CD 只留一条：构建改 Git，Argo 唯一落地，禁止多路 kubectl apply。

和 Q6 GitOps 回滚同一条纪律——多路发布会抢。
...
先自己试试：先砍什么？和 Q4「先别加机器」是同一类思路吗？

### 完整解答

> 🎯 **一句话**：流水线慢先看最慢 stage：缓存、并行、path filter，不要先堆 Agent 数量。
...
部署 Job 成功，Pod Ready。支付成功率从 99.9% 掉到 80%。健康检查一直绿。就是开头那个「全绿但业务红」现场。

先自己试试：对齐哪三件？门禁缺了什么？

### 完整解答

> 🎯 **一句话**：Pipeline green 只说明探针过了；要对齐 deploy 时间、指标拐点和 digest 再决定是否回滚。

就是 Q5「health 不够」的实战版。
...
先自己试试：资源不够蓝绿怎么办？滚动时协议要注意什么？

### 完整解答

> 🎯 **一句话**：滚动省资源但要协议兼容；蓝绿要双倍资源；高风险默认金丝雀 + 业务指标。
...
先自己试试：DORA 四个数字是什么？四条变更各走哪条？

### 完整解答

> 🎯 **一句话**：改造用 DORA 四数字说话；服务端镜像、客户端包、数值/超时必须拆到不同通道。

接 Q1 的 Delivery 口径——改造要有数字，通道要拆开。
...
**★★★★☆｜SBOM、扫描、签名三道供应链门禁怎么焊死？**

**覆盖**：必会 ｜ **对应**：知识点 [5. 质量闸门](知识点.md)

### 场景

安全要求 merge 前必须有 SBOM+无 Critical。

先自己试试：误报能口头 skip 吗？

### 完整解答

> 🎯 **一句话**：CI 生成 SBOM；扫描 fail on critical；cosign sign 后才进 prod 库；**不可 skip**。

就是 Q5 门禁的供应链加强版——仍不可 skip。

```groovy
stage('Scan') { when { branch 'main' } ... }
```

### 再追问

扫描误报能 waive 吗？——要 ticket+过期，**不要**口头 skip（Q5）。

---

## Q13

**★★★★☆｜开服冻结期 CI 还能自动 deploy 吗？**

**覆盖**：必会 ｜ **对应**：知识点 [三、原理](知识点.md)

### 场景

活动前冻结，Argo 仍 auto-sync。

先自己试试：冻结期还能自动 deploy 吗？这和 Q1 Delivery 人闸什么关系？

### 完整解答

> 🎯 **一句话**：CI 人工 gate + Argo sync window；冻结只放行 hotfix 流水线。

staging 自动、prod 手动 = Delivery（Q1）——冻结是人闸的强化版。

链 [08](08-运维平台与Oncall/) Q2、[23](../../03-Kubernetes/23-GitOps-ArgoCD与Tekton/)。

### 再追问

staging 自动、prod 手动算 CI 还是 CD？——Delivery：自动到预发，人点生产（Q1）。

---

## Q14

**★★★★★｜队列堵、绿但业务红、回滚失败，60 秒 CI 定责**

**覆盖**：核心 · 必会 ｜ **对应**：知识点 [7. 生产排障](知识点.md)

### 场景

Jenkins 全 Pending；pipeline green 但 5xx；rollback job 找不到 digest。60 秒定责，把前面的题串起来。

先自己试试：三件事各先查什么？

### 完整解答

> 🎯 **一句话**：Pending→agent label；绿红→晋升错 digest/没冒烟；回滚→问 schema+旧 digest（Q6）。

链 Q3/Q4/Q6/Q9——开头事故现场的急救卡。
...
每环境 rebuild 一次为什么禁止？——digest 变了，staging 测过无意义（Q3）。
...
- [ ] **口述分段**：能说出 CI / Delivery / Deployment，游戏生产默认 Delivery（Q1｜知识点 §3.1）
- [ ] **口述流水线**：能画出从 Git 到全量的分段，每段失败怎么停（Q2｜§2）
- [ ] **口述晋升**：能证明 staging 与 prod 同一 digest，拒绝每环境再 build（Q3｜§3.2）
- [ ] **口述门禁**：能设不可 skip 的闸门：测 / 扫 / 冒烟 / 金丝雀业务 SLI / 窗口（Q5｜§3.3）
- [ ] **口述 Jenkins**：能讲四块：Master=0、凭证、弹性 Agent、队列先查 label（Q4｜§4）
- [ ] **口述回滚**：能回滚前问 schema / 缓存 / 旧 digest；GitOps 改 Git（Q6｜§6）
- [ ] **口述拆分与改造**：能把服务端镜像、客户端包、CDN 热更、Ansible 配置拆开，并用 DORA 讲改造（Q11、Q14｜§3.4、§8）

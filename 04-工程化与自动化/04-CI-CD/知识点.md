...
> 大家好，欢迎来到这次分享。开讲之前，先带大家回到一个发布事故现场：
...
> 这一章讲完，你会知道当时那个人错在哪：为什么 CI 绿不等于玩家能登录、为什么不能每环境 rebuild、金丝雀为什么不能只看 `/health`。
> **本章回答的问题**：门禁怎么卡、制品怎么晋升、金丝雀看什么、失败怎么回滚、队列 Pending 先查哪；CI / Delivery / Deployment 分段，是为了让你动手时不把游戏生产做成无脑 Deployment。
> **本章不讲**：workflow YAML、自托管 runner、fork PR 密钥 → [05](../05-GitHub-Actions与GitLab-CI/)；镜像分层、ENTRYPOINT、overlay2 → [02](../02-Docker深度/)；Harbor、扫描细节、禁 latest、digest 复制 → [03](../03-制品与镜像/)；GitOps、Argo selfHeal → [03-23](../../03-Kubernetes/23-GitOps-ArgoCD与Tekton/)；发布策略 YAML、Ingress 权重 → [03-16](../../03-Kubernetes/16-灰度与回滚/)；变更窗口、oncall 冻结 → [08](../08-运维平台与Oncall/)；分支、revert → [01](../01-Git工作流/)。
...
**怎么听这场分享**：先钉三条主线——分阶段、锁 digest、卡闸门；第三节四个难点放慢；第七节用决策树回收开头现场。
...
> 🎯 **一句话**：CI 管能不能合、能不能出包；CD 管同一份包怎么往上走——绿不等于玩家能登录。
...
🔥 这三个词第一次容易混：CI 是「出厂质检」；Delivery 是「合格品进冷库，出库要签字」；Deployment 是「全自动出库」——游戏生产默认停在 Delivery。
...
DORA 四个数字用来讲改造：部署频率、变更前置时间（lead time）、变更失败率、MTTR。讲改造时四个数字都要有前后：例如 lead time 2 天→2 小时，失败率、MTTR **小于 5 分钟**。没有数字就不要说「提升了效率」。失败率 = 需人工介入的发布 / 总发布。

> ⭐ **记住**：Pipeline green 只说明阶段脚本成功，不等于玩家能登录。staging 测过的镜像必须原样上 prod，禁止每个环境再 `docker build`。门禁一破口，后面全是人情发布，故障不可审计。口诀——**绿 ≠ 能登录；测过的是 digest，不是同 Dockerfile。**
...
> 🎯 **一句话**：Git → CI → Harbor digest → staging → 人闸 → 金丝雀 → 全量；prod 没有箭头指回 build。

好，对着主图找三件事：有没有从 prod 指回 docker build？扫描 Critical 能不能 skip？金丝雀红了往哪回滚？
...
有人说「合并 main 就上生产更 SRE」——大家想想，下周开活动、逻辑服还要改表，全自动合适吗？
...
回到开头现场：staging `sha256:abc`，prod 又 build 出 `sha256:def`。有人说「prod 基础镜像新一点更安全」——这是典型事故。
...
口诀：**同 SHA ≠ 同包；晋升只复制 digest。**（细节见 [03](../03-制品与镜像/)）
...
开头有人要 skip 扫描「活动先开」，金丝雀只看 `/health`——大家想想，health 200 时充值挂了，流水线会红吗？
...
有人把改 nginx 超时也塞进镜像 CI——大家想想，值得吗？
...
> 🎯 **一句话**：游戏 CD 用 Delivery；Jenkins Master 不干活；队列先查 label 再加机器。
...
> 🎯 **一句话**：改错最贵的是 skip 门禁和 prod 再 build。
...
细节 YAML 在 [03-16](../../03-Kubernetes/16-灰度与回滚/)。本章只要求选型理由和回滚动作。
...
活动高峰：确认是这次发布 → 停发版 → 回滚上一 digest → 恢复后先看登录/支付，再补自愈。现在再看群里那句 skip 扫描、prod 再 build、只看 `/health`——你知道该怎么拦住了。
...
- [ ] **口述分段**：CI、Delivery、Deployment 差在哪？游戏生产为什么默认停在人闸上？（对应练习题 Q1）
- [ ] **口述晋升**：为什么 staging 测过必须同一 digest 上 prod？怎么证明？（Q3）
- [ ] **口述门禁**：哪几道闸门硬阻断？扫描和金丝雀怎样被人情拆掉？（Q5、Q9）
- [ ] **口述 Jenkins**：四块红线？队列 Pending 先看什么？（Q4）
- [ ] **口述回滚**：回滚前问哪三件事？expand/contract 是什么？GitOps 怎么回？（Q6）
- [ ] **口述拆分与改造**：服务端镜像、客户端整包、数值热更、改超时各走哪条？DORA 四个数字？（Q10、Q11、Q14）

六题都能口述，这一章就过了——开头那个「全绿但登录不了」的现场，你现在能对症下药了。下一章把 GHA / GitLab YAML 和 runner 剖开。

...
**一句话**：Thanos：Sidecar 上传 2h block，Query 去重，Compactor 做 compact + downsample。
...
**一句话**：高基数先炸 Prometheus ingest，Thanos 救不了；万 Pod 分片 Prom，约 200 万 series / 实例。
...
**一句话**：Istiod 控制面发 xDS，Envoy sidecar 数据面转发；南北向仍走 Gateway / Ingress。
...
**一句话**：sidecar 每 Pod 多一跳 + Istiod 容量；战斗服默认不注入，VS 不替代 WAF。
...
**一句话**：集群独立用 GitOps 每集群 Application；要统一调度才 Karmada；KubeFed 不现行。
...
**一句话**：Deployment 无身份不适合战斗；STS 有名 / 盘 / DNS 但不是游戏热更；阶梯到 GameServerSet。
...
**一句话**：OpenKruise 原地升级 / SidecarSet；GameServerSet 房间级策略，没做过不要写精通。
...
**一句话**：Device Plugin 报 nvidia.com/gpu；游戏 / 推理 / 训练三池分开，先污点不自研调度。
...
**一句话**：高基数先 Thanos、战斗全注入、默认 Karmada、GPU 混战斗节点都是反模式。
...
**一句话**：架构面先问规模 / SLO / 团队 / 已有栈，再画图说清不选什么。

...
> 这一章讲完，你会知道当时缺的是哪一只眼，以及三件套怎么互相跳转。
> **本章回答**：Metrics / Logging / Tracing 三件套各回答什么问题、怎么互相跳转、什么时候上哪个组件，以及游戏/后端场景怎么选型。
> **不讲**：PromQL 语法细节 → [02](../02-Prometheus与PromQL/)；告警阈值/降噪 → [03](../03-告警设计与降噪/)；日志采集链路搭建 → [04](../04-日志链路与采集/)；trace span 传播与采样 → [05](../05-链路追踪与OpenTelemetry/)；USE/RED 本体 → [01-21](../../01-基础底座/21-性能方法论-USE与RED/)；主机资源命令 → [01-06](../../01-基础底座/06-CPU与Load/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节「一次故障被看见的一生」，把 Metrics 发现 → Logging 定性 → Tracing 定位装进脑子；后面按三件套各自能答什么、怎么跳转、怎么选型展开；作战节就是开头「充了钱不到账」的正确剧本。每一节讲完都会提示对应练习题。
...
大家想想：Metric 全绿就等于没事吗？很多人会答「绿了就结单」——**错了**。充值不到账这种，往往要日志和链路才能定性定位。
...
口诀：**「Metrics 看面」**。练习题 Q2～Q5。面看清了，Logging 定性。
...
口诀：**「Logging 看点」**。练习题 Q6。点清楚了，Tracing 定位。
...
口诀：**「Tracing 看链」**。练习题 Q7、Q8。链有了，三件套跳转。
...
⭐ trace_id / exemplar。练习题 Q9。跳转通了，选型。
...
**回到开头那个现场**：Metric 全绿、日志翻不到、链路没 ID。正确剧本是：Metrics 发现面 → Logging 带 trace_id 定性 → Tracing 定位 hop → exemplar 互跳。现在再看「有大屏就是可观测性做完了」，你知道该怎么纠正了。

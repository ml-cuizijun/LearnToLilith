...
> **不讲**：主机侧 `logrotate`/`journald`/`grep` 实操 → [01-23](../../01-基础底座/23-日志运维/)；K8s 日志落地与轮转接线 → [03-20](../../03-Kubernetes/20-日志管理/)；ES 本体运维 → [02-04](../../02-中间件与数据库/04-Elasticsearch/)；Kafka 本体 → [02-03](../../02-中间件与数据库/03-Kafka/)；trace_id 生成与传播 → [05](../05-链路追踪与OpenTelemetry/)；代码侧日志规范 → [05-02-05](../../05-开发能力/02-Python/05-logging日志规范/)。

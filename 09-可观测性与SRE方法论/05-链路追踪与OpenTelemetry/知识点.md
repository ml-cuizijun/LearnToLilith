...
> 这一章讲完，你会知道当时缺的是哪张「请求路线图」，以及怎么从 P99 一路跳到慢 span。
> **本章回答**：一个跨服务请求从进门到回家，trace/span/context 怎么一路跟下来，采样、看图、定责怎么落地。
> **不讲**：指标聚合口径与 PromQL → [02](../02-Prometheus与PromQL/)；日志采集存储与 trace_id 打日志 → [04](../04-日志链路与采集/)；SLO 与错误预算怎么管发布节奏 → [06](../06-SLI-SLO与错误预算/)；Service Mesh 无侵入埋点实现 → [03-24](../../03-Kubernetes/24-ServiceMesh-Istio与Envoy/)；eBPF 无侵入可观测 → [03-25](../../03-Kubernetes/25-eBPF与Cilium/)。可观测三件套全局选型 → [01](../01-可观测性三件套与选型/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节「一个跨服务请求的一生」，把进门 → 上路 → 沿途 → 取舍 → 回家 → 看图装进脑子；后面一站一站展开采样、断链、瀑布图定责；作战节就是开头「P99 涨了却不知道慢在哪」的正确剧本。每一节讲完都会提示对应练习题。
...
练习题 Q1、Q14。主线有了——先从进门生成 trace_id。
...
口诀：**「一棵树一个 trace_id，每跳一个 span_id」**。练习题 Q1、Q2。户口本有了，该上路传接力棒。
...
大家想想：同步 HTTP 明明能跟到，一进线程池/Kafka 怎么就断了？很多人会答「中间件不支持 tracing」——**不全对**。大白话：接力棒不会自己跳栏，弯道（异步）必须**主动交棒**。
...
⭐ 面试必考：`traceparent` 四段；异步/MQ 最易断链。练习题 Q3、Q4、Q5。传过去了，span 怎么拼树。
...
⭐ 这是面试必考的定责口诀：**CLIENT 长看网络/队列，SERVER 长看服务自身**——别一上来就重启下游。
...
口诀：**「CLIENT 长看网络，SERVER 长看服务」**。练习题 Q6。树拼好了，谁进相册看采样。
...
🔥 很多人会说「网关采 10%、下游再采 10% 更省」——这是 ⭐ 高频错答。口诀：**「采样决策全链一致，半截树比没树更误导」**。
...
⭐ 高频错答：各服务独立随机采样。练习题 Q7、Q8。取舍定了，数据怎么回家。
...
大家想想：换 Jaeger 要不要改业务埋点？很多人会答「要重写」——**错了**。OTel 管的是「怎么采」，Jaeger/Tempo 管的是「怎么存怎么看」——一次埋点，后端可换。
...
⭐ exemplar / trace_id 跳转。练习题 Q11。看图之外，生病怎么认。
...
**回到开头那个现场**：网关 P99 到 800ms，日志一地，却不知道慢在哪一跳。正确剧本是：exemplar → 完整 trace → 比 CLIENT/SERVER → 带 trace_id 下钻日志。现在再看「重启全部逻辑服」，你知道该怎么拦住他了。

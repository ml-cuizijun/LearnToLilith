...
**本章不讲：** 通用调度、污点 → [04-06 调度机制](../../04-Kubernetes/06-调度机制/)；节点 NotReady 驱逐 → [04-16 节点运行时与升级](../../04-Kubernetes/16-节点运行时与升级/)；模型怎么选、窗口 → [01](../01-LLM与Transformer基础/)；应用侧 RAG 打推理 → [03](../03-RAG检索增强/)；网关后的 Agent → [06](../06-Agent智能体/)；告警降噪不是 GPU 课 → [09](../09-AIOps落地/)。不训模型。

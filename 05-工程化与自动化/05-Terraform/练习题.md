...
先对照 [知识点](知识点.md) **本章主线** 自己口述，再对答案。关键字（state、lock、漂移、for_each、import）要能点名。每题标了覆盖范围：
...
- [ ] 能画出 HCL / state / 云 API 三方和四种 diff
- [ ] 能说出远程 + 锁 + versioning；丢了会双创；prod / staging 分 key
- [ ] 能区分 refresh / state rm / destroy；锁残留确认没人再 unlock
- [ ] 能讲漂移：detailed-exitcode 2、拉回或改 HCL、紧急控制台事后 import
- [ ] 能区分 count vs for_each，合服只许一个 key；module pin 版本
- [ ] 能说 TF 管集群外、Helm 管集群内；import 分批 plan 到 0
- [ ] 能用分流图处理 destroy / lock / 误 create，而不是 auto-approve

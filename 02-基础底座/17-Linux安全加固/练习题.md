...
**一句话**：加固是入口（堡垒机 + key only）→ 权限（sudo 白名单）→ 发现（auditd 离机）三层，改端口不是核心。
...
改 `sshd_config` 先 `sshd -t`，再 **reload**。语法错 + restart 可能把自己锁在外面——所以才要堡垒机 / 云控制台当逃生口（[25](../25-堡垒机与跳板机/)）。防火墙必须同步新端口。
...
**一句话**：默认 DROP + ESTABLISHED 最前 + SSH 只放堡垒 CIDR；云安全组是第一层，主机防火墙是第二层。
...
云安全组是第一层，主机防火墙是第二层。链细节在 [15](../15-iptables与conntrack/)。不要只开 syncookies 就说防火墙做好了。
...
**一句话**：auditd 是内核审计不可清；AIDE 只在干净安装时 init，补丁后更新基线不要 `--init`。

`history` 用户可清、可改。**auditd 是内核审计**：`-w /etc/sudoers -p wa` 对写入一定有事件。生产 = 规则 + **日志离机**。本机 audit.log 不够，root 能改（[22](../22-日志运维/)）。
...
安全 sysctl 改哪些？——redirects=0、source_route=0、syncookies=1、非路由器 ip_forward=0、rp_filter=1。连接队列在 [18](../18-内核参数与sysctl/)。
...
扫描过了不是 IDS。还要靠 AIDE、异常登录、离机审计、堡垒机（[25](../25-堡垒机与跳板机/)）。
...
- [ ] 能说出三层是什么；为什么改端口不是核心
- [ ] 能默写 sshd 必关项；改完为什么 reload 而不是 restart
- [ ] 能写 sudo 白名单并禁 su/bash；visudo 写错怎么逃
- [ ] 能说出防火墙默认 DROP、SSH/业务口来源收窄、出站也管
- [ ] 被 SELinux 拦了先跑哪条命令；发现入侵第一步

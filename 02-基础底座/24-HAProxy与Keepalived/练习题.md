...
Nginx 是 Web 全能。HAProxy 是 **LB 本职**：四层性能和健康检查更细，Stats 看得清会话。库、游戏 TCP、纯转发选 HAProxy；站点、静态、复杂 HTTP 选 Nginx。内核超大入口选 LVS（16），不要在这章讲 DR。
...
两节点 VRRP **没有多数派**（20）。心跳看不见对方就会双主。防火墙常把 112 当「UDP 112 端口」丢掉。
...
5. 脚本无执行权限、SELinux 拦（17）。

进程在但不 listen：要探端口/HTTP，不要只探 PID。

### 再追问

rise/fall 在 HAProxy 和 Keepalived 各管什么？——HAProxy：摘不摘 **后端 server**。Keepalived：这台还要不要 **VIP**。两层都要连续 N 次，抗抖（20）。
...
新建办公隧道：**WireGuard**（内核、配置短）。已有 OpenVPN 或必须 TCP 443 穿透：OpenVPN。**玩家加速用云 GAAP 一类产品**，不是把战斗包送进办公 VPN。跨云专线是 07。

### 再追问

WireGuard 密钥怎么管？——每端一对；Peer 的 AllowedIPs 收紧。私钥当生产密钥，进堡垒/密管，不要散落 wiki（25、17）。
...
SOCKS 和 HTTP 代理？——HTTP 代理管 HTTP(S)。SOCKS5（Dante）更通用。SSH `-D` 也是 SOCKS（25），临时排障可以，不是生产出网方案。
...
超大规模内核转发、三种模式、real server 配 RIP → 16 LVS。用户态代理、七层 ACL、精细 HTTP 检查、库端口 → 本章 HAProxy。VIP 漂移、VRRP 112、双主 → 本章 Keepalived（LVS 也常用它管 ipvs，模式仍在 16）。
...
超时只抄 defaults 30s 行不行？——不行。外短内长和 23 同一条链。游戏 TCP 还要加大、必要时 `timeout tunnel`。

---

## 合上书

- [ ] 能画出 VIP + 双机 HAProxy/Keepalived，并分层 HTTP / TCP / MySQL
- [ ] 能说出 VRRP 协议 112、GARP、priority 才是谁主
- [ ] 能处理双 VIP：先停一台，再抓包；能对比 etcd quorum
- [ ] 能解释 VIP 不漂：weight、脚本、只探 PID、nopreempt
- [ ] 能配 httpchk / tcp-check / 读写拆端口，并识假活
- [ ] 能选 WireGuard，拒绝玩家走办公 VPN；正向代理写 no_proxy
- [ ] 能把 LVS / HAProxy / Keepalived 三句话拆开而不搅 DR

...
先对照 [知识点](知识点.md) **本章主线** 自己口述，再对答案。关键字（LISTEN/ESTAB 两个 Q、backlog、TIME_WAIT、conntrack）要能值班。每题标了覆盖范围：
...
**一句话**：LISTEN 的 Recv-Q 是未 accept 连接数，ESTAB 的 Recv-Q 是未读字节——不是一回事。
...
**一句话**：TIME_WAIT 在主动关闭方用连接池治；CLOSE_WAIT 是被动方没 close，改代码。
...
**一句话**：conntrack 满只挡新建连接，旧长连接正常——「新挂旧不挂」是它的签名。
...
**一句话**：半连接满像丢包，全连接满 Recv-Q≈Send-Q；有效 backlog=min(listen,somaxconn)。
...
**一句话**：fd 耗尽看进程 limits，端口耗尽看本机出站四元组——网关 CCU 四者一起算。
...
**一句话**：ESTAB Recv-Q 持续增长是应用没 read，先看线程/GC，不要先调 TCP 窗口。
...
**一句话**：somaxconn 默认 128 生产要改，容器里 somaxconn 是命名空间自己的值。
...
**一句话**：新挂旧不挂优先 conntrack/表类故障，安全组误改通常新旧一起死。
...
**一句话**：tcp_tw_reuse 依赖 timestamps=1，仍优先应用层连接复用。
...
**一句话**：%si 满是包还在网卡层，Recv-Q 涨是包已进 socket 应用慢——分工不同。
...
- [ ] LISTEN Recv-Q 和 ESTAB Recv-Q 各是什么？加大 rmem 能治哪一个？
- [ ] 有效 backlog 怎么算？只改一边为什么无效？容器里 somaxconn 在哪？
- [ ] TIME_WAIT 和 CLOSE_WAIT 谁在主动关、谁忘了 close？recycle 还能提吗？
- [ ] conntrack 满什么现象？established 默认多久？K8s 为什么必查？
- [ ] fd 耗尽和端口耗尽怎么分？网关 CCU 上限怎么算？
- [ ] `%si` 打满和 Recv-Q 涨各调什么？游戏长连接该监控哪两个数？

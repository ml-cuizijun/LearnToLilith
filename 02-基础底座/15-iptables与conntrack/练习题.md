...
`conntrack -L` 按源 IP 聚合看谁占满。先证明满，再调大。只调大是推迟爆炸。sysctl 持久化在 18。K8s iptables kube-proxy 规则爆炸会一起把表打满。
...
切换动机：规则爆炸、kube-proxy 刷规则抖 P99、表反复满。IPVS 治 **规则规模**，不自动消灭表满，也不治短连接泄漏。切完仍要监控 count/max。Cilium 见 K8s 23。主机 LVS 见 16。
...
INVALID：常见加固会 DROP。但 PMTUD、某些 NAT 边角会被误判 INVALID，表现为大包不通（11 MTU）。加这条要能回滚、要测大包。
...
表满是本章的锅；SYN 出了网卡对端不回是 11 的锅。不要一上来改 iptables policy。大厅通房间不通：房间端口 / UDP 超时，不要只抓登录 443。

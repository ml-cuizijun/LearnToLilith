...
1. **画链**：进来给本机走 INPUT，转发走 FORWARD；DNAT 在 PREROUTING，SNAT 在 POSTROUTING。
2. **证表满**：新挂旧不挂 → `conntrack -C` vs max + dmesg `table full`。
3. **持久化**：规则 save；调大 max 是买时间，治本 Keep-Alive / IPVS。
...
1. 进来先 PREROUTING，给本机走 INPUT，转发走 FORWARD。
2. DNAT 在 PREROUTING 改目的，SNAT/MASQUERADE 在 POSTROUTING 改源——不在 filter 里找。
3. `-v` 看命中计数；0 = 没走到这条链，或前面已命中返回。

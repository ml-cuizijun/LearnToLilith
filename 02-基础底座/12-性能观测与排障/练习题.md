...
先对照 [知识点](知识点.md) **本章主线** 自己口述，再对答案。关键字（30 秒全景、USE/RED、perf/bpftrace、证据链）要能定域。每题标了覆盖范围：
...
- [Q2 USE](#q2)
- [Q3 RED 与一次故障](#q3)
- [Q4 CPU 链 vs IO Load](#q4)
- [Q5 OOM / 磁盘满](#q5)
- [Q6 awk 与 ss 状态](#q6)
- [Q7 历史 sar](#q7)
- [Q8 平均 CPU 骗人](#q8)
- [Q9 perf / bpftrace 何时用](#q9)
- [Q10 SOP 与重启](#q10)
- [合上书](#合上书)
...
登录延迟告警。你刚 ssh 上去。旁边有人已经在 top 里来回排序。
...
Load 高先看 vmstat 是 CPU 还是 IO。没有这套开口就 top 乱翻。30 秒不够做火焰图。

### 再追问

- 容器节点还要补什么？——`cpu.stat` / `memory.events`（07）。宿主机利用率低也可能是 throttle。
- 工具按阶段：全景之后才 pidstat / iotop / perf，不一次开齐。
...
**一句话**：USE 对每种资源看 Utilization/Saturation/Errors，Load 是 S 不是 U。
...
flowchart TB
    ...
    C[USE：CPU 不饱和，磁盘 await 高] --> D[IO 才是瓶颈]
```

1. Load 是饱和不是利用率。
2. throttle 是 CPU 配额饱和，宿主机利用率可以低。
3. 平均 CPU 40% 不能结案：单核热点、cgroup、锁、下游 RT。
...
单核打满用 mpstat。throttle 读 cgroup。pidstat `-w`/`-d` 找谁在切谁在 IO；iotop 确认写什么；perf 仅 CPU 型函数级。还没定域就开 perf，窗口浪费掉。

### 再追问

- CPU 不高但请求延迟大？——Off-CPU：等 IO / 锁 / 网络。BCC `offcputime`。不要对着空闲 CPU 采 On-CPU。
- 火焰图怎么读？——X 轴越宽越热；找顶部平顶 = 叶子热点；颜色无含义。
...
容量规划同学说 CPU 还有余量。玩家已经在排队。

### 完整解答

**一句话**：平均 CPU 40% 结不了案：查单核热点、throttle、锁、下游 RT。
...
USE 看饱和和错误，RED 看入口。单核、throttle、云盘 QoS 都要点名。这时候上 On-CPU 火焰图：先 vmstat / mpstat。如果 `%us` 不高，On-CPU 没有平顶，该看 Off-CPU 或下游。
...
**一句话**：perf 在 CPU 型 `%us` 高时用 30s 采样；IO 型/Off-CPU 不要 On-CPU 图。

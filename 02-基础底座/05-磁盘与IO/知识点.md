...
活动高峰接口全超时，CPU idle 60%，Load 却很高。有人盯 `%util` 100% 说盘满了；另一台 `df` 100% 刚 `rm` 了 40G 日志空间没降。这一章只做一件事——**SSD 看 await 不看 util 定论，df ≠ du 先查 deleted**。
...
1. `write` 返回快是因为进了 cache，IOPS 在 writeback 爆。
2. dirty_ratio 一到，**全机 write 阻塞**，不是某一个接口慢。
3. LVM 在线扩：PV → VG → LV → xfs_growfs。
...
云盘 burst 耗尽：await 从 2ms 变 50ms，对着 burst balance 看。盘用到 **85%** 当故障。SSD 写放大：随机小写、接近满盘时 GC，表现 await 升高。

典型 iostat 输出（瓶颈在排队）：

```text
Device   r/s   w/s  rkB/s  wkB/s  await  aqu-sz  %util
nvme0n1  120  8900  4800  72000  45.2   402.1   98.5
```

await 45ms + aqu-sz 402 → 提交太多在排队，降 IO 或拆盘，不是换 SSD。
...
治理是限写和独立日志盘，不是盲目加大 dirty_ratio（推迟爆炸，阻塞更久）。available 这时会偏乐观（[04](../04-内存与OOM/)）。

O_DIRECT：绕过 Cache，避免数据库 Buffer Pool 和 OS Cache 双缓存。代价是延迟对盘更敏感。掉电安全靠 fsync，不靠「write 返回了」。普通日志/文件用 buffered，靠轮转。日志走 O_DIRECT 没有好处，只会把延迟绑死在盘上。
...
`kill -9` 无效 → [02](../02-进程与信号/)。wchan：`blk_mq_*` 本地盘，`nfs_wait_*` NFS，`wait_on_page_writeback` 脏页。

#### df 满、du 不满

`rm` 只摘文件名，进程还握着 fd 在写：
...
正确轮转：`copytruncate` 或 USR1 reopen。Docker `json-file` 必须 `max-size` / `max-file`。

#### 有空间 create 失败

`df -h` 有空间但 `touch` 报 No space → **`df -i`**。小文件海、overlay2 层。清文件**个数**。

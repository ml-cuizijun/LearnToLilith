...
- [ ] 能用 await + aqu-sz 分型，SSD 不用 %util 结案；NVMe 20ms 当事故
- [ ] 能分清 %wa / await / %util 三个视角
- [ ] 能处理 df ≠ du（lsof deleted 截断）和 inode 满（df -i）
- [ ] 能说明 dirty_ratio 一到全机写阻塞；buffered 下 write 返回不在盘上
- [ ] 能默写 LVM 在线扩四步，并说出 XFS 不能缩
- [ ] 能选 RAID 10 给数据库盘、云上不自建 RAID、库盘和日志盘分开
- [ ] 能处理云盘 burst、D/NFS、日志打满止血，而不是先 rm 或先改 cfq

...
dig @8.8.8.8 正常、运营商 DNS 错，下一步？——按劫持查：对比权威、换 4G/Wi-Fi、上 HTTPDNS。不要先改业务域名。
...
游戏登录入口切流失败会变成「部分渠道进错区」。排障必须让玩家提供 `dig` 结果或客户端日志里的 IP——智能 DNS 按地域返回不同 VIP。「DNS 已切」和「用户已切」不是同一时刻。摘流量要 VIP 级（LB 摘）+ DNS。
...
绕过 CDN，用源站 IP + Host 头测。绿锁只说明边缘证没问题，回源是另一跳。
...
HTTPDNS 拿到 IP 之后用 IP 校验证书可以吗？——不可以。证书是签给域名的。连接指定 IP，SNI / Host 仍用原域名。
...
健康检查已经把死 VIP 摘了，为什么还有人打上去？——DNS 还在 TTL 内。所以摘流量要 LB 摘 + DNS，不要只改 DNS。「DNS 已切」≠「用户已切」。
...
- [ ] 能画出客户端问递归、递归再迭代到权威；会用 +trace 和多 DNS 对比
- [ ] 能做切流五步：先降 TTL、旧 IP 托底；「DNS 已切」≠「用户已切」
- [ ] 能分 NXDOMAIN / SERVFAIL / 劫持 / 智能解析
- [ ] 能说明 hosts 只短路、resolv.conf 会被覆盖、K8s ndots 让外网先空转
- [ ] 能绕过 CDN 用源站 IP+Host 打源；MISS 不一定是故障；HIT 不应 502
- [ ] 能用哈希 URL + 按文件 Purge + 预热避免回源风暴；开服先稳定 DNS

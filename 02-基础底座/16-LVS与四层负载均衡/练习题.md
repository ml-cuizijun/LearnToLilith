...
- DR 后端为什么要抑制 ARP？——VIP 在 lo 上若不抑制，RS 会宣布「VIP 在我这」，和 LVS 抢地址，流量和会话全乱。`arp_ignore=1`、`arp_announce=2`。
...
七层不适合长驻 socket。不要用「Nginx 也能 stream 四层」硬扛百万连接——能，但不是这个量级的默认答案。HAProxy 怎么配见 [24](../24-HAProxy与Keepalived/)。
...
两台 LVS 之间的心跳被安全组丢掉。玩家一会儿能登、一会儿进错房间。ARP 表里 VIP 对应两个 MAC。

### 完整解答

**一句话**：双 VIP 先下一台——Keepalived 两台不算 quorum，心跳断会双主，VRRP 是 IP 协议 112。

keepalived 用 VRRP 做主备：主持 VIP，主挂则漂移。同时健康检查后端，挂了从 ipvs 摘掉。配置细节在 [24](../24-HAProxy与Keepalived/)，本章要会影响面。
...
- Keepalived 两台算不算 quorum？——不算。只是优先级 + 心跳。真分区防脑裂见 [20](../20-高可用与容灾/)。
...
房间进程规格不同。有人用了 sh，NAT 出口后一台后端被打满。

### 完整解答

**一句话**：长连接入口用 lc/wlc 按连接数分；sh 在 NAT 大出口后会扎堆，算法只管新连接。

1. **wrr**：规格不同按权重。
2. **lc / wlc**：按当前连接数，**长连接入口常用**，避免新连接都打到连接已经很多的机器。
3. **sh**：同一源 IP 固定后端。NAT 后成千上万用户同一个出口 IP，会打到同一后端。

长连接建立后不切。算法只影响**第一次分配**。会话保持是短连接有状态才真正需要。`-p` persistent 在 NAT 出口场景同样扎堆。

### 再追问

改算法要不要重建 VIP？——`ipvsadm -E` 改调度器。已有连接仍按旧绑定。改完看的是**新连接**分布。存量要均衡只能等消亡、主动断、或扩容接新连接。
...
LB 摘了 ≠ 用户切了。DNS 还在 TTL 内（[10](../10-DNS与CDN/)）。存量长连接也不会被健康检查踢掉——检查管的是**新分配**。四层不会把一条 TCP 搬到另一台。
...
三台房间，一台 CPU 打满，另外两台很闲。权重一样。有人要先改算法重建 VIP。

### 完整解答

**一句话**：先 `ipvsadm --stats/--rate` 和 `ip addr`，双 VIP 是脑裂不是算法问题。
...
看每台 ActiveConn / 速率。sh + 大 NAT 出口会打偏。lc 仍打偏：检查那台是不是健康检查失败后又被加回来、或长连接都是以前分的（算法不管存量）。

DR 下 stats **入站涨、出站几乎没有**；NAT 进出都涨。两台 `ip addr` 都有 VIP → 脑裂，先下一台，不是先改算法。
...
加：RS 先配好（DR：lo VIP + ARP 抑制）→ `ipvsadm -a ... -g -w n` → 看 stats。摘：先权 0，等新连接不再来，再 `-d`。
...
四个工单：① ARP 里 VIP 两个 MAC ② DR 会话乱 ③ 入口网卡满 ④ 探活已摘仍有人打到死机器。

### 完整解答

**一句话**：双 VIP 下一台；DR 乱查 ARP 抑制；入口满查 NAT 回程；摘了仍有人打查 DNS TTL + 存量 TCP。
...
Keepalived conf、notify、单播去哪答？——**24**。本章收口：心跳不通会双主；出现双主立刻下一台；漂移会断存量。脑裂总论在 [20](../20-高可用与容灾/)。
...
证书、Host 路由、灰度在七层（[09](../09-HTTP与TLS/)、[23](../23-Nginx/)）。长连接不要塞进七层。Keepalived 只护四层 VIP；七层 Nginx 的 HA 在 23/24。
...
- [ ] 能说出战斗长连接走四层；ipvsadm 挂了玩家不会立刻掉
- [ ] 能画出 NAT/DR/TUN 回程；游戏默认 DR 及 arp_ignore/arp_announce
- [ ] 能说出 TUN 何时上；长连接用 lc 不是 rr；sh+NAT 会打偏
- [ ] 能说明健康检查只摘新分配；探活口=业务口；DNS TTL 和存量 TCP
- [ ] 能处理双 VIP：下一台、IP 112、nopreempt；VIP 漂移会断存量
- [ ] 能用 stats/rate + ARP + lo 分流，升级怎么滚

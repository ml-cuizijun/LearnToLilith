...
先对照 [知识点](知识点.md) **本章主线** 自己口述，再对答案。关键字（namespace/cgroup、cpu.stat、memory.events、GOMAXPROCS）要能找路径。每题标了覆盖范围：
...
**一句话**：cgroup 管配额（OOM/throttle），namespace 管视图；容器内 free/nproc 读宿主机不可信。
...
**一句话**：throttle 看 cpu.stat 的 nr_throttled 增量，100ms 周期里 quota 用完就被踢出队列。
...
**一句话**：137 但节点还空是 cgroup memory.max 到了，读 memory.events 的 oom_kill 增量。
...
**一句话**：nproc 返回宿主机核数，GOMAXPROCS 不对齐 limit 会导致大量 throttle。
...
**一句话**：v1 usage_in_bytes 含 cache 易误杀，v2 能拆 file/anon 并有 PSI/memory.high。
...
**一句话**：kubelet 写 cpu.max/memory.max；request 影响调度不是硬顶，limit 才是 throttle/OOM 线。
...
**一句话**：延迟敏感有状态服倾向宽 limit 或只设 request，100m limit 突发必被 throttle。
...
**一句话**：Pod 共享内存 limit 时 sidecar 泄漏能带走主容器；pids.max 防 fork 炸弹。
...
**一句话**：不要猜 cgroup 路径：host PID → /proc/pid/cgroup → /sys/fs/cgroup 读 cpu.stat/memory.events。
...
- [ ] cgroup 和 namespace 各管什么？free/nproc 为什么撒谎？
- [ ] 从容器内现象怎么跳到宿主机 cgroup 文件？cpu.stat / memory.events 读哪几行？
- [ ] 100m 在 100ms 周期里是多少配额？扩节点为什么修不好 throttle？
- [ ] 137 但节点还有内存，怎么证明是 cgroup 杀的？Guaranteed 还会吗？
- [ ] v1 和 v2 对内存排障差在哪？PSI、memory.high 是什么？
- [ ] sidecar 泄漏谁死？pids.max 和 io.max 各防什么？

...
**一句话**：盘满先 df→du→lsof deleted；rm 不释放空间，会 reopen 用 USR1，不会用 copytruncate。
...
空间不释放是 [14](../14-文件系统与inode/) 的经典：删的是目录项，inode 还在。治理是 **轮转 + reopen**，不是加一块盘当根因。不要先 kill 逻辑服。
...
转发：`*.* @@logserver:514` 把系统日志离机（[17](../17-Linux安全加固/) 审计也要离机）。应用更常用 Filebeat 直读文件。两套抢一个文件会轮转打架。
...
**一句话**：生产 JSON + UTC + 级别当开关；必备 trace_id/service/level，DEBUG 生产关掉。
...
时间戳用本机 localtime 行不行？——不行。存 UTC（[19](../19-NTP与时间同步/)）。Kibana 再转。否则合服、跨 Region 时间线错。
...
**一句话**：丢了按 queue→lag→rejected 分段查，不要猜；Kafka 削峰不是无限水坝。
...
**一句话**：入口死活用 blackbox/SLO（21）；日志告警看错误速率，不是单条 DEBUG。

入口死活用 **blackbox / 进程 / SLO**（[21](../21-监控与告警/)）。日志告警证明「错误在涨、有异常模式」，阈值要有 burst 和级别，避免活动 INFO/WARN 噪声。FATAL、支付 ERROR 可以 P0；DEBUG 泄漏不能进生产。

ElastAlert frequency、Grafana LogQL、或日志抽指标进 Prom，都是同一思想：速率，不是单条。

### 再追问

audit.log 要不要进 ES？——要离机（[17](../17-Linux安全加固/)）。可以进独立索引、更长保留、更严权限。不要和游戏 DEBUG 混一个 index。
...
**一句话**：裸 Docker 主机要在 daemon.json 设 json-file max-size/max-file，lsof 在写文件的 mount 命名空间看。
...
5. SELinux 拦 rename（[17](../17-Linux安全加固/) AVC）。
...
- [ ] 能说出盘满三步；`rm` 后空间不回来先找什么
- [ ] 能讲 journald/rsyslog/应用目录分工；SystemMaxUse 不设会怎样
- [ ] 能区分会 reopen 和不会 reopen 各配哪套 rotate；Docker 呢
- [ ] 能配 Filebeat multiline；registry 各解决什么；丢失按哪几段查
- [ ] 能按场景拆 ELK/Loki；禁止 player_id 当 label
- [ ] 能把日志告警和探活分开；JSON 必备字段有哪些

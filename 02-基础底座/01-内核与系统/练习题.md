...
先对照 [知识点](知识点.md) **本章主线**（分层 → 归类 → 留证）自己口述，再对答案。关键字（syscall、`%sy`、journal、dmesg、After/Requires、cgroup、fstab）要能点名。每题标了覆盖范围：
...
**一句话**：小 IO 把 syscall 打到百万时 `%sy` 爆；先分 sy/si，再用 `strace -c` 看 calls，别加核。
...
**一句话**：`strace -c` 看 syscall 分布；perf/火焰图看函数热点；生产慎挂完整 strace。
...
**一句话**：fstab 写错用 VNC/救援改成 UUID；内核升级要窗口、回滚、grub 留旧版。

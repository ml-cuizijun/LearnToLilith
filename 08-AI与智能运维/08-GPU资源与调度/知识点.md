...
> 活动日 20:05，有人要删大厅 Deployment「腾 GPU」——卡根本不在普通池；有人 `kubectl delete pod` 全重启推理，权重加载三分钟，TTFT 从 300ms 飙到 8s。三个人分别喊「删大厅」「重启救命」「先扩副本」，三句都不对。
> **本章回答**：一块 GPU 从驱动可见、Plugin 上报、Scheduler 绑定、Pod 占卡、显存账本到故障下线——每一段怎么坏、现场怎么定责。
> **不讲**：TTFT/TPOT/排队/vLLM 服务质量 → [09](../09-推理服务性能与稳定性/)；通用 K8s 调度 → [03-06](../../03-Kubernetes/06-调度机制/)；节点 NotReady → [03-17](../../03-Kubernetes/17-节点运行时与升级/)。
...
flowchart LR
    ...
    F -.->|照哪段坏了| K["describe · smi · Events"]
```

- 📖 **读图**
  - 🛤️ 主线是一块卡从硬件到被 Pod 占用的资源链——**不是**「拷个模型文件就能推理」
  - 🔧 第一刀永远照「卡还在不在、分没分对、坏没坏」；延迟/排队/KV 打满后的服务质量在 09（Q1、Q2、Q11）

> ⚠️ **别混**：值班群说「GPU 还有空但很慢」——**08 看 smi 是否真空**；不空或 queue 涨是 **09**；两章联查，不是二选一甩锅。口诀：**08 管卡，09 管慢**。

本节对应练习题 Q1、Q2、Q11。边界钉死——先认卡。
...
- 📖 **读图**
  ...
  - **Container Toolkit** 注入设备；**Device Plugin** 把「有几张卡」报给 K8s——两层不同；Allocatable 0 查 Plugin

#### ❓ 为什么宿主机有卡、Pod 里却看不到？

- ❌ **先改 Deployment / delete 推理 Pod**：卡在宿主机，业务 YAML 救不了注入链
- ✅ **分层定责**：宿主机 smi 失败 → 驱动/硬件；宿主机 OK、Pod 无卡 → Toolkit/runtime；Allocatable 0 → Plugin DS
- 🔗 **版本要对齐**：驱动 vs Plugin 不匹配是常见 Pending / CrashLoop 根因
...
- 🗺️ **症状 → 定责层**
  - 宿主机 smi 失败 → 驱动/硬件；`dmesg` 查 Xid，先 cordon
  - 宿主机 OK、Pod 无卡 → Toolkit/runtime；`nvidia-container-cli`
  - Allocatable 0 → Device Plugin；查 DaemonSet logs
...
> 📝 **面试**：GPU 节点排障 **从宿主机 smi 向外**——不是先 delete 推理 Pod。⭐（练习题 Q1、Q12）

本节对应练习题 Q1、Q12。上报通了，整卡怎么报。
...
- 📖 **读图**：Scheduler 只给 **申请了 GPU 且 toleration 匹配 GPU 节点 taint** 的 Pod 绑卡；训练/推理分池，活动日在线不与训练抢 time-slicing。

#### ❓ 为什么不能像 CPU 写 0.3 卡？

- ❌ **小数卡**：`nvidia.com/gpu` 是 **整数扩展资源**，写 `0.3` 调度器不认或永远 Pending
- ✅ **整卡或 MIG 资源名**：`nvidia.com/gpu: "1"`，或 `nvidia.com/mig-1g.10gb: "1"`
- 🏊 **独立池**：GPU 节点带 taint；没有 toleration 的大厅 Pod **进不了**——所以删大厅腾不出卡
...
> 📝 **面试**：K8s GPU 调度 = Device Plugin 报 `nvidia.com/gpu` + 独立池 taint + **整数卡**；不能像 CPU 写 0.3。⭐（练习题 Q3）

口诀：**「活动日整卡，开发可 MIG」**。练习题 Q3。切分定了，看怎么绑。
...
graph TD
    A["A100 80GB 物理卡"]
    A --> B["MIG 1g.10gb → 小模型 Pod A"]
    A --> C["MIG 1g.10gb → 小模型 Pod B"]
    A --> D["剩余切片 → 中大模型 Pod C"]
```

- 📖 **读图**：MIG 适合 **多租户小模型**；70B 级通常仍要整卡或多卡张量并行（引擎侧见 09）。

#### ❓ 为什么活动日不能用 time-slicing 扛 SLA？

- ❌ **软件分时**：两进程共享同一块显存/算力，峰值互抢 → 同时 OOM 或延迟抖
- ✅ **整卡或 MIG**：硬件边界清晰，配额可预期
- 🎮 **活动口径**：在线推理 **禁 time-slicing**；开发试模型才用

> ⚠️ **别混**：MIG = **硬件切分**；time-slicing = **软件分时**。活动日在线不用 time-slicing 扛 SLA。
...
#### ❓ 为什么集群总卡够，4 卡 Pod 还 Pending？

- ❌ **只看集群总和**：8 节点各剩 1 卡 ≠ 任一节点有 4 卡
- ✅ **同节点凑齐 N 张**：扩 8 卡节点、降 TP 度数、或等释放
- 🔀 **08 vs 09**：凑不齐 N 卡是 **08 Pending**；NCCL 通信慢是 **09 延迟**——先 describe Events 分责
...
> 📝 **面试**：Insufficient 不一定是「集群没卡」——可能是 **单节点凑不齐 N 卡**。⭐（练习题 Q4、Q9、Q13）

本节对应练习题 Q4、Q9、Q13。绑上了，显存账本。
...
- 🔑 **日志关键词怎么定责**
  ...
  - `CUDA OOM` + 一放量就炸 → 常是 KV（09 详讲）；本章只认「卡被吃满」
  - 显存 100% 长期 → 限新 Pod 调度；扩池
...
- 🏢 **大厅在普通池**：无 `nvidia.com/gpu` 请求、无 GPU taint toleration → **根本不占卡**
- 🏊 **卡在独立 GPU 池**：删普通 Deployment 释放的是 CPU/内存，不是 GPU
- ✅ **真腾卡**：扩 GPU 池、等推理 Pod 释放、或迁训练任务——别动大厅

> 📝 **面试**：删大厅腾 GPU？**不能**，卡不在普通池。OOM 先分 **权重 vs KV**——权重装不下是资源选型；一放量就炸常是 KV（09）。⭐（练习题 Q5、Q7）
...
- 📊 **指标归属**
  - GPU-Util % / Memory-Usage / Xid → **08**（算力忙、显存水位、硬件）
  - `queue_depth` / TTFT / 429 → **09**（服务质量）
...
- 📖 **读图**：四件套管 **有没有卡、分给谁、坏没坏**；TTFT/排队/continuous batching 是 09。

> ⚠️ **别混**：**08 管资源边界**（卡、池、MIG、Xid）；**09 管服务质量边界**（TTFT、TPOT、网关排队、vLLM 参数）。

本节对应练习题 Q5、Q7、Q13。Pending 外，Xid 掉卡。
...
- 🗺️ **smi 画面 → 定责**
  - Xid / 卡数少 → 硬件/驱动；**别** delete Pod 指望自愈
  - Allocatable gpu: 0 → Plugin/驱动；**别**删业务 Pod
  - 显存 100% → 占满（查 09 是否该限流）；**别**删大厅
  - 全 Pod 重启后更慢 → 冷启动加载（09）；**别**「重启救命」
...
- 🔍 **现场**：先修 Plugin/驱动，再谈扩推理副本（09）。反复 delete 业务 Pod **不会**修好 Allocatable 0。
...
- 🏊 **池怎么拆**
  - `gpu-train`（taint train）→ 微调、离线批
  - `gpu-infer`（taint infer）→ vLLM 在线
  - 普通（无 taint）→ 大厅（无 GPU）
...
- 💥 **回收 = 掉卡**：节点 NotReady，Pod 被抢占，权重重新加载分钟级
- 📉 **表现在 09**：TTFT 爆炸、探针失败——根因却是 **08 的实例选型**
- ✅ **口径**：离线训练可 Spot（有 checkpoint）；活动在线推理 **禁止**；开发试模型可

> 🔍 **现场**：活动日推理池 label 不应含 `lifecycle=spot`；若 TTFT 飙且 node 刚 NotReady，先查是否误调度 Spot。
...
- 📎 **常见 Xid（资源层）**：13 / 31 / 43 / 79（fallen off bus）/ 119（GSP RPC）→ **一律先 cordon**，再查驱动/硬件

> 📝 **面试**：删大厅不腾卡；训练推理分池；活动在线不用 Spot；Xid 先 cordon。⭐（练习题 Q6、Q7、Q14）

口诀：**「Xid 先 cordon，delete Pod 没用」**。练习题 Q6、Q7、Q14。坏卡外——作战。
...
- 📖 **读图**：先问 Pending 还是 Running；Pending 拆三种 Events；Running 先 smi，再决定留在 08 还是切 09。
...
> 🔍 **现场**：接到「GPU 不够 / Pending / 掉卡」报障，60 秒走完这条线再下结论。
...
| 4 卡 TP Pending？ | 同节点凑不齐 N 卡，不是集群总和够。 |
| 活动能用 Spot？ | 在线推理禁止；回收=掉卡。 |
...
- Allocatable 0 靠删业务 Pod → 查 Plugin/驱动

### 数字墙

> FP16 **2** bytes/参数 · **7B** ≈ **14GB** 权重 · **72B** ≈ **144GB** 权重 · GPU 请求必须 **整数** · MIG **硬件**切 · time-slicing **无**隔离 · Toolkit 链 **4** 层 · 多卡 TP **同节点 N 张** · Xid **先 cordon** · 活动在线 **不用** Spot
...
对着第一节主图口述一遍，卡住就回对应小节。开头那个现场——删大厅、重启救命、先扩副本——三个人各自错在哪，现在该能说清了。

- [ ] **默画主图**：驱动 → Plugin → Scheduler → Pod 占卡 → 显存账 → 故障下线；`smi` / Events 照哪一段。（→ Q1 · Q11）
- [ ] **口述 08 vs 09**：各管什么？TTFT 在哪章？queue+smi 怎么联查？（→ Q2）
- [ ] **口述软件栈**：驱动 → Toolkit → Plugin → Pod；宿主机有卡 Pod 无卡查哪层？（→ Q12）
- [ ] **口述 Device Plugin**：报什么资源？Allocatable 0 查什么？为何不能 0.3 卡？（→ Q3）
- [ ] **口述整卡/MIG/time-slicing**：活动日选哪个？为什么？（→ Q3）
- [ ] **心算权重 footprint**：7B、72B FP16 各多少 GB？权重 vs KV 怎么分责？（→ Q4 · Q9）
- [ ] **口述 Pending 三种 Events**：Insufficient / Allocatable 0 / 无 toleration；4 卡 TP 同节点凑不齐。（→ Q5 · Q13）
- [ ] **口述 Xid 处理**：cordon 后做什么？为什么 delete Pod 没用？（→ Q6）
- [ ] **口述独立池与 Spot**：taint、训练推理分池、活动为何禁 Spot、删大厅为何不腾卡。（→ Q7 · Q8 · Q14）
- [ ] **定责**：Pending vs Xid vs「慢」各走哪章？60 秒剧本四步。（→ Q10 · Q14）

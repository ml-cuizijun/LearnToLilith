...
> 这一章讲完，你会知道当时两个人错在出域和没看 diff，以及 Vibe Coding 的正确姿势。
> **本章回答**：一次「用 AI 辅助排障或写代码」从提需求到合入会经历什么——人管两头、模型出草稿、验收清单挡住出域和自动执行。
> **不讲**：FC 审批闸门 → [04](../04-Function-Calling与Tool-Use/)；MCP Server 部署 → [05](../05-MCP协议/)；Agent 多步循环 → [06](../06-Agent智能体/)；GPU/推理队列 → [08](../08-GPU资源与调度/) · [09](../09-推理服务性能与稳定性/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节「AI 辅助的一生」，把人管两头、模型出草稿钉死；后面按排障开场、工单四要素、工具选型、review 七条展开；作战节拆出域与自动 apply。骨架不动。每一节讲完都会提示对应练习题。
...
大家想想：Cursor 一键 apply 省事吗？很多人觉得「AI 写的差不多就行」——**错了**。人管两头：需求与验收；模型只出草稿。
...
口诀：**「先证据，再假设，只要只读」**。练习题 Q2。开场对了，工单四要素。
...
⭐ 看不懂的代码不能合。练习题 Q5。review 外，绝不能给的清单。
...
口诀：**「kubeconfig 不出域」**。练习题 Q1。清单钉死，staging 验收。
...
**回到开头那个现场**：prod kubeconfig 贴公有云、diff 没看就合。正确剧本是：证据开场 → 工单四要素 → review 七条 → staging 验收 → 人签字。现在再看那两个人的操作，你知道该怎么拦住他们了。

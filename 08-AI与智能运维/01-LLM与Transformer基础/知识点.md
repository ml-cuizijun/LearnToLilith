...
> 这一章讲完，你会知道当时群里那两个人错在哪，以及正确做法是什么。
> **本章回答**：一次推理从 token 切分、Attention 看上下文、采样出下一个 token，到拼成最终输出，每个阶段会出什么岔子、SRE 该怎么用、不该怎么用。
> **不讲**：Prompt 结构化写法 → [02](../02-Prompt工程/)；RAG 开卷 → [03](../03-RAG检索增强/)；只读工具查实时 → [04](../04-Function-Calling与Tool-Use/)；GPU 推理部署 → [08](../08-GPU资源与调度/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节「一次推理的一生」主图，把 Tokenizer → Attention → 采样装进脑子；后面按入场/理解/窗口/采样/幻觉一站一站展开；边界与作战节专门拆开头那个现场——贴公有云、换模型、temperature=0。08 章特例：口述模板能背就过关。每一节讲完都会提示对应练习题。
...
大家想想：模型「答」你的问题，到底是在查库还是在续写？很多人会答「它懂了所以会答」——**错了**。它是按训练分布往下拼 token，像不像和对不对是两件事。
...
口诀：**「token ≠ 字节 ≠ 汉字」**。练习题 Q2。切完了，该看上下文。
...
⭐ 面试必考：Q/K/V 直觉，SRE 不必推公式。练习题 Q3。懂了「看」，再谈窗口能塞多少。
...
口诀：**「窗口共享，首尾放重点」**。练习题 Q4。窗口管住了，出场看采样。
...
⭐ 高频错答：temperature 不防幻觉。练习题 Q5。采样定了，看交付与幻觉。
...
练习题 Q4（cutoff）、Q13（TTFT）。好，该谈边界红线了。
...
口诀：**「出域/幻觉/只读/人审」**。练习题 Q7、Q8、Q11、Q12。红线钉死，作战定责。
...
**回到开头那个现场**：贴公有云、喊换大模型、喊 temperature=0。正确剧本是：出域先拦 → 数值靠 FC → 低温只求稳 → 变更人审。现在再看群里那两句话，你知道该怎么拦住他们了。

...
> 活动日 20:05，告警 bot、RAG、GM 问答三条线同时打满推理网关——TTFT 从 300ms 飙到 8s。有人 `kubectl delete pod` 全重启，权重再加载三分钟；有人只看 `nvidia-smi` 说「GPU 还有空」，却没看网关 `queue_depth=847`。两句都不对。
...
> 🎯 **一句话**：推理请求六段——进网关、排队、prefill（TTFT）、decode（TPOT）、流式吐出、过载拒绝——慢了先拆 **哪一段**，别混成一个「慢」。
...
- 📖 **读图**
  - 🛤️ 主线是一条请求的时间线——**总延迟 = 排队 + TTFT + TPOT×输出长度**；流式改善感知，不降低真实 TTFT
  - 🔧 第一刀：`queue_depth` + `nvidia-smi` + trace 三段拆开（Q1、Q8、Q10）——**08 管卡，09 管慢**

- 🗺️ **08 vs 09 联查三条分支**
  - queue↑ + smi 满 → 真过载：429 + 预扩（08 确认有卡）
  - queue↑ + smi 空 → 查 tokenizer / 拼批 / 网关，不是加卡
  - KV OOM → 先降 `max_num_seqs`（09）；加卡是备选（08）

口诀：**「429 是保护，无限排队是自杀」**。练习题 Q1、Q10。六段有了——先进网关水龙头。

---

## 二、进网关：鉴权、路由、限流阀

> 🎯 **一句话**：客户端不直连 GPU——**网关**是水龙头：鉴权、按业务路由、`max_queue`、超额 **429**，不无限排队拖死全体。
...
- 📖 **读图**：三条业务共池要 **按 route 分 QPS**——全打 70B 会挤爆；短 JSON 摘要路由小模型。Agent（[06](../06-Agent智能体/)）每步调一次推理，和网关限流 **同一等级**。

### 🚰 max_queue 与 route QPS 🎯⭐
...
- 🔍 **现场**：`reject_rate>0` = 过载保护在工作；`reject_rate=0` 且 queue 持续涨 = **无限排队**，P99 会先炸

#### ❓ 为什么要 429，不能无限排队？

- ❌ **无限排队**：门口不拦人、里面没桌 → 人人等超时，比拒绝更糟
- ✅ **429 + max_queue**：超额立刻拒绝，保住已在飞的请求 P99
- 🗺️ **按 route 分 QPS**：alert 短摘要不该和 GM 长文抢同一把 70B 水龙头

> ⚠️ **别混**：**拒绝（429）** 是过载保护，不是「服务坏了」——比全超时好。

> 📝 **面试**：网关 = 水龙头；429 是保护；无 route 分 QPS → 70B 被 alert 挤占。⭐（练习题 Q7）
...
#### ❓ 为什么 GPU 空但 queue 还在涨？

- ❌ **「GPU 空 = 没问题」**：卡闲着，请求可能卡在网关 / tokenizer / 拼批调度
- ✅ **先对 queue 与 smi**：同向满才是真容量；真空则查软件路径，不是盲目加卡（08）
- 🔧 **第一刀**：`curl … queue_depth` 与 `nvidia-smi` **同一分钟**对照

本节对应练习题 Q2、Q3。排队清了，TTFT / prefill。
...
- 📖 **读图**：8s 里 **6.2s 是排队** → 不是「模型变笨」；修 prefill 救不了排队。

- 🔥 **拉高 TTFT 的常见刀**
  - 排队：并发超容量 → 429、HPA、预扩
  - 长 prompt：RAG Top 50 进窗 → Top 3~5（[03](../03-RAG检索增强/)）
  - 冷启动：新副本权重未加载 → `initialDelaySeconds` > 加载时间
  - 假 Ready：只探端口 → 权重未就绪也接流

> ⚠️ **别混**：**流式（SSE）** 让 **感知** 变快——首 chunk 早到；**真实 TTFT**（prefill 完成时刻）不变，别当优化完成。

#### ❓ 为什么流式不能算「优化了 TTFT」？

- ❌ **「开了 SSE，TTFT 就降了」**：流式只是更早推送已生成的 token
- ✅ **真实 TTFT** 仍要等 prefill 跑完；看板看 **引擎侧 first_token**，不看体感
- 🧾 **优化真 TTFT**：减排队、减 prompt、避免半加载接流——不是开流式开关

> 📝 **面试**：TTFT 含排队 + prefill；先拆排队 vs prefill；流式不降真实 TTFT。⭐（练习题 Q1、Q5）

### 📉 量化与服务质量权衡 🔥

**INT8 / INT4 量化** 减 **权重 footprint（08）** 和 **decode 算力**——可能损质量，活动前要回归评测。
...
本节对应练习题 Q1、Q4、Q5。首字之后，TPOT。

---

## 五、decode 与 TPOT：逐 token 生成

> 🎯 **一句话**：**TPOT（Time Per Output Token）** = 每输出一个 token 的平均耗时——decode 自回归，**总生成时间 ≈ TPOT × 输出 token 数**。
...
- 🔑 **TPOT 高常见原因**
  - GPU Util 满 → 量化、加副本、降并发
  - 输出过长 → 网关 cap `max_tokens`
  - batch 过大 → 调 `max_num_seqs`（拼批牺牲单请求）

- 🎬 **continuous batching（连续批处理）**：vLLM 在 decode 时 **动态拼批**——新请求插入、完成的踢出，提高 GPU 利用率；代价是调度复杂度，**不是** Ollama 默认路径

### ✂️ max_tokens 与输出长度治理 ⭐
...
本节对应练习题 Q5、Q13。decode 外，KV Cache。
...
#### ❓ 为什么一放量 OOM 常是 KV 不是权重？

- ❌ **「OOM = 模型太大」**：权重装不上发生在 **启动 / 换模型**，日志常见 `failed to load weights`
- ✅ **一放量才炸**：并发 × 上下文把 KV 打满，日志常见 `KV cache allocate`
- 🔪 **第一刀**：降 `max_num_seqs` / 限长 / RAG TopK；加卡是备选（08）

> 📝 **面试**：显存三块权重 + KV + 碎片；一放量 OOM 常 KV；PagedAttention 减碎片不是无限。⭐（练习题 Q3、Q9）
...
口诀：**「一放量 OOM 常是 KV」**。练习题 Q3、Q6、Q9。显存外，引擎选型。
...
- 📖 **读图**：Running ≠ Ready——权重加载完、探针过了才能接流；半加载接流 = TTFT 再炸一轮。

#### ❓ 为什么活动链路不能用 Ollama？

- ❌ **「能跑就行」**：缺 continuous batching / PagedAttention，并发一上来 P99 崩
- ✅ **vLLM + 网关限流 + probe delay**：权重钉 hash，禁 `latest`
- 🧊 **冷启动**：`minReplicas≥1`，`initialDelaySeconds` > 加载时间
...
- 🔍 **现场**：`queue_wait` 占 6.2s → **09 容量**（429 / HPA）；`prefill` 1.1s 且 prompt 4k tokens → **RAG 减窗（03）** 或换小模型

> 📝 **面试**：Ollama 不进活动链路；vLLM + 网关限流 + probe delay；权重钉 hash 禁 latest。⭐（练习题 Q4）

本节对应练习题 Q4、Q14。vLLM 外，过载与 HPA。
...
#### ❓ 为什么 HPA 不能按 CPU、也不能 min=0？

- ❌ **按 CPU 扩**：推理瓶颈在 GPU / queue，CPU 可能一直不高
- ❌ **minReplicas=0**：从 0 扩 = 冷启动 TTFT 爆炸
- ✅ **信号用 queue_depth / GPU util**；probe delay > 权重加载；扩前 08 确认有卡
...
- 📖 **读图**：先问 queue 涨不涨 → 再对 smi → OOM 分 KV / 权重 → 卡故障走 08 → 最后查探针是否半加载接流。
...
> 🔍 **现场**：接到「TTFT / P99 爆」报障，别先全 restart，60 秒走完这条线再下结论。
...
本节对应练习题 Q8、Q12。现在回开头那个现场。

---

**回到开头那个现场**：全 restart 推理 Pod、只看 smi 说 GPU 还有空。正确剧本是：queue → smi → 权重 vs KV → Xid（08）→ 半加载探针。现在再看那两个人的操作，你知道该怎么拦住他们了。
...
对着第一节主图口述一遍，卡住就回对应小节。开头那个现场——全 restart、只看 smi——两个人各自错在哪，现在该能说清了。

- [ ] **默画主图**：网关 → 排队 → prefill / TTFT → decode / TPOT → 流式 → 过载；`queue · trace · smi` 照哪一段。（→ Q1 · Q8）
- [ ] **口述 08 vs 09 边界**：各管什么？联查三条分支？（→ Q10）
- [ ] **拆解 8s 延迟**：排队 6.2s + prefill 1.1s 说明什么？（→ Q2）
- [ ] **口述 TTFT vs TPOT vs 流式**：真实 TTFT 变了吗？（→ Q1 · Q5）
- [ ] **口述 KV OOM vs 权重 OOM**：日志怎么分？第一刀？（→ Q3 · Q9）
- [ ] **口述 PagedAttention 与 continuous batching**：解决什么、不保证什么？（→ Q4 · Q9）
- [ ] **口述 vLLM vs Ollama**：活动日怎么选？（→ Q4）
- [ ] **口述 HPA / 探针 / min=0**：各会怎样？（→ Q6）
- [ ] **口述五刀排障**：TTFT 爆的顺序。（→ Q8 · Q12）
- [ ] **口述过载策略**：429 vs 无限排队。（→ Q7）

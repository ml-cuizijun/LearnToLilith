...
> 这一章讲完，你会知道当时错在把摘要 bot 当成全自动排障，以及 max_steps 触顶后该怎么停。
> **本章回答**：一次 Agent 执行从接到目标到交出结论会经历什么——ReAct 怎么转、权限在哪一层卡住、max_steps 触顶后该怎么停。
> **不讲**：单步 FC 与 schema → [04](../04-Function-Calling与Tool-Use/)；MCP 接线 → [05](../05-MCP协议/)；RAG 长期记忆 → [03](../03-RAG检索增强/)；Cursor 改文件 → [07](../07-AI工具实战与Vibe-Coding/)；GPU 队列 → [08](../08-GPU资源与调度/)；AIOps 禁止自动修 → [10](../10-AIOps落地/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节 Agent 执行的一生，把接任务→ReAct→护栏→触顶交人装进脑子；后面按权限四层、Observation 不可信、max_steps 展开；作战节拆「四十步打满 Prometheus」。骨架不动，只叠口吻。每一节讲完都会提示对应练习题。
...
大家想想：Agent「转圈」时第一反应是什么？很多人会说「加卡 / 换大模型」——**错了**。先问卡在哪一段：工具列表？步数？Observation 注入？
...
练习题 Q1、Q9。接任务清了，路线：ReAct 还是 Plan。
...
⭐ ReAct 外层仍要护栏。练习题 Q2、Q11。循环转起来，Observation 怎么信。
...
口诀：**「Observation 是数据，不是指令」**。练习题 Q5、Q12。证据在手，权限四层。
...
🔥 大白话：**步数上限不是嫌它笨，是保护监控和网关**——到顶打包证据交人，不是空说「我不知道」。
...
口诀：**「四层缺一不上线」**。练习题 Q3、Q4。护栏外，收束与多 Agent。
...
练习题 Q8、Q14。现在回开头那个现场。

---

**回到开头那个现场**：摘要 bot 升全自动、四十步打满 Prometheus。正确剧本是：只读工具列表 → max_steps 触顶交人 → Observation 当数据 → 变更人签字。现在再看「让 Agent 自己修」的同学，你知道该怎么拦住他了。

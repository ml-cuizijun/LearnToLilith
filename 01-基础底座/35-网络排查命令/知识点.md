...
> 晚上九点 oncall：「登录一直转圈」——有人已经 `tcpdump -i any` 打满终端、SSH 眼看卡死；有人看了眼 `ip route show` 说「有 default，网络没问题」；还有人 ping 通就结了案，开发反手甩来 `Connection timed out`。
> **本章回答**：一次「连不上 / 慢 / 重置」从接警到定责，每一步第一刀敲什么、输出怎么读、什么时候升级邻章。
> **不讲**：Recv-Q / backlog → [12](../12-网络栈与Socket/)；窗口 / 重传 / `ss -ti` → [13](../13-TCP与UDP/)；TLS / HTTP 语义 → [14](../14-HTTP与TLS/)；DNS 劫持 / CDN → [15](../15-DNS与CDN/)；抓包深纪律 / PMTUD / TSO → [16](../16-网络排障与抓包/)；iptables / conntrack → [17](../17-iptables与conntrack/)。
...
- 📖 **读图**
  - 🛤️ 五步各占一节：接警（二）→ 定层（三）→ 缩范围（四～六，dns/tcp/tls）→ 留证（七）→ 收工（八）
  - 🎯 80% 工单定完层就有答案——dns / tcp / tls / ttfb 总有一层背锅，**轮不到抓包**
  - 💥 底部虚线是翻车四件套，后文每把刀都会点名 ⭐（Q1）
...
> 🎯 **一句话**：先问「卡在哪一步失败」，再选刀——人话对应五种包行为，第一刀完全不同。
...
- ⚡ **最值钱的分界线：失败得快不快**
  - **立刻失败**（`Connection refused`，毫秒级）：有人回了话——没人监听回 RST，或墙明确 reject → 先查对端（`ss`）
  - **转圈到底**（`Connection timed out`）：石沉大海——SYN 被吃（drop）、走错路、或源 IP 不被放行 → 先查本机路由和路径（`route get` / `nc`）

> ⚠️ **别混**：`Connection refused` ≠ `Connection timed out`。refused =「有人明确说不」，timeout =「没人应」——前者查对端监听和 reject，后者查 drop、路由和源 IP，**顺序对调就是白忙一晚上**。

> 🔍 **现场**：接完电话先复述：「是进不了大厅，还是大厅能进、进房间转圈？」——大厅（HTTPS 登录域名）和房间（长连接 IP:port）是两个目标，刀也不一样（三节末、Q12）。

> 📝 **面试**：「连不上你第一刀敲什么？」——HTTP 业务答 `curl -w` 做差定层；非 HTTP / 长连接答「先确认目标 IP:端口，`nc -zv` + `ss -lntp`」。**任何情况下第一刀都不是 tcpdump**。

本节对应练习题 Q12。话翻译完了——HTTP 业务第一刀：定层。
...
🔥 拿到 `dns:0.01 tcp:0.02` 就念「TCP 20ms」——**错了**。五个数全是累计值，`time_connect` **包含 DNS**。不减就把 DNS 记在 TCP 头上。

> 💡 **打个比方**：跑步 app 记的是「出发到现在累计几分几秒」，想知道某一段多快，得相邻打卡点相减。curl 的五个数就是五个累计打卡点。
...
- ❌ **五个数当每段耗时读**：`time_connect` 是从 0 累计到 TCP 握手完成
- ✅ **本段 = 后减前**：`tcp = connect − namelookup`；口诀「五个累计值，本段后减前；connect 含 DNS」
- 🧮 口算自检：五段相加 ≈ `total`
...
F  同一 URL 两边各跑一次：跳板 tcp:0.012 / 玩家侧 tcp:1.800，两边 ttfb 本段都短
   → 路径/出口差异；应用本身不慢——单看一边的五段说不出这句话
```

> ⚠️ **别混**：`time_connect` 不是纯 TCP 时间——从 0 累计到握手完成，**包含 DNS**。把 `tcp:0.02` 念成「TCP 20ms」前，先减 namelookup。

- 🗺️ **没有 `-w` 时，`curl -v` 卡点也能对上五段**
...
- 🎮 **长连接房间**：curl 五段只覆盖「HTTP(S) 一问一答」。先确认房间 IP:port，走 `nc -zv` + `ss`，仍说不清才抓**房间端口**（七节）。

> 📝 **面试**：「curl -w 怎么定位慢？」——先背「五个数都是累计值，本段 = 后减前，connect 含 DNS」；再背五段各找谁：dns→dig、tcp→nc/route get/mtr、tls→openssl、ttfb→应用、body→带宽。

> 🔍 **现场**：值班机存成 alias（如 `cw`）；HTTP / HTTPS 各一版，HTTP 的 `tls` 是 0.000，别当异常。

练习题 Q2 专门练做差。下一站：DNS 岔路。
...
# 198.51.100.20           ← 公共 DNS 的答案：和上面不一致 = 本地 DNS 有故事
dig login.example.com | grep -E 'login.*IN\s+A'
# login.example.com.  60  IN  A  203.0.113.10
#                     ↑ TTL 60s：刚切过解析的话玩家侧缓存还没过期
getent hosts login.example.com
# 203.0.113.10 login.example.com   ← 走本机 nsswitch（/etc/hosts 优先），应用实际拿到的就是它
```

- 🔀 **跳板和玩家侧不一致**：让玩家侧 `nslookup` 对比。多半是 split DNS 或劫持 → [15](../15-DNS与CDN/)
- 📌 **解析可疑 /「偶发连错库」**：`curl --resolve login.example.com:443:198.51.100.20 https://login.example.com ...`（`-w` 照用）把域名钉到指定 IP 重跑五段——慢跟着哪个 IP 走，解析嫌疑立刻见分晓
- 📛 **`NXDOMAIN`**：域名不存在（查错 / 记录删了 / 授权断）；**`SERVFAIL`**：权威自己病了。换 `dig @8.8.8.8` 再问，能把「本地解析器病」和「权威病」分开
- 🌲 `dig +trace` 从根逐级问——日常工单 `+short` + `@公共DNS` 对比就够

> 🔍 **现场**：工单永远并排贴 `dig +short` 和 `dig @8.8.8.8 +short` 再附 TTL；怀疑应用拿到的 IP 和你不一样时补 `getent hosts`。

> 📝 **面试**：「DNS 可疑你查什么？」——`dig +short` 对答案、`dig @8.8.8.8` 对比两边、看 TTL、NXDOMAIN 和 SERVFAIL 分开说。深挖引到 15。

本节对应练习题 Q10 的 dig 半边。DNS 过了还不通？进 TCP 岔路。
...
- LISTEN 里没有这个端口 = 进程没起 / 端口配错，**后面全不用查**
- 有这行但只绑了 `10.0.0.8:443`，从 VIP 或公网侧连就是「端口在听却连不上」→ [12](../12-网络栈与Socket/)
...
ss -ant state close-wait | head -3 # close-wait：对端已 FIN、本机一直没 close——泄漏在代码，修代码
ls /proc/$(pgrep -o nginx)/fd | wc -l   # close-wait 只增不减时看 fd 数是否同步涨 → 05
ss -ti dst 10.0.1.50:3306          # 重传/cwnd/rcv_space → 13
```

> ⚠️ **别混**：LISTEN 的 Recv-Q 是**未 accept 的连接个数**，ESTAB 的 Recv-Q 是**未读取的字节数**——同名不同义。机制深挖在 12。
...
timeout 3 bash -c 'cat < /dev/null > /dev/tcp/10.0.1.50/3306' && echo open || echo closed   # 没装 nc 时的替代
```

- 📡 **UDP 口**：别用 `nc -zuv` 下结论——UDP 没有握手，「succeeded」只代表包发出去了；要看到业务回包或抓到包才算通
- 🔌 **老机器只有 telnet**：`telnet IP PORT` 能连**只证明 TCP 口开了**，敲 HTTP 拿到响应才算应用活着——「telnet 通」≠「HTTP 200」

### 🧱 drop 与 reject：两种「不通」的分水岭 🔥

#### ❓ 为什么同样「连不上」，有人秒失败、有人干等几十秒？

- 很多人会答「都是防火墙拦了」——太粗了。nc 的 refused / timeout 背后是墙的两种动作，第一刀方向完全不同：
...
- 📖 **读图**
  - 🚪 refused 有两个来源（墙 reject 或对端没监听）→ 第一刀 `ss -lntp` 看对端
  - 🕳️ timeout 只有一类（包被吃 / 走丢 / 走错）→ 第一刀 `nc` + `route get`

> 💡 **打个比方**：reject 是门卫明确说「不让进」——你立刻知道被拒了；drop 是门卫装没听见——你在外面一直等到怀疑人生。**等得越久，越说明没人回话**。
...
> ⚠️ **别混**：安全组 / iptables 里 **reject ≠ drop**——一个秒失败，一个干等。加规则时选错动作，工单现象会完全换一副面孔。

> 📝 **面试**：「drop 和 reject 的区别？」——drop 丢包不回应、客户端超时，查路径和源 IP；reject 回 RST、立刻 refused，查对端监听和墙规则。收尾一句「**超时查路，拒绝查口**」。

### 🧭 第二问 · 包从哪出：ip route get 🔥

🔥 开头那个现场里，有人看了眼 `ip route show` 说「有 default，网络没问题」——**错了**。双网卡机器上，show 是地图册，真正替你算「这一包走哪」的是 `ip route get`。⭐

#### ❓ 为什么不能拿 ip route show 结案？

- ❌ **show = 地图册**：列出所有可能的路，不告诉你这一包实际走哪
- ✅ **get = 导航**：替你把「这个源、这个目的」实际走哪条算一遍；双网卡 / 策略路由上两者经常不一致，**以 get 为准**
...
实际：     ip route get 10.0.1.50 → via 10.0.2.1 dev eth1 src 10.0.2.88
           ...
           拿 eth0 源 IP 去 ping 管理口 → 「ping 通」——两件事根本不在一张卡上
```

改完路由表如果现象不变，先 `ip route flush cache` 再 get 一次。策略路由 / 多表深排 → [16](../16-网络排障与抓包/)。

> 🔍 **现场**：双网卡工单永远贴 `ip route get 对端IP` 而不是 `ip route show`——`dev` 和 `src` 两个词就是结论。
...
- 📊 **状态速读**：REACHABLE 可用 / STALE 缓存待确认 / DELAY、PROBE 正在探测 / FAILED 二层不通 / 无条目 = 还没试过
- 同网段 FAILED → 查线缆、网卡、VLAN、对端起没起来，**不是改默认网关**；跨网段时 ARP 问的是网关 → [12](../12-网络栈与Socket/)

### 🛤️ 第四问 · 路上丢不丢：ping 与 mtr ⭐
...
ping -M do -s 1472 203.0.113.10   # DF 位 + 1472 字节 payload（+28 头 ≈ 1500）：小包通大包死 → PMTUD → 16
```
...
ping 通 + TCP 超时    墙 drop 了 TCP（或只放 ICMP）——「三层通四层不通」，锅在墙
ping 不通 + TCP 通    ICMP 被禁，正常现象，不当事故
ping 不通 + TCP 不通  可能真不通；仍要先 route get 排除「走错卡」
小包通 + -M do 大包不通   PMTUD 黑洞的经典指纹 → [16](../16-网络排障与抓包/)
```

> ⚠️ **别混**：**ping 通 ≠ 网络没问题**——ICMP 和 TCP 可能不是同一条策略；**ping 不通 ≠ 网络断了**——ICMP 常被整体禁掉。验收业务永远用 TCP 口径（nc / curl）。
...
全程 0 丢但 TCP 挂 ← ICMP 路径 ≠ TCP 路径：回到抓 TCP / 查墙
```

> 💡 **打个比方**：中间路由器对「回 ICMP 回执」限了速——它照样转发数据包，只是懒得每个都回执。判断它好不好，**看它后面那几跳**：后面不丢，说明转发得好好的。

没有 mtr 用 `traceroute -n`；业务端口用 `traceroute -T -p 443`；`tracepath` 顺带看路径 MTU。`* * *` 之后仍能到达 = 那一跳不回 ICMP，常见且无害。
...
- 📖 **读图**
  - 🔪 四刀有先后——口没开就查路径是白忙；refused 根本不用出本机；timeout 才按 路由 → 二层 → 路径 往外推
  - 📋 可背清单：**refused 查口（ss + reject），timeout 查路（get → neigh → mtr），谁在听看绑定，从哪出看 src**

> ⚠️ **别混**：这套刀法**不保证**能找出「已 ESTAB 仍然卡死」——那是窗口/重传，`ss -ti` → [13](../13-TCP与UDP/)；也不覆盖「小包通大包死」→ [16](../16-网络排障与抓包/)。

> 📝 **面试**：「TCP 连不上你怎么缩范围？」——口（ss/nc）、路（route get 的 dev/src）、二层（neigh）、路径（mtr 形态），四步各一句；补「refused 和 timeout 的第一刀不同」就是满分版。

口诀：**「超时查路，拒绝查口」**；**「show 是地图册，get 是导航」**。练习题 Q3～Q7。TCP 通了还慢？可能是 TLS。
...
CONNECTED(00000003)            ← TCP 已通；若紧接着断，是 TLS 层拒（版本 / SNI / 证书）
  0 s:CN = login.example.com   ← 证书主体（subject）要匹配访问的域名
    ...
    Protocol  : TLSv1.3        ← 协商版本；握手失败出 alert → 14
    Cipher    : TLS_AES_256_GCM_SHA384
notAfter=Sep 28 12:00:00 2026 GMT   ← 有没有过期；tls 段偶发秒级慢也可能是吊销检查拉链慢
```

> ⚠️ **别混**：IP 直连时不带 `-servername`，服务器给的是**默认证书**，证书名必然对不上——不是证书坏了，是你没报 **SNI（Server Name Indication）**。

> 📝 **面试**：「HTTPS 慢 / 证书告警你怎么查？」——`openssl s_client -connect host:443 -servername host`，看 CONNECTED 后有没有断、CN 是否匹配、notAfter、Protocol/Cipher；语义层引到 14。

练习题 Q10 的 openssl 半边。前三步仍说不清，才上留证。
...
- 📖 **读图**
  - 🔌 闭环第一步永远是 route get——**网卡选错，抓一晚上都是空气**
  - 💾 写文件不打终端，抓够包数就停，离线再慢慢过滤

#### ❓ 为什么第一刀不是 tcpdump？

- ❌ **裸 `-i any` 打终端**：看不出包从哪张卡出入、多口聚合出假象、流量打爆 SSH
- ✅ **前三步说得清就别抓**；真要抓 = route get 定网卡 → host/port 过滤 → `-nn -s0 -c -w`
...
> 🔍 **现场**：第一轮先抓 SYN/RST 摘要（`-c 200`，几秒就停），看清是「重复 [S] 无 [S.]」还是「[S] 后立刻 [R]」再决定要不要全量。
...
- 🔍 **四种现象对号入座**
  - **重复 [S] 无 [S.]** = SYN 黑洞（先回 ss 查监听，再查 drop）
  - **[S] 后立刻 [R]** = refused（监听 / reject）
  - **ESTAB 之后突发 [R]** = 先看 RST 的**源 IP**——本机内核、对端、还是中间盒（→ [16](../16-网络排障与抓包/)）
  - **大厅域名上抓不到任何包** = 抓错目标，换成房间 IP:port

> ⚠️ **别混**：`tcp[tcpflags] & tcp-syn != 0` 会把 **SYN-ACK 也抓进来**；只要纯 SYN 必须用 `tcp[tcpflags] = tcp-syn`。

> ⚠️ **别混**：**禁止裸 `tcpdump -i any`**——看不出网卡、聚合出假象、打爆 SSH。要看多接口就分接口抓或上镜像；TSO 假 checksum → [16](../16-网络排障与抓包/)。

> 📝 **面试**：「生产环境怎么安全抓包？」——route get 定网卡 → host/port 过滤 → `-nn -s0 -c 1000 -w 文件` → 离线 `-r`；一句「不带过滤的 `-i any` 在生产等于事故」收尾。

⭐ 口诀：**「先定网卡再抓；禁裸 -i any」**。练习题 Q8。留完证，该收工了。
...
- 📖 **读图**
  - 🚪 入口先分「HTTP 还是长连接」和「新挂旧还在」两个特例
  - 🧮 中段是五段做差分流；底部三个出口（16 / 12 / 13）对应三种「本章到顶」的形态
...
主机侧 CPU / IO 的刀法                      → [34](../34-主机排查命令/)
```

本节对应练习题 Q9、Q11、Q13、Q14。

**回到开头那个现场**：有人裸 `tcpdump -i any`、有人拿 `route show` 当结论、有人 ping 通就结案。正确剧本是：先把玩家的话翻译成包语言 → `curl -w` 做差定层 → 按层缩范围 → 仍说不清才按纪律抓包。
...
- [ ] **默画主图**：报障的一生五步 + 翻车四件套；说清 80% 工单死在前三步（对应练习题 Q1、Q14）。
- [ ] 口述**接警翻译**：五种人话各对应包行为、第一刀是什么（Q12）。
- [ ] 口述**做差**：五个累计值、相邻相减；ttfb 慢定责给谁、connect 含 DNS（Q2）。
- [ ] 口述 **DNS 岔路**：dig、@8.8.8.8 对比、NXDOMAIN vs SERVFAIL（Q10）。
- [ ] 口述 **TCP 三问四刀**：ss+nc / route get / neigh / mtr（Q3、Q4、Q5、Q7）。
- [ ] **默画** drop/reject：SYN 四种结局；「超时查路，拒绝查口」（Q6）。
- [ ] 口述 **ping 边界**：四组合 + `-M do -s 1472`（Q5）。
- [ ] 口述 **TLS**：openssl 看哪四样、为何带 `-servername`（Q10）。
- [ ] 默写一条**合法 tcpdump**；纯 SYN 用 `=` 不用 `&`；禁裸 `-i any`（Q8）。
- [ ] 口述**收工**：新挂旧不挂、60 秒剧本、六条升级路径（Q9、Q11、Q13、Q14）。

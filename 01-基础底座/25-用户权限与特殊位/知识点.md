...
> 这一章讲完，你会知道当时那个人错在哪，以及 Permission denied 该怎么定责而不是乱开 777。
> **本章回答**：一次 Permission denied 从内核判定的那一微秒，到路径哪一级挡了你、该改哪个位、谁有权代办、怎么结案审计的一生。
> **不讲**：权限位存放在 inode 哪一格 → [09](../09-文件系统与inode/)；进程身份的 fork 继承与切换系统调用 → [05](../05-进程与信号/)；SELinux/AppArmor、fail2ban 与加固全景 → [27](../27-Linux安全加固/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节的主图，把「一次 Permission denied 的一生」装进脑子；第二到第七节顺着身份 → 九位 → chmod → umask → 特殊位 → sudo 往下走；第八节专门讲定责，也就是开头那个现场的正确解法。每一节讲完都会提示对应练习题，学完就去练。
...
咱们先不急着背 chmod 数字。学权限最怕的就是上来 `chmod -R 777`。先看图，把「卡在链上哪一段」装进脑子里：
...
**读图**：带着三个问题看这张图，比背数字位有用得多：主线是值班处理一次 Permission denied 的完整时间线。前两段靠命令看清，中间「定责」是三选一，最后一段结案是多数人跳过的——`chmod -R 777` 压报错就是跳过定责直接乱修。后面每节对应图上一个方块：二、三讲「发生」的原料（身份与九个位），四～六讲「位」怎么改，七讲「代办」，八讲「定位与结案」。

---

---

好，主图有了。咱们正式出发——第一站：内核认的是数字还是名字？
...
出身段到这儿。本节对应练习题 Q3。好，身份有了——大家想想：文件 777 为什么还会 Permission denied？

---

## 三、长相：三组九位怎么读

🔥 文件 777 还是 Permission denied——**高频错答是「权限坏了」**。大白话：**挡你的往往是路径上某一级目录缺 x，不是文件本身**。口诀：**「文件看三位，进门看目录 x」**（练习题 Q1）。
...
长相段到这儿。本节对应练习题 Q1。好，会读九位了——chmod 数字法和符号法怎么用？大写 X 防什么坑？
...
易容段到这儿。本节对应练习题 Q4。好，会改位了——大家想想：服务建出来为什么全是 600？
...
umask 段到这儿。本节对应练习题 Q5。回到开头那个 777 共享目录——三个特殊位各管什么？

---

## 六、特权：SUID、SGID 与 Sticky

🔥 777 共享目录里躺着 SUID 后门——**结案审计漏掉就会出事**。大白话：**SUID 让别人以文件主人身份跑；Sticky 才是「只能删自己的」**。口诀：**「SUID 提权，Sticky 防删，SGID 传组」**（练习题 Q2、Q6、Q7）。
...
特殊位段到这儿。本节对应练习题 Q2、Q6、Q7、Q10。好，特权位懂了——大家想想：su 和 sudo 差在哪？sudo 重定向为什么失败？

---

## 七、代办：sudo 与 sudoers

🔥 有人给 deploy 开 `(ALL) NOPASSWD: ALL`——**加固等于没做**。大白话：**sudo 是白名单代办，不是把 root 钥匙整串扔出去**。⭐ 面试必考（练习题 Q8、Q9）。
...
sudo 段到这儿。本节对应练习题 Q8、Q9。好，代办会了——大家想想：namei 怎么定位挡路的那一级？find 怎么审计多余 SUID？结案为什么不能停在「改完能写」？
...
结案段到这儿。本节对应练习题 Q10、Q11、Q13。现在再看开头那个 `chmod -R 777`——接到 Permission denied，第一分钟怎么走？
...
作战段到这儿。本节对应练习题 Q11、Q12、Q13、Q14。现在再看开头那两场事故——777 压报错、SUID 后门——你已经知道该怎么拦住他了。
...
对齐练习题「合上书」，逐条口述，卡住就回对应节 / 题号。现在再看开头那两场事故——`chmod -R 777` 压报错、共享目录 SUID 后门——你该知道先定责什么了。

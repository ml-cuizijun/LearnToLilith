...
| **13** | TCP/UDP：握手机制、窗口、重传、心跳 |
| **16（本章）** | **定层手法、抓包纪律、路由/ARP/mtr、握手在包里怎么认、MTU/PMTUD** |

本章是**排障手法主场**。12/11 告诉你「机制上可能卡哪」；本章告诉你「现场第一刀敲什么、抓什么、怎么读输出」。
...
| 段 | 记住 |
|----|------|
| ① | 五段做差定层（§4）；dns→15，tls→14，ttfb→应用 |
...
│  链出：队列/Recv-Q→12 · 窗口重传→13 · 502/证书→14 · DNS→15 · 表满→17     │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| 本段 | 结果 | 定责 |
|------|------|------|
| dns | **1.2s** | dig、本机 resolve、ndots、劫持 → 15 |
| tcp | 5ms | 路通 |
...
| `-v` 停在 | 对应五段 | 想什么 |
|-----------|----------|--------|
| `Could not resolve host` | dns | dig → 15 |
| `Trying x.x.x.x...` 后长时间 | tcp | SYN / 墙 / route get |
| `Connected` 后卡 SSL | tls | openssl s_client → 14 |
...
| 4 | 仍不明 → 抓该端口 SYN/心跳（§6、§7）；窗口/重传 → 13 |
...
| 已 ESTAB 仍慢 | **别回头改握手** | 五段 ttfb / TLS / `ss -ti`→13 |
...
玩家：「能进大厅进不了房间」
  → 不要在 443 上找房间握手失败
  → nc/抓 9000；看 SYN 还是心跳后 RST（心跳机制见 13）
```
...
flowchart TB
    ...
    subgraph l4 ["建连"]
        ...
        F -->|有 SYN-ACK 无 ACK| I[回程 / 全连接队列 12]
    end
...
**① Connection refused：** `ss -lntp` 没监听 → journal 是否 failed → Address already in use 则旧进程占口（Type=forking，见 11）。
...
**数字**：以太网 MTU **1500** · `ping -M do -s 1472`（+28=1500）· MSS 常 **~1460** · tcpdump 必带 **`-c` 和 `-w`** · 五段做差 **四次** · 心跳/NAT 数字见 13 · 表满见 17

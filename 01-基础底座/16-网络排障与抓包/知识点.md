...
> 登录偶发挂：curl 五段做差全正常、`nc` 秒通、mtr 全程 0 丢——[35](../35-网络排查命令/) 的五步走完了，还是说不清。群里开始甩锅，有人已经在生产机 `tcpdump -i any` 打终端，SSH 眼看卡死。
> **本章回答**：一次说不清的网络问题深排障怎么走——过滤器进阶、两端抓对比、MTU/PMTUD、读 pcap 认病，最后把锅钉在证据上。
> **不讲**：第一轮刀法（curl -w / dig / nc / mtr / tcpdump 最小纪律）→ [35](../35-网络排查命令/)；握手/窗口/重传 → [13](../13-TCP与UDP/)；分层封装与收包路径 → [12](../12-网络栈与Socket/)；TLS 看不到明文 → [14](../14-HTTP与TLS/)。
...
下一章：[iptables 与 conntrack](../17-iptables与conntrack/)。第一轮刀法：[35](../35-网络排查命令/) · 传输机制：[13](../13-TCP与UDP/) · 封装与收包：[12](../12-网络栈与Socket/) · NTP：[32](../32-NTP与时间同步/)。

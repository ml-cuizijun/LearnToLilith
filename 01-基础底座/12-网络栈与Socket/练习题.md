...
先对照 [知识点](知识点.md)：**先 §2 总图，再 §3 名词，再 §5～§8**。能讲清跨网段/ARP 再谈队列。每题标了覆盖范围：
...
flowchart TB
    ...
    C[ESTAB Send-Q 涨] --> C1[对端或网络 ZeroWindow 见 13]
...
SYN Cookie 干什么？——半连接满时不占队列槽，把状态编码进序列号。防 SYN Flood。正常负载不应长期靠 Cookie。握手细节见 13。
...
重启为什么都好？——进程退出内核收 fd；TW 也会随时间消散。不是根因消失。见 05。
...
- [ ] **§2 总图**：跨网段第一跳帧给谁？IP 目的是谁？路由器每跳剥帧→看 IP→ARP→换 MAC？
- [ ] **ARP**：为何封装时还没有目的 MAC？谁问？
- [ ] LISTEN Recv-Q vs ESTAB Recv-Q；backlog / somaxconn
- [ ] 新挂旧不挂 / 四个「满」
- [ ] 握手、抓包、软中断去哪章？

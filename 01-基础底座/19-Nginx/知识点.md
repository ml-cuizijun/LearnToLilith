...
> **本章回答**：一个 HTTP 请求从被 Worker 接住到回到玩家手里的一生——每一段什么会坏、499/502/504/503 怎么一眼定责、改配置为什么必须 reload。
> **不讲**：499/502/504 的 HTTP 语义与超时链「外短内长」→ [14](../14-HTTP与TLS/)；包进主机、backlog/Recv-Q → [12](../12-网络栈与Socket/)；TIME_WAIT → [13](../13-TCP与UDP/)；DNS/CDN 回源 → [15](../15-DNS与CDN/)；抓包 → [16](../16-网络排障与抓包/)；四层 LB → [18](../18-LVS与四层负载均衡/)、[20](../20-HAProxy与Keepalived/)。
...
- 📖 **读图**
  - 🛤️ 主线五段人生；「换岗」不在请求生命里，是这台 Nginx 自己的换代（七节）
  - 🩺 「归宿」最常出入：码在 access.log，痛可能在 Worker / 上游 / DB——先看码，再顺着 error.log 与两列时间往回定位段
  - ⚔️ 战斗 TCP 长连接**不在这条线上**——七层反代挂长连接是事故源，走四层 [18](../18-LVS与四层负载均衡/)/[20](../20-HAProxy与Keepalived/)
...
### 👷 Master 管进程，Worker 接连接 🎯⭐

- **Master** = 大堂经理：读配置、收信号、拉起/回收 Worker——**不端盘子**
- **Worker** = 接连接干活：`worker_processes auto` 时一核一个；每个是**单线程 + epoll 事件循环**
- **epoll** = 事件通知机制：哪个 fd 就绪内核才叫醒，不就绪的不耗 CPU——地基在 [12](../12-网络栈与Socket/)，本章只讲它怎么定容量

> 💡 **打个比方**：老式餐厅一服务员盯一桌（一连接一线程）；Nginx 是一个服务员站大堂中央，哪桌举手（fd 就绪）就先服务——桌子再多也不要等量人手。

#### ❓ 为什么不是一连接一线程？

- ❌ **一连接一线程**：几万连接 = 几万线程切换 → 上下文开销把 CPU 吃光
- ✅ **单线程 epoll**：一个 Worker 同时盯成千上万 fd，就绪才干活
- 👉 多 Worker **只为用多核**，不是「越多越快」——`auto` = 核数就是上限口径

> ⚠️ **别混**：Worker 多 ≠ 更快。写多于核数 = 几个 Worker 抢同一颗 CPU，延迟反而差。Master 挂了但 Worker 还在的窗口里，存量连接继续服务，只是没人收新配置和新 Worker。

### 📐 容量：一条反代请求占 2 个 fd 🎯⭐🔧

Worker 同时握**客户端 fd** 和**上游 fd**，反代按 **2 倍 fd** 估上限：
...
- `worker_processes auto` = 核数
- `worker_connections 10240+` 常见；`worker_rlimit_nofile 65535`，systemd `LimitNOFILE=65535` 要对齐（[24](../24-内核参数与sysctl/)）
- `multi_accept on`：一次事件循环 accept 多个新连接——**不是**「多 Worker」
- 入口 fd 满（新连接失败）和「上游 refused 的 502」是两种病：前者玩家连 Nginx 都进不来
...
- `Active` 顶着 `worker_processes × worker_connections` → 入口 fd 满
- `Reading` / `Writing` / `Waiting` = 读请求头 / 写响应 / 空闲 keepalive
- `accepts ≈ handled` 正常；`handled` 明显小于 `accepts` → 曾经打到连接上限
- 公网暴露 stub_status = 泄露容量，必须 `allow/deny` 钉死

> 📝 **面试**：Nginx 为什么能扛几万并发——「Master/Worker + 单线程 epoll，没有每连接一线程的切换开销；反代按 2 fd 估容量，上限 `worker_processes × worker_connections` 且对齐 nofile」。⭐（练习题 Q9）

本节对应练习题 Q9。Worker 会接了——同一个 URI，location 怎么匹配？proxy_pass 带不带斜杠差在哪？
...
反代是**七层（应用层）**动作：Worker 认得 Host、路径、头，能改写、缓冲、终结 TLS。
...
- 口诀：`=` > `^~`（跳过正则）> 正则按**书写顺序** > 最长前缀
- 经典事故：没写 ②③，所有 `.js` 被 ⑤ 的反代吃掉打进 Java，CPU 爆
- 查法：`nginx -T | grep -n location` 看生效顺序 → 加静态块 → `-t` → reload → `curl -I` 验证不再走 upstream

### 🔀 proxy_pass 尾斜杠：改路径的隐形开关 ⭐

```nginx
proxy_pass http://lobby;      # /api/login → 上游仍收 /api/login（原样带过）
proxy_pass http://lobby/;     # location /api/ 前缀被替换 → 上游收 /login
proxy_pass http://lobby/v2/;  # 换前缀 → 上游收 /v2/login
```

#### ❓ 为什么带不带尾斜杠差这么大？

- 🔑 **带 `/`** =「用 proxy_pass 的 URI **替换** location 匹配到的前缀」
- 🔑 **不带 `/`** = 原样把客户端 URI 转上游
- ❌ 改错 → 上游 **404**（路径不存在），**不是** 502
- ✅ 改完必须 `curl -v` 看上游 access 里的真实路径，不靠猜

> ⚠️ **别混**：URI 改错 = 404；502 = 连不上或响应非法。两码事。
...
- `X-Forwarded-For` 客户端可伪造——只信你管控的上一跳：`set_real_ip_from` + `real_ip_header` + `real_ip_recursive on`
- 活动限流用 `$binary_remote_addr` 当 key 时：前面还有 SLB → 看到的是 SLB 地址，整台会被当成「一个 IP」限死（NAT 出口同理，五节）
...
- 证书轮转 = 新 fullchain → `nginx -t` → reload
- 告警用 [22](../22-监控与告警/) blackbox `probe_ssl_earliest_cert_expiry`，**7 天 P1**
- 握手/证书链/0-RTT 语义 → [14](../14-HTTP与TLS/)，本章只钉指令

> ⚠️ **别混**：客户端到 Nginx 是 HTTP/2，Nginx 到上游仍常是 **HTTP/1.1**（keepalive 池）——两段协议别混成一段。

本节对应练习题 Q11、Q12。路认对了——上游挂了一台怎么摘？keepalive 三件套缺一不可吗？
...
- **轮询（默认）**：后端无状态、规格相近
- **weight**：机器规格不一，大的多扛
- **ip_hash**：按源 IP 粘滞——玩家换 NAT 出口/换网就破功
- **least_conn**：长连接、单请求耗时不均时优先；慢机可能反而吸走更多连接（它最快「变闲」）
- **hash / random**：按 key 粘滞 / 超大规模，按需

### 🩺 被动摘除：max_fails / fail_timeout 🔥

开源 Nginx 健康检查是**被动**的：失败累计到 `max_fails`（默认 1）就在 `fail_timeout`（默认 10s）窗口内摘掉，窗口过了再试。

```text
一台真死 → 被动摘 → 流量挤到活的（正常自愈）
...
- ❌ **被动** = 等真实请求失败才记账；窗口内仍可能把玩家打进死机
- ✅ **主动** HTTP 探活开源版很弱——要模块/商业版，或前面挂 HAProxy httpchk（[20](../20-HAProxy与Keepalived/)）
- 👉 发布摘机正确顺序：先 `server ... down;` → `-t` → reload → 再停进程；直接杀进程会吃一段 max_fails 滞后的 502 雨
- 灰度用 `weight=1` 对大 weight

### 🚕 keepalive 三件套 🔥⭐

> 💡 **打个比方**：keepalive 池像出租车不熄火接下一单；少一行配置 = 每单都熄火重新打火——后端 TIME_WAIT 照样爆。
...
#### ❓ 为什么写了 keepalive 还等于没开池？

- ❌ 只写 `upstream keepalive N`：代理默认易带 `Connection: close` → 每次请求新建 TCP → 后端 TW 爆、端口耗尽（[13](../13-TCP与UDP/)、[24](../24-内核参数与sysctl/)）
- ✅ 必须同时有 ②③；缺任一行 = 没开池
- 📦 **池是每个 Worker 各一份**：`keepalive 32` × 8 Worker ≈ 最多 256 条空闲连接——调到 2000 不是优化，是把上游 fd 打满
- ⏱ 上游空闲超时比池更短 → 上游先关、Nginx 拿旧连接 → `upstream prematurely closed connection` → **502**

> ⚠️ **别混**：`keepalive_timeout`（http 块，**客户端→Nginx** 空闲保活，常 65s + `keepalive_requests 1000`）≠ upstream 的 `keepalive N`（**Nginx→后端** 池大小）——一个管入口一个管出口。

> 📝 **面试**：「keepalive 三件套：`upstream keepalive N` + `proxy_http_version 1.1` + `Connection ""`；开源探活是 max_fails 被动摘，主动探活上 HAProxy 或商业版」。⭐（练习题 Q5）

本节对应练习题 Q5、Q10。池会管了——大响应、限流、静态各卡在哪？
...
- ✅ **on**：上游发完就解放——慢的是玩家侧，不该拖死后端
- ❌ **off「更快」**：慢客户端更易把 Worker 拖死；一般 API **保持 on**
- 👉 旋钮改之前先有证据：
...
- 请求体姐妹篇：`client_max_body_size` 太小 → 热更大包报 **413**，不是 502——`nginx -T | grep client_max_body_size` 一眼
...
location /download/ { limit_conn conn_limit 10; limit_conn_status 503; }
```

- `burst=200 nodelay`：前 200 个突发立刻处理；超了直接拒。不写 `nodelay` 则突发排队匀速放行，延迟变差
- 活动出口 NAT 会让「按 IP」变成「按整栋楼」——换 key，或上 [02-09](../../02-中间件与数据库/09-API网关与服务治理/) 按玩家 ID 限

> ⚠️ **别混**：限流的 503 和「上游全摘」的 503 是两回事——前者 error.log 是 limit 相关；后者是 `no live upstreams`。看日志分，不看码猜。

### 📁 静态：sendfile 与动静分离 ⭐

静态文件不过 proxy：`root` 直接由 Worker 让内核把文件页丢进 socket（**sendfile**——数据不进用户态来回拷贝），配 `tcp_nopush`（攒满报文再发，和 sendfile 成对）、`tcp_nodelay on`（关 Nagle，细节 → [13](../13-TCP与UDP/)）、`gzip`、长 `expires` + 可 `access_log off`。
...
资源版本与 CDN → [15](../15-DNS与CDN/)。

本节对应练习题 Q7、Q8、Q13。回到开头那个现场：登录全 502——码怎么一眼定责？
...
> 💡 **打个比方**：502 是快递到仓库门口没人签收；504 是签收员等太久放弃了；499 是买家等不及取消订单；503 是仓库整体关门或闸机限流。

#### ❓ 为什么 502 不是「上游超时」？

- ❌ 「502 = 上游慢/超时」——**超时读完是 504**
- ✅ **502** = 没拿到合法上游响应（拒连、RST、非 HTTP、prematurely closed）
- ✅ **504** = 连上了但 `proxy_read_timeout` 到点没读完
- 口诀：一句话「**连没连上**」分家
...
- 📖 **读图**
  - 第一步永远是 error.log——`upstream:` 写清打哪台，errno 写清为什么
  - `111` = 这台端口没人听：上游机 `ss -tlnp | grep 8080`、`curl -v` 复现 → 起进程/修发布——**不是**重启 Nginx，更不是重启战斗服
  - 若同一批是 `upstream timed out`，已经是 504 的故事

> ⚠️ **别混**：**502 vs 504** 是本章最高频混答。499 是**客户端**先走，锅不在 Nginx 也不在上游（语义全表 → [14](../14-HTTP与TLS/)）。

### ⏱️ 两列时间做差：慢在谁身上 🔧⭐🔥

排障 P99 之前，log_format 必须先打进这两列：
...
#### ❓ 为什么 urt≈rt 和 rt≫urt 要分家？

- **`$request_time`** = 从收到客户端第一字节到发完最后一字节的总时间
- **`$upstream_response_time`** = 花在上游身上的时间（建连 + 发请求 + 等响应）
- **差值 `rt − urt`** = 耗在 Nginx 自己和客户端身上的时间
  - `urt ≈ rt` 且都大 → 慢在上游，打后端/DB
  - `rt ≫ urt` → 慢在 Nginx 侧，查 `df`（缓冲落盘）、慢客户端、TLS
- 502 的 `urt` 是 `-`（没连上就没时间）；499 的 `urt` 可以很大——「客户端不等了」和「上游慢」常同时出现

> 📝 **面试**：「慢不先优 gzip。先看两列时间做差：`urt≈rt` 打后端，`rt≫urt` 查 Nginx 侧；502 的 urt 是 `-`，499 的 urt 可以很大。」⭐（练习题 Q2）
...
对齐原则只有一句「**外短内长**」（完整定责 → [14](../14-HTTP与TLS/)）：

```text
APP 15s  <  SLB 20s  <  Nginx proxy_read_timeout 25s  <  Java 硬超时 30s
```

- 只把 Nginx 调到 300s、APP 仍 15s → 499 照样飞
- 外长内短 → Nginx 先 504，玩家看到失败而 SLB 还没到自己的超时
- `proxy_next_upstream` 可对部分错误自动换下一台——**支付等非幂等 POST 默认不要开大**（幂等 → [14](../14-HTTP与TLS/)）

### 🧩 收束：一次 5xx 凭什么 60 秒定责 🎯⭐

面试问「Nginx 5xx 你怎么查」，答这条链——**六件套，一步不跳**：
...
- 📖 **读图**
  - ①②定性质（谁先放手、连没连上），③定段（上游/Nginx 侧），④⑤⑥拿证据
  - 六步走完还没结论，才轮到抓包（[16](../16-网络排障与抓包/)）和抓 DB 慢查询

> ⚠️ **别混**：这条链只保证「定位到段」，**不保证**告诉你「为什么慢」——上游慢的根因在业务/DB/依赖；在 Nginx 上加大 timeout 是把判决书涂掉，不是治病人。

> 📝 **面试**：背法「**码 → 字 → 差 → 验 → 真 → 盘**」六字诀。⭐（练习题 Q1）

本节对应练习题 Q1、Q2、Q3、Q4。码会认了——改一个 header，为什么不能 restart？
...
- 📖 **读图**
  - reload 的「平滑」= 新旧 Worker 短暂并存——新连接全走新配置，旧连接在旧 Worker 里跑完最后一程
  - `ps -o pid,ppid,etime,cmd -C nginx` 能看到「一批新、一批旧在退出」的交替

#### ❓ 为什么改配置不能 restart？

- ❌ `systemctl restart nginx`：杀光 Worker → 大厅 HTTP 长轮询当场全断
- ✅ `nginx -s reload`（HUP）：新 Worker 接新、旧 Worker 把存量做完再退
- 🔑 **为什么必须先 `-t`**：语法错时新 Worker 起不来，Master **拒绝换代**、旧配置继续服务——你以为「reload 了」其实没换上；`-t` 把坑变成桌面上的一次报错
...
- 回滚也是「还原 conf → `-t` → reload」，不是 restart
- 其余信号备查：QUIT 优雅退 Master、TERM/INT 快速关、USR2 热升级二进制（先在预发练）。生产日常 **reload + reopen** 两个就够
...
⑤ 验证：curl -sI https://login.game.com/health   # 200 + 新行为生效；盯 5xx 一个观察周期
```

> ⚠️ **别混**：reload 平滑 ≠ 战斗 TCP 能挂上来。reload 只救「配置变更」，救不了「七层扛长连接」——四层回 [18](../18-LVS与四层负载均衡/)/[20](../20-HAProxy与Keepalived/)。

本节对应练习题 Q6。现在再看群里那句「重启全部逻辑服」——60 秒怎么定责？
...
- 📖 **读图**
  - 入口永远是码，不是「重启试试」
  - 502 往上游查；504 先做时间差再分上游/本机；499 出本层（→ [14](../14-HTTP与TLS/)）；503 看日志分「池空」还是「限流」
...
本节对应练习题 Q1、Q14。开头那个现场——502 就重启逻辑服、改配置就 restart——你已经知道该怎么拦住他了。
...
| 问 | 30 秒答 |
|----|---------|
| Nginx 为什么高并发？ | Master/Worker + 单线程 epoll；多 Worker 只为多核，`auto`=核数。 |

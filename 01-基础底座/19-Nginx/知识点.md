...
> **不讲**：499/502/504 的 HTTP 语义与超时链「外短内长」口径 → [14](../14-HTTP与TLS/)；包怎么进主机、backlog/Recv-Q → [12](../12-网络栈与Socket/)；TIME_WAIT 机制 → [13](../13-TCP与UDP/)；DNS/CDN 回源 → [15](../15-DNS与CDN/)；抓包 → [16](../16-网络排障与抓包/)；四层 LB → [18](../18-LVS与四层负载均衡/)、[20](../20-HAProxy与Keepalived/)。

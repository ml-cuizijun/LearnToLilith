...
| **14** | **HTTP 语义、超时链口径、Keep-Alive 两边、TLS/证书、499/502/504 语义定责** |
| **19（本章）** | **Nginx 反代指令、upstream、缓冲、reload、limit_req、error.log 关键字落地** |
| **18 / 20** | 四层 LVS / HAProxy TCP、Keepalived |
| **03-04** | APISIX、动态路由、插件、etcd 热更新 |

本章是 **Nginx 作战手册**。14 告诉你「码是什么意思、超时链怎么对齐」；本章告诉你「在 Nginx 上敲哪条指令、看哪行日志、reload 会不会闪断」。
...
│  语义/超时链/TLS → 14   ｜ 四层 → 24/20   ｜ 动态网关 → 06-04               │
└──────────────────────────────────────────────────────────────────────────┘
```
...
若上游活着但 SQL 跑 40s，APP 15s 先走 → access **499**；Nginx `proxy_read_timeout 30s` 先到 → **504**。语义分工见 [14 §6](../14-HTTP与TLS/知识点.md)。
...
客户端 HTTP/2 到 Nginx，到上游仍常是 **1.1 池**——两段别混。入口 Keep-Alive 与上游池的**语义**见 [14 §4.4](../14-HTTP与TLS/知识点.md)；本章钉指令。

> **记住**：keepalive 必须三行。超时必须外短内长（口径见 14）。
...
> 499/超时链「谁先放手」的完整定责树 → [14 §6](../14-HTTP与TLS/知识点.md)。
...
只把 Nginx 调到 300、APP 仍 15 → **499** 照样飞。详见 [14 超时链](../14-HTTP与TLS/知识点.md)。
...
2. nginx -t
3. 业务低峰或可承受窗口
07. nginx -s reload（或 systemctl reload nginx）
...
支付等非幂等接口：慎用 `proxy_next_upstream` 对 502 盲目重试——口径见 [14](../14-HTTP与TLS/知识点.md)。
...
证书轮转：新 fullchain → `-t` → reload → blackbox 盯 `probe_ssl_earliest_cert_expiry`。**7 天 P1**（[22](../22-监控与告警/)）。握手/链/0-RTT → 14。
...
| 战斗长连接走 Nginx HTTP 反代 | 错 | 四层 18/20 |

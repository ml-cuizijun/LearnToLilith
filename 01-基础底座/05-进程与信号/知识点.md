...
- 主线是一个进程的一生，五个阶段串成一条时间线，每节讲一段；「挨打」那一段是全章枢纽——它决定了前面「为什么杀不掉」和后面「怎么死得体面」。
- 值班遇到「进程不对」，`ps` 的 STAT 就是 X 光机：D 照 IO、Z 照父进程、进程没了照退出码——先认状态再动手。
...
> 💡 **打个比方**：COW（copy-on-write，**写时复制**）像合租房——fork 时父子先共用同一套家具（物理页），谁要改动谁自己复印一份搬出去，没改的一直共用。

fork 不真的复制内存：只复制**页表**，父子页表指向同一批物理页并标成只读。谁先写谁触发缺页异常，内核才复制那一页、各持一份。**省的是「立刻复制物理页」，不省「内核的预留」**——fork 时内核仍按「子进程可能把所有页都写一遍」做检查，账不够就拒绝。
...
> ⚠️ **别混**：**S（可中断睡眠）vs D（不可中断睡眠）**——差在「事件能不能被信号打断」。S 等的是允许打断的事件（socket、锁、定时器），`kill` 一发就醒；D 等的是**不能中途拆**的 IO（拆了会破坏块设备或 NFS 协议一致性），所以信号进不去。**Load = 可运行队列（R）+ 不可中断睡眠（D），S 不计入**——大厅网关几万个连接全是 S，Load 可以接近 0。
...
**信号（signal）** = 内核发给进程的异步事件通知。每个信号有**默认动作**（多数是终止进程），应用也可以注册 **handler（信号处理函数）** 接管它——除了两个内核特批挡不住的：
...
**信号只在进程从内核态返回用户态的那一刻递送**。运行中（R）、可中断睡（S）都回得了用户态，信号立刻生效；D 状态回不了，任何信号（包括 KILL）都压在 pending 里等 IO 完成。
...
> 🔍 **现场**：Load 40、idle 70%，群里已经 `kill -9` 三连、进程还在——走这条线：
...
> ⚠️ **别混**：OOM Killer 发的是 **SIGKILL**——没有 handler 可跑，「OOM 时优雅退出失败」这个说法不成立，**根本没有**优雅退出。指望 OOM 前刷盘，只能靠 limit 余量、探针、降载（[07](../07-内存与OOM/)）。
...
> ⚠️ **别混**：**HUP = 重读配置换 worker，USR2 = 热升级二进制，TERM = 停服**——三件事三个信号，别答成「reload 就是重启 Nginx」。
...
> 📝 **面试**：按「**送达条件 → 挡不住的两个 → PID 1 特例 → 退出码 128+n**」分组背：先说信号要回用户态才递送（D 是反例），再说 KILL/STOP 不可捕获，再补容器 PID 1 没注册的信号被内核丢弃，最后用 137/139/143 收尾。四道检查一条线，比背零散结论高一档。
...
**僵尸进程（zombie process）** = 子进程已 `exit`、内核保留进程表槽位和退出状态，等父进程 `wait`/`waitpid` 来取。`ps` 里显示 `Z` 和 `<defunct>`。**占的是 PID 号，不是用户内存**——所以用 `free` 判断僵尸严重程度是错的。
...
> ⚠️ **别混**：**PPID=1 ≠ 僵尸**——孤儿的标志恰恰是 PPID=1 且活得好好的；僵尸的标志是 **STAT=Z**，PPID 指着那个没收尸的父。同理「杀僵尸 PID」永远没用，要么让父收尸、要么杀父让 init 代收。
...
进程被信号杀死时，**退出码 = 128 + 信号编号**——这三个数是值班路由表：
...
> 📝 **面试**：「容器里的进程为什么收不到 SIGTERM？」——两层：① 应用当了 PID 1，内核对 init 有特殊保护，没注册 handler 的信号一律丢弃；② PID 1 是 `/bin/sh -c` 这类不转发信号的 shell，TERM 停在 shell 手里。修法：**tini / dumb-init 当 PID 1、`docker run --init`、或 CMD 用 exec 形式**让应用自己当 PID 1 并 Listen TERM；现象侧的指纹是「每次停服都等满 grace 才退、日志里 exit 137」。
...
- [ ] **默画一节的图**：出生 → 干活 → 挨打 → 死亡 → 善终五个阶段各记什么；`ps` STAT 照哪几段。
- [ ] **口述出生**：fork 和 exec 各干什么、COW 省什么不省什么、贴着 limit fork 为什么 ENOMEM。
- [ ] **口述进程树**：PID 1 两个天职、孤儿怎么被收养、daemon 双 fork + setsid 是干嘛的。
- [ ] **口述干活**：五个 STAT 各是什么、Load = R+D 为什么不含 S、T 状态怎么解。
- [ ] **默画挨打**：信号从发出到递送的链路（pending → 回用户态 → 生效），D 那条支路为什么 KILL 也进不去。
- [ ] **口述挨打**：TERM vs KILL、OOM 走哪条、wchan 三种字样各修什么。
- [ ] **默画收束**：一次 kill 生效的四道检查（送得达 / 挡不住 / PID 1 特例 / 递送之后），并说清它不保证什么。
- [ ] **口述挨打余下**：SIGPIPE 为什么杀进程、HUP/USR2/TERM 三件事、信号合并、handler 安全。
- [ ] **口述死亡**：僵尸怎么来、杀僵尸为什么没用、孤儿 vs 僵尸、137/139/143 各查哪、core 落盘四检查。
- [ ] **口述善终**：停服一条链的顺序、grace 为什么 10s 不够、容器收不到 TERM 的两层坑、tini 解决什么。
- [ ] **定责**：STAT 决策树第一刀怎么切、fd 泄漏和 strace 怎么分工、60 秒剧本四步。

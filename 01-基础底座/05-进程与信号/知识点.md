...
| 章 | 负责 |
|----|------|
| **01** | 用户态/内核、syscall、双日志、不可用第一眼 |
| **02（本章）** | 进程状态、信号递送、僵尸/孤儿、优雅停服、容器 PID 1 |
| **03** | Load = R+D；高 Load 分型（CPU 型 / IO 型） |
| **04** | OOM 发 SIGKILL、137 对 dmesg |
| **07** | cgroup 限额、fork ENOMEM |
| **13** | systemd unit、TimeoutStopSec 全文 |

本章是**进程生命周期主场**。01 告诉你「服务不可用先看 status」；本章告诉你「进程还在时 STAT 是什么、能不能杀、怎么优雅停」。
...
| 28454 `Rl` | R + 多线程标记；在跑或可跑 | 高 `%us` → 03 |

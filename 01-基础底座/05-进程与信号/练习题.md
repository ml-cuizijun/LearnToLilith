...
先对照 [知识点](知识点.md)：**先 §2 总图，再 §5 僵尸 / §6 信号，再 §7～§9 定责**。能分清 STAT 与 TERM/KILL 再谈加核。每题标了覆盖范围：
...
- [Q8 fd 泄漏](#q8)
- [Q9 strace 与 futex](#q9)
- [Q10 fork ENOMEM](#q10)
- [Q11 Nginx reload / SIGPIPE](#q11)
...
处理的是 **IO 路径**，不是再杀一次。`gdb attach` 也会卡。D 很多时普通 reboot 可能卡在杀进程，走带外强制重启。D 计入 load、几乎不占用户态 CPU，所以「load 高先加 CPU」会走错（[06](../06-CPU与Load/)）。

### 再追问

- 能用 kill -9 以外的信号吗？——都不能在回到用户态前生效。STOP/TERM/KILL 同一类。见 §6.2。
...
- 僵尸占内存吗？——几乎不占用户内存。占的是进程表。用 Z 的数量和 `pid_max`。见 §5.4。
...
- 连发 10 次 TERM 会处理 10 次吗？——不一定。标准信号会合并，可能只递送一次。见 §6.5。
...
- shareProcessNamespace 能当停服方案吗？——那是调试用。正道是 PID 1 + 应用处理 TERM。见 §7.2。
...
- 全是 S 会不会把机器拖慢？——S 本身不占 CPU。若 S 是在等锁，真正干活的可能是持锁的那个 R。要看 `%us` 和锁，不要看 S 的个数吓人。细账 → [06](../06-CPU与Load/)。

---

## Q6

**★★★★☆｜孤儿进程和僵尸有什么区别？看到「没父进程」先想哪？**
...
有人指着 `ps` 说「这进程 PPID=1，是僵尸」。另一台有一排 `Z`。

### 完整解答

**一句话**：孤儿是活着被 PID 1 收养；僵尸是已死等父 wait——STAT 一眼分清。

| | 孤儿 | 僵尸 |
|--|------|------|
| 还活着吗 | **活** | **死** |
| STAT | 通常 S/R | **Z** |
| PPID | 变成 **1** | 仍是没 wait 的父 |
| 要慌吗 | 一般是故意 daemon | PID 槽涨要慌 |
| 处理 | 确认是否预期 | 修父 wait / 杀父 |

```bash
ps -eo pid,ppid,stat,comm | head
# PPID=1 且 STAT 不是 Z → 孤儿，正常收养
# STAT=Z → 僵尸，查原来的 ppid（变成 Z 前）或看谁下面一排 Z
```

> **别混**：PPID=1 ≠ 僵尸。僵尸的标志是 **STAT=Z**，不是 PPID。

### 再追问

- 父被杀后僵尸怎么办？——父死了，僵尸被 PID 1 收养并收尸，会消失。所以「杀父」是止血手段之一。见 §5。
...
| 码 | 信号 | 先查 |
|----|------|------|
| 137 | 9 SIGKILL | dmesg OOM、cgroup `memory.events`、人手 -9、kubelet 强杀（[07](../07-内存与OOM/)） |
...
- exit 1 呢？——那是进程自己 `exit(1)`，不是信号。看 journal 和业务日志，不是 dmesg（[04](../04-内核与系统/)）。
...
- **socket fd 涨**：连接没 close，常和 `CLOSE_WAIT` 一起出现——对端关了，本地应用没读完/没关（[12](../12-网络栈与Socket/)）。HTTP 客户端不关 Body 是经典。
...
- 只加 ulimit 行不行？——把爆的时间推迟。不修 close，还会再爆。见 §8.5。
...
| 看到什么 | 含义 | 下一步 |
|----------|------|--------|
| futex 极多 | 用户态锁竞争 | 减锁、查全局锁；不是加核（[04](../04-内核与系统/)、[06](../06-CPU与Load/)） |
...
不要在逻辑服里 fork curl。改成 sidecar、本地 HTTP、或把 limit 留出 COW 余量。细节见 [10](../10-cgroup与容器/)。

### 再追问

- clone 线程也会这样吗？——线程共享地址空间，比 fork 整个进程便宜得多。这个坑主要是 **fork 子进程**。见 §4.1、§4.2。

---

## Q11

**★★★☆☆｜`nginx -s reload` 用什么信号？SIGPIPE 为什么要 Ign？卡在 D 的 worker 呢？**
...
改了 nginx.conf，`nginx -s reload` 后长连接还在。有人问「是不是没生效」。另一边自写 C 网关偶发「写 socket 就整个进程没了」。

### 完整解答

**一句话**：reload 对 master 发 SIGHUP，旧 worker 处理完再退；写已关闭连接默认 SIGPIPE 会杀进程，网关常 Ign。

对 **master** 发 **SIGHUP**。master 重读配置、拉起**新 worker**；**旧 worker** 处理完手上的连接再退出。连接还绑在旧 worker 上，所以不断开。新连接由新 worker 接，配置已经是新的。

卡在 **D** 的旧 worker：HUP 换不掉它（回不了用户态，退不出）。要修 IO，或杀 master（会断连）。长连接 + 慢退的 worker 还会让内存看起来「reload 后翻倍」，直到旧 worker 退完。配合 `worker_shutdown_timeout`（[19](../19-Nginx/)）。

**SIGPIPE**：对端已关仍 `write`，默认把进程干掉。C/C++ 网关应 `signal(SIGPIPE, SIG_IGN)` 并检查 EPIPE。Go/Java 运行时通常已处理。

### 再追问

- USR2 呢？——那是热升级二进制，和 reload 配置不是一回事。不要混。见 §6.4。

---

## Q12

**★★★★☆｜给一段 ps / pstree，你怎么口述定责？kill 默认发几号？**
...
白板给你三行 `ps` 和一截 `pstree`，让你 60 秒定责。

```text
USER   PID  %CPU %MEM  STAT COMMAND
game 28451  0.0  2.1   D    logic-server
game 28452  5.2  1.8   S    nginx: worker
game 28453  0.0  0.0   Z    [curl] <defunct>
```

```text
logic-server(28451)─┬─{logic-server}(28455)
                    └─curl(28453)   # 若已是 Z，显示为 defunct
```

### 完整解答

**一句话**：先读 STAT 主字母——D 查 wchan，Z 查 ppid，S 通常正常；`kill` 默认 15 TERM。

| PID | STAT | 口述 |
|-----|------|------|
| 28451 | D | 卡 IO，kill 无效 → `wchan`；进 Load |
| 28452 | S | 可中断睡，不进 Load；5.2% 间歇醒 |
| 28453 | Z | 僵尸；父是 28451，修 wait，不杀 28453 |

`kill PID` = **SIGTERM(15)**。有状态服先 TERM 再确认；`-9` 是处决。`pstree -ap` 用来确认父子和容器里是不是 tini。

### 再追问

- STAT 里的 `l`、`s`、`+` 要背吗？——认主状态字母即可；`l`=多线程，细节见 §8.2。

---

## 合上书

- [ ] 默画 §2.0：进程树 + 状态机 + 信号递送 + 停服？
- [ ] 五个 STAT 各是什么？Load 包不包含 S？D 为什么 `kill -9` 无效？
- [ ] wchan 三种常见字样（nfs_wait / blk_mq / writeback）分别修什么？
- [ ] 僵尸占的是 PID 不是 RSS；孤儿 vs 僵尸？`kill -9` 僵尸 PID 有用吗？
- [ ] 游戏逻辑服怎么停？OOM 走哪条？grace 为什么 10 秒不够？
- [ ] 容器收不到 TERM 的两层原因？tini 解决什么？
- [ ] 137 / 139 / 143？`ps`/`pstree`/`kill` 怎么读？fd 与 strace 怎么分工？

七题能口述，这一章就过了。下一章：[CPU 与 Load](../06-CPU与Load/)。

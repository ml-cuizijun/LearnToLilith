...
开始做题前，先给大家一个小建议：合上知识点，对着**第一节的主图**，把「一个内核参数的一生（写下来 → 加载 → 定居 → 到进程 → 验收 → 回滚）」口述一遍。能顺下来，下面这些题你会发现大半都会了；卡壳的地方，就是你该回去重读的那一节。
...
题目的顺序也是有意排的：Q1～Q4 先钉三道门；Q5～Q10 常调参数坑；Q11～Q13 角色清单与纪律；Q14 白板收口。
...
就是开头 Send-Q 仍 128——min 关系。

先自己试试：somaxconn 和 backlog 谁截谁？
...
地图细考——TIME_WAIT 与端口耗尽。

先自己试试：tw_reuse 能不能当万能药？
...
地图细考——swappiness 与 overcommit。

先自己试试：调小 swappiness 就能防 OOM 吗？
...
先自己试试：fs.file-max / nproc / LimitNOFILE 各管哪层？
...
对齐知识点「合上书」，逐条口述，卡住就回对应节 / 题号。现在再看开头那个现场——万能清单、Send-Q 仍 128——你该知道先验哪道门了。

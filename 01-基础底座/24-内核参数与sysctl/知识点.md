...
> **不讲**：各参数的机制深挖——somaxconn/半连接全连接队列 → [12](../12-网络栈与Socket/)，TIME_WAIT/窗口/内核心跳 → [13](../13-TCP与UDP/)，swappiness/overcommit/OOM → [07](../07-内存与OOM/)，fd 与 rlimit 原理 → [05](../05-进程与信号/)，conntrack 表与 NAT → [17](../17-iptables与conntrack/)，安全向 sysctl（redirects/rp_filter）→ [27](../27-Linux安全加固/)，cgroup 限额与 sysctl 的分工 → [10](../10-cgroup与容器/)。

...
| 章 | 负责 |
|----|------|
| **12** | 包怎么进主机、Socket/队列、Recv-Q、半/全连接、「四个满」**含义与定责** |
| **13** | TCP/UDP：握手、窗口、重传、心跳；**tcp_* 旋钮在传输层干什么** |
| **17** | conntrack 表满、NAT；**nf_conntrack_*** 的表侧含义 |
| **24（本章）** | sysctl **怎么写、怎么持久、谁听、危险不危险、按角色裁剪** |

本章是**内核旋钮的落盘与边界主场**。12/13/17 告诉你「这个数管什么、满了怎么认」；本章告诉你「改到哪一层才算改了、哪些千万别碰」。
...
| ⑥ | 旋钮**含义**回 12/13/15；本章管**怎么改、改不改得动** |
...
│  含义：队列→12 · 窗口/心跳→13 · table full→17 · 安全 sysctl→27            │
└──────────────────────────────────────────────────────────────────────────┘
```
...
> **记住**：名词扫盲只建「门牌」。队列/窗口/表满的故事在 12/13/15。
...
/etc/sysctl.d/99-security.conf   # 安全向单独文件 → 27
```
...
**面试怎么说**：「tcp_* 的含义我按 12/13 定责；18 负责落盘、namespaced、和听谁的；LISTEN 顶满我绝不先加 rmem。」
...
08. **落盘**：角色 `sysctl.d` + `--system`；fd 改 unit 并**重启进程**  
...
flowchart TB
    ...
    B -->|dmesg table full| J["conntrack 80% · 含义见 17"]
...
| somaxconn 默认 128？ | 全连接 = min(listen, somaxconn)；活动开闸 Recv-Q 顶满。含义见 12。 |
...
| conntrack 满？ | 间歇丢新连接；>80% 告警；调 max 买时间，治本见 17。 |

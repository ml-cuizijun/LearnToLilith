...
> 逻辑服 `/data` 用到 95%，工单批了一块 500G 新盘。新同事直接 `mount /dev/vdb /data`，报 `wrong fs type`——从一块裸盘到业务能写的卷，中间隔着认盘、分区、LVM、文件系统、fstab 五道门，跳过任何一道都挂不上；还有人在 `umount` 时撞上 `target is busy` 不敢动，群里已经有人在喊「reboot 一下试试」。
> 这一章讲完，你会知道当时那个人错在哪，以及「扩完了 df 不变」该先看哪一层账本。
> **本章回答**：一块裸盘从被内核认出来、变成 PV/VG/LV、格式化挂载、在线扩容到安全下线的全链，以及「扩了盘 df 不变」「umount 说忙」这类工单的第一刀。
> **不讲**：盘慢与 IO 定责（await/脏页/iostat）→ [08](../08-磁盘与IO/)；文件系统机制（inode/journal/链接/fsck）→ [09](../09-文件系统与inode/)；备份策略与容灾 → [31](../31-备份与恢复/)、[33](../33-高可用与容灾/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节的主图，把「一块裸盘的一生」装进脑子；第二到第十节顺着认盘 → 分区 → RAID → 成砖 → 切卷 → 挂载 → 扩容 → 快照 → 退场往下走；作战节专门讲定责，也就是开头那个现场的正确解法。每一节讲完都会提示对应练习题，学完就去练。
...
咱们先不急着敲 `pvcreate`。学 LVM 最怕的就是上来背命令，背完还是 `wrong fs type`。先看图，把「五道门」装进脑子里：
...
**读图**：带着三个问题看这张图，比背命令名有用得多——主线从左往右是一次加盘的施工顺序——分区和 RAID 两道门是可选的（云盘常两道都跳过），但 PV → VG → LV 这三道门一张也少不了，它们就是 LVM 的三层。虚线是每层的观测命令：`lsblk` 的 TYPE 列、`pvs`/`vgs`/`lvs` 三兄弟分别照砖、池、卷——后面每一节都是把图上一个方块放大。扩容、快照、下线不是新状态，是这个卷「活着」期间会经历的三件事。
...
好，主图有了。咱们正式出发——第一站：怎么把盘认清楚？

---

## 二、认盘：lsblk 与磁盘命名 ⭐

🔥 开头那位直接 `mount /dev/vdb`——**连认盘都跳了**。大白话：**盘符是临时工号，UUID 才是身份证**。口诀：**「lsblk 认亲，UUID 落户」**（练习题 Q1）。
...
认盘段到这儿。本节对应练习题 Q1。好，盘认清了——要不要分区？MBR 还是 GPT？
...
分割段到这儿。本节对应练习题 Q2。好，分区定了——RAID 和 LVM 谁管冗余？
...
护体段到这儿。本节对应练习题 Q3。好，冗余归 RAID——LVM 三层到底是什么？

---

## 五、成砖：PV 与 LVM 三层结构 ⭐🔥

🔥 大家想想：分区焊死后满了怎么办？很多人会答「再分一个区挂过来」——**半对**。大白话：**PV 是砖、VG 是池、LV 是切出来的卷**；弹性全来自中间那层池。⭐ 面试必考（练习题 Q4）。
...
成砖段到这儿。本节对应练习题 Q4。好，三层懂了——从裸盘到 `/data` 可写，六步怎么敲？
...
创建段到这儿。本节对应练习题 Q5、Q6。好，卷起来了——重启还在吗？fstab 怎么写才不翻车？
...
落地段到这儿。本节对应练习题 Q7。好，挂上了——业务涨了要扩容，为什么有人扩完 df 不变？

---

## 八、长大：在线扩容与 df 不变的两大坑 🔥⭐🔧

🔥 这是值班天天见的坑。大家想想：`vgextend` 成功了，`df` 该不该立刻变大？——**不该**。口诀：**「三层账本：vgs 看池、lvs 看卷、df 看文件系统；哪层没动补哪层」**（练习题 Q8、Q9）。
...
长大段到这儿。本节对应练习题 Q8、Q9。好，扩容链通了——快照为什么会让 df 比 du 大？

---

## 九、分身：快照原理与 df 不等于 du ⭐🔥

🔥 「做了快照空间泄漏」是**高频错答**。大白话：**COW = 改块前先把旧块复印进快照区，旧块不释放**。⭐ 面试必考（练习题 Q10）。
...
分身段到这儿。本节对应练习题 Q10。好，快照懂了——想缩容、umount 报 busy，怎么退场？
...
退场段到这儿。本节对应练习题 Q11、Q12。现在再看开头那个 `wrong fs type` 和 `target is busy`——60 秒怎么定责？
...
作战段到这儿。本节对应练习题 Q13、Q14。现在再看开头那位直接 `mount /dev/vdb` 的同事——你已经知道该怎么拦住他了。
...
对齐练习题「合上书」，逐条口述，卡住就回对应节 / 题号。现在再看开头那个现场——直接 `mount /dev/vdb`、`wrong fs type`、有人喊 reboot——你该知道先查哪一层了。

...
| 章 | 负责 |
|----|------|
| **12** | 包怎么走、怎么进主机、Socket/队列、本机「满」了怎么定责 |
| **13** | TCP/UDP：握手、窗口、重传、心跳；战斗长连接 |
| **14（本章）** | **HTTP 语义、超时链、Keep-Alive、TLS/证书、499/502/504 定责** |
| **15** | DNS / CDN 回源、HIT/MISS、TTL |
| **16** | 抓包手法、`curl -w` 五段完整 SOP、路由/MTU |
| **19** | Nginx timeout / upstream keepalive / 限流 / reload |

本章是**应用入口层主场**。12/11 告诉你「连没连上、传输卡不卡」；本章告诉你「已经是 HTTPS 了，码是谁写的、证是哪一套、超时谁先放手」。
...
│  抓包 / 五段完整 SOP → 16　队列满 → 12　窗口 → 13　CDN → 15               │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| 在 HTTP / TLS 里 | 不在这一层 |
|------------------|------------|
| 499 / 502 / 504 / 503 / 500 语义 | iptables、conntrack（12/17） |
| 超时链、Keep-Alive、证书链、SNI | Nginx 指令细节（19） |
| TLS 04.2 2-RTT / 1.3 1-RTT / 0-RTT | 完整 curl -w 五段 SOP（11） |
...
```text
APP
  │  dig / 系统解析 → CDN VIP（细节 → 15）
  │  TCP 三次握手到边缘 :443（机制 → 13；连不上 → 16）
...
```text
t=0     活动开始，登录 QPS 暴涨
t=30s   上游部分 Pod OOM → exit 137（细节 → 07）
...
| **0-RTT** | TLS 04.3 早期数据；有重放风险 | §5.2 |
| **session 复用** | ticket/cache 少握手；≠ 0-RTT 早期数据 | §5.2 |
| **ttfb** | 到首字节；常 = 应用「开始拣货」 | §7；完整五段 → 16 |
| **队头阻塞** | 一条 TCP 丢包堵整连接；HTTP/2 仍在 | §4.5；战斗 → 13 |
...
| 「能进大厅进不了房间」 | **两条连接**（常不是本章 HTTP） | 别混战斗长连接 → 11/16 |
...
1. 活动 SQL 慢查询，登录 P99 从 200ms → 21s。  
2. 核对各层超时（上表）。  
...
**打个比方**：短连接像每次购物都重新办会员卡、过安检——TLS 04.2 要 **2-RTT** 握手，高峰每秒 1 万新连接，光「办手续」就把网关 CPU 打满。Keep-Alive 像会员卡刷一下直接进。
...
4. 修复：入口 + upstream 都开；再叠加 ECDSA + TLS 04.3 + session 复用（§5）。  

TIME_WAIT 堆积的 `ss` 认法 → [12](../12-网络栈与Socket/)；连接池治 TW 的传输侧口径 → [13](../13-TCP与UDP/)。

> **别混**：战斗长连接是 TCP/KCP（08），**不是** HTTP Keep-Alive。

**面试怎么说**：「短连接把 TW 和 TLS CPU 一起打爆。入口和 upstream 都要 Keep-Alive。TLS 04.2 是 2-RTT，1.3 是 1-RTT，高峰必须复用。」
...
| TLS | ECDSA + TLS 04.3 + session 复用 | 高峰仍用 RSA 当唯一入口 |
...
| TCP 都连不上 | 12/13/16，不是先换证 |

完整五段做差手法 → [16](../16-网络排障与抓包/)；本章负责 **TLS 语义与证书定责**。

### 5.2 握手 RTT：1.2 / 1.3 / 0-RTT

**一句话：** TLS 04.2 约 **2-RTT**，1.3 约 **1-RTT**；0-RTT 再省一轮但有**重放**——支付/登录态变更禁止。

| 版本 / 模式 | 大致成本 | 生产口径 |
|-------------|----------|----------|
| TLS 04.2 | ~2-RTT | 仍常见；高峰更吃复用 |
| TLS 04.3 | ~1-RTT | 标配方向 |
...
| 信号 | 含义 | 治法 |
|------|------|------|
| tls 段高 + worker `%us` 高 + RSA 证 | 握手吃 CPU | ECDSA + TLS 04.3 + session |
...
| 上游 localhost 很快，经网关 504 | 缓冲盘满 / 客户端极慢 / 路径问题 | buffering → 19；路径 → 16 |
...
> 🎯 值班第一刀：日志分码 → curl 做差定层 → openssl 看链。抓包不是本章第一刀（→ 16）。
...
| TLS vs 业务 | `curl -w` 做差 | tls 高 vs ttfb 高 | 完整五段见 16 |
| 强制 VIP | `curl --resolve` | Host=SNI=域名 | 别只改 hosts |
| 强制 1.2 | `openssl s_client -tls1_2` | 对比 1.3 | — |
| 连接复用概况 | `ss -s` / nginx keepalive | ESTAB 是否爆炸 | TW 认法见 12 |
...
**TLS CPU：** 先确认慢在 TLS 段，再换 ECDSA / TLS 04.3 / session。Keep-Alive 也能少握手。
...
| TLS vs 业务 | curl -w 做差（§7）；完整 SOP → 16 |
...
flowchart TB
    ...
    B -->|CDN 502 源站 localhost 正常| K[回源 IP + Host + SNI · 15]
    B -->|连不上 / 队列怀疑| L[06 / 11]
...
| HTTP Keep-Alive 就是战斗长连接 | 错 | 两层；战斗见 13 |
...
| HTTPS 慢第一刀抓包 | 错 | 先日志分码 + curl -w；手法在 16 |

**数字**：APP 常见 **8s** · Nginx `proxy_read_timeout` 常 **60s** · TLS 04.2 **2-RTT** · TLS 04.3 **1-RTT** · 证书告警 **14 天** · 建议超时链示例 **8 < 10 < 12 < 15**

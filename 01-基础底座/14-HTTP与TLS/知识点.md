...
- 📖 **读图**
  ...
  - 🔪 第一刀分流：dns/tcp/tls/ttfb 用 `curl -w` 做差（SOP → [02](../02-网络排查命令/)）；状态码类先看网关 error.log（Q1、Q3）
...
- 📖 **读图**：四段对上 `curl -w` 四段——dns↔host 解析、tcp↔到边缘 443、tls↔握手、ttfb↔「网关+上游开始拣货」。连采/误用见 [02](../02-网络排查命令/)，本章只读数。
...
- curl 侧等价物是 `--resolve`（七节）；命令 SOP → [02](../02-网络排查命令/)
...
- 反过来：`tls` 正常、`ttfb` 高 → 慢在应用/DB，跟握手无关（四节）。做差口径 → [02](../02-网络排查命令/)

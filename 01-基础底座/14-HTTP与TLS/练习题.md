...
先对照 [知识点](知识点.md) **§2 总图 + §6 499/502/504** 自己口述，再对答案。关键字（502/504/499、curl -w 做差、证书链、Keep-Alive）要能分码。
...
看 `curl -w` 做差。时间戳是从开始累计的，本段要用减法。完整五段 SOP 在 16，本章只要会拆 TLS 和业务。
...
短连接 = 每次完整 TCP + TLS。TW 和网关 CPU 一起涨。TLS 04.2 约 2-RTT，1.3 约 1-RTT。登录/支付 HTTP 入口必须开。上游连接池同样要开，只开入口不够。`keepalive_timeout` 过长占 fd，过短等于短连接。战斗长连接走 TCP/KCP，和 HTTP Keep-Alive 是两层（08）。TW 的 `ss` 认法见 12。
...
网关开了 TLS 04.3 session 复用，有人想把 0-RTT 开到支付回调「再省 1-RTT」。
...
RSA 握手吃 CPU。先确认慢在 TLS 段，再换算法，不要先加机器。curl -w 看 tls 段（appconnect − connect）。tls 高 + nginx worker %us 高 + RSA 证书 → ECDSA、TLS 04.3、session ticket、SSL 卸载或加入口实例。tls 低、ttfb 高 → 不是握手问题，去查上游。生产标配：ECDSA + TLS 04.3 + session 复用。Keep-Alive 也能少握手。
...
- [ ] **默画 §2.0**：四层路径 + 语义 + TLS + 499/502/504 + 第一刀
- [ ] 502 / 504 / 499 各是谁先放手？error.log 关键字是什么？（§6）
- [ ] 超时为什么必须从外到内递减？timeout=300s 为什么不是修复？（§4.3）
- [ ] HTTP/1.1 和 HTTP/2 对队头阻塞差在哪？战斗长连接能用 Keep-Alive 回答吗？（§4.4–4.5）
- [ ] 证书链三截是什么？过期、缺中间证、SNI 错各什么表现？（§5）
- [ ] 支付为什么禁止 0-RTT？RSA 高峰怎么治？（§5.2）
- [ ] CDN 502、源站 localhost 正常、边缘绿锁，先查哪一跳？（§5.4）
- [ ] 边界：队列→12，窗口→13，抓包五段→16，CDN→15，Nginx 指令→19

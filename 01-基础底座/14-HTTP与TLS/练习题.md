...
先对照 [知识点](知识点.md) **一节总图 + 四节 499/502/504** 自己口述，再对答案。关键字（502/504/499、curl -w 做差、证书链、Keep-Alive）要能分码。
...
**一句话**：502 是网关没拿到合法 HTTP 响应，504 是等到 proxy_read_timeout——断点不同。
...
**一句话**：499 是客户端先断开，玩家已失败但网关/上游可能还在跑 SQL。
...
**对应**：知识点 三节「RSA 高峰 CPU」 · 完整五段 SOP 见 [02](../02-网络排查命令/)
...
**一句话**：curl -w 时间戳是累计值，本段做差：tls=appconnect−connect，ttfb=starttransfer−appconnect。

看 `curl -w` 做差。时间戳是从开始累计的，本段要用减法。完整五段 SOP 在 [02](../02-网络排查命令/)，本章只要会拆 TLS 和业务。
...
**一句话**：证书链缺中间证部分客户端失败，过期几乎全挂，SNI 错拿到默认证。
...
**对应**：知识点 五节「Connection 与 Keep-Alive」
...
**一句话**：短连接每次 TCP+TLS，TW 和 CPU 一起涨——入口和 upstream 都要 Keep-Alive。

短连接 = 每次完整 TCP + TLS。TW 和网关 CPU 一起涨。TLS 1.2 约 2-RTT，1.3 约 1-RTT。登录/支付 HTTP 入口必须开。上游连接池同样要开，只开入口不够。`keepalive_timeout` 过长占 fd，过短等于短连接。战斗长连接走 TCP/KCP，和 HTTP Keep-Alive 是两层（[13](../13-TCP与UDP/)）。TW 的 `ss` 认法见 12。
...
**一句话**：HTTP/2 解决应用层并发，TCP 丢包仍堵所有 stream；HTTP/3 用 QUIC 流独立。

HTTP/1.1：一条连接一次一个请求，并发靠多开 TCP。HTTP/2 解决的是应用层队头阻塞：一条 TCP 上多 stream 并行。**TCP 丢包仍堵整条连接上的所有 stream。** HTTP/3 跑 QUIC/UDP，流之间不堵。

内网延迟已经很低时 HTTP/2 通常够（gRPC 常用）。公网是否 HTTP/3 看客户端和证书。不要答「上了 HTTP/2 就不会堵」。战斗长连接的队头阻塞见 13，不是给战斗上 HTTP/2。
...
**一句话**：503 是没有健康上游或主动限流，502 是有上游但响应非法或连接失败。
...
**一句话**：超时从外到内递减，APP 最短；timeout=300s 会让慢查询继续打 DB。
...
**一句话**：0-RTT 有重放风险，支付/登录态变更禁止，静态资源才考虑。
...
**对应**：知识点 三节「RSA 高峰 CPU」「会话复用与 0-RTT」
...
**一句话**：RSA 握手吃 CPU，tls 段高先换 ECDSA+TLS1.3+session 复用，不是先加机器。
...
**对应**：知识点 七节「入口证 vs 回源证」 · 交叉 [15](../15-DNS与CDN/)
...
**一句话**：CDN 502 查回源那一跳：源站 IP+Host+SNI，边缘绿锁只说明边缘证没问题。

入口证书和回源证书是两套。边缘绿锁只说明**边缘证**没问题。502 出在回源那一跳。用「源站 IP + Host 头 + SNI」从公网或模拟回源测，不要只在源站 localhost 测。安全组放行 CDN 回源网段。HIT 不应 502；若 X-Cache 是 HIT 还 502，那是脏缓存或边缘自己的问题，交叉 [15](../15-DNS与CDN/)。登录域名不要和包体 CDN 共用一张要频繁 Purge 的证。
...
**★★★★☆｜反向代理下 Host 头、X-Forwarded-For 各管什么？配错会怎样？**

**覆盖**：必会 · 常用 ｜ **对应**：知识点 [三、认路：location 与头改写](知识点.md) · 交叉 [19](../19-Nginx/)

### 场景

多租户登录共用一个网关 IP，按 Host 路由。有人用 IP 直连测试「明明通」，线上 Host 路由全 404。

### 完整解答

> 🎯 **一句话**：**Host 决定虚拟主机路由**；`X-Forwarded-For` 传递客户端真实 IP——IP 直连不带 Host 会打到默认 server。

```bash
curl -H 'Host: login.example.com' http://10.0.0.5/health
...
|----|------|------|
| Host | 七层选站 | 404/错证书 |
| X-Forwarded-For | 上游记真实 IP | 限流/审计失效 |
| X-Forwarded-Proto | 上游知是否 HTTPS | 重定向循环 |

### 再追问

- 回源 HTTPS 要带什么？——Host + SNI 一致（Q11）。

---

## Q13

**★★★☆☆｜HSTS 和混合内容（Mixed Content）各是什么坑？**

**覆盖**：必会 · 常考 ｜ **对应**：知识点 [三节 TLS 策略](知识点.md)

### 场景

页面 HTTPS 绿锁，但活动页引了 `http://` 的 JS，部分渠道白屏。有人只续服务器证。

### 完整解答

> 🎯 **一句话**：**HSTS** 强制后续走 HTTPS（防降级）；**混合内容**是 HTTPS 页里夹 HTTP 子资源——浏览器直接拦。

```http
Strict-Transport-Security: max-age=31536000; includeSubDomains
```

活动页所有静态必须 `https://` 或相对路径；`http` 资源在 WebView 里常被整页 block。

### 再追问

- HSTS 配错能回退吗？——`max-age` 内浏览器记住，别在测试域乱开 `includeSubDomains`。

---

## Q14

**★★★★★｜白板：499/502/504 + TLS 慢 + CDN 502，一张定责表怎么讲？**

**覆盖**：核心 · 必会 · 常考 ｜ **对应**：知识点 [四、归宿](知识点.md) · [七、入口证 vs 回源证](知识点.md) · [九、收口](知识点.md)

### 场景

白板综合：五种 HTTP/TLS 现象各一句签名、一条命令、一个邻章。

### 完整解答

> 🎯 **一句话**：**先分码再分段时间**——5xx 看谁先放手；慢用 curl -w 做差；CDN 502 查回源一跳。

| 现象 | 签名 | 第一刀 | 邻章 |
|------|------|--------|------|
| 499 | 客户端先走 | 对齐超时链 | 19 upstream P99 |
| 502 | 上游非法/断 | error.log + 上游退出码 | 16 抓 FIN/RST |
| 504 | 网关等满 | 超时链外→内 | 上游硬超时 |
| tls 段高 | 握手 CPU | ECDSA/TLS1.3/session | 19 SSL 配置 |
| CDN 502 源站 localhost 通 | 回源证/Host/SNI | 公网模拟回源 | 15 |

```bash
curl -o /dev/null -s -w 'dns:%{time_namelookup} tcp:%{time_connect} tls:%{time_appconnect} ttfb:%{time_starttransfer}\n' https://...
```

> 📝 **面试**：讲完表补「支付禁止 0-RTT」「timeout=300s 不是修复」。

### 再追问

- 队列/窗口问题去哪？——12/13，别在本章硬抠 TCP。

---

## 合上书

- [ ] **默画一节**：四层路径 + 语义 + TLS + 499/502/504 + 第一刀
- [ ] 502 / 504 / 499 各是谁先放手？error.log 关键字是什么？（四节）
- [ ] 超时为什么必须从外到内递减？timeout=300s 为什么不是修复？（四节·超时链）
- [ ] HTTP/1.1 和 HTTP/2 对队头阻塞差在哪？战斗长连接能用 Keep-Alive 回答吗？（五、六节）
- [ ] 证书链三截是什么？过期、缺中间证、SNI 错各什么表现？（三节）
- [ ] 支付为什么禁止 0-RTT？RSA 高峰怎么治？（三节）
- [ ] CDN 502、源站 localhost 正常、边缘绿锁，先查哪一跳？（七节）
- [ ] Host / X-Forwarded-* 反代头（Q12）
- [ ] HSTS 与混合内容（Q13）
- [ ] 五现象定责白板表（Q14）
- [ ] 边界：队列→12，窗口→13，curl -w 五段→02，抓包→16，CDN→15，Nginx 指令→19

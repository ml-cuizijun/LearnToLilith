...
> 凌晨两点告警群炸了：「支付 5xx 飙红」。有人 `vim` 打开 5GB access.log，终端当场卡死；有人 `grep` 翻了一小时，天亮了还答不出「哪个接口、几点开始、怨谁」。
> **本章回答**：一份 5GB 日志怎么在 5 分钟内切成一条能定责的证据——列号是合同，先缩小再聚合，改文件必先预览。
> **不讲**：正则引擎与回溯深挖 → [05-04](../../05-开发能力/04-正则与文本处理/)；Shell 脚本骨架与 pipefail → [05-01-02](../../05-开发能力/01-Shell/02-脚本骨架与三开关/)；日志采集与告警体系 → [23](../23-日志运维/)。
...
- 📖 **读图**
  - 🛤️ 主线从告警进、从证据出；中间四步每步是一条可复制命令，不是「思路」
  - 🔀 右下两条虚线是同晚最常见岔路：改配置走 sed、JSON 走 jq——不改主线，只换其中一步的工具

> 💡 **打个比方**：三剑客是淘金流水线——grep 是筛子（5GB 泥沙 → 一脸盆），awk 是秤（按成色分堆），sed 是镊子（改之前先在纸上比划）。没有筛子直接上秤，秤会被压垮。

> 📝 **面试**：「grep 把行数变少（过滤），awk 把行变成数（聚合），sed 把字改掉（编辑）；JSON 走 jq，不让 awk 硬拆。」——先说分工，再谈细节。

- 🚫 **四个禁区**（整章回扣）🔥⭐
  - **5GB 直接 `sort`**——内存扛不住全量排序；先 grep 缩小再聚合
  - **不验列就信 `$9`**——`log_format` 一改，统计全错还不报错
  - **裸 `sed -i` 改生产**——没预览、没备份，改坏即事故
  - **awk 硬拆 JSON**——转义嵌套一层就崩；结构化数据走 jq

练习题 Q1、Q14。主线有了——先把列号合同钉死。
...
🔥 很多人 grep 完就直接 `$9` 当状态码——**错了**。口诀：**「列号是合同，先验列再统计」**。

- 📜 **列号合同**：Nginx access.log 按空格切开，第 N 个字段 = awk 的 `$N`；合同由 `log_format` 定义。标准 combined 再追加 `rt=`/`urt=` 后，本钱是：
...
- ❌ **列号漂了不报错**：`log_format` 在 `$status` 前加 `$host`，状态码从 `$9` → `$10`；旧脚本拿 `$9` 统计出来是 0 或全是字节数——**静默出错数**
- ✅ **防法就一条**：`log_format` 变更必须同步吃这张表的脚本；怀疑漂移时重跑验列仪式
- 🔑 `$NF` 是「最后一列」，不是「耗时列」——只有验列确认 `rt=` 恰好最后一列时才碰巧等于耗时；抽耗时用 `match()` 按内容抓（四节）

> ⚠️ **别混**：`$NF` ≠ 耗时列。格式一变它抓到的是 `urt=` 或别的东西。

练习题 Q4、Q5。列对了，下一刀 grep 缩小。
...
- 📖 **读图**
  - 📉 数据量每步掉 1~2 个数量级 = 「先缩小再聚合」的全部含义
  - 🧭 在 4000 万行上只跑**单次流式扫描**（grep/awk）；排序/去重要全量进内存，必须等缩到万~百万行
  - 🛡️ 真有全量排序硬需求：`LC_ALL=C sort -S 2G -T /data/tmp`——`-S` 限内存、`-T` 指定临时目录，别把 `/tmp`（常在内存盘）和业务一起打爆
...
- 🧰 **常用选项口诀**（记用法别背表）
  - `-E` 扩展正则（生产默认）· `-F` 固定字符串（查 IP/报错原文，快且不踩转义）
  - `-i` 忽略大小写 · `-v` 反选（排除健康检查全靠它）
  - `-n` 行号 · `-c` 计数 · `-o` 只出命中片段 · `-R --include` 递归圈范围

> ⚠️ **别混**：**`grep -c` 数的是「命中的行数」，不是「命中次数」**——一行命中 3 次也只计 1。要数次数用 `grep -o pat f | wc -l`。工单里「报错 5000 条」和「报错 5000 行」是两个口径。`-F` 的快不只是省转义：固定串走确定算法，比正则回溯快一个量级。

- 📐 **姿势纪律**
  - 写 `grep pat file`，不要 `cat file | grep pat`——多一个进程，少一分利落
  - 文件大时先 `wc -l` 和 `tail -1` 确认在滚动、没被 logrotate 切空（切走见 [23](../23-日志运维/)）
  - 要前后文：`-A 3` / `-B 2` / `-C 1`；Java 堆栈典型写法 `grep -n -A 20 'Exception' app.log`；多组命中之间 grep 打 `--` 分隔
...
> ⚠️ **别混**：**`-m` 是「够了就停」，不是 head**——`grep x | head` 仍要等 grep 吐满管道缓冲，`-m` 才是让 grep 自己刹车。`Binary file xxx matches` 不是报错：混进二进制段时 grep 只回这一句，要真看内容加 `-a`，但先怀疑文件本身是否被写坏。

### 🪤 三个正则深坑 ⭐

#### ❓ 为什么 `grep 'error+'` 匹配不到？

- ⚠️ **BRE vs ERE**：grep 默认是 **BRE（Basic Regular Expression）**，`+ ( ) { } |` 必须反斜杠转义才当元字符；`grep -E`（等价 `egrep`）才是 **ERE（Extended Regular Expression）**
- ✅ 生产脚本一律写 `-E`，省下每一次「为什么匹配不到」的排查
...
- 🔥 **贪婪匹配（greedy matching）**：`.*` 一路吃到行尾再回吐——`".*"` 从第一个引号吃到最后一个。抽定界符之间用否定字符类 `[^"]*`。IP 别写 `[0-9.]+`，写 `([0-9]{1,3}\.){3}[0-9]{1,3}`
- 💥 **回溯爆炸（catastrophic backtracking）**：`(.+)+` 这类嵌套量词会指数级回溯——本节只立规矩「量词套量词不写」，机理 → [05-04](../../05-开发能力/04-正则与文本处理/)

> 📝 **面试**：「我写 grep 一律带 `-E`；查固定字符串用 `-F`；抽片段用 `-oE`；正则里尽量用字符类和定界符，少用 `.*`。」
...
### 📂 除了 access.log：error.log、journalctl、OOM 🔧

- **error.log 按关键字进**，四个高频词：`upstream timed out`（上游慢）、`connect() failed` / `Connection refused`（上游拒连）、`no live upstreams`（上游全下线）。命中 `upstream timed out` 基本指向上游或网络，再深走 [19](../19-Nginx/)
...
- **OOM 线索**：`grep -iE 'out of memory|oom-killer' /var/log/messages`——命中只是线索，根因 → [07](../07-内存与OOM/)
...
- ⚙️ **执行模型**：`模式 { 动作}`——每行先过模式（正则、比较、`BEGIN`/`END`），命中才执行；没写模式就对每行执行；没写动作就打印整行
- 📦 **术语内联**
  - **字段（field）** = 一行按**分隔符（delimiter）**切出的第 N 块：`$0` 整行、`$1` 第一块、`$NF` 最后一块
  - **NR** = 全文件累计行号；**FNR** = 当前文件内行号；**NF** = 当前行字段数
  - **FS** = 输入分隔符（`-F` 改，默认空格）；**OFS** = 输出分隔符
  - **BEGIN/END** = 第一行之前 / 最后一行之后各执行一次的钩子
  - **关联数组（associative array）** = 下标可以是任意字符串的哈希表，`c[$7]++` =「以 URI 为 key 计数」
...
- ❌ `sort | uniq -c` 先把 16 万行完整排序再数——数据量小时没毛病，**5GB 全量进 sort = 禁区 ①**
- ✅ awk 关联数组是**一次流式扫描**：不排序、不落盘，过滤 + 计数 + 分组塞进同一条命令
...
`substr($4,14,2)` 只取「小时」，**跨天会叠加**——昨天 10 点的一小波和今天 10 点的爆发会被数成同一个格子。跨天 key 必须带日期：`substr($4,2,11)` 从第 2 位（跳过 `[`）取 11 位拿到 `21/Jul/2026`。
...
- ❌ `rt=`/`urt=` 带前缀，直接拿 `$11` 还得剥壳；拿 `$NF` 又踩格式漂移
- ✅ 用 `match()` 加捕获组按**内容**抓：
...
- 📌 `match()` 第三个参数 `m` 是 gawk 扩展：`m[1]` = 第一个捕获组。同理抓 `urt=`——rt ≫ urt 说明慢在 Nginx 本机/排队，两者接近说明慢在上游

### 🧹 坏行与多文件

日志里混着残行（写一半崩了、多进程交错），按列统计前先设防；多机对比用 `FILENAME` 分组：
...
- 🔑 **NR vs FNR**
  - **NR** = 全局行号（所有文件连着数）；**FNR** = 当前文件内行号
  - 多文件都要跳表头：`FNR == 1 { next }`（写成 `NR == 1` 只会跳第一个文件的表头）
  - 第一个文件当「名单」查第二个：`NR == FNR`（还在第一个文件里）做分支
...
> ⚠️ **别混**：`FNR == 1` 不是「第一行数据」，是「每个文件的第 1 行」——单文件时和 `NR == 1` 等价，多文件时天差地别。
...
- 📖 **读图**
  - ⬅️➡️ 一行进来：切字段 → 过模式 → 进数组，全程一次扫描不回头
  - 🪝 BEGIN 在最左端之前跑一次（表头/初始化），END 在最右端之后跑一次（汇总输出）
  - 📦 四件套证据全部发生在「数组累积 + END 输出」这两格
...
> ⚠️ **别混**：Top IP 是**线索**不是**判决**。量级最大那个 IP 十有八九是 LB/NAT 出口或运营商代理；真要定位单点看 `X-Forwarded-For` 或网关日志。结论落在 Top URL + 时间窗 + 样本日志上，封 IP 必须走变更流程。

> 📝 **面试**：「awk 记一个骨架：模式-动作 + 关联数组。Top N 固定写法 `$9~/^5/ {c[$7]++} END{for(u in c) print c[u],u}` 再 `sort -rn | head`；时间窗用 substr 切 $4，跨天 key 带日期；耗时列用 match 捕获组抓，不拿 $NF 赌运气。」
...
> 💡 **打个比方**：模式空间是流水线上的一个小工作台——复印一页上来，改这一页，发出去，再复印下一页。它从不同时拿着整本书，所以 5GB 文件也改得动，内存里永远只有一行。
...
- 📖 **读图**
  - ➡️ 从左到右是唯一合法路径：缺 diff = 盲改，缺校验 = 赌 reload 不炸
  - 🛑 任何一步不符合预期，回上一步重写表达式，**不许跳步**
...
### 🍎 macOS 的坑与「什么时候别用 sed」

> ⚠️ **别混**：**GNU sed**（Linux）与 **BSD sed**（macOS）的 `-i` 不是一回事。GNU 写 `sed -i 's/a/b/' f`；BSD 的 `-i` 必须跟备份后缀，`sed -i '' 's/a/b/' f`（空串 = 不留备份）。在 Mac 上照抄 GNU 写法，sed 会把 `s/a/b/` 当成后缀、把 `f` 当成脚本。跨平台统一写 `-i.bak`。

- 🚫 **三条红线，过线就换工具**
  - **多台机器** → Ansible（[04-06](../../04-工程化与自动化/06-Ansible/)），别挨个 ssh 手敲
  - **YAML/JSON** → yq/jq，缩进即语义，sed 改 YAML = 蒙眼动刀
  - **带条件分支**（改 A 行当且仅当 B 行存在）→ Python（[05-02](../../05-开发能力/02-Python/)），sed 没有靠谱的条件流

> 📝 **面试**：「sed 只做单机纯文本的确定性替换，流程固定四步：grep -n 圈行、预览、diff -u、-i.bak 落盘加服务校验。多机走 Ansible，结构化配置走 yq/jq。」

### 🔗 sed -i 的暗坑：inode 会换，硬链接会断 ⭐

#### ❓ 为什么 sed -i 会断硬链接？

- ❌ **`sed -i` 不是「原地改字节」，是「写临时文件再改名顶替」**——改完 inode 换了一个
- 🧨 三件事会露馅：
  - **硬链接断开**——别的名字还指向旧 inode，共享配置硬链悄悄分叉
  - **属主和权限重置**——root 跑的 sed -i，文件可能变成 root 属主，服务 reload 直接 Permission denied
  - **软链接被替换成实体文件**——改的是软链本身，不是它指向的那份
...
- 🛡️ **保 inode**：预览通过后 `sed 's/…/…/' f > f.new && cat f.new > f`——用 cat 重定向写回原文件，inode / 属主 / 权限 / 硬链接全部保住。多数「重写式」编辑器同理，改完关键配置都值得 `stat` 一眼
...
- ⚠️ **三剑客管无 schema 的纯文本，jq 管结构化 JSON**
- ❌ `awk -F'"'` 拆 JSON 看着能跑，字段里带转义引号、嵌套对象一层就崩，而且字段取错也不报错
- ✅ 判据一句话：能被 `jq .` 解析的就是 jq 的地盘
...
- 🧭 **陌生 JSON 先探结构**：`jq 'keys' pod.json` 看顶层字段、`jq '.[0]' events.json` 抽第一条样本、`jq 'keys_unsorted'` 保留字段顺序——路径先验过再进管道

> ⚠️ **别混**：`jq` 路径写错**不报错只出 null**——和 awk 列号漂移一样是「静默出错数」；`// "default"` 是给 null 兜底，不是掩盖路径错误。

jq 的本职在 JSON 查询与改写；配 kubectl 只是最高频场景。Pod 排障主线在 [03-Kubernetes](../../03-Kubernetes/)，这里只钉住「取数用 jq」。
...
- 📏 **三条纪律**
  - **`-r` 必带**——上游没命中时 xargs 仍会执行一次，`xargs rm` 变成裸 rm
  - **文件名走 `-print0 | xargs -0`**——默认按空白切，`game log 01.log` 会被拆成三个
  - **先打印再动手**——把 `xargs rm` 换成 `xargs -n1 echo` 跑一遍，眼见名单再换回真命令
  - 批量控节奏：`-n 20` 分批、`-P 4` 限并发——`xargs -n1 -P4 -I{} curl -sf http://{}/health` 就是四路探活最小写法
  - 同族：`find -exec cmd {} \;` 每文件起一进程；`find -exec cmd {} +` 批量传参（等价 xargs，且天然不拆文件名）；简单场景 `-exec +` 更省心，要并发和分批才是 xargs 的地盘
...
- 📖 **读图**
  - 1️⃣ 第一层判形态（JSON 直接去 jq——最常走错的岔口）
  - 2️⃣ 第二层判目的（行→grep、数→awk、改→sed）
  - 3️⃣ 第三层两道安检：改文件前问「多机/结构化/带条件」，聚合前问「数据量缩够了没」
...
- **10–30s**：`grep -c '" 50[0-9] '` 估量；`grep -E 'upstream timed out|refused' error.log | head -3` 抓方向；看堆栈加 `-A 20`，只要样本加 `-m 10`（三）
- **30–45s**：一条 awk 出 Top URL + 小时分布（四）
- **45–60s**：Top IP 交叉 + 平均 rt；把「Top URL、开始时间、样本 5 行、怀疑方向」贴进工单

- 📋 **两条值班纪律**
  - one-liner 只是**当场听诊器**，长期指标走监控（[22](../22-监控与告警/)）
  - 贴工单前脱敏——IP、token、用户数据打码，样本 5 行足矣
  - 数据量大到没信心时先抽样：`tail -n 500000` 或 `awk 'NR % 50 == 0'`，量级够了再决定全量

> 📝 **面试**：「四个不做：不做全量 sort、不盲信列号、不裸 sed -i、不让 awk 碰 JSON；Top IP 只是线索不直接封；one-liner 结论要落成工单证据，长期看的东西进监控。」

练习题 Q1、Q9、Q12、Q14。回到开头那个现场：正确剧本是先验列 → grep 缩小 → awk 聚合 → 需要改再 sed 预览。

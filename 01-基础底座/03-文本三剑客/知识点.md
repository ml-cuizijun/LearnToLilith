...
| 章 | 负责 |
|----|------|
| **28（本章）** | 值班 **one-liner**：grep 缩小 → awk 聚合 → sed 改（先预览）；Nginx access / journal / error 的**列纪律**与可复制模板 |
| **06-05** | 正则引擎、贪婪/回溯、Python `re`、复杂命名组 |
| **06-01** | Shell 编排、`set -euo pipefail`、管道退出码 |
| **22** | 日志从哪来、轮转 reopen、盘满、采集 |
| **23** | Nginx 超时链、502/504/499 语义、`log_format` 该打什么字段 |
| **26 / 27** | 主机 / 网络排查**命令刀法**；本章只管「文本怎么切」 |

本章是**排查命令刀法线的文本段**。26/27 告诉你敲什么看主机和网络；本章告诉你对着几 GB 日志，**怎么不把跳板机打挂、怎么不把列号抄错**。
...
告警 / 客服喊 5xx
    │
    ├─→ 23 Nginx：502/504/499 语义、upstream、两列时间
    ├─→ 22 日志：文件还在不在、有没有被 rotate 切走、盘满没
    ├─→ 28 本章：对着现有日志，5 分钟出 Top URL / Top IP
    └─→ 06-05：要写可入库脚本、命名组、复杂正则再下去
```
...
| 多行 YAML 审计镜像 | grep `image:` | **yq** / Python `safe_load`（06-05） |
...
若 `log_format` 把时间打在行尾 `rt=0.045 urt=0.042`（见 23 推荐格式），**不要假设固定 `$NF`**：
...
解读：504 且 urt≈30 → 上游读超时；502 且 urt=`-` → 没连上 upstream（23）。
...
**③ 回溯爆炸**（知道即可）：嵌套量词 `(.+)+` 遇畸形行可 CPU 100%——生产过滤用字符类与长度限制；原理 → 06-05。
...
# --- 499 Top URL（客户端取消，常与超时相关，语义见 23）---
awk '$9 == 499 {c[$7]++}
     END {for (u in c) print c[u], u}' /var/log/nginx/access.log \
  | sort -rn | head
...
| Top IP 直接封禁 | 出口 NAT / 误伤整栋 | 只当线索；变更单 + 限流（23） |
...
23 Nginx ：502/504/499 各是谁的锅
09 HTTP  ：协议与网关语义
22 日志  ：文件为何不在 / 盘为何满
21 监控  ：要不要叫醒、SLO
06-05    ：正则为什么这样写、Python 怎么沉淀
```
...
| 和 06-05 怎么分？ | 本章 one-liner 刀法；正则原理与 Python re 在 06-05。 |

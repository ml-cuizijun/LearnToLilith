...
- [ ] **口述**主线：5GB 日志变证据的五步，每步的输入输出量级（一）
- [ ] **默画**主图：对表→缩小→数数→交叉→落盘，加上 sed 和 jq 两条岔路（一）
- [ ] 说出四个禁区，以及各自对应的事故画面（一）
- [ ] **默写**验列仪式那条 awk，并指出 `$4` 为什么要偏移 1 位再取小时（二）
- [ ] 列号为什么会漂移？统计为 0 和全错数分别怎么暴露出来？（二）
- [ ] grep 生产姿势三件事：`-E` 默认、估量先行、别 `cat \|`（三）
- [ ] `grep 'error+'` 为什么匹配不到？BRE/ERE 转义差在哪？（三）
- [ ] 抽引号内内容为什么不能用 `.*`，正确写法是什么？（三）
- [ ] **默写** Top URL 那条 awk，并说清跨天时 key 怎么改（四）
- [ ] 一条 awk 一次扫描能交出哪五件证据？分别回答什么问题？（四·收束）
- [ ] sed 改生产配置的安全四步，macOS 上 `-i` 的坑在哪？（五）
- [ ] sed -i 为什么会断硬链接、重置属主？保 inode 的写法是什么？（五·5.4）
- [ ] xargs 三条纪律：`-r`、`-print0/-0`、先 echo 后动手，各防什么事故？（六）
- [ ] 决策树走一遍：什么形态走 jq、什么情况 sed 也不许上？（七）

...
先对照 [知识点](知识点.md) **[一、一张图：一份 5GB 日志变证据的主线](知识点.md) 总图 + 走读五步 + [易错一句话对照](知识点.md) 易混** 自己口述，再对答案。关键字（grep 缩小 → awk 聚合、log_format 列号、sed 预览、禁止全量 sort）要能定责。每题标了覆盖范围：
...
白板综合：「给你 access+error，怎么在 10 分钟内拿出 5xx Top URL 和是否上游锅的证据？」
...
- [ ] 默画管道：grep → awk → sed；jq 何时介入？四个禁区？  
- [ ] 5GB 日志 5xx Top URL 的正确顺序？  
- [ ] grep 默认 BRE 还是 ERE？生产怎么写？  
- [ ] 为什么不能盲信 `$9`？验列命令？  
- [ ] sed 预览一行命令？何时不能用裸 sed？  
- [ ] Top IP 集中能否直接封？  
- [ ] 跨天小时统计 key 怎么写？`$NF` 当 RT 错在哪？
- [ ] pipefail 为什么监控必开（Q13）
- [ ] 白板五步走读与四禁区（Q14）

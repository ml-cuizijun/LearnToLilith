...
开始做题前，先给大家一个小建议：合上知识点，对着**第一节的主图**，把「5GB 日志变证据」的五步口述一遍——对表 → 缩小 → 数数 → 交叉 → 落盘。能顺下来，下面这些题你会发现大半都会了。

做题的方式：每道题**先看标题和场景，自己口述一遍答案，再往下看解答**。关键字（grep 缩小 → awk 聚合、log_format 列号、sed 预览、禁止全量 sort）要能定责。

题目顺序：Q1～Q3 钉主线与列号；Q4～Q10 按 grep/awk/sed/jq 展开；Q11～Q14 收尾综合。
...
**对应**：[一、一张图：一份 5GB 日志变证据的主线](知识点.md) · [七、作战：定责与四个禁区](知识点.md)
...
- 和 05-04 怎么分？——本章 one-liner 刀法；引擎/回溯/Python re → 12-05。
...
**对应**：[3.1 先看漏斗，再敲命令](知识点.md) · [4.2 证据四件套：全靠关联数组 ⭐](知识点.md)

### 场景

活动第一小时，反代机 access.log ≈ 5GB。群里有人已经在跑：这是 Q1 主线的走读版——5GB 怎么切成证据。
...
- 如何一次打印所有列？——`for(i=1;i<=NF;i++) printf ...`（见 [二、对表：列号是合同，先验再信](知识点.md)）。
...
**对应**：[4.2 证据四件套：全靠关联数组 ⭐](知识点.md) · [七、作战：定责与四个禁区](知识点.md)

### 场景

你跑出：很多人 grep 完就直接 `$9`——这道题专门拆这个坑。
...
**对应**：[5.1 安全四步 🔧](知识点.md) · [5.3 macOS 的坑与「什么时候别用 sed」](知识点.md)
...
**对应**：[3.5 除了 access.log：error.log、journalctl、OOM 🔧](知识点.md)
...
**对应**：[六、旁路：jq 管结构化数据](知识点.md)

### 场景

有人：四个禁区里「JSON 用 jq」那一条的现场版。
...
- Pending 没有 `containerStatuses`？——先判 null 再取 restartCount（见 [六、旁路：jq 管结构化数据](知识点.md)）。  
- YAML 审计 `image:`？——别纯 grep，用 yq / safe_load（05-04）。
...
**对应**：[4.4 按「内容」抓列：rt 与 urt](知识点.md)；语义深化 → 19
...
- 本章验收？——默画 [一、一张图：一份 5GB 日志变证据的主线](知识点.md) + 走读五步 + 易混三钉。  
...
**★★★★☆｜`grep | awk` 管道统计为 0，脚本却 exit 0，为什么？**

**覆盖**：必会 · 常用 ｜ **对应**：知识点 [七、作战：定责与四个禁区](知识点.md)

### 场景

5xx 监控脚本：`grep ' 5' access.log | awk '{print $7}' | sort | uniq -c`。日志里没有 5xx 时 Cron 绿灯，但没人发现 `grep` 其实因权限失败。
...
> 🎯 **一句话**：管道默认只信**最后一个**命令的退出码——中间 `grep`/`awk` 失败被吞，必须 `set -o pipefail`。

```bash
#!/bin/bash
set -euo pipefail
grep ' 5' /var/log/nginx/access.log | awk '{print $7}' | sort | uniq -c | head
```

| 写法 | 风险 |
|------|------|
| 裸管道 | 中间失败仍 exit 0 |
| `pipefail` | 任一环节非 0 即失败 |
| `set -e` | 单独不够，要配 pipefail |

监控脚本还要把 stderr 重定向进日志，别静默失败。

### 再追问

- 为什么不用 `if grep ...; then awk ...`？——可以，但管道+pipefail 更短；关键是**别让 Cron 假绿**。

---

## Q14

**★★★★★｜白板：5GB 日志从原始文件到证据链，五步走读 + 四个禁区？**

**覆盖**：核心 · 必会 · 常考 ｜ **对应**：知识点 [一、一张图](知识点.md) · [九、收口](知识点.md)

### 场景

白板综合：「给你 access+error，怎么在 10 分钟内拿出 5xx Top URL 和是否上游锅的证据？」本套题收尾，把五步走读与四禁区合到一起。
...
> 🎯 **一句话**：**grep 缩小 → awk 聚合 → sed 预览改配置**；禁区：盲信 `$9`、全量 sort、裸 sed -i、awk 拆 JSON。

```text
① 验列：head -1 + 对 log_format（Q4）
② 缩小：grep ' 5' 或时间窗 awk（Q8）
③ 聚合：awk 按 URL 计数，禁止全量 sort 5GB（Q2）
④ 交叉：error.log / journal 同一时间窗（Q7）
⑤ 产出：Top N + 样例 request_id，甩 19/23
```

四个禁区：统计 0 先验列；OOM 先缩范围；改 conf 先 diff；JSON 用 jq（Q9）。

> 📝 **面试**：强调「本章只割证据，502 语义去 19，盘满去 09」。

### 再追问

- 要落 Cron？——pipefail + 输出文件 + 非零告警（Q13）。
...
- [ ] **默画管道**：grep → awk → sed；jq 何时介入；四个禁区（→ 知识点「一」；Q1、Q9、Q14）。
- [ ] **口述 5GB→证据**：5xx Top URL 的正确顺序（→「一」「三」「四」；Q2）。
- [ ] **口述 grep**：默认 BRE/ERE、生产姿势、error+ 为什么匹配不到（→「三」；Q3、Q4）。
- [ ] **口述列号**：为何不能盲信 `$9`、验列命令、漂移怎么暴露（→「二」；Q5）。
- [ ] **口述 sed**：预览一行、何时不能裸 sed、硬链接与 inode（→「五」；Q6、Q7）。
- [ ] **口述聚合**：Top IP 集中能否直接封、跨天小时 key、`$NF` 当 RT（→「四」；Q8、Q10）。
- [ ] **口述 xargs / pipefail**：三条纪律、监控为何必开 pipefail（→「六」；Q11、Q13）。
- [ ] **白板五步与四禁区**（Q14）。

全部打勾后，找同事十分钟讲一遍——讲得下来，你就是下一个能拦住「vim 开 5GB 日志」的人。

对照 [知识点 · 合上书](知识点.md#合上书)。深度正则 → [05-04](../../05-开发能力/04-正则与文本处理/)；Nginx 语义 → [19](../19-Nginx/)；日志平台 → [23](../23-日志运维/)。

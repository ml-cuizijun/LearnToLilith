...
- 和 06-05 怎么分？——本章 one-liner 刀法；引擎/回溯/Python re → 06-05。
...
不要无 `-since` 灌几年 journal 再 grep。504 语义与两列时间 → 23。
...
- YAML 审计 `image:`？——别纯 grep，用 yq / safe_load（06-05）。
...
嵌套量词 `(.+)+` 遇畸形行可能 CPU 100%——生产过滤保持窄模式；原理 → 06-05。
...
- 没有两列时间？——先改 log_format 再排障，否则蒙眼（23）。  

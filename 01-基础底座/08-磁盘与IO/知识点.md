...
- [ ] **默画主图**：`write → Page Cache(脏页) → journal → 块层队列 → 介质`，fsync/O_DIRECT 的分叉在哪汇入
- [ ] **口述**：write 返回时数据在哪？掉电安全靠什么保证？
- [ ] **讲清**：脏页三参数各管什么档位；超 dirty_ratio 后全机发生什么；加大它行不行
- [ ] **讲清**：buffered / fsync / O_DIRECT 怎么选；日志能不能上 O_DIRECT；为什么库要防双缓冲
- [ ] **默画**：journal 事务四步 + 崩溃重放；三层日志（库 WAL / FS journal / journald）各保什么
- [ ] **分型**：await 高 + aqu-sz 高 vs 低各指向什么；SSD %util 100% 怎么结案
- [ ] **讲清**：顺序 IO vs 随机 IO 谁吃亏；调度器怎么选；IONice 用在什么场景
- [ ] **口述**：%wa 为什么会骗人（NFS、CPU 挤 0）；磁盘慢最终听谁的
- [ ] **分型**：块满 / inode 满 / deleted 各自的签名和第一刀；df 和 du 为什么对不上；一排 D 看 wchan
- [ ] **落地**：LVM 在线扩四步（XFS 跟挂载点、ext4 跟设备）；XFS 能否缩；库盘 RAID；盘位规划三句
- [ ] **作战**：60 秒剧本的三个时间窗各敲什么；值班最小三联

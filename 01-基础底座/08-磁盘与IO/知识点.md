...
| 章 | 负责 |
|----|------|
| **08（本章）** | 写路径、iostat 定责、脏页、空间三类、journal、排队分型 |
| **06** | Load / `%wa` / 软中断与 CPU 视角 |
| **07** | Page Cache、available、OOM |
| **09** | inode 结构、fstab、ext4/xfs 选型细节 |
...
| cgroup `io.max` 隔离备份（见 10） | 当单接口超时去重启乱杀 |
...
1. 凌晨全服备份打同一块数据盘 → 逻辑服 P99 20ms→05s  

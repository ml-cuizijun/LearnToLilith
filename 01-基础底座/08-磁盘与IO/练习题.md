...
开始做题前，先给大家一个小建议：合上知识点，对着**第一节的主图**，把「一次 write 的一生」口述一遍——应用 → Page Cache → journal → 块层 → 介质；再说清 await 分型和空间三类。能顺下来，下面这些题你会发现大半都会了；卡壳的地方，就是你该回去重读的那一节。
...
题目的顺序也是有意排的：Q1～Q3 先把 await / 写语义 / 脏页钉死；Q4～Q13 按空间与落地展开；Q14 收尾到白板综合。
...
先自己试试：SSD 上听 await 还是 %util？aqu-sz 高低怎么分型？
...
先自己试试：write 返回了数据在盘上吗？三种写语义怎么选？
...
空间病的第一种签名——和 Q5 inode、Q9 日志打满是一家。
有人刚 `rm` 了 40G 的 `logic.log`，`df` 一点没降，业务还在写。他准备再 rm 一遍或者干脆重启。

先自己试试：df 满、du 对不上，少的空间去哪了？再 rm 一次管用吗？
...
先自己试试：FS journal / 库 WAL / journald 各保什么？能关 journal 提速吗？
...
先自己试试：云盘 await 从 2ms 飙到 50ms，先查什么？
...
先自己试试：顺序 vs 随机谁吃亏？IONice 用在什么场景？
...
先自己试试：不看下面，你能默画 write 一生 + await 分型吗？
...
- [ ] **默画主图**：write → Page Cache（脏页）→ journal → 块层 → 介质；fsync/O_DIRECT 分叉（→ 知识点「一、一张图」；Q14）。
- [ ] **口述写语义**：write 返回时数据在哪；buffered / fsync / O_DIRECT 怎么选（→「二、出发」·「四、要落盘」；Q2）。
- [ ] **口述脏页**：三参数各管什么；超 dirty_ratio 全机发生什么；加大行不行（→「三、中转」；Q3）。
- [ ] **默画 journal**：事务四步 + 三层日志各保什么（→「五、路上」；Q6）。
- [ ] **分型 await**：队列高低怎么定责；SSD %util 100% 怎么结案（→「六、排队」；Q1、Q7）。
- [ ] **口述顺序/随机与 IONice**（→「六、排队」；Q13）。
- [ ] **口述 %wa 为什么骗人**（→「六、排队」·「七、生病」；Q8）。
- [ ] **分型空间三类**：块满 / inode / deleted；一排 D 看 wchan（→「七、生病」；Q4、Q5、Q9）。
- [ ] **落地**：LVM 四步、XFS 不能缩、库盘 RAID、云盘 burst（→「八、落地」；Q10、Q11、Q12）。
- [ ] **作战**：60 秒剧本与白板综合（→「九、作战」；Q14）。

全部打勾之后，给自己出一个终极测试：找一位同事，用十分钟把「一次 write 的一生 + await 分型」讲一遍。讲得下来，你就是下一个能拦住「先换 SSD」的人。

...
| 章 | 负责 |
|----|------|
| **12** | 包怎么走、怎么进主机、Socket/队列 |
| **13** | TCP/UDP：握手、窗口、重传 |
| **14** | HTTP 状态码、入口证 / 回源证、SNI |
| **15（本章）** | **DNS 解析链、TTL 切流、dig 定责、CDN HIT/MISS/Purge、劫持** |
| **16** | `curl -w` 五段定层、抓包手法、路由 / MTU |

本章是**名字与分发主场**。16 告诉你「连不上先看 dns 段」；本章告诉你 dns 段慢 / 错了之后，**权威、递归、劫持、CDN 回源**怎么分锅。
...
│  链出：curl -w dns→16 · 回源证/SNI→14 · 队列→12 · TCP→13                   │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| X-Cache HIT/MISS、回源、Purge | Nginx 指令细节（19） |
...
```bash
# 11 的定层入口（细节见 16）；dns 慢再回本章 dig
curl -o /dev/null -s -w 'dns:%{time_namelookup} tcp:%{time_connect}\n' https://login.game.com/
```

> **别混**：`time_connect` 是累计值（含 DNS），做差才是纯 TCP——细节在 16，本章不展开。
...
| HIT 还 502 | 边缘脏缓存 / 边缘故障 | **不是回源**；Purge 该 URL（见 14） |
...
# 或 HTTPS 回源时配合 --resolve / SNI（证书细节 → 14）
...
| curl -w dns 慢？ | 转 dig（本章）；五段 SOP 在 16。 |

...
先对照 [知识点](知识点.md) **§2 总图 + §5 TTL + §6 dig + §8 CDN** 自己口述，再对答案。关键字（递归/权威、TTL 切流、X-Cache、HTTPDNS、ndots）要能对比。每题标了覆盖范围：
...
- dig @8.8.8.8 正常、运营商 DNS 错，下一步？——按劫持查：对比权威、换 4G/Wi-Fi、上 HTTPDNS。不要先改业务域名。见 §9。
...
绕过 CDN，用源站 IP + Host 头测。绿锁只说明边缘证没问题，回源是另一跳（入口证 ≠ 回源证，见 09）。
...
- HTTPDNS 拿到 IP 之后用 IP 校验证书可以吗？——不可以。证书是签给域名的。连接指定 IP，SNI / Host 仍用原域名。见 09。
...
- curl -w 里哪一段会高？——`time_namelookup`（dns 段）；定层手法见 11，根因在本章 ndots。
...
- 健康检查已经把死 VIP 摘了，为什么还有人打上去？——DNS 还在 TTL 内。所以摘流量要 LB 摘 + DNS，不要只改 DNS。「DNS 已切」≠「用户已切」。见 §5.5。
- 四路 dig 表怎么记？——权威错改记录；权威对+广告 IP=劫持；权威对+合法另一 VIP=智能 DNS。见 §6.2。
...
3. 按 §6.2 表定责：权威 / 缓存 / 劫持 / 智能 DNS。  
4. dns 很快、connect 慢 → **不是 DNS**，回 11 查 TCP/路径。

> **别混**：`time_connect` 含 DNS（累计值），要做差——细节在 11。
...
- [ ] 默画 §2.0：解析链 + 切流五步 + CDN HIT/MISS/Purge + 翻车点  

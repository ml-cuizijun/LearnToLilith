...
| 章 | 负责 |
|----|------|
| **12** | 包怎么走、Socket/队列、Recv-Q、backlog、本机「满」 |
| **13** | TCP 窗口 / 重传 / 心跳；`ss -ti` 读传输态 |
| **14** | HTTP 状态码、TLS 握手语义、证书链 |
| **15** | DNS 权威 / 劫持 / TTL / CDN |
| **16** | 分层排障理论、抓包深纪律、PMTUD、TSO 假 checksum、两端抓协议 |
| **02（本章）** | **命令配方、过滤器、读输出、场景闭环；何时升级到 16** |

本章是**网络命令刀法手册**：速查 + 场景闭环。16 告诉你「为什么要定层、抓包纪律背后的坑」；本章告诉你「值班键盘上第一刀敲什么、屏幕长什么样、下一步去哪」。
...
4. 写出合法 tcpdump 最小闭环（指定网卡 + 过滤 + `-nn -c -w`），并知道何时链到 16。  
...
flowchart TB
    ...
    N2["窗口 / 重传 → 11"]:::note
    N3["TLS 语义 → 14 · DNS 劫持 → 17"]:::note
    N4["PMTUD / 两端抓 → 18"]:::note
...
| 段 | 记住 |
|----|------|
| ① | `curl -w` 累计要做差；dns→15，tls→14，ttfb→应用 |
...
│  openssl    │  neigh / ping      │  仍不明 → 16 深排                     │
├─────────────┴────────────────────┴───────────────────────────────────────┤
│  翻车：-i any · 不做差 · route show 猜双网卡 · ping 通就结案              │
│  邻章：队列 12 · 窗口 13 · TLS 14 · DNS 15 · PMTUD/两端抓 16              │
└──────────────────────────────────────────────────────────────────────────┘
```
...
ss -ti dst 10.0.1.50:3306   # 传输细节 → 深读见 13
```
...
| 字段 | 含义 | 定责 |
|------|------|------|
| `Recv-Q 245760` | **未读字节**（不是连接数） | 应用卡住没 `read` → 12 |
...
| 异常形态 | 想什么 |
|----------|--------|
| `synrecv` 很高 | 半连接打满 / SYN flood → 12 |
| `timewait` 爆炸 | 短连接、缺连接池 → 12 |
...
| `Connected` 后卡 SSL | tls | openssl → 14 |
| HTTP 行出来很慢 | ttfb | 应用 / DB |
| 下载很慢 | body | 体积 / 带宽 |

**长连接房间：** curl 打不上业务语义 → 确认**房间 IP+端口** → `nc -zv` + `ss` → 仍不明才抓该端口（§4.10）。别在登录域名上抓房间。

**升级到 16：** 五段已定层但路径/MTU/RST 源仍扯不清——分层理论与 PMTUD 深排见 [16 §4～§8](../16-网络排障与抓包/)。
...
**何时用：** 粗测路径、配合大包探 MTU（深复现链 16）。

```bash
ping -c 3 10.0.1.50
ping -c 3 -W 1 203.0.113.10
ping -M do -s 1472 203.0.113.10    # DF + 1472 ≈ 路径 1500；不通先想 PMTUD → 16
```
...
**升级到 16：** 策略路由 / 多表纠缠、PBR 与 conntrack 交叉——见 [16 §5](../16-网络排障与抓包/)。
...
| 已 ESTAB 后突发 `[R]` | 谁发 RST | **看源 IP**；中间盒 → 16 两端抓 |
...
| ESTAB Recv-Q 涨 | 应用 read → 12 | 调 rmem 当 LISTEN 满 |
| 小包通大包死 | 标记 PMTUD → 16 | 本章硬撑深排 |
...
| LISTEN Recv-Q 满？ | 未 accept 连接数 → 12；不是字节 rmem。 |
| ttfb 慢找谁？ | 应用/DB，不是专线。 |
| 何时升级 041？ | PMTUD 深排、TSO 假 checksum、NAT 后两端抓对不上。 |

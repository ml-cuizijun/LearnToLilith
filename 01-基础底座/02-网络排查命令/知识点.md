...
> 这一章讲完，你会知道当时那三个人各错在哪，以及正确的第一刀该敲什么。
> **本章回答**：一次「连不上 / 慢 / 重置」的报障从接警到定责收工，每一步第一刀敲什么、输出怎么读、什么时候升级邻章。
> **不讲**：Recv-Q / backlog / 队列机制 → [12](../12-网络栈与Socket/)；窗口 / 重传 / `ss -ti` 深读 → [13](../13-TCP与UDP/)；TLS / HTTP 语义 → [14](../14-HTTP与TLS/)；DNS 劫持 / CDN → [15](../15-DNS与CDN/)；抓包深纪律 / PMTUD / TSO / 两端抓 → [16](../16-网络排障与抓包/)；iptables / conntrack → [17](../17-iptables与conntrack/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节的主图，把「报障的一生」五步装进脑子；第二到第七节顺着接警 → 定层 → 缩范围（DNS/TCP/TLS）→ 留证往下走；第八节专门讲定责收工——也就是开头那个现场的正确解法。每一节讲完都会提示对应练习题，学完就去练。
...
⭐ 翻车四件套是面试常考点（练习题 Q1）。好，主图有了——第一站：接警翻译。
...
这一节对应练习题 Q12。好，话翻译完了。大家想想：HTTP 业务第一刀该敲什么？——定层。
...
🔥 很多人拿到 `dns:0.01 tcp:0.02` 就念「TCP 20ms」——**错了**。五个数全是从 0 起算的累计值，`time_connect` **包含 DNS**。不减就把 DNS 记在 TCP 头上，整晚查错方向。
...
🔥 口诀记一句：**「五个累计值，本段后减前；connect 含 DNS」**。练习题 Q2 专门练做差。下一站：DNS 岔路。
...
这一节对应练习题 Q10 的 dig 半边。DNS 过了还不通？进 TCP 岔路。
...
大家想想：同样是「连不上」，有人秒失败、有人干等几十秒——差在哪？很多人会答「都是防火墙拦了」——太粗了。nc 的 refused / timeout 背后是防火墙的两种动作，第一刀的方向完全不同：
...
🔥 开头那个现场里，有人看了眼 `ip route show` 说「有 default，网络没问题」——**错了**。双网卡机器上，show 是地图册，真正替你算「这一包走哪」的是 `ip route get`。⭐ 这是高频错答。
...
这一节对应练习题 Q9、Q11、Q13、Q14——综合题都在这，务必练熟。


**回到开头那个现场**：有人裸 `tcpdump -i any`、有人拿 `route show` 当结论、有人 ping 通就结案。正确剧本是：先把玩家的话翻译成包语言 → `curl -w` 做差定层 → 按层缩范围 → 仍说不清才按纪律抓包。现在再看群里那三个人的操作，你知道该怎么拦住他们了。

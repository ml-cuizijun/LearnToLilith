...
> 这一章讲完，你会知道当时那三个人各自错在哪，以及纵深防御为什么不能只改端口。
> **本章回答**：一台裸机从出厂到扛得住一次入侵的一生——少开门、管住人、收住权、留下痕，出事了怎么收场。
> **不讲**：文件权限与 SUID/特殊位原理 → [25](../25-用户权限与特殊位/)；SSH 协议与 sshd 深配 → [26](../26-SSH与远程访问/)；五链四表与 conntrack → [17](../17-iptables与conntrack/)；sysctl 机制与落盘 → [24](../24-内核参数与sysctl/)；人从哪进生产的通道治理 → [28](../28-堡垒机与跳板机/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节的主图，把「一台裸机的加固一生」装进脑子；第二到后续节顺着减面 → 管人 → 收权 → 锁门 → 筑墙 → SELinux → 留痕 → 闭环往下走；作战节专门讲定责，也就是开头那个现场的正确解法。每一节讲完都会提示对应练习题，学完就去练。
...
咱们先不急着改端口。学加固最怕的就是「只做一件事就以为安全了」。先看图，把「哪一道闸还没装」装进脑子里：
...
**读图**：带着三个问题看这张图，比背 checklist 有用得多：
...
减面段到这儿。本节对应练习题 Q2。好，门少了——大家想想：空密码、锁定、过期怎么管？
...
管人段到这儿。本节对应练习题 Q3。回到开头那位给 deploy 开 NOPASSWD ALL 的同学——收权怎么写？

---

## 四、收权：sudo 白名单与文件底线 ⭐

🔥 `NOPASSWD: ALL`——**加固等于没做**。大白话：**sudo 白名单是「只准这几条」，不是「全部免密 root」**。口诀：**「visudo 编辑，白名单到命令」**（练习题 Q4）。
...
收权段到这儿。本节对应练习题 Q4。好，权收住了——SSH 加固清单哪些是底线？改完怎么不把自己锁死？

---

## 五、锁门：SSH 加固清单 ⭐

🔥 有人喊「把 22 改成 2222」当加固——**不够**。大白话：**改端口只是藏门牌，禁密码、禁 root、限来源才是锁门**。⭐ 面试必考（练习题 Q5）。
...
锁门段到这儿。本节对应练习题 Q5。好，SSH 锁了——防火墙默认 DROP 怎么白名单化？
...
筑墙段到这儿。本节对应练习题 Q6。好，包过滤有了——安全向 sysctl 调哪些？
...
旋钮段到这儿。本节对应练习题 Q7。回到开头那位 `setenforce 0` 的同学——DAC 和 MAC 差在哪？

---

## 八、第二道墙：SELinux 与 AppArmor 🔥

🔥 反代 502 但 curl 通就 `setenforce 0`——**拆第二道墙**。大白话：**DAC 是主人说了算，MAC 是系统强制标签；SELinux 拦了先查审计再放行，别关**。口诀：**「先 ausearch，再放行；别 setenforce 0」**（练习题 Q8、Q9、Q10）。
...
第二道墙到这儿。本节对应练习题 Q8～Q10。好，MAC 懂了——fail2ban、auditd、AIDE 各留什么痕？
...
留痕段到这儿。本节对应练习题 Q11、Q12。好，站岗有了——漏洞管理与 CIS 基线怎么闭环？
...
前面几道闸到这儿。本节对应练习题 Q11～Q13。现在再看开头告警炸群——发现入侵 60 秒怎么走？
...
作战段到这儿。本节对应练习题 Q14。现在再看开头那三个人——改端口、setenforce 0、NOPASSWD ALL——你已经知道该怎么拦住他们了。
...
对齐练习题「合上书」，逐条口述，卡住就回对应节 / 题号。现在再看开头那三个人——改端口、setenforce 0、NOPASSWD ALL——你该知道先守哪一层了。

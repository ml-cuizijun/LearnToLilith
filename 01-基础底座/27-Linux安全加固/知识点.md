...
| 章 | 负责 |
|----|------|
| **27（本章）** | 主机加固三层：入口（sshd + 来源）→ 权限（sudo / DAC / MAC）→ 发现（auditd / AIDE）；被拦查 AVC、入侵先隔离 |
| **17** | 五链四表、NAT、conntrack table full——**怎么写规则**、表满怎么认 |
| **28** | 堡垒机产品、审批、录屏、人怎么跳 |
| **04-13** | 容器 / K8s 侧 RBAC、PSS、NetworkPolicy——**不替代**宿主机 MAC |
| **24** | 性能旋钮；本章只留**安全向** sysctl（redirects / syncookies / ip_forward / rp_filter） |

本章是**主机安全入口**。17 告诉你「规则写在哪条链」；本章告诉你「生产机该不该让世界连上、进来能跑什么、出事能不能对到人」。

...
| **12（本章）** | **先定域、再下钻**：USE/RED、30 秒全景、profiling 边界、证据链 |
...
flowchart TB
    ...
    N1["Load/steal细节 → 03"]:::note
    N2["OOM/cgroup → 04"]:::note
    N3["await/脏页 → 05"]:::note
...
| ③ 四链 | r/us→03；b/wa→05；available/137→04；ss→06/11（§8） |
...
**【2 分钟】**  
「USE 看资源 U/S/E，RED 看入口。定域后四链：CPU→03，盘→05，内存/137→04，连接→06/11。profiling 只给确认过的 CPU 型，短窗口 `-p PID`；bpftrace 短窗口计数。证据链三条：当时 vmstat、监控图、error 日志。止血可以重启单实例，结案要根因和预防。」
...
**数字**：全景 **6** 条命令 · 火焰图采样常见 **30s** · 监控粒度 **1s** · 证据链 **3** 条（vmstat 片段、监控图、error 日志）· conntrack 告警常看 **80%** 水位（细表 → 15）

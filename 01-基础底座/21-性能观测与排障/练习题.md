...
磁盘满：`df -h` / `-i` 定位，**截断**日志或清 deleted fd，不要先 rm。根因是轮转 / inode。进程持有已删文件时 rm 不释放空间，见 09。
...
**一句话**：现场 awk TOP IP 和 5xx 百分比，ss 状态分流决定进 12/13/15 哪章。

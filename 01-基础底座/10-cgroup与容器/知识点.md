...
> **不讲**：OOM Killer 打分、dmesg 细读、Java 堆外明细 → [07](../07-内存与OOM/)；CPU 时间分类 us/sy/wa 与 Load → [06](../06-CPU与Load/)；systemd 的 slice 封装 → [11](../11-Systemd与服务管理/)；requests/limits、QoS、驱逐 → [03-04](../../03-Kubernetes/04-Pod生命周期/)。
...
- 主线是一份配额的一生：被写下 → 被看见（看歪）→ 被撞上 → 被定责，后面每节认领一段。
...
一台宿主机跑几十个服务，CPU、内存、盘都是共享的：不划边界，一个备份进程就能把同盘邻居全部拖慢。Linux 划边界用的是两套内核设施：**cgroup（control group，控制组）限制一组进程能用多少资源**；**namespace 限制一组进程能看见什么**。容器就是拿这两套设施包出来的普通进程——**容器 = 普通进程 + namespace（视图）+ cgroup（配额）**，不是微型虚拟机。
...
> ⚠️ **别混**：**request vs limit**。request 影响**调度**（这个 Pod 算占多少资源）和**争抢权重**（cpu.weight/shares）；limit 才是**配额**（quota），**只有 limit 造成 throttle**。request 写多大都不会让进程被踢出运行队列。

### cgroup v1 vs v2 🎯⭐🔥

v1 每个控制器一棵树、文件名带下划线；v2 一棵统一树、能拆内存明细、带压力信号。**K8s 1.27+ 默认推 v2**，老集群 v1 还很多，两套文件名都要认：
...
> 📝 **面试**：「Docker 和 K8s 写配额有什么区别」——答「没有本质区别，都是写手：`docker run --cpus/--memory` 和 Pod `limits.*` 最后都由 runtime/kubelet 写进同一批 cgroup 文件；区别在多容器时 K8s 的 Pod 级内存常共享（六节）、Docker 通常一容器一 cgroup」。
...
十分钟后 Pod `OOMKilled`。「明明还有 40G」——**容器内的 `free`/`nproc`/`top` 默认读的是宿主机的 `/proc`（meminfo、cpuinfo、stat），不是本 cgroup 的配额**。namespace 换了进程表和网卡，**没有**把这三份文件改写成「只显示你的 limit」（除非额外挂了 lxcfs 之类，生产默认别假设有）。`top` 的 CPU% 还按宿主机核数归一化——2 核 limit 的容器跑满配额，`top` 里只显示 6%。
...
> ⚠️ **别混**：**用容器内 `free` 反驳 137、用 `nproc` 反驳 throttle——永远无效**。这不是误差大小的问题，是根本读的不是同一本账。

### 谎言最贵的代价：运行时按谎言配并发 🎯⭐🔥

谎言不只是让人误判，还会**带动运行时配错**：Go 服务不设 `GOMAXPROCS`，默认跟 `runtime.NumCPU()` 走（≈ nproc=32）；limit=2 核 → 32 个 P 抢 2 核配额 → 上下文切换和 throttle 双重放大，P99 毛刺。
...
CPU limit 的底层是 **CFS bandwidth control（CFS 带宽控制）**：每个**周期（period，v1 文件 `cpu.cfs_period_us`，默认 100000µs = 100ms）**内，这个 cgroup 最多消耗 **quota（v1 文件 `cpu.cfs_quota_us`）**的 CPU 时间；用完，**整个组**被踢出运行队列，晾到下个周期再放回来——这就是 **throttle**。
...
> 📝 **面试**：「P99 涨但节点 idle，我先看 `cpu.stat` 的 `nr_throttled` 增量。100m = 每 100ms 只有 10ms。只有 limit 造成 throttle，request 不会；扩节点修不好，配额在这个 cgroup 上。」
...
内存配额三个文件：**`memory.max` 硬顶（就是 limit）**、**`memory.current` 此刻计入的用量**、**`memory.events` 事件计数**（`oom` = 触发过 OOM 的次数，`oom_kill` = 真杀了几个）。v1 对应 `memory.limit_in_bytes` / `memory.usage_in_bytes` / `memory.oom_control`。
...
> 📝 **面试**：「137 但节点还空，我看 `memory.events` 的 oom_kill 和 dmesg：dmesg 没有 Out of memory 就是 cgroup max。Guaranteed 也挡不住自己的 limit。内存贴边先看 `memory.stat` 拆 file/anon——cache 贴顶是常态，anon 斜线才是病。」
...
> 📝 **面试**：按动词背「**等、杀、拒、慢**」四字诀——throttle 等、OOM 杀、pids 拒、io 慢；每种再配一个文件名和签名字段，就是完整答案。
...
- [ ] **默画一节的图**：出生（写手 → cgroup 文件）→ 看界（视图/谎言）→ 撞界（CPU / 内存 / pids·io）→ 作战（host PID → 文件定责）。
- [ ] **口述出生**：为什么要隔离、容器 = 进程 + namespace + cgroup、limit 从 YAML 到哪个文件、100m 换算成什么、request 和 limit 谁造成 throttle。
- [ ] **口述 v1/v2**：怎么一眼判断节点版本、两套文件名怎么对照、v1 usage 含 cache 为什么易误判、v1 多棵树路径怎么拼、迁 v2 的三个坑。
- [ ] **口述看界**：六种 namespace 各一句、free/nproc 为什么是谎言、GOMAXPROCS 为什么要对齐 limit。
- [ ] **默画撞界 CPU**：20ms 的活撞 100m 配额的时间线，说清为什么 P99 炸而节点 idle、为什么容器里没有 %st、定责算术三步各算出什么（被踢比例 / 平均晾多久 / 换算核对）。
- [ ] **口述撞界内存**：137 三叉怎么分、cgroup OOM vs 整机 Killer 对照、Guaranteed 为什么还会死、page cache 为什么让内存贴边是常态、三本账（RSS / current / swap）差在哪、杀（max）和图表（working_set）口径怎么分工。
- [ ] **默画收束**：撞界四种下场（等/杀/拒/慢）结构图，每种配一个文件和一个签名。
- [ ] **口述邻居**：pids 满和 ENOMEM 怎么分、pids 为什么按线程计数、fork 炸弹为什么是全节点灾难、io.max 管什么、sidecar 怎么带走主容器。
- [ ] **定责**：决策树从哪个动作开场、四个现象各读哪个文件哪个字段、60 秒剧本四步各干什么。

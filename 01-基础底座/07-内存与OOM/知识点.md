...
> **刻意不展开**：容器内 free 为什么撒谎的内核细节 → [04](../04-内核与系统/)；cgroup v1/v2 文件、PSI → [10](../10-cgroup与容器/)；K8s QoS YAML → [03-Pod](../../03-Kubernetes/04-Pod生命周期/)；脏页 / iostat → [08](../08-磁盘与IO/)；fork ENOMEM、信号 137 语义 → [05](../05-进程与信号/)；Load 里的 si/so 格子 → [06](../06-CPU与Load/)。
...
| 章 | 负责 |
|----|------|
| **07（本章）** | 物理内存怎么读；RSS/VSZ/cache；整机 OOM vs cgroup OOM；overcommit / Swap；137 定责 |
| **04** | 容器里 `free` 读宿主机账的根因 |
| **05** | SIGKILL / 137、fork ENOMEM |
| **06** | Load、`vmstat` 的 si/so 格子（换页信号往本章结案） |
| **08** | Dirty、writeback、盘慢导致回收卡住 |
| **10** | cgroup memory 文件、v1/v2、PSI |
...
1. 拉监控：流量/CCU 变没变？  
2. 不变还斜线 → 泄漏嫌疑；CCU 5k→05 万上台阶 → 容量。  
...
2. **活动 CCU 5k→05 万**，RSS 上台阶后平稳 → 容量；按单玩家内存 × CCU 算 limit，不要先骂泄漏。  

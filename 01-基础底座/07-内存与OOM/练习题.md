...
开始做题前，先给大家一个小建议：合上知识点，对着**第一节的主图**，把「一页内存的一生」口述一遍——申请 → 入账 → 紧张 → 击杀 → 复盘；再说清「够不够看 available」和 137 三叉。能顺下来，下面这些题你会发现大半都会了；卡壳的地方，就是你该回去重读的那一节。
...
题目的顺序也是有意排的：Q1～Q3 先把账本和 137 钉死；Q4～Q12 按击杀与紧张展开；Q13～Q14 收尾到定责综合。
...
- [Q11 RSS / VSZ](#q11)
- [Q12 overcommit](#q12)
- [Q13 OOM 60 秒定责剧本](#q13)
- [Q14 内存/OOM 白板综合题](#q14)
- [合上书](#合上书)
...
**对应**：知识点 [free -h：先读 available 再决定要不要慌 🔧⭐](知识点.md) · [收束：内存够不够，三本账各看一列 🎯⭐](知识点.md)

### 场景

`free -h` 显示 used 90%。开发说内存爆了要加机器。buff/cache 占了大半。available 还有约总量的 15%。

先自己试试：够不够看哪一列？used 90% 要不要加机器？
...
**对应**：知识点 [五、击杀：OOM Killer 与两条杀法 🎯🔥](知识点.md) · [六、复盘：定责与作战 🎯🔧](知识点.md)
...
**对应**：知识点 [两条杀法对照 🎯⭐🔥](知识点.md)

### 场景

这是 Q2 结论的容器版——同一条 137，换一本账。
Pod limit 2Gi。进容器 `free -h` 看到 64G。十分钟后 137。开发说「宿主机明明还有内存」。

先自己试试：宿主机空、Pod 137，是不是宿主机没内存？
...
**对应**：知识点 [涨法分型：斜线 · 台阶 · 热更 ⭐](知识点.md)

### 场景

杀完人还要复盘形态——斜线才像泄漏，台阶常是容量。
活动 CCU 5k → 05 万，RSS 上去了。开发说是泄漏要 dump。另一次是热更后流量不变 RSS 斜线涨。

先自己试试：斜线 / 台阶 / 热更怎么分？重启回落能定泄漏吗？
...
**对应**：知识点 [oom_score_adj：保护谁、别保护谁 ⭐](知识点.md)

### 场景

整机 OOM 时把 sshd 杀了，你登不进去。有人要把逻辑服 `oom_score_adj` 设成 -1000。

先自己试试：oom_score 高 = 泄漏吗？业务能不能 adj=-1000？
...
**对应**：知识点 [Java / Go：limit 怎么留 ⭐](知识点.md)

### 场景

`-Xmx=2g`，Pod limit 也是 2Gi。堆看起来没满，反复 OOMKilled。用了 Netty。

先自己试试：Java Xmx=limit 行不行？堆没满却 137 查什么？
...
**对应**：知识点 [Swap：用了就是紧 ⭐🔧](知识点.md)

### 场景

`free` 里 Swap used 2G。有人说「还有 Swap，先不用加内存」。战斗服 P99 开始抖。

先自己试试：Swap 有余量 = 还能撑吗？看 si/so 还是看总量？
...
**对应**：知识点 [free -h：先读 available 再决定要不要慌 🔧⭐](知识点.md) · [drop_caches：生产禁止日常 ⭐](知识点.md)

### 场景

和 Q1「cache 大常健康」是同一条红线的反面操作。
cache 占 80%。有人准备 `echo 3 > /proc/sys/vm/drop_caches`「腾内存」。

先自己试试：cache 80% 该不该 drop_caches？
...
**对应**：知识点 [RSS / VSZ / working_set：三个口径 🎯⭐](知识点.md)

### 场景

监控 working_set 已经 90% limit，容器里 `ps` RSS 只有 60%。开发不信要 OOM。

先自己试试：容器告警跟 RSS 还是 working_set？
...
**对应**：知识点 [整机 OOM Killer：按分数挑 victim 🔥⭐](知识点.md) · [两条杀法对照 🎯⭐🔥](知识点.md)

### 场景

Q3 是撞自己的 max；本题补上节点级 QoS 选杀。
节点 MemAvailable 见底。有 Guaranteed 的逻辑服，也有 BestEffort 的工具 Pod。另一次是逻辑服自己 137，节点还很空。

先自己试试：request=limit 还会被杀吗？QoS 和 cgroup OOM 怎么分？
...
**★★★★☆｜137 来了，60 秒怎么分清整机 OOM、cgroup OOM、人手 -9？**

**覆盖**：核心 · 必会 ｜ **对应**：知识点 [六、复盘：定责与作战](知识点.md)

### 场景

Pod 重启，exit 137。宿主机 `free` 还有 15G。有人要加节点内存。

先自己试试：60 秒 OOM 定责剧本能默敲吗？

### 完整解答

> 🎯 **一句话**：**dmesg 整机 / memory.events cgroup / journal signal**——三条证据互斥，别混。

```bash
dmesg -T | grep -i 'killed process'
cat /sys/fs/cgroup/.../memory.events | grep oom
journalctl -u <unit> -b | grep -i 'killed\|signal'
```

| 证据 | 定责 |
|------|------|
| dmesg OOM killer | 整机 → 07 调 limit/泄漏 |
| memory.events oom_kill | cgroup → 10 对齐 Xmx |
| signal 9 无 oom | 人手 kill / 别家脚本 |

### 再追问

- 留证顺序？——先 dmesg/journal 再重启（Q2）。
...
**覆盖**：核心 · 必会 · 常考 ｜ **对应**：知识点 [一、一张图](知识点.md) · [七、收口](知识点.md)

### 场景

这张白板把前面十四道题串起来了。
白板：「画 malloc 到 OOM 的路径，说 available 为什么比 free 重要。」

先自己试试：不看下面，你能默画一页内存的一生吗？

### 完整解答

> 🎯 **一句话**：**申请(虚拟)→缺页(物理)→入账(RSS/cache/available)→紧张(回收/swap)→击杀(整机/cgroup)**。

```text
决策看 available，不是 free
斜线涨=泄漏  台阶=批量加载  热更=正常
禁止：drop_caches 日常、oom_score_adj=-1000 业务、overcommit=1 当万能
```

> 📝 **面试**：容器先看 cgroup 文件，别看宿主机 free（Q3、Q10）。

### 再追问

- Swap si/so 高说明什么？——内存压力，不是磁盘章（Q7）。

---

## 合上书

14 道题做完了，回到这个清单自查一遍——每条都能讲顺，这章才算真的拿下了。注意每条后面标注的题号：

- [ ] **默画主图**：申请 → 入账 → 紧张 → 击杀 → 复盘；`free` / `vmstat` / `dmesg`/`events` 各照哪段（→ 知识点「一、一张图」；Q14）。
- [ ] **口述申请**：虚拟内存、malloc 之后占了吗、minor vs major、泄漏 vs 溢出（→「二、申请」；Q11、Q12）。
- [ ] **口述入账**：free / available / cache；used 高为什么常是假阳性（→「三、入账」；Q1）。
- [ ] **口述三个占用口径**：VSZ / RSS / working_set vs limit（→「三、入账」；Q9、Q11）。
- [ ] **口述紧张**：kswapd vs direct reclaim、si/so、为何禁 drop_caches（→「四、紧张」；Q7、Q8）。
- [ ] **默画击杀**：整机 OOM 与 cgroup OOM 两条杀法（→「五、击杀」；Q2、Q3）。
- [ ] **口述击杀**：137 三叉、oom_score vs adj、Java limit、QoS（→「五、击杀」；Q2、Q5、Q6、Q10）。
- [ ] **口述复盘**：斜线 / 台阶 / 热更；60 秒定责（→「六、复盘」；Q4、Q13）。
- [ ] **白板综合**（Q14）

全部打勾之后，给自己出一个终极测试：找一位同事，用十分钟把「三本账 + 137 三叉」讲一遍。讲得下来，你就是下一个能对齐「内存 90%」争吵的人。

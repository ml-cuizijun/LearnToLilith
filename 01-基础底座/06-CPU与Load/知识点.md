...
| 章 | 负责 |
|----|------|
| **06（本章）** | Load 与 CPU% 两把尺；格子定责；高 Load / 高延迟时先分型再动手 |
| **05** | D 状态、wchan、信号杀不掉 |
| **08** | 磁盘 await / %util / 脏页 |
| **07** | 换页 si/so、OOM |
| **12** | 包怎么进主机、Socket/队列；`%si` 只在本章定责「单核软中断」 |
| **10** | cgroup / 容器 quota 文件怎么读 |
| **21** | perf / 火焰图采样手法 |
...
```text
                    ┌─────────────────┐
    进程想跑 / 卡IO  │   Load 04/5/15   │  ← 人多不多、排不排队
                    └────────┬────────┘
                             │ 对不上时
                             ▼
                    ┌─────────────────┐
...
```text
正在恶化：  40.1,  22.0,  8.0     → 04m 远大于 15m，先止血
...
| ④ 热点型 | Load 低/不高，单核 100% | **不要水平加核当根治** | 匹配服 1 核满、Load 04.2 |
...
%wa · await · %util  不是同一个数
  ...
  await   = 请求在设备队列里待了多久（盘视角）→ 08
  %util   = 设备有活干的时间占比（SSD 上别当唯一结案）→ 08
```
...
CFS 周期默认 **100ms**。例如 `cpu.max` = `200000 100000` → 05 核；`500m` ≈ 每周期 50ms 配额。突发跨过边界，wall time 直接加上「被踢出队列」的等待。
...
**Go/Java 坑**：容器里 `nproc` 常返回**宿主机核数**；`GOMAXPROCS` / GC 线程按宿主机核数跑，limit=1 核时会严重加重 throttle。修：`automaxprocs`、`-XX:ActiveProcessorCount`，**对齐 limit**。cgroup 文件与容器细节 → [10](../10-cgroup与容器/)；K8s request/limit YAML → [03-Pod](../../03-Kubernetes/04-Pod生命周期/)。
...
| K8s request/limit YAML | [03-Pod](../../03-Kubernetes/04-Pod生命周期/) |

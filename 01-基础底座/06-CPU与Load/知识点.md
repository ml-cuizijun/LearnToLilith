...
**Load（load average，平均负载）** = 过去 1 / 5 / 15 分钟，「可运行（R）+ 不可中断睡眠（D）」任务个数的**指数加权滑动平均**——越近的采样权重越大，所以 1m 对突变最敏感。三个数不是三种 Load，是同一个数的三种记忆长度。
...
- [ ] **默画一节的图**：出生 → 排队 → 被执行 → 切换；两条出事岔路（Load 飙升 / 假象）；Load 照哪段、CPU% 照哪段。
- [ ] **口述两把尺**：Load 数什么（R+D、1/5/15 指数加权）、对比谁（核数 N、>2N）、`1m >> 15m` 说明什么、高 Load 低 CPU 为什么先查 D 不加核。
- [ ] **默画超载三形态**：CPU 饱和 / IO 等待 / D 堆积各看什么字段（r+us / b+wa / b+id）、各走哪条出口；说清「Load 低不等于健康」。
- [ ] **口述格子定责**：us / sy / si / wa / st / id 下一刀各去哪；wa=0 为什么不能证明盘快；%wa、await、%util 三个数差在哪。
- [ ] **口述到函数的链**：mpstat -P ALL → pidstat → top -H → TID；Java 怎么对 jstack、C/Go 用什么；什么时候不上火焰图、没有 perf 怎么办。
- [ ] **口述切换**：cswch 与 nvcswch 谁是抢时间片；cs 多少才当故障信号；一连接一线程为什么死。
- [ ] **口述热点与软中断**：整机平均为什么骗人；单核 %si 打满先看什么、扩副本有没有用；和 12 章 Recv-Q 定责怎么分界。
- [ ] **口述被抢**：%st 和 throttle 怎么分、各看哪条命令；GOMAXPROCS 为什么要对齐 limit；只设 request 不设 limit 的代价；频率调节什么时候咬人。
- [ ] **定责**：对着 mpstat 一屏（整机平均 / 单核 us / 单核 soft / %steal）逐条说结论；vmstat 第一行为什么丢掉；NUMA 什么时候才绑。

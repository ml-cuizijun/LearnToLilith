...
> **不讲**：D 为什么杀不掉、信号 → [05](../05-进程与信号/)；换页 si/so、OOM → [07](../07-内存与OOM/)；await / %util / 脏页深排 → [08](../08-磁盘与IO/)；cgroup 文件与容器细节 → [10](../10-cgroup与容器/)；包怎么进 Socket → [12](../12-网络栈与Socket/)；火焰图采样手法 → [21](../21-性能方法论-USE与RED/)。
...
- 📖 **读图**
  - 🛤️ 主线四个实线阶段；两条虚线是出事方向——队列爆了（Load 飙升，三节）/ 没排满却慢（假象，六、七节）
  - 📏 监控 Load 照「排队」段；`top` / `mpstat` 照「被执行」段。**两段对不上——Load 高 CPU 闲——就是本章最高频题面**
  - 🔧 `vmstat` / `mpstat` 是 X 光机：不换器官，只答「现在卡在哪一段」

好，主图有了。第一站：任务怎么进运行队列。
...
一个任务要跑，不是直接上 CPU，而是先进**运行队列（runqueue）**：每个 CPU 一条，调度器从里面挑下一个。
...
- 🔑 **R（runnable）** = 想跑、正在队列里等 CPU——`ps` 状态列的 `R`、`vmstat` 的 `r`，数的都是它
- 🔑 **D（uninterruptible sleep）** = 卡在不能被打断的 IO（磁盘、NFS），`kill -9` 也无效——信号要等它回到可运行态才被处理（细节见 [05](../05-进程与信号/)）
...
Linux 用 **CFS（Completely Fair Scheduler，完全公平调度器）**：每个任务记一个 **vruntime（虚拟运行时间）**，谁跑得少挑谁；**nice 值**（−20 ～ 19，负数更高优先）只调权重，不是硬保证。生产里几乎不靠调 nice 救命——它救不了一颗被打满的核。

- ⭐ **优先级反转（priority inversion）**：低优先级任务持着锁，高优先级任务等这把锁，这时一堆中优先级任务把低优先级挤下 CPU——高优先级被「反转」成连中优先级都跑不过
  - 内核 mutex 带**优先级继承（priority inheritance）**缓解
  - 现实版：「日志线程持锁 → 主循环等锁 → GC 线程插队」。P99 有尖刺却查不到 CPU 热点时，这是一个候选答案
...
本节对应练习题 Q1。R 和 D 认清了——监控大屏 Load 到 40，到底在数什么？
...
- 📏 **Load（load average，平均负载）**：过去 1 / 5 / 15 分钟，「可运行（R）+ 不可中断睡眠（D）」任务个数的**指数加权滑动平均**——越近的采样权重越大，所以 1m 对突变最敏感
  - 三个数不是三种 Load，是同一个数的三种记忆长度
  - 口诀：**「Load 数排队，CPU% 数干活」**（练习题 Q1）
- 🐧 **Linux 特有把 D 也数进去**：很多教材画「Load = CPU 就绪队列长度」只数 runnable；正是 D 这一笔，才有「CPU 很闲、Load 很高」这个必考现象

#### ❓ 为什么 Load 高 ≠ 该加核？

- ❌ **把 Load 当 CPU%**：以为队列长 = 核不够 → 直接开加核工单
- ✅ **Load 是排队长度（含 D）**：CPU% 是「核的时间花在哪」——两把尺
  - USE 里：Load ≈ **Saturation（饱和度）**，CPU% ≈ **Utilization（利用率）**（[21](../21-性能方法论-USE与RED/)）
- 🔀 **一对不上时怎么走**
  - Load 高 · CPU 闲 → 几乎必然有 D 在抬队列（查 NFS / 盘，**不加核**）
  - Load 低 · CPU 也低却超时 → 去看单核热点和假象（六、七节）

> ⚠️ **别混**：**Load ≠ CPU 利用率**。前者数「多少人在等」，后者数「核在干什么」。见上 ❓，别处不重复。
...
活动开场，8 核逻辑服 Load 飙到 40，群里喊「加 8 核」，`top` 显示 idle 70%、`%us` 不高，游戏日志打在 NFS 上：
...
到这里，Load 这把尺的零件齐了。面试问「Load 高怎么排查」，要的就是这张图——**三种形态，先认字段再下刀**：
...
- 📖 **读图**
  - 🔀 三条分支用 `vmstat` 一屏分开：**看 r、b、us、wa、id**——r+us 高是饱和，b+wa 高是 IO，b+id 高是 D 堆积（CPU 闲着队还长）
  - ② 和 ③ 常同时出现：D 堆在 IO 上，wa 跟着涨；本地 `iostat` 干净而 b 仍高 → NFS / 远程盘
  - 📌 **CPU 饱和（saturation）** = 队列里的任务超过核能立刻消费的量，排队时间开始进 wall time

> ⚠️ **别混**：这张图**不保证**「Load 低 = 健康」——单核热点、cgroup throttle、steal 都不抬 Load，P99 照样炸（六、七节）；也不保证「Load 高 = CPU 不够」——形态②③加核全无效。

> 📝 **面试**：按「**有人跑不着 CPU（饱和）/ 有人跑着但等盘（IO）/ 有人压根起不来（D）**」分组背三种形态，各配字段签名（r+us / b+wa / b+id）。收尾补「高 Load 低 CPU 先查 D，不加核；Load 低也要看单核和 throttle」——这句是区分「背过」和「懂了」的分水岭。

本节对应练习题 Q1、Q7。排队段讲完了——任务终于上 CPU，这一秒的时间账单怎么切？
...
任务被调度上 CPU 后，时间记进账单。**%us / %sy / %si / %wa / %st / %id 六格加起来 ≈ 100%**：
...
- 📖 **读图**
  - 从账单进，每格出一条固定「下一刀」——六条出口互不通用，先认**最大格**再选工具
  - `top` 抬头 `%Cpu(s)` 与 `mpstat` 是同一份账单（`%soft`=%si、`%iowait`=%wa、`%steal`=%st）
...
- 🔥 **火焰图什么时候上**：确认 CPU 型、需要函数占比时，采 **10–30s** 足够（手法见 [21](../21-性能方法论-USE与RED/)）
  - **不上**：IO 型 / wa 高、throttle、丢包（先查 si）、steal——型没分对，采样是浪费还加重负载
  - 没有 perf 也不卡壳：Java 至少 jstack，Go 至少 pprof，C 至少 `top -H` 给到 TID
...
- ✅ **%wa 高且 b>0 → IO 型**（三节收束）
- ❌ **wa=0 ≠ 盘快**：CPU 被算力打满时没有 idle 可记到 wa——盘再慢也挤不出这一格
- 📐 **三个数分清，别混着结案**
...
- `%st` 被 Hypervisor 抢走 → 七节
- `%id` 真闲
  - **id 高 · Load 仍高 → 必有 D**（三节）
  - **id 高 · Load 不高但 P99 炸 → 假象**（throttle / steal / 单核热点，六、七节）
- 顺带：nice 调过优先级的用户态时间单独记在 `%ni`，正常几乎为 0（二节）

> 📝 **面试**：对着格子口述定责——「us 找函数，sy 找 syscall，wa/b 去盘，si 去网卡，st 找云厂商，id 高 Load 高必有 D」；再补「wa=0 不能证明盘快，CPU 打满时 wa 被挤掉，盘慢听 await」。

本节对应练习题 Q3、Q12。账单会切，进程也会被切走——cs 很高，是不是该加核？
...
- 物理机每秒几万次切换常见，**持续 > 10 万/s 且延迟涨**才当故障信号
- 典型事故：网关「一连接一线程」，cs 30 万/s、P99 涨——CPU 大量时间花在「换人」而不是干活上
  - 治法：线程池按核数收敛、改 epoll / 协程模型（epoll 见 [12](../12-网络栈与Socket/)）
- ❌ 线程过多时加核只是推迟爆炸；先把并发模型收敛到核数量级，再谈扩容

> ⚠️ **别混**：**cs 高 ≠ 该加核**。见上 ❓。

> 📝 **面试**：「cs 数十万且延迟涨，我先 pidstat -w 分自愿和非自愿——自愿高查锁和 IO，非自愿高说明线程太多；一连接一线程的网关要改 epoll，不会先加核。」

本节对应练习题 Q6。下一型假象：整机 CPU 才 40%，P99 却炸了——根因常在单核。
...
- 一台机器可以**同时有两条病**：CPU3 是业务主循环热点、CPU0 是收包软中断——定责要分开说，工具链不同（四节 vs 本节）
- 单核热点时 Load 可以只有 1.x——Load 是全机平均，看不见一颗核的主循环
- 死循环、正则回溯、单线程主循环都是常客，**扩整机核不拆这一核**
...
- 📖 **读图**
  - 问题不在「CPU 不够」，在「中断没打散」——网卡每条队列的中断绑死一颗核，包全走一条队列时全堆 CPU0
  - 扩两台网关副本，**每台的 CPU0 照样满**，因为包还是进同一块网卡的同一条队列

#### ❓ 为什么扩副本治不了单核 %si？

- ❌ 以为「整机有余量 / 副本多」→ 软中断压力会均摊
- ✅ 包仍进**同一块网卡同一条硬件队列** → 中断仍绑原核 → 每台副本的对应核照样满
- ✅ 正道是把中断/软中断**打散到多核**（RSS），不是水平扩业务副本
...
本节对应练习题 Q2、Q8。单核之外，还有两类「机器很闲、业务却很慢」——虚机被抢和 cgroup 限流。
...
- **判据**：`nr_throttled / nr_periods` **增量 > 5%** 且延迟涨 → 当故障
- cgroup 文件细节 → [10](../10-cgroup与容器/)；K8s request/limit YAML → [03-04](../../03-Kubernetes/04-Pod生命周期/)
- ⭐ **GOMAXPROCS 坑**：容器里 `nproc` 常返回**宿主机核数**；Go 的 `GOMAXPROCS`、JVM 的 GC 线程就按宿主机核数开——16 个线程抢 0.5 核配额，throttle 更凶
  - 修法：`automaxprocs` / `-XX:ActiveProcessorCount`，**并发度对齐 limit**
  - 只设 request 不设 limit 能避免 throttle（request 不是硬顶），代价是可能吵邻居——配 HPA + 节点水位兜底，匹配/逻辑服常用这套

#### ❓ 为什么 %st 和 throttle 不是一回事？
...
- ❌ 把两者当成「机器被抢走时间」的同义词
- ✅ K8s 里 Pod 被 throttle 时宿主机的 %st 可以是 0——一层虚机、一层 cgroup，认法不同

> ⚠️ **别混**：**st 不是 throttle**。见上 ❓。
...
- `r` 持续 > 核数 = CPU 排队；`b` 持续 > 0 = D 堆积
- `cs` / `in` 见五、六节；`si`/`so` 是 swap 换页，非 0 先去 [07](../07-内存与OOM/)
...
本节对应练习题 Q3、Q9、Q11。四把刀会敲了——接到「Load 高 / 延迟高」怎么 60 秒定责？
...
- 📖 **读图**：先进 `vmstat` 定型，再按 r/b/us/wa/id/st/si/cs 分流；假象走六、七节，IO/D 走盘与 wchan。
...
**回到开头那个现场**：Load 到 40，工单写了「加 8 核」，idle 还有 70%。正确处理：uptime 对比核数 → vmstat 丢第一行看 b/wa → 一排 D 查 NFS/盘，**不是加核**。现在再看那张工单，你知道该怎么拦下了。

本节对应练习题 Q9、Q10、Q13、Q14。

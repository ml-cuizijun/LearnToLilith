...
> **不讲**：TLS 握手与证书链本身 → [14](../14-HTTP与TLS/)；API Server/kubelet 证书轮换 → [03-03](../../03-Kubernetes/03-API-Server/)；etcd Raft 细节 → [03-02](../../03-Kubernetes/02-etcd/)；合服流程本身 → [07-06](../../07-游戏运维专题/06-开服合服/)；日志字段规范 → [23](../23-日志运维/)；cron 的时区坑 → [30](../30-定时任务与Cron/)。
...
- 📖 **读图**
  - 🛤️ 左半段（钟源→Relay→业务机）是**对表链**：业务机只指内网 Relay，Relay 再指云/公网——不许每台直连公网池。**Stratum** = 离钟源几层，**16 = 没对上表**
  - 💥 右边是**打崩面**：时钟差长得像业务 bug——证书报过期、日志对不齐、排行榜乱，第一刀都是 `chronyc tracking`（→ 四节）
  - 📦 容器是「搭车者」：Pod 不跑 chronyd，宿主机对准了它就对准了（→ 七节）；但它**可以**有自己的时区——差 8 小时常在时区不在时钟（→ 六节）
...
- 🏔️ **Stratum 0**：原子钟、GPS——真·时间本体，不直接对外
- 🏛️ **Stratum 1**：直接连着 Stratum 0 的授时——国家授时中心、云厂商 NTP
- 🏢 **Stratum 2**：指 Stratum 1 的机器——典型是**内网 Relay**（企业自建 2~3 台 chronyd）
- 🖥️ **Stratum 3+**：业务机，指 Relay。生产上业务机 **Stratum ≤4** 为好；**>10** 当链路有问题；**16 = 未同步**

> 💡 **打个比方**：像问路。「直接看过钟的人」是 Stratum 1，「问了 Stratum 1 的人」是 Stratum 2——**问的手数越多，误差累积越大**。所以「Stratum 数字大」从来不是「更准」，而是「离钟更远」；16 则是「根本没问到人」。

#### ❓ 为什么数字大不是更准？

- ❌ **误解**：Stratum 16 / 12 「层级更高所以更权威」
- ✅ **真相**：数字 = 离钟源的跳数；越大越远、误差越容易叠；**16 = 根本没同步**
- 🎯 业务机应 **≤4**；看到 **>10** 先怀疑链路断了或源坏了，不是「换更高档的层」
...
- 📖 **读图**
  - 应用读的永远是**内核系统时钟**（`CLOCK_REALTIME`）——chronyd 负责拧准，应用自己不会「再对一次表」
  - `rtcsync` 把校准写回硬件钟（RTC），防止**重启后又偏回去**；完整闭环：上游 → 系统时钟 → 写回 RTC

> ⚠️ **别混**：**时钟（UTC 这一刻对不对）≠ 时区（显示成东八还是 UTC）**。差 8 小时先 `timedatectl` 查时区（→ 六节），不是先换 NTP 源——NTP 同步的是 UTC 时刻，与时区无关。

本节对应练习题 Q1。分层懂了——chrony 怎么对表？
...
- ⚡ **Chrony 为什么快**（面试够用版）：不是换了另一套 NTP 协议，而是 ① 滤波/估算更敢用短采样收敛；② 允许受控 step（makestep）开机对齐；③ 实现轻，虚拟机/休眠场景恢复好

#### ❓ 为什么 Chrony 和 ntpd 不能双开？

- 🥊 **两个进程抢着拧**同一把 `CLOCK_REALTIME` → 时间线来回抖，比不同步更糟
- ☠️ **ntpdate 进 crontab** = 每小时猛掰一次指针，和 chronyd 的 slew 对着干
- ✅ **处置四步**：`systemctl disable --now ntpd` → 删 crontab ntpdate → 只留 chronyd → `chronyc sources -v` 确认有 `^*`
...
- 🕳️ **step 制造空洞或重叠**：一秒被跳过或经历两次 → 结算双扣/漏扣、lease 错乱、证书窗口瞬移
- ✅ **运行中默认 slew**：拧频率连续追，业务无感
- 🎛️ **makestep** = 「允许跳」的开机策略，不是「随时可以跳」：
...
- 🔒 **三句铁律** ⭐🔥
  - `makestep 1.0 3` 是**开机对齐策略**——大头在开机头几次，之后常态是 slew
  - **运行中手打 `chronyc -a makestep` = 主动制造事故**；真要应急，低峰+广播变更窗口
  - `date -s` **绝对禁止**：手改后 chronyd 会再拉回来，时间线二次抖动——正确路径永远是「修源 → 必要时低峰 makestep → 确认 tracking」

> ⚠️ **别混**：跳钟 ≠ 校准。「校准」的默认答案是 slew；step 是受控的极端手段。面试问「怎么处理偏差 3 秒」，答「先查为什么偏（源/硬件），别上来就 makestep」。

本节对应练习题 Q2、Q3。拧准了——tracking 各字段怎么读？
...
- 🔑 **Reach 377** = 八进制满值：最近 8 次探测全成功；掉到 0 就是从来不通
- 🏠 业务机的 sources 里**只应出现内网 Relay**——再堆一堆公网池说明配置没管住
- 🔋 硬件钟一侧用 `hwclock -r` 对照：RTC 与系统时钟差 **>1s** 当故障（电池/主板）；修好之前**不该被提成 Relay**——否则把错钟批发给整个内网
...
- 规模化第一道防线是监控：node_exporter 的 **`node_timex_*`**（如 `node_timex_offset`）做**偏移 >0.5s 告警**（→ [22](../22-监控与告警/)）
- `chronyc tracking` = 告警响后的**人肉第一刀**
- `sources -v` 定责源还是本机；`hwclock -r` 与 Frequency 看硬件；`timedatectl` 分时区
- 四层由粗到细，别跳着用
...
本节对应练习题 Q4、Q5。开头证书 expired、etcd 丢 Leader——第一刀查什么？
...
口诀：**「证书/etcd/排行榜先 chronyc，别先重签」**（练习题 Q6、Q7）。⭐
...
#### ❓ 为什么证书明明还有 60 天却 expired？

- ⏱️ TLS 校验证书窗口用的是**本机时钟**，不是「真实世界」
- ⏩ 本机快了几十秒 → `notAfter` 在本机眼里已经过去 → 集体报 expired
- ⏪ 反过来慢太多 → `notYetValid`
- ✅ **处置顺序铁的**：先对表（修源，低峰才 makestep）→ 再 openssl 看窗口 → 最后才轮换重签。对表后往往立刻好；别上来就重签一地证书

### 📝 日志时间线

多机排障靠时间窗关联，同一请求在两台机器日志里差几百 ms～数秒，因果顺序就搅糊了——**小于 1 秒就开始难受**。两种病要分叉：
...
- 表现：`etcd_has_leader==0`、选主抖、lease 提前过期或双主错觉
- 机制（够用版）：Raft 的心跳/选举超时按**物理时钟流逝**感知，节点间墙钟大幅偏移时超时判断失真——恰恰是**控制面节点 NTP 必须健康**
- 并列三刀：① 各 etcd 节点 tracking ② 磁盘延迟/fsync（→ [03-02](../../03-Kubernetes/02-etcd/)）③ peer 证书是否也被时钟歪了
- 跨 region 不要靠拧 `election-timeout` 硬扛时钟问题

### 🎫 Kerberos 与其他一刀

- **Kerberos**：票据默认时间窗口约 **5 分钟**；域控和业务机各指各的公网 NTP，就集体 401——统一内网 Relay，先 tracking 两侧再查 KDC
...
- **分布式锁 lease**：时钟跳变导致锁双持有/误释放
- **游戏排行榜/合服结算**：时间戳不一致 → 差 **>1s 停止合服**（门禁见七节）
- **定时任务错位**：cron 时区坑归 [30](../30-定时任务与Cron/)；CI 因时间回跳重复触发也是时钟病
...
- 📖 **读图**
  - ①②管「谁跟谁对」，③管「怎么对」（不打跳），④管「偏了怎么知道」，⑤管「用什么钟、显示成什么区」
  - 出事时反向走：现象 → 打崩面对号 → tracking 读数 → 源/本机/硬件/时区四选一
...
本节对应练习题 Q6、Q7。偏了会炸——时区和时钟差是一回事吗？
...
- 🐳 **容器差 8 小时**：镜像默认 UTC，业务要东八——给 Pod 设 `TZ=Asia/Shanghai`，或挂载 `/etc/localtime`
- 🌍 **出海策略**：服务器、库、日志存 UTC，展示层转本地，活动开关写绝对时刻（UTC 或带时区）——不要每台机器 set-timezone 设当地时区当唯一真相

#### ❓ 为什么时钟对了还差 8 小时？

- ✅ tracking 全绿（System time 微秒级）但日志比 `date` 快整 8 小时 → **时区病，不是 NTP**
- 🧭 `timedatectl` 里两个字段两个病：`NTP synchronized: no` = 时钟没同步（→ 三、四节）；`Time zone: UTC` = 时区显示（本节）
- ❌ 别一起治：差 8 小时换 NTP 源 = 答非所问

> ⚠️ **别混**：`timedatectl` 的 `NTP synchronized` 与 `Time zone` 是两件事——别把时区坑当 chrony 事故。

### ⏱️ 墙上钟 vs 单调钟 🔥⭐

- **墙上钟（wall clock，CLOCK_REALTIME）**：`date` 看到的「现在几点」——**会被 NTP 调整**（slew / step / `date -s` 都会动它）
...
- ❌ 用墙上钟做超时/重试退避：一次跳钟或闰秒 → 间隔翻倍或归零
- ✅ **「间隔」用单调钟**（超时、重试、结算间隔）；**「时间点」用墙上钟**（日志时间戳、活动开始时刻）
- 🎮 反作弊和结算的隐性 bug 多出于「拿挂钟当秒表」
...
- `Leap status`：`Normal` 无闰秒；`Insert second`/`Delete second` = 上游公告了闰秒；`Not synchronized` = 没同步（常伴 Stratum 16）
- 处置：关注 IERS/上游公告，确认 chrony 版本与源支持，结算/发奖窗口避开闰秒边界
- 应用若假设「每分钟永远 60 秒」，闰秒窗口会踩坑——又一个「间隔用单调钟」的理由

> ⚠️ **别混**：**Leap status ≠ Frequency 漂移**。Leap 非 Normal 且已同步 = 闰秒事件（跟公告走）；Frequency **>100ppm** 且 offset 持续爬 = **单机硬件问题**（换电池/换实例，且这台**不许当 Relay**）。闰秒是全球的，硬件漂是单机的。

本节对应练习题 Q8、Q9。时区分清了——容器要跑 chronyd 吗？Relay 怎么摆？
...
#### ❓ 为什么 Pod 里不要跑 chronyd？

- 🧩 **容器共享宿主机内核时钟**：节点 Chrony 好了，所有 Pod 的 UTC 就对
- ❌ sidecar/Pod 里再跑 chronyd → **抢不到内核时钟还添乱**
- ✅ Pod 能改的只有**时区**（TZ 或挂 `/etc/localtime`，→ 六节）
- 一句口诀：**NTP 做在节点，时区做在镜像/Pod**

### ☁️ 云厂商 NTP ⭐

- **阿里云**：`ntp.aliyun.com` / `ntp1–7.aliyun.com`——免费、低延迟
- **AWS**：**169.254.169.123**——VPC 内 link-local，不必出公网
- **火山引擎**：`ntp.volcengine.com`

云主机优先厂商 NTP（延迟低、安全组好放行）；虚拟化环境看清 hypervisor 是否注入时钟，**不要**客机+宿主两套猛调。最常踩的：**安全组不放 UDP 123，再好的 Chrony 也是 sources 全 `^?`**——十次有九次是网络策略。

#### ❓ 为什么业务机不直连公网 NTP？

- 🎛️ **源不可控**：公网池质量参差、毒源难排；每台各指各的 = 集群内互相对不齐
- 🔒 **安全组/出口**：大规模业务机直出 UDP 123 难管；Relay 集中放行、集中观测
- ✅ **拓扑铁律**：业务只指内网 Relay ×2~3；Relay 再指云/公网——源可控、故障面收敛
...
设定：A/B 两区要合服，监控先报 A 区某逻辑服偏移 2s。
...
- 📖 **读图**
  - 告警线 **0.5s** =「叫人」，合服门禁 **1s** =「停合」——两档别混
  - Stratum 16 先修源，不要手改 date；证书/排行榜乱看起来像业务 bug，根因常在第 ③④ 步
  - 放行后结算仍建议盯 **小于 500ms**；回档比校时贵一个数量级，别指望「先合再补」

本节对应练习题 Q10。开头偏了 62 秒——60 秒怎么定责？
...
- 📖 **读图**
  - 第一问先排「进程不在/被抢」，第二问按 tracking 读数分流
  - 16 和全 `^?` 走网络，偏大有源走源质量，Frequency 走硬件，8 小时走时区
  - 证书、etcd、合服三个业务现象各自有明确归属，但入口都是同一刀 tracking
...
本节对应练习题 Q11～Q14。现在再看开头那些重签证书、重启 etcd 的人——你已经知道该先对表了。

...
> 这一章讲完，你会知道当时那个人错在哪，以及脑裂怎么确认、track_script 为什么必须有、为什么战斗口不该塞进七层。
> **本章回答**：一次故障里 VIP 的一生——平时谁挂它（VRRP）、主机倒了它怎么漂（GARP）、漂过去谁分流量（HAProxy）、后端坏了怎么摘（健康检查）、最坏时它怎么一分为二（脑裂）。
> **不讲**：LVS 的 NAT/DR/TUN 与 ipvsadm → [18](../18-LVS与四层负载均衡/)；Nginx 七层反代与 502/504 → [19](../19-Nginx/)；高可用总论与 RTO → [33](../33-高可用与容灾/)；超时链语义 → [14](../14-HTTP与TLS/)；抓包手法 → [16](../16-网络排障与抓包/)；云 SLB/专线 → [06](../../06-云平台/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节的主图，把「一次故障里 VIP 的一生」装进脑子；第二到第七节顺着这条线一站一站往下走——出生、上岗与故障、漂移、干活、体检、脑裂；第八节专门讲定责，也就是开头那个现场的正确解法。每一节讲完都会提示对应练习题，学完就去练。
...
咱们先不急着背 keepalived.conf。学入口高可用最怕的就是 Keepalived 和 HAProxy 职责搅在一起。先看图，把「VIP 在哪一段出事」装进脑子里：
...
好，主图有了。咱们正式出发——第一站：两台怎么商量谁挂 VIP。
...
出生段到这儿。本节对应练习题 Q1、Q9。好，主备谈妥了——大家想想：升主之后邻居交换机怎么知道 VIP 搬家了？进程死了通告为什么还在发？

---

## 三、上岗与故障：GARP 宣告，以及通告怎么停

🔥 haproxy 进程死了 VIP 还挂在本机——真凶往往是**没配 track_script**。大白话：**机器活着、通告照发，备机不知道该抢**；必须把「本机 haproxy 活不活」绑进 priority。⭐ 值班天天见（练习题 Q6）。
...
上岗段到这儿。本节对应练习题 Q6。好，通告会停了——大家想想：主机倒了，VIP 怎么一秒搬到备机？存量 TCP 还在吗？

---

## 四、漂移：主机倒了，VIP 怎么搬到备机 🔥⭐

🔥 很多人以为 VIP 漂移连接无感——**错了**。大白话：**四层不会 mid-flight 把 TCP 搬到另一台；漂移瞬间打到旧 MAC 的帧会丢，客户端要重连**。⭐ 面试必考（练习题 Q3）。
...
漂移段到这儿。本节对应练习题 Q3、Q4。好，VIP 会漂了——漂过去之后，谁按七层/四层分流量？为什么战斗 TCP 不该塞进 Nginx？

---

## 五、干活：HAProxy 怎么分流量

🔥 有人要把战斗 TCP 和登录 HTTP 一起塞进 Nginx「统一入口」——**别这么干**。大白话：**战斗长连接走四层（LVS/HAProxy TCP mode），登录 HTTP 走七层；七层解析/缓冲长连接是事故源**。口诀：**「长连接四层，HTTP 七层」**（练习题 Q2）。
...
干活段到这儿。本节对应练习题 Q2、Q10、Q11、Q12。好，流量会分了——大家想想：后端挂了怎么摘？sticky 粘得住跨机吗？
...
体检段到这儿。本节对应练习题 Q7、Q8。回到开头那个现场：两台都有 VIP——脑裂怎么确认、怎么止血？

---

## 七、病上加病：脑裂 🔥⭐

🔥 防火墙一改两台都有 VIP，有人说「再等一会会自己好」——**错了**。大白话：**VRRP 两节点没有仲裁，双方都会继续挂 VIP 服务流量**——不会像 etcd 那样少数派停写。口诀：**「双 VIP = 脑裂，先下一台」**（练习题 Q5）。
...
脑裂段到这儿。本节对应练习题 Q5。现在再看群里那句「统一入口塞进 Nginx」——60 秒怎么定责？
...
作战段到这儿。本节对应练习题 Q13、Q14。现在再看开头那三个现场——双 VIP、进程死 VIP 还在、战斗口塞七层——你已经知道该怎么拦住他了。
...
对齐练习题「合上书」，逐条口述，卡住就回对应节 / 题号。现在再看开头那三个现场——双 VIP、进程死 VIP 还在、战斗口塞七层——你该知道先查什么了。

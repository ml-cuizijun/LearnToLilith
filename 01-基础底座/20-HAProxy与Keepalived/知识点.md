...
> **刻意不展开**：LVS NAT/DR/TUN、ipvsadm、RS ARP → [18](../18-LVS与四层负载均衡/)；Nginx 反代 / 502·504 / reload → [19](../19-Nginx/)；脑裂总论 / RTO → [26](../26-高可用与容灾/)；超时链语义 → [14](../14-HTTP与TLS/)；抓包手法 → [16](../16-网络排障与抓包/)；云 SLB / 专线 → [06](../../06-云平台/)；SSH 跳板 → [28](../28-堡垒机与跳板机/)。
...
| **13** | TCP 握手后慢/断/卡（窗口、重传、心跳） |
| **18** | **内核四层 LVS**：NAT/DR/TUN、ipvs、RS ARP |
| **19** | **Nginx 七层**：反代、upstream、502/504、reload |
| **20（本章）** | **用户态 LB + VIP**：HAProxy 四/七层、健康检查、sticky；Keepalived VRRP、脑裂、track_script |
| **26** | 高可用总论、脑裂概念、RTO |

本章是 **入口双机作战手册**。18 告诉你「内核怎么转」；19 告诉你「HTTP 反代怎么配」；本章告诉你「VIP 漂在哪台、HAProxy 怎么分流量、探活假活怎么认、会话粘住怎么配」。
...
| ④ | HTTP 可走 Nginx（19）；战斗/库常走 HAProxy mode tcp；超大入口看 LVS（18） |
...
│  分层：登录可 Nginx(19) · 战斗/库 HAProxy · 超大内核入口 LVS(18)           │
...
```mermaid
flowchart LR
    p[玩家] --> http[登录 HTTP · Nginx 25]
    p --> fight[战斗 TCP · HAProxy 24 或 LVS 18]
...
> **记住**：Keepalived 摘的是「这台还要不要 VIP」；HAProxy 摘的是「后端 server 还接不接新连接」。rise/fall 两层都要连续 N 次，抗抖（见 26）。
...
> **别混**：战斗长连接挂 Nginx `proxy_pass http://` 是七层事故源（19）。要四层就 HAProxy `mode tcp` 或 LVS（18）。
...
**一句话：** 库/战斗 TCP、精细探活、Stats → HAProxy；静态/复杂 HTTP → Nginx（19）；超大内核四层 → LVS（18）。

| | HAProxy（本章） | Nginx（19） | LVS（18） |
|--|-----------------|-------------|-----------|
...
2. **说明**：这是七层 HTTP 反代；战斗帧同步走这里会变成解析、缓冲、超时事故源（19）。
...
| `timeout client` / `server` | HTTP 17～60s；TCP 数分钟 | 战斗只抄 30s →  idle 被踢 |
...
**一句话：** HAProxy ACL 做 Host/Path/灰度分流够用；复杂插件化网关去 APISIX（03-04），静态/证书花活去 Nginx（19）。
...
> **别混**：etcd 脑裂靠 quorum 停写；Keepalived 两台**停不了写**。重要数据不要指望 VRRP 当一致性协议。概念总论见 26；本章钉现场止血。
...
改配置：`haproxy -c -f /etc/haproxy/haproxy.cfg` 通过后再 `systemctl reload haproxy`。生产避免无脑 restart 闪断存量（与 Nginx reload 口径同思路，见 19）。
...
**说明：** 禁止 reload；修好再 `-c`。和 Nginx `-t` 同一纪律（19）。
...
flowchart TB
    ...
    subgraph layer ["分层误用"]
        A --> L{是不是战斗塞进七层}
        L -->|是| M[改 HAProxy mode tcp 或 LVS 18]
        L -->|登录 HTTP| N[Nginx 25]
    end
...
| 战斗 TCP 走哪？ | HAProxy `mode tcp` 或 LVS（18）；**不**走 Nginx HTTP 反代。 |

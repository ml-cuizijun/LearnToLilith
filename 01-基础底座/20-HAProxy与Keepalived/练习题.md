...
开始做题前，先给大家一个小建议：合上知识点，对着**第一节的主图**，把「一次故障里 VIP 的一生（出生 → 上岗 → 漂移 → 干活 → 体检 → 脑裂）」口述一遍。能顺下来，下面这些题你会发现大半都会了；卡壳的地方，就是你该回去重读的那一节。
...
题目的顺序也是有意排的：Q1～Q2 先钉两层分工与选型；Q3～Q6 漂移与 track_script；Q5、Q7～Q9 脑裂与体检；Q10～Q14 超时/边界与定责。
...
整章地基——Keepalived 查本机、HAProxy 查后端。

先自己试试：两层各查谁？通告停了和后端摘了差在哪？
...
选型边界——战斗 TCP 走哪一层，和开头「塞 Nginx」呼应。

先自己试试：战斗 TCP、登录 HTTP 各走哪个？为什么别塞统一七层？
...
先自己试试：通告停 → 备机升主 → GARP，存量 TCP 还在吗？
...
先自己试试：通告还在发还是真停了？track_script / 防火墙先查谁？
...
就是开头「进程死了 VIP 还在」——track_script。

先自己试试：为什么要 track_script？killall -0 测得出假活吗？
...
体检段——tcp-check 与 httpchk、假活。

先自己试试：tcp-check 和 httpchk 差在哪？什么叫假活？
...
先自己试试：nopreempt 防什么？修好原主要不要自动切回？
...
干活细考——mode tcp 超时与战斗被掐。

先自己试试：mode tcp 超时怎么配？空闲超时要对齐什么？
...
干活配套——MySQL 入口为什么 mode tcp。

先自己试试：MySQL 为什么用 mode tcp？读写怎么拆？
...
先自己试试：正向 vs 反向差在「代表谁」？no_proxy 干什么？
...
定责收口——VIP / HAProxy / 后端三层分流。

先自己试试：三问决策树怎么走？0–60s 敲什么？
...
先自己试试：默画 VIP 一生 + 脑裂止血 + GARP，90 秒怎么讲？
...
对齐知识点「合上书」，逐条口述，卡住就回对应节 / 题号。现在再看开头那三个现场——双 VIP、进程死 VIP 还在、战斗口塞七层——你该知道先查什么了。

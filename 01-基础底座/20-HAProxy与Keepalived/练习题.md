...
**一句话：** HAProxy 是用户态 LB 本职（四层/TCP/精细探活）；Nginx 是 Web 全能；超大入口内核转发看 LVS（16）。

```mermaid
flowchart TB
    a[玩家 HTTP API] --> n[Nginx 23]
    b[玩家 TCP 战斗] --> h[HAProxy mode tcp 或 LVS 16]
...
- **Nginx（23）**：静态、证书、复杂 HTTP、reload。
- **HAProxy（本章）**：`mode tcp`、rise/fall、Stats、库端口、sticky。
- **LVS（16）**：内核 NAT/DR/TUN，超大战斗入口；回程路径与本章用户态代理不同。

不要「统一入口」把战斗塞进 Nginx `proxy_pass http://`。

### 再追问

HAProxy 回程和 LVS DR 一样吗？——不一样。HAProxy 是用户态代理，回程过它；DR 回包常直出 RS（见 16）。
...
战斗长连接在 Nginx 上出现缓冲/超时怪象？——先确认是否误走七层；应改 HAProxy `mode tcp` 或 LVS（16），别在 Nginx 上硬加 timeout。
...
**一句话：** 不行——长连接要分钟级 timeout，必要时 `timeout tunnel`；外短内长语义见 09，Nginx 侧见 23。

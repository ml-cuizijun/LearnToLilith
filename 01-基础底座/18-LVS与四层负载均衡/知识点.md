...
| **08** | TCP 握手后慢/断/卡（窗口、重传） |
| **15** | iptables / conntrack 表与链 |
| **16（本章）** | **四层 LB**：NAT/DR/TUN、VIP、探活摘机、会话打偏 |
| **23 / 24** | 七层 Nginx；Keepalived 配置细节 |

本章是**入口四层主场**。06 告诉你「连没连上」；本章告诉你「连上了分到哪台、回程过不过 LVS、VIP 漂了会怎样」。
...
④ 同二层帧到达 RS11 物理网卡
        ...
        ▼
⑤ RS11 上房间进程 accept / read（后续 ESTAB 见 06/08）
        │
        ▼
...
| 注意点 | 说明 |
|--------|------|
| MTU | 外层多 ~20 字节；不分片就要调小 MTU / MSS（深排 → 11） |
...
| **keepalived** | 常兼 VIP（VRRP）+ RS 探活；conf 细节在 24 |
| **VIP** | 对外虚拟 IP；客户端永远连它 |

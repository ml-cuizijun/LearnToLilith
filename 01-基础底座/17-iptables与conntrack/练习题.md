...
开始做题前，先给大家一个小建议：合上知识点，对着**第一节的主图**，把「一个包过墙的一生（进站 → 登记 → 改名 → 过闸 → 生病 → 换壳）」口述一遍。能顺下来，下面这些题你会发现大半都会了；卡壳的地方，就是你该回去重读的那一节。
...
题目的顺序也是有意排的：Q1～Q3 先钉链表与过闸；Q4～Q7 改名与记账；Q8～Q10 表满与手术刀；Q11～Q12 加固与换壳；Q13～Q14 定责收口。
...
整章地基——后面 VIP、表满都踩在「走对链」上。

先自己试试：五条链、四张表各管什么？本机 SSH 和 VIP 转发差在哪条链？
...
命中即停的经典坑——和 Q1「链走对」配套，这题考「顺序写对」。

先自己试试：为什么 ACCEPT 加了还登不上？当前会话为什么没断？验证该怎么做？
...
**对应**：知识点 [五、过闸：filter，命中即停](知识点.md)（与 [35](../35-网络排查命令/) 同口径）

### 场景

公网入口只对白名单开放，其余全 policy DROP；值班问：为什么用户反馈「卡 30 秒才失败」而不是「立刻拒绝」。

过闸细考——和 Q2 同节，换个角度问客户端感受。

先自己试试：DROP 和 REJECT 客户端各什么表现？排障方向差在哪？口诀是什么？
...
就是 Q1「VIP 走 FORWARD」的现场版——别先重启后端。

先自己试试：第一刀查哪张表？DNAT 有计数之后还查什么？
...
Q4 的配套——DNAT 之后还要会选 SNAT / MASQ，以及回包怎么逆写。

先自己试试：固定出口用哪个？弹性 IP 用哪个？回包靠什么「改回去」？
...
游戏双 NAT——Q4 的「另一条 DNAT」版，再叠 UDP 超时。

先自己试试：大厅通房间不通，防火墙侧先查什么？心跳 40s 和 udp_timeout 30s 会怎样？
...
记账地基——下一题 Q8「表满」全踩在能读懂这一行上。

先自己试试：一行两个五元组各是什么？NEW / ESTABLISHED / INVALID 各代表什么？
...
先自己试试：铁证哪两条命令？为什么不是光纤/安全组？止血和治本怎么排？
...
这是 Q8 的三分对照——表满 vs 队列满 vs 安全组。

先自己试试：三个签名各是什么？第一刀各敲什么？
...
Q8 止血之后有人要动手术刀——这两刀都能误伤。

先自己试试：NOTRACK 什么时候能用？为什么禁止全 UDP？DROP INVALID 会误伤什么？
...
过闸加固——和 Q2「命中即停」同一纪律：限速 ACCEPT 必须在兜底 DROP 前。

先自己试试：limit 怎么写？和 syncookies / 上游清洗怎么配合？
...
先自己试试：IPVS 治什么不治什么？firewalld 和手写规则为什么二选一？
...
定责收口——把 Q2、Q4、Q8 串成 60 秒剧本。

先自己试试：三单各走决策树哪个入口？0–60s 敲什么？
...
综合收口——就是开头那个现场的正确解法，把前面十道题串起来。

先自己试试：四本账怎么背？表满铁证哪两条命令？
...
对齐知识点「合上书」，逐条口述，卡住就回对应节 / 题号。现在再看开头那个现场——新挂旧不挂、有人要改安全组 / 重启战斗服——你该知道先查哪两条命令了。

- [ ] 默画一节的图：进站 / 出站 / 记账三块，DNAT、SNAT、conntrack 各标在哪（知识点一；Q1、Q14）
- [ ] 五链四表：谁管什么、为何 NAT 不在 filter、外网 SSH 和 VIP 转发差在哪条链（知识点二；Q1）
- [ ] conntrack 表项：两套五元组、四态、[ASSURED]、use=；UDP 30s 和心跳怎么对齐（知识点三；Q7）
- [ ] 回包逆写：DNAT+SNAT 后客户端为何仍以为在跟 VIP 说话、NOTRACK 为什么弄坏它（知识点四；Q5、Q10）
- [ ] 命中即停：`-I` vs `-A`、`-v` 计数怎么读、DROP vs REJECT「超时查路拒绝查口」、limit 怎么限 SYN（知识点五；Q2、Q3、Q11）
- [ ] 表满一例钉死：铁证两条命令、为何不是安全组、止血→治本六步、和 12 的队列满怎么分（知识点六；Q8、Q9）
- [ ] 换壳：iptables-nft 怎么认、firewalld 为什么二选一、IPVS 治什么不治什么（知识点七；Q12）
- [ ] 定责：决策树三个入口各接什么报障、60 秒剧本怎么走（知识点八；Q13）
- [ ] 白板过墙一生综合（Q14）

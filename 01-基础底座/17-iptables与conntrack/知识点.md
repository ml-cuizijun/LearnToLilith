...
> 这一章讲完，你会知道当时那个人错在哪，以及「新挂旧不挂」怎么两条命令钉死、为什么不该先重启还活着的战斗服。
> **本章回答**：一个包过主机防火墙的一生——走哪条链、被谁改地址、在哪记账、在哪被丢；「新挂旧不挂」怎么两条命令钉死。
> **不讲**：队列 / fd / 临时端口那三个「满」 → [12](../12-网络栈与Socket/)；握手/重传/心跳机制 → [13](../13-TCP与UDP/)；抓包与 MTU 深排障 → [16](../16-网络排障与抓包/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节的主图，把「一个包过墙的一生」装进脑子；第二到第七节顺着这条线一站一站往下走——进站、登记、改名、过闸、生病、换壳；第八节专门讲定责，也就是开头那个现场的正确解法。每一节讲完都会提示对应练习题，学完就去练。
...
咱们先不急着背五链四表。学防火墙最怕的就是上来背链名，背完还是不会查——规则写得再对，查错链也看不见计数。先看图，把「包从哪进、往哪走、谁在旁边记账」装进脑子里：
...
**读图**：带着三个问题看这张图，比背链名有用得多：

- 一个包的一生三段：**进站**（PREROUTING → 路由 → INPUT 或 FORWARD → POSTROUTING）、**出站**（OUTPUT → POSTROUTING，**没有 PREROUTING**）、**记账**（conntrack 挂在旁边，nat 和 filter 都看它）。⭐ 面试必考「本机发出有没有 PREROUTING」（练习题 Q1）。
- 排障从「包该走哪条链」入手：外网连本机端口查 INPUT；本机当网关/VIP 转发查 FORWARD；本机主动出站查 OUTPUT——查错链，规则写得再对也看不到计数。
- 本章的照妖镜只有三件：`iptables -t <表> -L -v`（规则吃没吃到包）、`conntrack -C`（账本还剩多少行）、`dmesg`（内核丢包铁证）。开头那个「新挂旧不挂」现场，照妖镜的第二、三件就能钉死（练习题 Q8）。

---

好，主图有了。咱们正式出发——第一站：包到底走哪条链。
...
大家想想：面试官递来白板笔问「五链四表」，你先画什么？很多人上来就列规则——错了。先把**链和表**分清，后面所有排障都踩在这上面。
...
不带 `-v` 只能「看见规则」，不能证明「吃到了包」——这是本章排障第一纪律。口诀：**「不带 -v 等于没查」**。

---

进站段到这儿。本节对应练习题 Q1。好，链走对了——大家想想：回包为什么不用再写一条反向规则？
...
🔥 很多人会答「因为写了双向规则」——**错了**。大白话：**内核先给这条流建了一行账，回包翻账对得上就放，不必再写反向规则**。这本账叫 **conntrack（connection tracking，连接跟踪）**。
...
> ⚠️ **别混**：conntrack 表项 ≠ Socket——`ss` 看的是进程手里的连接（[12](../12-网络栈与Socket/)），`conntrack` 看的是内核的流账。一条 Socket 连接对应一行账，但账满时**Socket 完全健康**，这就是六节「新挂旧不挂」的机制根源。⭐ 这是高频错答——看到 ESTAB 还活着就说「防火墙没事」（练习题 Q7、Q8）。

---

登记段到这儿。本节对应练习题 Q7、Q10。好，账本会看了——大家想想：VIP 上的包，目的地址是谁改的？回包又怎么改回去？
...
🔥 很多人会在 filter 表里翻 DNAT——**白翻**。大白话：**改地址是 nat 表的活，filter 只管放不放行**。口诀：**「nat 改地址，filter 管放行」**。
...
🔥 客户端明明打的是 VIP，后端回包怎么还能「看起来像 VIP 回的」？很多人会答「后端自己写了 VIP 当地址」——**错了**。大白话：**网关靠 conntrack 那行账把地址逆变换回去**——所以 NAT 依赖 conntrack，NOTRACK 掉的流做 NAT 会坏。
...
改名段到这儿。本节对应练习题 Q4、Q5、Q6。好，地址会改了——大家想想：放行写在拒绝后面，会发生什么？
...
🔥 有人先 `-A DROP` 再 `-A ACCEPT`，自己 ssh 还活着就喊「没事」——**错了**。大白话：**命中即停，排在前面的规则吃掉包，后面永远轮不到；老会话是 ESTABLISHED，靠总闸活着，新开才会死**。口诀：**「-I 插队，-A 排队」**（练习题 Q2）。
...
过闸段到这儿。本节对应练习题 Q2、Q3、Q11。好，规则会写了——回到开头那个现场：新挂旧不挂，真凶是什么？
...
🔥 群里一半人喊改安全组、一半人喊重启战斗服——**两边都错了**。大白话：**账本写满了，新客进不来，场里已停好的车照常待着**——签名就是新挂旧不挂。光纤断 / 安全组误改通常**新旧一起死**；不对称优先查表满。⭐ 这是值班天天见、面试必考（练习题 Q8、Q9）。
...
生病段到这儿。本节对应练习题 Q8、Q9。好，表满会认了——大家想想：换了 nft / IPVS，这套账还看吗？
...
大家想想：切了 IPVS 还要不要看 conntrack？很多人会答「不用了」——**错了**。IPVS 治的是规则爆炸，**多数 DNAT 路径仍吃表**。
...
换壳段到这儿。本节对应练习题 Q12。现在再看群里那句「改安全组 / 重启战斗服」——60 秒怎么定责？
...
作战段到这儿。本节对应练习题 Q13、Q14。现在再看群里那句「改安全组 / 重启战斗服」——你已经知道该怎么拦住他了：先 `conntrack -C` vs max，再看 dmesg。
...
对齐练习题「合上书」，逐条口述，卡住就回对应节 / 题号。现在再看开头那个现场——新挂旧不挂、有人要改安全组 / 重启战斗服——你该知道先查哪两条命令了。
...
- [ ] **口述登记**：conntrack 表项两套五元组、NEW/ESTABLISHED/RELATED/INVALID 各是什么、回包为什么不用写反向规则、UDP 超时和心跳怎么对齐。（练习题 Q7、Q10）

...
> 这一章讲完，你会知道当时那个人错在哪，以及卡 30 秒和指纹告警各自第一刀砍哪。
> **本章回答**：一次 SSH 连接从敲门（TCP）、对暗号（密钥交换）、认门（主机指纹）、验身（用户认证）到落 Shell、挖隧道的一生，每一段会怎么坏、第一刀砍哪。
> **不讲**：TCP 握手与队列机制 → [13](../13-TCP与UDP/)；ping/traceroute/ss 排查刀法 → [02](../02-网络排查命令/)；iptables/conntrack → [17](../17-iptables与conntrack/)；TLS 与证书体系 → [14](../14-HTTP与TLS/)；fail2ban、sshd 加固全景 → [27](../27-Linux安全加固/)；跳板机与批量密钥分发 → [28](../28-堡垒机与跳板机/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节的主图，把「一次 SSH 连接的一生」装进脑子；第二到第七节顺着敲门 → 对暗号 → 认门 → 验身 → 落 Shell / 隧道往下走；后面作战节专门讲定责，也就是开头那个现场的正确解法。每一节讲完都会提示对应练习题，学完就去练。
...
咱们先不急着背 sshd_config。学 SSH 最怕的就是「连不上」和「登不进」搅在一起。先看图，把「卡在哪道门」装进脑子里：
...
**读图**：带着三个问题看这张图，比背配置项有用得多：主线是五道门，每道门各有一类典型病：敲门段连不上（二节）、对暗号段几乎不坏但它解释「为什么安全」（三节）、认门段出指纹告警（四节）、验身段出 Permission denied（五节）、落 Shell 之后是隧道与传文件（七节）。「生病」不是新阶段，是任何一段坏了的样子——八、九节集中定责；`ssh -v` 的调试输出就是照这条线走的，卡在哪一步就是哪道门的病。

---

---

好，主图有了。咱们正式出发——第一站：TCP 都还没连上时，refused 和 timeout 差在哪？

---

## 二、敲门：到 22 端口之前

🔥 Connection refused vs timed out——**值班天天见**。大白话：**refused 是对方回话了但 22 没人听；timeout 是喊了没人应**。口诀：**「拒绝查本机，超时查网络」**（练习题 Q1）。
...
敲门段到这儿。本节对应练习题 Q1。好，TCP 通了——大家想想：对暗号这段几乎不坏，它到底在保证什么？
...
对暗号段到这儿。本节对应练习题 Q2。回到开头那个指纹告警——手起刀落 rm known_hosts 为什么危险？

---

## 四、认门：known_hosts 与主机指纹

🔥 `REMOTE HOST IDENTIFICATION HAS CHANGED` 就 `rm known_hosts`——**高频错答**。大白话：**指纹变了可能是重装，也可能是中间人；先核实再改，别直接删**。⭐ 面试必考（练习题 Q3）。
...
认门段到这儿。本节对应练习题 Q3。好，门认清了——大家想想：密钥认证为什么比密码安全？公钥放好了为什么还登不进？

---

## 五、验身：密码认证 vs 密钥认证

🔥 公钥放好了就是登不进——**权限位往往是真凶**。大白话：**authorized_keys 和家目录太宽，sshd 会直接拒绝**。口诀：**「家目录不组写，密钥文件 600」**（练习题 Q4、Q5）。
...
验身段到这儿。本节对应练习题 Q4、Q5。好，能登了——passphrase、ssh-agent、`-A` 各自解决什么？加固怎么不把自己锁死？
...
认证配套到这儿。本节对应练习题 Q6、Q7。好，会话在了——隧道 -L/-R 差在哪？scp 和 rsync 怎么选？
...
干活段到这儿。本节对应练习题 Q8～Q11。现在再看开头两个现场——卡 30 秒、指纹告警就 rm——第一分钟怎么定责？

---

## 九、作战：60 秒定责

🔥 十台机器卡 30 秒就喊「内网挂了」——**常常是 UseDNS**。大白话：**sshd 反解客户端 IP 卡住，不是光纤断了**。口诀：**「先 ssh -v，再怪网络」**（练习题 Q11、Q12）。
...
作战段到这儿。本节对应练习题 Q12、Q13、Q14。现在再看开头那两个人——喊内网挂了、rm known_hosts——你已经知道该怎么拦住他们了。
...
对齐练习题「合上书」，逐条口述，卡住就回对应节 / 题号。现在再看开头那两个现场——卡 30 秒喊内网挂了、指纹告警就 rm——你该知道先查哪道门了。

...
> **不讲**：TCP 握手与队列 → [13](../13-TCP与UDP/)；ping/traceroute/ss → [35](../35-网络排查命令/)；iptables/conntrack → [17](../17-iptables与conntrack/)；TLS 证书体系 → [14](../14-HTTP与TLS/)；fail2ban / sshd 加固全景 → [27](../27-Linux安全加固/)；跳板机与批量密钥 → [28](../28-堡垒机与跳板机/)。
...
- 🗺️ **timeout 刀法**：按 [35](../35-网络排查命令/) 走 `ping` → `traceroute`；云上安全组 / iptables DROP 最常见（→ [17](../17-iptables与conntrack/)）
...
| Connection timed out | ping/traceroute → [35](../35-网络排查命令/) | 反复重试 ssh |
...
下一章：[Linux 安全加固](../27-Linux安全加固/)。排查刀法：[35](../35-网络排查命令/) · 权限位：[25](../25-用户权限与特殊位/) · 跳板机：[28](../28-堡垒机与跳板机/)。

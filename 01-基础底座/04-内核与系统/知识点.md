...
| 章 | 负责 |
|----|------|
| **04（本章）** | 整机骨架：用户态/内核/syscall、分层归类、双日志、第一眼命令 |
| **02～07** | 各域细账（进程、CPU、内存、磁盘、网络、cgroup） |
| **11** | systemd unit 全文与落地 |
...
├─────────────┴────────────────────┴───────────────────────────────────────┤
│  双日志：journal(退出码) · dmesg(OOM/盘/网卡)  ｜ 五域 → 06～07            │
...
慢 / 挂 / 连不上
        │
        ├─ Load 高 + %us 高     → 调度 → 06
        ├─ MemAvailable 低 / OOM → 内存 → 07
        ├─ %wa 高 / await 高    → 磁盘 → 08
        ├─ %si 高 / ss 异常     → 网络 → 12
        └─ 容器 P99 炸、宿主机闲 → cgroup → 10
```
...
- 第 4 列 `calls`：**15234 次 write** → 08～10 秒内一万多次，每条日志一行就 write 一次。  
...
| # | 现象 | 丢进哪格 | 不要做 |
|---|------|----------|--------|
| ① | Load 40、idle 70% | D / IO → 08、02 | 加 CPU |
| ② | 进程 137、宿主机还有内存 | 内存 / cgroup OOM → 07、07 | 只看 journal 说崩了 |
...
```mermaid
flowchart LR
    free["free → 64G<br/>宿主机视图"] -.->|≠| cg["memory.max → 05G<br/>真限额"]
...
flowchart TB
    ...
    C -->|15/143| F["对发版窗口"]
    C -->|活着| G["ss -lntp → curl → 21"]
    B -->|慢| H["五域归类 §7"]
    H --> I["%wa→08  %si→12  %sy→§5  Mem→07  throttle→15"]

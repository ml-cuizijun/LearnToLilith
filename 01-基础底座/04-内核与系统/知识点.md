...
> 凌晨三点告警「逻辑服不可用」，群里已经有人敲「重启试试」、有人开了 top 不知道看哪个格子——重启会清掉现场，dmesg 里那行 OOM 证据跟着一起没。
> **本章回答**：一行业务代码（一次 `write`）从用户态穿过系统调用进内核、落到硬件要走哪条路；进程死了谁记账、机器慢了先归哪一格。
> **不讲**：Load/CPU 细账 → [06](../06-CPU与Load/)；OOM 细账 → [07](../07-内存与OOM/)；磁盘 IO → [08](../08-磁盘与IO/)；进程与信号 → [05](../05-进程与信号/)；cgroup 文件树 → [10](../10-cgroup与容器/)；unit 全文 → [11](../11-Systemd与服务管理/)；sysctl → [24](../24-内核参数与sysctl/)。
...
- 📖 **读图**
  - 🛤️ 主线只有一根：出生 → 用户态 → 正门 → 内核 → 硬件；**没有任何箭头从业务直连磁盘/网卡**——合上书要能默画（Q1）
  - 📒 两条虚线是记账：journal 收应用（含退出码），dmesg 收内核（OOM、盘错、网卡 reset）——进程死了两边各说半句（六节）
  - 🔭 `top` 格子照不同段：`%us` 用户态、`%sy` 正门、`%si` 内核收包、`%wa` 等盘——「CPU 不高但很慢」多半是后两个（四节、七节）
...
- 📖 **读图**
  - 从左到右是开机顺序，任何一环卡住后面全起不来
  - fstab 写错卡在「挂其它盘」；sshd 排在它后面——**ping 通、SSH 超时、刚改过 fstab** 三现象凑齐就是这个签名

- 🔑 **systemd = PID 1**，三件事都跟值班有关：
  - 拉起/停止服务（`systemctl`）
  - 收应用日志（`journalctl -u`）
  - 给服务挂 cgroup（容器/K8s 限额最终落这里 → [10](../10-cgroup与容器/)）
...
- 📖 **读图**
  - Requires 只绑「生死」不绑「先后」——只有 Requires 时可能并行拉两个，逻辑服连上一个还没 listen 的端口
  - 硬依赖必须 `After=` 和 `Requires=` **一起写**；对方挂掉时 systemd 给依赖方发 **SIGTERM（15）**——「数据库抖两秒、整组逻辑服被停」就是 Requires 写太狠

#### ❓ 为什么有了 Requires 还要写 After？

- ❌ **只写 Requires**：管「没它不能活」，**不管先后** → 可能并行起 → 连未就绪的库刷屏
- ❌ **只写 After**：管顺序，对方挂了**自己照跑**——「After 了就保险」是错的
- ✅ **硬依赖 = After + Requires**：先等它起来，它挂了我也停
- 🎮 游戏服常见折中：对 MySQL 只写 `After`、重连放应用里；`Requires` 留给「没有绝对不能跑」的本地 agent
...
> ⚠️ **别混**：`After` 只管顺序，不管对方死活；`Requires` 只管生死，不管先后——两个维度，缺一不可（硬依赖场景）。

### 👻 Type 与 PIDFile：幽灵进程从哪来 🔧

- **`Type=simple`**（默认首选）：进程前台跑（如 Nginx `daemon off`）
- **`Type=forking`**：启动命令 fork 后**父进程退出**，真服务是子进程——systemd 靠 **PIDFile** 认主
  - PIDFile 路径写错/写晚 → unit **failed**，但**子进程可能还占着端口**
...
- 🔍 **现场**：再 `start` 就抢端口（`Address already in use`）。先对 PIDFile/Type，别盲目 start
  - 看生效配置（含 drop-in）：`systemctl cat myapp`
  - 能前台就 `simple`；配了会 fork 的老脚本却写 `simple`，父进程一退出 systemd 会把整棵 cgroup 杀掉
...
- 坑在**设备名重启会变**（sdb1→sdc1），UUID 稳定；细账 → [09](../09-文件系统与inode/)
- 已经能 SSH 时直接改 fstab + `mount -a` 验证，不必进救援
...
🔥 业务代码里写一句 `write`，它真的直接碰了磁盘吗？——没有。CPU 分两种特权级：
...
- 🗺️ 配套两个地址空间名词：
  - **用户空间（user space）/ 内核空间（kernel space）**——每个进程的虚拟地址空间分两块
  - 内核空间全机共享；进程「进入内核」就是开始执行内核空间里的代码

- 🔑 **系统调用（system call，syscall）** = 用户程序请求内核服务的受控入口，用户空间进内核空间的**唯一正门**：
...
- 🛡️ **隔离**：任意进程都能碰磁盘/网卡 → 一台机器立刻乱套；特权级把「能碰硬件」锁在 ring 0
- 🚪 **唯一正门**：要碰，只能走 syscall——内核检查权限、记账、再代劳
- ⏱️ 一次 syscall 往返约 **100～300ns**——单次可忽略，打到十万百万次就是事故（四节）

### 🔔 进内核的三种方式，别混成一种 🔥

- **系统调用**：进程**主动**发起——`write`、`futex` 都是业务代码自己调的，耗时记 `%sy`
- **中断（interrupt）**：**被动**叫门
  - **硬中断（hardirq）**：硬件拉线叫 CPU（网卡「包到了」，`top` 的 `%hi`）
  - **软中断（softirq）**：内核把能推迟的活稍后批量做（收包剥头进协议栈，`%si`）→ [06](../06-CPU与Load/)、[12](../12-网络栈与Socket/)
- **上下文切换（context switch）**：调度器把 CPU 从一个进程换给另一个，保存/恢复寄存器，`vmstat` 的 `cs` 列——换的是**进程**，代价高于一次进出内核
...
- 📖 **读图**
  - write 的「快」快在内存——**页缓存（page cache）** 是内核拿空闲内存缓存磁盘数据的那层 RAM
  - write 返回 ≠ 落盘（掉电会丢，`fsync` 才强制落盘）
  - 日志场景经典结论：**盘被打满通常打满的是 IOPS（每秒 IO 次数），不是容量写满**

#### ❓ 为什么 write 返回了数据还可能丢？

- ✅ 内核先把数据拷进 **page cache**，立刻告诉应用「写完了」——所以快
- ❌ 此时磁盘上可能还没有这一笔；掉电 / 崩机 → 未刷盘的丢了
- 🔒 要「真落盘」才调 `fsync` / `fdatasync`；生产日志该不该每条 fsync，是吞吐与安全的取舍
...
- `calls` 列 `write` 一万多次 → 每条日志一次 write
- 治理是**减少次数**：攒到 **8KB** 再写，100 万次变约 125 次；不是加核
- 100 万次 1 字节 write ≈ **0.1～0.3s CPU**——`%sy` 高、`%us` 不高的账就是这么来的
...
- 📖 **读图**
  - 先分 sy 还是 si，再往下走
  - `%si` 高时 strace 业务进程是白忙——收包活根本不在业务线程里
...
- **/proc 是伪文件系统（procfs）**——不是磁盘上的文件，是内核把内部状态实时渲染成文本；读它就是问内核本人
- 常用几扇窗：
  - `/proc/meminfo`（free 的数据源）
  - `/proc/cpuinfo`（CPU 型号核数）
  - `/proc/<pid>/status`（状态、VmRSS、Threads）
  - `/proc/<pid>/fd`（打开了什么）
  - `/proc/cmdline`（本次启动内核参数）、`/proc/uptime`
...
- 内核升级 = 变更：有窗口、有值守、grub **留上一版**，验收 `uname -r` 到位 + `dmesg` 无 oops；关掉无人值守自动升级，别在活动窗口升
- 主机名用 `hostnamectl set-hostname` 改；`/etc/hosts` 里本机名没对上，sudo 解析自己会很慢；云上还要防 cloud-init 把名字改回去
...
#### ❓ 为什么容器里 free 显示 62Gi 还会 OOM？

- 📕 free 读的是 **`/proc/meminfo`**——这是**宿主机**账，mount namespace **不换**它
- 📗 真限额在 **cgroup**（`memory.max` / v1 `memory.limit_in_bytes`）；K8s `limits` 落在这里
- 💀 超了 cgroup 限额 → 这组进程被杀（常表现为 137）——和 free 显示多少无关
- `nproc` 同理返回宿主机核数：limit 1 核的容器里 Go 默认 `GOMAXPROCS=16` → CPU **throttle（限流）**——宿主机 `%id` 很高、容器 P99 照炸
- 监控看 `container_memory_working_set_bytes` 对比 limit；**不要**把容器里的 `node_memory_MemAvailable` 当容器可用内存。细账 → [10](../10-cgroup与容器/)
...
- 📖 **读图**
  - 🧱 **墙**：怎么进内核（ring + syscall）
  - 管家：进程谁拉起、日志谁收（PID 1 三职）
  - 账：死了怎么验尸（journal × dmesg）
  - 限：容器世界谁说了算（cgroup）
  - 诊：告警先去哪（五格出口）——前四件是**结构**，第五件是**动作**
...
🔥 很多人看到 137 就结案「OOM」——**错了**。137 只说明被 SIGKILL 杀了，**谁杀的**要去 dmesg 找证据。
...
#### ❓ 为什么 journal 里 9/KILL 不能直接定 OOM？

- journal 只证明**死法** = SIGKILL（9），凶手可能是：
  - 内核 OOM Killer（整机）
  - cgroup 限额杀（容器）
  - 人手 `kill -9` / kubelet 强制杀
- ✅ 必须拿 **dmesg**（或宿主机 kmsg / `memory.events`）对时间和 PID，才结案
- 容器内 `dmesg` 常为空——很多镜像不挂宿主 kmsg，上**宿主机**看，或看 K8s `OOMKilled`
...
常用参数：`-b` 本次开机、`-b -1` 上次开机、`--since "10:00"`。
...
- 📖 **读图**
  - 环上每一步都在放大事故：杀 → 拉 → 吃满 → 再杀
  - 同机服务被冷启动 CPU 峰值拖死；StartLimit 用尽停在 failed，看起来像「服务起不来」
  - 更隐蔽：热更脚本让进程正常退出换新包，`always` 把旧进程拉回来和新进程抢端口

#### ❓ 为什么 Restart=always 反而更不稳？

- `always` + `RestartSec=0` = 无论怎么死立刻拉起——OOM 现场变成**杀→冷启→再杀**正反馈
- 热更 `exit 0` 也会被拉回旧进程 → 抢端口
- ✅ 生产写法：`on-failure` + 间隔 + 次数上限
...
- 📖 **读图**
  - 入口先分「挂 / 慢 / 连不上」；「慢」才进五格
  - 第一眼各格看：load 与 `%us/%sy/%st`（调度）、MemAvailable 与 RSS（内存）、`%wa` 与 await（磁盘）、`%si` 与 `ss -s`（网络）、`memory.max`/`cpu.stat`（cgroup）
...
- 📖 **读图**
  - 第一刀分「进程在不在」——挂了走退出码（左），活着走五域（右），fstab 是独立入口
  - 分支全部是真实现象，对号入座再动手
...
练习题 Q10、Q13、Q14。开头那个「重启试试」——正确剧本是：先分域 → 留证（journal × dmesg）→ 再谈重启。

...
- **`status=9/KILL`**（退出码 **137 = 128+9**）：收到 SIGKILL——**还不知道是谁杀的**，去 dmesg。
...
- [ ] **默画一节的图**：出生 → 用户态 → 正门 → 内核 → 硬件，说清「没有任何箭头从业务直连硬件」；两条虚线（journal/dmesg）各收谁说的话。
- [ ] **口述出生**：开机路径 BIOS/GRUB→内核→systemd→fstab→sshd；systemd 三职；After/Requires/Wants 各管什么、硬依赖怎么写；fstab 写错的签名与救援步骤。
- [ ] **口述穿门**：用户态/内核态、特权级 ring 0/3、系统调用是唯一正门；系统调用 / 中断（硬中断、软中断）/ 上下文切换三件事怎么分。
- [ ] **默画干活**：一次 write 的时序——计 `%sy`、进 page cache、稍后回写打 IOPS；`%sy` 高的证明步骤（strace -c 看 calls）与 8KB 合并的账。
- [ ] **口述看世界**：/proc 是伪文件系统、free 读宿主机账、容器真限额在 cgroup `memory.max`、nproc/throttle 同理；uname -r 与内核升级验收。
- [ ] **默画骨架五件套**：墙、管家、账、限、诊——各对应哪一节、各不保证什么。
- [ ] **口述死亡**：退出码 137/143/1 各是什么死法；journal×dmesg 四象限怎么交叉结案；Restart 生产写法与死亡螺旋。
- [ ] **定责**：慢/挂/连不上先分哪三种；五域各看什么第一眼指标、各开哪章；vmstat 的 r/b/wa 怎么读。
- [ ] **口述作战**：60 秒剧本四步、三问（进程在不在/能不能变更/流量通不通）、为什么不能先重启。

...
- 📖 **读图**
  ...
  - 📦 「收工」= **定域 + 证据**，不是修好；四线全平但像网络 → 停主机刀转 [02](../02-网络排查命令/)
...
> ⚠️ **别混**：定域 ≠ 定根因。这张图**不保证**「为什么盘忙 / 为什么 OOM」——只保证别把 IO 型当 CPU 型治。处方互不通用：IO 型加核没用、CPU 型调 `somaxconn` 没用、内存型别先 `drop_caches`。另有独立出口：四线全平但像网络 → **停主机刀**，转 [02](../02-网络排查命令/) / [16](../16-网络排障与抓包/)。
...
> ⚠️ **别混**：**active ≠ 健康**——进程在但玩家进不去，主机刀扫一轮没异常 → 转 [02](../02-网络排查命令/) 或探活（[21](../21-性能方法论-USE与RED/) · [22](../22-监控与告警/)）。
...
下一章：网络排查刀法 → [02 网络排查命令](../02-网络排查命令/)。机制深排：[06](../06-CPU与Load/) · [07](../07-内存与OOM/) · [08](../08-磁盘与IO/) · [09](../09-文件系统与inode/) · [11](../11-Systemd与服务管理/) · [21](../21-性能方法论-USE与RED/)。

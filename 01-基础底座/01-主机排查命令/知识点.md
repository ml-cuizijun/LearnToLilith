...
| 章 | 负责 |
|----|------|
| **26（本章）** | 主机侧**刀法**：怎么敲、怎么读、怎么定下一刀；30 秒 / 3 分钟套餐；高 Load / OOM 迹象 / 盘满 / 服务挂的决策树 |
| **03** | Load 数什么、CPU% 格子、软中断 / steal / throttle **为什么** |
| **04** | OOM killer、`available` 背后、换页 si/so |
| **05** | await、%util、脏页、队列深度公式 |
| **12** | USE/RED、何时 profiling、证据链哲学 |
| **13** | unit、依赖、重启策略 |
| **27 / 11** | 网络刀法、抓包 |
...
3. **交接**：定域证据（命令片段）交给 03～05 / 13 深排；网络嫌疑甩给 27 / 11。
...
├─────────────┬────────────────────┬───────────────────────────────────────┤
│  开场 30 秒  │  分域挥刀           │  出口（别章）                          │
│  uptime     │  CPU: top/mpstat   │  Load/%si → 03                        │
│  → vmstat   │  MEM: free/pidstat │  OOM → 04                             │
│  → 分型     │  DISK: iostat/df   │  await → 05                           │
│             │  SVC: systemctl    │  unit → 13 · 网 → 27/11               │
├─────────────┴────────────────────┴───────────────────────────────────────┤
...
1. `uptime` → `load average: 35.2, 12.1, 4.0`；`lscpu` 确认 8 核 → 1m Load **远大于核数**，但还**不能**说是 CPU 不够。
...
|------|------|
| 只看 all | 单核热点被平均稀释 |
| `%soft` 高就调 TCP 窗口 | 先看是否单核打满软中断；网刀在 27/11，机制在 03 |
...
**一句话：** iostat 看设备是否忙、等多久；iotop 看是谁在刷。公式回 05，本章只练读屏。
...
> **记住**：iostat 定「盘忙」；iotop 定「谁忙」。await 数学去 05。
...
flowchart TB
    ...
    D --> D1["mpstat -P ALL → top/pidstat → 03"]
    E --> E1["iostat → iotop → 05"]
    F --> F1["free -h → dmesg OOM → 04"]
    G --> G1["du → lsof deleted → 扩或清"]
    H --> H1["status → journalctl -u → 13"]
```
...
| 现象 | 先做 | 别做 |
|------|------|------|
| Load≫核、`r`↑、`us`↑、单核 100% | mpstat → top -H → 03/12 | 不看单核就加核 |
| Load≫核、`b`↑、`wa`↑、idle 仍高 | iostat → iotop → 05 | **加核** |
| Load 高、`%soft` 单核满 | 03 RSS；网可疑再 27 | 先改 TCP 窗口 |
| Load 高、`%st` 高 | 超卖/换机 → 03 | 先调应用参数 |
...
| 容器里被杀、宿主机还闲 | cgroup 内存限额 → 04/07 | 只看宿主机 free |
...
| active 但玩家进不去 | 主机刀转网刀 27/11；或应用探活 | 看见 active 就结案 |
...
5. **边界**：机制回哪章？网络何时甩给 27/11？strace 为什么不是首刀？  

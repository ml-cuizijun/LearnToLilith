...
> **不讲**：iostat/await/脏页水位/fsync 落盘路径 → [08](../08-磁盘与IO/)；D 进程与信号语义 → [05](../05-进程与信号/)；LVM/RAID/分区加盘全流程 → [29](../29-磁盘管理与LVM/)；logrotate 轮转 → [23](../23-日志运维/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节的主图，把「一个文件的一生」六个阶段装进脑子；第二到第八节顺着这条线一站一站往下走——出生、被写、有名字、被删、满、对账、修复；第九节专门讲定责，也就是开头那个现场的正确解法。每一节讲完都会提示对应练习题，学完就去练。
...
咱们先不急着背 `df -i`、`lsof`。学文件系统最怕的就是上来背命令——背完还是分不清「满的是块还是 inode」。先看图，把「一个文件从哪来、到哪去」装进脑子里：
...
- 主线是一个文件的一生，六个阶段串成一条时间线，后面每节讲一段；「被删」和「满了」两段是值班高频区，也最厚。⭐ 面试必考就挂在这两段（练习题 Q1、Q2）。
...
好，主图有了。咱们正式出发——第一站：文件出生时，内核到底登记了什么。
...
- **目录项（dentry）** = 目录里的一条「文件名 → inode 号」登记。目录本身也是个文件，内容就是一张名字表；内核把这些登记缓存成 **dentry cache（目录项缓存）** 加速路径解析。**文件名不在 inode 里**——这是全章第一句，也是 ⭐ 面试高频错答点。
...
🔥 大家先问自己：`df` 显示 100%，root 还能写几 MB——是不是玄学？很多人会答「缓存还没刷」——**错了**。真相是预留块。
...
> ⚠️ **别混**：`Avail=0` 是「非 root 配额用光」，不是「块物理用光」——预留块救 root 的写入，救不了普通用户业务。`-m` 调 0 会在真满时连 syslog 都写不进，数据盘调到 1% 是底线，不是 0。口诀记一句：**「Avail 是非 root 视角，差的那 5% 是预留」**（练习题 Q11）。
...
**fd（file descriptor，文件描述符）** = 进程手里指向这个打开文件的小整数（`/proc/PID/fd` 怎么用它见 [05](../05-进程与信号/)）。内核真正回收一份 inode+块的条件是**两个计数同时归零**：nlink（名字数）和打开它的 fd 数。这句话是第五节的全部伏笔——记住它，开头那个「rm 了空间不释放」就有答案了。

> 📝 **面试**：问「一个文件在磁盘上怎么存」，答三件套——名字在目录项里、元数据和块指针在 inode 里、内容在数据块里；再补一句「文件名不在 inode 里，所以删名字和还空间是两件事」，就比背定义高一档。

这一节对应练习题 Q11、Q13——预留块和 dentry 路径解析，学完可以直接去练。

好，三件套登记完了。大家想想：`write()` 返回成功，数据真的已经在盘上了吗？很多人会答「是」——咱们看看错在哪。
...
🔥 `write()` 返回成功只代表数据进了 **Page Cache（页缓存）**、该页标成**脏页（dirty page）**——大白话：**先进内存候车室，还没上车**。回写水位、fsync/O_DIRECT、脏页堵全机的完整故事在 [08](../08-磁盘与IO/)。本章只看文件系统这一段：**账本怎么改才不会改一半**。
...
> ⚠️ **别混**：journal 保的是**文件系统结构一致**（inode/目录项/位图不半拉子），**不保你写的数据内容不丢**——掉电时最后一个未提交事务的数据照样丢。保业务数据靠库 WAL + fsync（[08](../08-磁盘与IO/)），两层不能互相替代。口诀：**「journal 保账本，不保你写的字」**。

好，账本怎么改讲清了。下一问自然是：同一个文件能不能挂好几个名字？硬链和软链差在哪？
...
> 📝 **面试**：问到「删了会不会影响数据」，答「看 nlink：硬链是删名字，nlink 归零且无 fd 才回收；软链删的是纸条本身，目标毫发无损」。分工一句：「版本切换软链（绝对路径 + mv 原子替换）、增量备份硬链（rsync --link-dest）」。⭐ 口诀：**「版本切软链，备份用硬链」**（练习题 Q4）。

这一节对应练习题 Q4。好，名字讲完了——回到开头那个现场：有人 `rm` 了 40G，`df` 纹丝不动。大家想想，空间到底什么时候才真正回来？
...
> 🔍 **现场**：凌晨有人 `rm` 了 40G `logic.log`，`du` 立刻小、`df` 还是 100%，群里要重启逻辑服——先抓人，别重启。⭐ 这是值班天天见的坑（练习题 Q2、Q5）。
...
⭐ 截错 fd 会把 socket 或数据文件截成 0——动手前 `lsof` 对上**日志路径**再执行。这是磁盘满时对「正在写的日志」的标准止血；长期解法是让进程 reopen（nginx `kill -USR1`）或 logrotate `copytruncate`，轮转姿势在 [23](../23-日志运维/)。Docker 容器 `json-file` 不设 `max-size` 把节点盘写满是同族事故（→ [10](../10-cgroup与容器/)）。口诀：**「rm 不降先截断，别杀业务进程」**。

### 「rm 了 df 不降」的三大场景 🔥

同一个症状，三个完全不同的病，治法互斥——很多人只会答「deleted」，那就只答对了三分之一：
...
这一节对应练习题 Q2、Q5——deleted 止血的两道题，学完趁热打铁。

好，被删讲透了。回到开头另一个错：`df -h` 才 18%，群里喊扩云盘。大家想想——No space 一定是块满吗？
...
> 🔍 **现场**：活动服报 No space，两条一起敲，别只看 `-h`。⭐ 这是开头那个现场的正解入口（练习题 Q1）。
...
⭐ **append vs create**：inode 满时对**已存在**文件 append 有时还能写（不新建 inode），**新建一定挂**——「日志还在滚、热更解压却失败」这个组合，先想 inode。口诀：**「还能 append、不能 create → 先查 inode」**。

### 算术：为什么空间才 7% inode 就光了 🔥

很多人会问：空间明明还有 93%，inode 怎么会光？咱们算一遍就明白了。ext4 默认密度约 **1 个 inode / 16KB 磁盘**：
...
🔥 有人查到 `mke2fs -i` 想在线改——**别动手**。很多人会答「能调大」——**错了**。

> ⚠️ **别混**：**不能**。ext4 的 inode 数量**格式化时定死**——`mkfs.ext4 -i 4096` 调密度、`-N` 直接定个数，都是**格式化参数**，有数据的盘跑它等于清盘。XFS 的 inode **动态分配**，海量小文件下照样会被打满，「换 xfs 就不用管 inode」是错的，监控一样带 `df -i`。口诀：**「inode 密度是格式化参数，不是在线旋钮」**（练习题 Q6）。
...
这一节对应练习题 Q1、Q6——两种满和「能不能在线加」，开头那个扩云盘的同学就错在这儿。

好，两种满分清了。下一题更绕：`du` 说 200G，`df` 说 800G，`lsof deleted` 还是空的——差在哪？
...
> 📝 **面试**：du 和 df 的定义差一句——「du 是目录树视角，看不见 fd 和挂载点下面的；df 是文件系统账本视角，只认已分配块」。差一截先 deleted、再挂载点，别只会一个答案。练习题 Q3、Q9 专门练这条链。

这一节对应练习题 Q3、Q9——对账四条路和关键盘 nofail。

好，对账讲完。最后一站落地：修盘、选型、fstab——动手前必须知道的三条纪律。
...
🔥 有人看到只读就想 `mount -o remount,rw` 硬写回去——**别急**。journal 让 ext4/XFS 崩溃重启时**重放日志秒级恢复**，日常轮不到修复工具；要出手的场景是断电后重放失败、superblock/journal 本身损坏、或强制卸载后报错。
...
这一节对应练习题 Q7、Q8、Q10、Q12——选型、fstab、开服 checklist、只读挂载。

好，机制讲完了。回到开头那个现场——群里在喊扩云盘、rm、重启——正确的 60 秒该怎么走？
...
这一节对应练习题 Q14——白板综合定责，把前面所有题串起来。

现在再看群里那两句「扩云盘」「重启逻辑服」，你知道该怎么拦住他了：先 `df -h && df -i` 分型，再 `lsof +L1` 找握 fd 的手——扩容治不了 inode，kill 治不了 deleted。
...
- [ ] **口述出生**：三件套各存什么、文件名在哪、superblock 与 dentry cache 各是什么、预留块 5% 造成的两个现象（→ Q11、Q13）。
- [ ] **口述被写**：write 返回时数据在哪、journal 先记什么再改什么、journal 保什么不保什么（与 08 的分界）。
- [ ] **默画有名字**：硬链共享 inode（nlink=2）与软链存路径两张小图；跨 FS / 链目录 / 悬空三个判定；版本切换与增量备份各用哪个、为什么（→ Q4）。
- [ ] **口述被删**：回收条件（nlink × fd）、rm / `> file` / `> /proc/PID/fd/N` 三动作对 df 的影响、「rm 不降」三大场景各用什么命令（→ Q2、Q5）。
- [ ] **默画收束判定链**：du → lsof → mount/NFS 三步分流，三条支路各是什么病（→ Q3）。
- [ ] **口述满了**：两种满的签名对照、1TB/16KB/1KB 的算术、为什么不能在线加 inode、append vs create（→ Q1、Q6）。
- [ ] **口述对账**：du 和 df 的定义差一句、四条路的顺序与各自 10 秒判据、关键盘为什么不要 nofail（→ Q3、Q9）。
- [ ] **口述修复与落地**：fsck / xfs_repair 的前提、`-L` 是什么、fstab 六字段、UUID / nofail / `mount -a`、XFS 扩容跟谁（→ Q7、Q8、Q12）。
- [ ] **定责**：60 秒剧本四步各敲什么；症状是「慢」时转哪章看什么命令（→ Q10、Q14）。

...
> **刻意不展开**：await / aqu-sz / `%util` / 脏页 / journal 写路径 → [08 磁盘与IO](../08-磁盘与IO/)（**iostat 定责去 05**）；logrotate / copytruncate / journald → [23 日志运维](../23-日志运维/)；overlay 打满 inode → [10 cgroup与容器](../10-cgroup与容器/)；救援进不了系统 → [04 内核与系统](../04-内核与系统/)。
...
| 章 | 负责 |
|----|------|
| **14（本章）** | 文件名/inode/块、三种满、deleted、df≠du、软硬链、ext4/xfs、fstab UUID |
| **05** | 写路径、**iostat**、await/aqu-sz、脏页、journal、排队分型；空间三类只给入口 |
| **22** | logrotate、copytruncate、reopen、journal 打满 |
| **07** | 容器 overlay 层把 inode 打满 |

本章是**文件系统元数据线**。05 告诉你「盘慢不慢、脏不脏」；本章告诉你「满的是哪一类账、名字没了块还在不在」。
...
│  慢？await / %util → 回 05 看 iostat（本章不展开）                        │
└──────────────────────────────────────────────────────────────────────────┘
```
...
🎯 **别混**：本章不是磁盘性能章。看到 await / aqu-sz / `%util` 立刻回 05。
...
```bash
iostat -xd 1 3
# await / aqu-sz / %util 怎么读 → 05，本章不展开
```
...
journal / 写路径细节 → [08](../08-磁盘与IO/)。await 与调度器 → 同样回 05。
...
flowchart TB
    subgraph gate ["先分叉"]
        ...
        A2["接口超时 Load 高 CPU idle"] --> IO["iostat → 05"]
    end
...
| 接口超时、await 高 | **iostat → 05** | 在本章纠结 inode |

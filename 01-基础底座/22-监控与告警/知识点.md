...
> 活动开场 3 分钟，电话被 CPU、连接数、QPS 打爆——登录入口其实已经 5xx，黑盒探活夹在四十条「机器黄了」里没人点；另一头 Grafana 全绿、玩家进不去。三拨人分别喊「再加 CPU 规则」「先关通知」「刷新面板」，三句都不对。
> **本章回答**：一条告警的一生——信号怎么采（白盒 / 黑盒）、怎么算（PromQL）、怎么写成规则和分级（P0–P3）、怎么降噪（分组 / 抑制 / 静默）、怎么把正确的人在正确的时间叫醒。
...
> 🎯 **一句话**：信号从采集到叫人要过五站：采、算、规、降、人；卡在哪一站，就修哪一站。
...
- 📖 **读图**
  - 🚪 左边两源：黑盒从玩家视角探「通不通」，白盒是进程自报——缺黑盒则内部全绿也能入口挂（三节）
  - 🛤️ 中间机器判断：规则给持续性（`for`）与级别；AlertManager 给降噪（七节）；右边才是人——**只有 P0 打电话**，对齐 SLI 不对齐「CPU 圆不圆」（五、六节）
  - 🩺 虚线：Prometheus 挂了会「全绿」，靠 `up==0` 与 Watchdog 兜底（八节）
...
> 💡 **打个比方**：Metrics 是仪表盘转速表，Logs 是行车记录仪，Traces 是快递单。半夜决定要不要停车看仪表盘；修车才看记录仪和快递单。

- 🗺️ **值班四个场景怎么选**
  - 登录成功率跌破 SLO → **Metrics**（SLI 曲线）+ 黑盒；别先 grep 全量日志当告警源
  - 某玩家订单号失败 → **Logs**（按 request_id 检索 → [23](../23-日志运维/)）；别指望 Metrics 标签里塞订单号
  - 支付 P99 高、跨网关与支付 → **Traces** 看哪一跳慢；别只看单机 CPU
  - 磁盘将满 → **Metrics** `predict_linear`；别等日志写失败再叫

- 🚫 **高基数红线** ⭐：`http_requests_total{user_id="u182937"}` 错——标签基数爆炸，存储先死。正确：Metrics 聚合做告警，日志带 `request_id` / `trace_id` 下钻。「有 Metrics 就不需要 Logs」「有 Traces 就不需要黑盒」同样错——Trace 采不到「连不上」。

> ⚠️ **别混**：**告警主战场是 Metrics，不是 Logs**。每个 ERROR 行都推 IM = 没有分级；日志告警只做少量致命关键字。

> 📝 **面试**：问「三支柱怎么选」，答「Metrics 定叫不叫、趋势和 SLO；Logs 定这一次为什么；Traces 定跨服务哪一跳。主机值班主战场是 Metrics 加黑盒，日志告警只做少量致命关键字」。⭐（练习题 Q2）

本节对应练习题 Q2。选材定了——为什么 Grafana 全绿玩家还能进不去？
...
🔥 开头「Grafana 全绿、玩家进不去」——真凶往往是**缺黑盒**。⭐（练习题 Q6、Q7）

### 📦 白盒与黑盒 🎯⭐🔥

- ⬜ **白盒监控**：进程自己暴露 `/metrics`，回答「内部怎么样」——节点资源（node_exporter）、入口 RED（网关）、中间件（mysqld / redis）、进程在不在
- ⬛ **黑盒监控**：blackbox_exporter 从「玩家能碰到的地方」往里探，回答「外面通不通」

#### ❓ 为什么白盒全绿玩家还能进不去？

- 👻 **白盒看不见四样**：LB 摘流量、DNS 指错、证书换错、安全组没放行——都在进程**外面**，白盒照样全绿
- ✅ **黑盒存在的全部理由**：从玩家路径探通不通；探的还是玩家点的那个 URL，不是静态 `/health`
- 🔁 **反过来**：黑盒挂了白盒全绿 → 病在路径不在机器
...
③ 中间件层：mysqld / redis / nginx exporter —— 组件自身指标
④ 业务层：网关 / 服务自带 metrics —— 入口 RED（QPS / 5xx / P99）
```

- 📖 **读图**：叫醒线主要钉在 ① 和 ④；②③ 是饱和信号，最多 P2（六节）

### 🔌 Pull、Push 与 Pushgateway

| 模式 | 谁主动 | 优点 | 坑 |
|------|--------|------|------|
| **Pull（Prometheus）** | Server 来 scrape | `curl` 即排障；服务发现自然 | 目标入站要开端口 |
| **Push（Zabbix Agent 等）** | Agent 上报 | 出站好做、防火墙友好 | 发现与标签能力弱 |
| **Pushgateway** | 短命 Job 推一下 | 批任务 / CI 跑完也能报 | 序列**不过期**，Job 结束必须删 |
...
```text
node_cpu_seconds_total{cpu="0",mode="idle"} 1.234e7   # ← 有行 = 采到了
node_memory_MemAvailable_bytes 8.5e9
node_filesystem_avail_bytes{mountpoint="/var"} 2.1e10
```

- `curl` 不通 = 没采到。「Grafana 有旧图」不代表还在采
- blackbox 同理：`curl 'http://blackbox:9115/probe?target=<URL>&module=http_2xx'` 直接看 `probe_success`

### 📊 四种指标类型 ⭐

- **Counter（计数器）**：只增不减——请求总数、错误总数。**必须 `rate()` 取增速**再比阈值；直接 `> 1e8` 会永远 firing
- **Gauge（仪表值）**：可上可下——内存、在线人数、队列长度，直接比
- **Histogram（直方图）**：分桶存（`le` 标签），服务端可聚合出 P95 / P99——**跨实例算分位数只能用它**
- **Summary（摘要）**：客户端预先算好分位数——单实例方便，**跨实例加不起来**

> 💡 **打个比方**：Counter 是水表——不能说「读数大于一亿就漏水」，要看每小时流了多少（`rate`）；Gauge 是油箱指针，随时 readable。

- 🔧 **落地最小集**：node_exporter **:9100** 每台都装、blackbox **:9115** 放在用户侧路径上、Prometheus 能访问全部 target、AlertManager 电话网关**单独演练**过、Pushgateway 只给短任务。`scrape_interval` 主机 **15s~30s**——过短打爆 exporter

> ⚠️ **别混**：**白盒绿 ≠ 玩家能进**。白盒只证明「进程自己觉得自己健康」。

> 📝 **面试**：问「为什么内部全绿玩家还是进不去」，答「白盒看不见 LB / DNS / 证书 / 安全组——它们在进程外面。必须有 blackbox 从玩家路径探活，探的还是玩家点的那个 URL，不是静态 /health」。⭐（练习题 Q6、Q7）

本节对应练习题 Q6、Q7。信号采上来了——`rate` 和 `irate` 差在哪？为什么 P99 必须用 Histogram？
...
🔥 很多人把 Counter 当 Gauge 画——口诀：**「Counter 看速率，Gauge 看水位」**（练习题 Q5）。

### 📈 rate 与 irate ⭐

#### ❓ 为什么 Counter 不能直接比阈值？

- ❌ **直接 `> 1e8`**：Counter 只增不减，读数迟早超阈值 → 永远 firing
- ✅ **`rate(x[5m])`**：窗口内**平均每秒增速**，自动处理 Counter 重置——告警规则的标准选择，平滑、抗抖动
- ⚡ **`irate(x[5m])`**：只取窗口内**最后两个样本**算瞬时速率——尖峰敏感，画短面板看毛刺；做告警会被两个样本的抖动骗 firing
- 📌 **背板**：面板毛刺看 irate，告警阈值用 rate；两者都**只能用于 Counter**，Gauge 上用是错的

### 📐 分位数：histogram_quantile 的 P95/P99 🔥
...
#### ❓ 为什么跨实例 P99 不能用 Summary？

- 🪣 **Histogram**：每个延迟区间一个 `_bucket` 计数器（`le` =「小于等于」边界），`histogram_quantile` 在桶间**线性插值**还原分位数——所以是**估算**，桶太粗就不准
- 🧮 **跨实例**：先把各实例的桶 `sum by (le)` 加起来再取分位
- ❌ **Summary**：客户端算死了分位数，**0.9 分位的分位数没有可加性**——多副本网关加不起来
...
- 🟡 **静态 70%**：大盘上会**永远黄**——喊两周没人理，真的快满也没人信
- 📈 **`predict_linear`**：用过去 6h 增速外推「24h 后还剩多少」——**比阈值早半步**：70% 但增速为零不叫，40% 但增速凶猛要叫
- 🎮 **活动服可加紧**：`[6h] → 20h` 预测 + `[1h] → 5h` 紧急线

> ⚠️ **别混**：**Histogram vs Summary**——跨实例只能 Histogram；**rate vs irate**——告警用 rate，面板毛刺用 irate；都只对 Counter。

> 📝 **面试**：问「跨三个网关副本算 P99」，答「服务端用 Histogram 暴露 bucket，PromQL 里 sum by (le) rate 之后 histogram_quantile(0.99, ...)；Summary 跨实例加不起来」。再补「分位数是桶间插值估算，桶边界要按业务延迟量级设」。⭐（练习题 Q5）

本节对应练习题 Q5。会算信号了——叫醒线该对齐 CPU，还是对齐玩家体验？
...
🔥 大白话：**SLI 是量什么（玩家体验）、SLO 是允许多差、Budget 是还能烧多少错误额度**——叫醒线对齐 SLI，不对齐 CPU 圆不圆。⭐（练习题 Q1、Q3）
...
- 🧮 **口算** ⭐：SLO 99.9%，一个月 30 天 = 43200 分钟，Budget = 43200 × 0.1% ≈ **43.2 分钟**。今晚登录失败率 20% 持续 10 分钟 = 猛吃 Budget：该叫人、该冻非紧急发布
...
- 📖 **读图**：Budget 是闸门——充足可发、将尽冻结、耗尽复盘；叫醒线钉在 SLI 破 SLO，不钉在 CPU%

#### ❓ 为什么叫醒线对齐 SLI 不对齐 CPU？

- 🎮 **玩家疼的是体验**：登录成功率、支付成功率、探活通不通——不是某台机器 CPU 圆不圆
- 📉 **CPU 70% 但登录 99.95%** → IM 即可（活动则静默）；**登录成功率 80%** → **P0/P1** 跟业务
- 💾 **盘**：60% 且 predict 不满 → 不叫；predict 24h 内满 → P1；95%+ 或 inode 满 → P0
- 📜 **证书剩 3 天** → P1/P0
- 🛑 **主机 CPU 是饱和信号，最多 P2，永远不能当玩家体验 SLO**

- 🎯 **游戏服常见 SLI**：登录——成功率、P99、blackbox；支付——成功率、耗时分位；大厅入口——探活、5xx；战斗长连接——掉线率、心跳超时率。别拿单机 CPU% / 连接数绝对值 / Load 平均值当 SLI

> 📝 **面试**：问「半夜哪些指标能叫醒你」，答「对齐 SLI 的才能叫：登录 / 支付成功率破了 SLO、探活连续失败、盘 predict 将满、OOM 风暴、证书临期。CPU 70% 不是——它是饱和信号，先看登录曲线正不正常」。Budget 细算 → [SRE 06](../../09-可观测性与SRE方法论/06-SLI-SLO与错误预算/)。⭐（练习题 Q1、Q3）

本节对应练习题 Q1、Q3。叫醒线钉死了——黄金信号怎么写成带 `for` 的规则？
...
- 📡 **黄金四信号**（Google SRE）：**Latency**（多慢，看 P99 不看平均）、**Traffic**（多忙，QPS / CCU）、**Errors**（多错，5xx / 失败 / drop）、**Saturation**（多满，CPU / 盘 / 连接表）
  - RED = 服务侧投影（Rate=Traffic、Errors、Duration=Latency）
  - USE = 资源侧投影（方法论详见 [21](../21-性能方法论-USE与RED/)）
...
- 📖 **读图**：第一行定级（玩家疼不疼），第二行定饱和（机器满不满）；5xx 12% 是 P0，CPU 72% 是容量旁证
...
- ⏱️ **`for` 的取值口径** ⭐：探活 **1~2m**（挂了要快知道）、CPU **5m**（瞬时毛刺不该叫）、盘 **10m**（增速预测要稳）。一事一级：同一件事不能既是 P0 又是 P2
- ⚠️ **`for` 不是把阈值改松**：它是防抖（持续才算 firing），不是把 70 改成 95
...
> 📝 **面试**：问「面板怎么摆」，答「第一行 RED——QPS、5xx、P99；第二行 USE——CPU、Load、MemAvailable、盘、inode、fd；时间轴打发布 Annotation。规则三要素 expr + for + severity，for 探活 2 分钟、CPU 5 分钟、盘 10 分钟；P0 才电话」。⭐（练习题 Q4）

本节对应练习题 Q4。回到开头那个电话被打爆的现场——四十条「机器黄了」里埋着真 P0，怎么降噪？
...
🔥 电话被 CPU/连接数/QPS 打爆、真 P0 没人点——这就是**告警疲劳**。口诀：**「分组压量、抑制父子、静默窗口、P0 才打电话」**（练习题 Q8、Q9）。

- 🧨 **疲劳从哪来**：静态阈值永远黄；一台机器挂了没抑制，四十条 IM 一起推；容量类当 P0 电话；每个 ERROR 日志行都推 IM；长期静默等于删规则
...
- 📖 **读图**：四因对四刀——永远黄用 `for`、风暴用分组+抑制、容量电话用分级路由、日志刷屏用砍规则；疲劳和漏报要一起治

### 🧩 四个机制别混 ⭐

| 机制 | 干什么 | 典型 | 别当成 |
|------|--------|------|--------|
| **`for`（持续时间）** | 条件持续多久才 firing | CPU `for: 5m` | 把阈值从 70 改到 95 |
...
#### ❓ 为什么抑制不是静默？一台挂了人该看到几条？

- 🧯 **抑制**：故障发生时**自动**少吵——父压子。一台 hang 死，人**只收到一条 InstanceDown**，同 instance 的 CPU / 进程 / 磁盘全部 suppressed
- 🔇 **静默**：计划内**手动**不吵——今晚装修，烟感暂时屏蔽；明天必须恢复
- 🔑 **配抑制**（「故障越大噪音越小」的关键配置）：
...
> 💡 **打个比方**：AlertManager 是大楼前台分拣——同一栋楼着火（InstanceDown），不要给每个房间再打四十通电话；**静默**是「今晚这层装修，烟感暂时屏蔽」——装完不恢复等于把烟感拆了。

### 🎮 活动窗口红线 ⭐

- ✅ 活动中：容量类 P2（CPU、QPS 绝对值）**Silence**；登录、支付、blackbox、盘 predict、证书**死也不静默**；发布打 Annotation
- ❌ **长期 Silence = 删规则**。漏报是另一头的病：没黑盒、没 Watchdog、阈值太宽——只砍规则会漏报
...
- 📖 **读图**：五关任一关漏了，开头那个「电话炸 / 全绿进不去」就会重演

> ⚠️ **别混**：**抑制 ≠ 静默**——一个是机器的应激反应（父压子），一个是人的排期决定（窗口）。

> 📝 **面试**：问「告警风暴怎么治」，答「一台挂了我配 InstanceDown 抑制同 instance 子告警，人只收一条；活动只 Silence 容量类，SLO 和探活死也不静默；疲劳四步——分组、for、分级路由、每月砍从未被处理的规则」。⭐（练习题 Q8、Q9）

本节对应练习题 Q8、Q9。噪声压住了——盘满怎么提前叫？Prometheus 自己挂了谁叫醒你？
...
- **盘**：`predict_linear` 斜率（四、六节）；已 95%+ 或 inode 满 → P0——`df -h` 看着空、inode 满，日志和 Nginx 照样写挂（[09](../09-文件系统与inode/)）
...
- 💀 Prometheus 自己挂了 → 所有面板停在最后一帧、告警全部消失——**「没有告警」≠「一切正常」**
- ✅ **三层兜底**：
...
- ✅ **验收五条** ⭐：`curl :9100` 有 `node_`；targets 全 UP 且 `up==0` 有告警；对登录 URL 故意断安全组能触发 P0；测试 firing 时电话 / IM 真的响；Watchdog 在、停掉 Prometheus 外层能叫人
- 🔗 **图和告警同源**：面板和规则用同一条 PromQL，否则「图上红了脚本还没叫」各说各话

> 📝 **面试**：问「监控系统自己挂了怎么办」，答「三件事：up 指标盯 scrape 目标；Watchdog 永远 firing 的心跳由外部 Deadman 确认；再用独立的外部拨测探监控自己。核心口径是『没告警 ≠ 没故障』」。⭐（练习题 Q10、Q11、Q12）

本节对应练习题 Q10、Q11、Q12。现在再看开头那个现场——电话炸、黑盒被埋、全绿进不去——60 秒怎么走？
...
- 📖 **读图**：先黑盒定「外路径通不通」；通了再看 SLI；只有 CPU/QPS 高才问是不是活动容量；风暴另开一支查抑制
...
> 🔍 **现场**：接到「电话炸 / 进不去 / 全绿」，60 秒走完这条线再下结论。

```text
① 0–15s  黑盒探活：probe_success？失败 → P0 查 SLB/DNS/证书/安全组
② 15–30s 通了看登录/支付 SLI：破 SLO → 跟业务扩容/回滚/摘机
③ 30–45s 只有 CPU/QPS 高 → 问活动窗口？该 Silence 容量类，不是打电话
④ 45–60s 一条根因几十条 IM → amtool 查抑制/静默；人只留 InstanceDown + 真 P0
```
...
对着第一节主图口述一遍，卡住就回对应小节。开头那两个现场——电话被噪声打爆、Grafana 全绿玩家进不去——现在该能说清先查什么了。

- [ ] **默画主图**：黑盒 / 白盒 → Prometheus → 规则 → AlertManager → P0~P3，五站各干什么；监控自监控的虚线画上。（→ Q13、Q14）
- [ ] **口述三支柱**：各答哪一问；四个值班场景怎么选；为什么日志 ERROR 全量当告警是错的；高基数红线是什么。（→ Q2）
- [ ] **口述采集**：白盒 vs 黑盒各补什么盲区；监控四层分层；Pull / Push / Pushgateway 的坑；Counter 为什么必须 rate。（→ Q6、Q7）
- [ ] **口述 PromQL**：rate vs irate；跨实例 P99 怎么写（Histogram + histogram_quantile）；盘为什么用 predict_linear 不用静态阈值。（→ Q5）
- [ ] **口述 SLI**：SLI / SLO / SLA / Budget 四个词；99.9% 一个月多少分钟；CPU 为什么不能当玩家 SLO。（→ Q1、Q3）
- [ ] **口述规则与分级**：expr + for + severity 三要素；for 的三档取值；P0~P3 响应时限；该叫 / 不该叫名单；面板三板斧。（→ Q4）
- [ ] **口述降噪**：疲劳四刀；抑制 vs 静默 vs for vs 分组；inhibit_rules 怎么配；活动窗口红线；amtool 怎么加静默。（→ Q8、Q9）
- [ ] **口述兜底**：饱和度清单（盘 / conntrack / fd / inode / throttle / 证书）；Watchdog 三层自监控；验收五条。（→ Q10、Q11、Q12）
- [ ] **定责走读**：活动开场告警风暴，从黑盒 → SLI → AM → 容量 vs 入口，口述四步和留证三样。（→ Q13、Q14）

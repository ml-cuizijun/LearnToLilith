...
> 这一章讲完，你会知道当时叫醒线错在哪，以及「全绿却进不去」该先查什么。
> **本章回答**：一条告警的一生——信号怎么采（白盒 / 黑盒）、怎么算（PromQL）、怎么变成规则和分级（P0–P3）、怎么降噪（分组 / 抑制 / 静默）、最后怎么把正确的人在正确的时间叫醒。
> **不讲**：告警响了之后怎么定域排障 → [21](../21-性能方法论-USE与RED/)；日志采集与轮转 → [23](../23-日志运维/)；PromQL 深水与高基数 → [K8s 19](../../03-Kubernetes/19-监控与可观测性/) · [SRE 02](../../09-可观测性与SRE方法论/02-Prometheus与PromQL/)；告警体系化设计 → [SRE 03](../../09-可观测性与SRE方法论/03-告警设计与降噪/)；SLO 细算 → [SRE 06](../../09-可观测性与SRE方法论/06-SLI-SLO与错误预算/)；Oncall 排班 → [工程 08](../../04-工程化与自动化/08-运维平台与Oncall/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节的主图，把「一条告警的一生」装进脑子；第二到第八节顺着选材 → 采集 → PromQL → SLI/SLO → 规则 → 降噪 → 兜底往下走；第九节专门讲定责，也就是开头那个现场的正确解法。每一节讲完都会提示对应练习题，学完就去练。
...
咱们先不急着背 PromQL。学监控最怕的就是规则堆了一百条，电话却叫错人。先看图，把「信号卡在哪一站」装进脑子里：
...
好，主图有了。咱们正式出发——第一站：Metrics / Logs / Traces 各答哪一问？
...
选材段到这儿。本节对应练习题 Q2。好，三支柱分清了——大家想想：为什么 Grafana 全绿玩家还能进不去？黑盒缺了会怎样？

---

## 三、出生：采集——白盒、黑盒与 Pull

🔥 开头「Grafana 全绿、玩家进不去」——真凶往往是**缺黑盒**。大白话：**白盒是自己报自己，黑盒是从玩家视角探通不通**——内部全绿入口照样可以挂。⭐ 面试必考（练习题 Q6、Q7）。
...
采集段到这儿。本节对应练习题 Q6、Q7。好，信号采上来了——大家想想：rate 和 irate 差在哪？为什么 P99 必须用 Histogram？

---

## 四、成形：PromQL 把指标算成信号

🔥 很多人把 Counter 当 Gauge 画——**高频错答**。大白话：**Counter 只增不减，必须用 rate/irate 看增速；Gauge 才是瞬时水位**。口诀：**「Counter 看速率，Gauge 看水位」**（练习题 Q5）。
...
成形段到这儿。本节对应练习题 Q5。好，会算信号了——大家想想：叫醒线该对齐 CPU，还是对齐玩家体验？

---

## 五、定标：SLI / SLO / Error Budget——叫醒线的数学

🔥 **SLI / SLO / Error Budget**第一次见容易背成三句话。大白话：**SLI 是量什么（玩家体验指标）、SLO 是允许多差、Budget 是还能烧多少错误额度**——叫醒线对齐 SLI，不对齐 CPU 圆不圆。⭐ 面试必考（练习题 Q1、Q3）。
...
定标段到这儿。本节对应练习题 Q1、Q3。好，叫醒线钉死了——黄金信号怎么写成带 `for` 的规则？
...
变规段到这儿。本节对应练习题 Q4。回到开头那个电话被打爆的现场——四十条「机器黄了」里埋着真 P0，怎么降噪？

---

## 七、降噪：告警疲劳治理 🔥

🔥 电话被 CPU/连接数/QPS 打爆、真 P0 没人点——这就是**告警疲劳**。大白话：**噪声太多，人就会对所有响铃脱敏**。口诀：**「分组压量、抑制父子、静默窗口、P0 才打电话」**（练习题 Q8、Q9）。
...
降噪段到这儿。本节对应练习题 Q8、Q9。好，噪声压住了——大家想想：盘满怎么提前叫？Prometheus 自己挂了谁叫醒你？
...
兜底段到这儿。本节对应练习题 Q10、Q11、Q12。现在再看开头那个现场——电话炸、黑盒被埋、全绿进不去——60 秒怎么走？
...
作战段到这儿。本节对应练习题 Q13、Q14。现在再看开头那两个现场——叫醒线错乱、全绿却进不去——你已经知道该怎么拦住他了。
...
对齐练习题「合上书」，逐条口述，卡住就回对应节 / 题号。现在再看开头那两个现场——电话被噪声打爆、Grafana 全绿玩家进不去——你该知道先查什么了。

...
> 这一章讲完，你会知道当时那个人错在哪，以及第一刀该砍在六段里的哪一段。
> **本章回答**：一行日志从「应用写下第一笔」到「客服在 Kibana 搜到它」的一生，每一段会怎么坏、值班第一刀砍哪。
> **不讲**：deleted inode 的账本原理 → [09](../09-文件系统与inode/)；await、脏页等盘慢 → [08](../08-磁盘与IO/)；探活与告警分级 → [22](../22-监控与告警/)；K8s 集群日志形态 → [03-20](../../03-Kubernetes/20-日志管理/)；ES 分片与 ILM 深调 → [02-04](../../02-中间件与数据库/04-Elasticsearch/)。这些都有专门的章节，本章只在用到时点到为止。

**标记**：🎯重点 · 🔥难点 · ⭐常考 · 🔧实操

**怎么听这场分享**：咱们先看第一节的主图，把「一行日志的一生」装进脑子；第二到第六节顺着出生 → 落地轮转 → 生病 → 出门 → 归宿往下走；第七节专门讲定责，也就是开头那个现场的正确解法。每一节讲完都会提示对应练习题，学完就去练。
...
咱们先不急着背 logrotate 配置。学日志运维最怕的就是从「搜不到」直接跳「扩 ES」。先看图，把「断在哪一段」装进脑子里：
...
出生段到这儿。本节对应练习题 Q1、Q2。好，知道去哪查了——大家想想：配了 logrotate 为什么文件还在疯长？copytruncate 和 USR1 差在哪？

---

## 三、落地与轮转：logrotate 全套与 reopen

🔥 很多人把 copytruncate 当默认——**有坑**。大白话：**copytruncate 是拷走再清空，高峰可能丢尾；rename+信号 reopen 才是正路**。口诀：**「能 reopen 就别 copytruncate」**（练习题 Q3、Q4）。
...
轮转段到这儿。本节对应练习题 Q3、Q4、Q5。回到开头那个 `rm` 现场——为什么 du 小了 df 不动？

---

## 四、生病：写满磁盘——du、df 与 deleted

🔥 `rm` 大日志后 `du` 立刻小、`df` 纹丝不动——**值班天天见**。大白话：**rm 只删目录项，进程还握着 fd，空间不释放**。口诀：**「先 lsof +L1，再截断」**（练习题 Q6、Q7）。
...
生病段到这儿。本节对应练习题 Q6、Q7。好，本机盘救回来了——大家想想：Kibana 缺十分钟，第一刀查 Filebeat 还是扩 ES？
...
出门段到这儿。本节对应练习题 Q8、Q9。好，采集通了——大家想想：有 JSON 为什么还对不上因果？ELK 和 Loki 怎么选？

---

## 六、归宿：结构化、trace_id 与 ELK/Loki

🔥 日志在但拼不出因果——真凶往往是**缺 trace_id**。大白话：**没有同一请求的身份证，多机多行日志对不上时间线**。⭐ 面试必考（练习题 Q10）。
...
归宿段到这儿。本节对应练习题 Q10、Q11、Q12。现在再看开头三个现场——盘满、rm 不降、Kibana 缺十分钟——60 秒怎么定责？
...
作战段到这儿。本节对应练习题 Q13、Q14。现在再看开头那三个人——rm 不降、喊扩 ES、盘满登录失败——你已经知道该怎么拦住他们了。
...
对齐练习题「合上书」，逐条口述，卡住就回对应节 / 题号。现在再看开头那三个现场——盘满、rm 不降、Kibana 缺十分钟——你该知道先查哪一段了。
